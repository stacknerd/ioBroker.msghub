...
- Refs: stable per run, bounded by `maxPool` and intentionally composed of safe characters (e.g. `IngestRandomChaos.0.<runId>.<kind>.<slot>`) to avoid ref normalization warnings
...
Options are stored in the plugin’s base object `native`.
The schema and defaults come from the plugin manifest (`lib/IngestRandomChaos/manifest.js`), and the plugin resolves
effective values via `ctx.meta.options.resolveInt(...)` on startup.

- `intervalMinMs` (`number`, default `2000`): minimum delay between ticks.
- `intervalMaxMs` (`number`, default `5000`): maximum delay between ticks (clamped to be >= `intervalMinMs`).
- `maxPool` (`number`, default `10`): maximum number of concurrently active messages managed by this plugin.

Note: when options are changed via the Admin Tab (or `IoPlugins.updateInstanceNative`) for an enabled instance, `IoPlugins`
restarts that single instance so changes apply immediately. If you edit `native` manually in the Objects view, do a
disable+enable toggle to apply.
...
- Base object id: `msghub.0.IngestRandomChaos.<instanceId>`
- Enable switch: `msghub.0.IngestRandomChaos.<instanceId>.enable`
- Status: `msghub.0.IngestRandomChaos.<instanceId>.status`
