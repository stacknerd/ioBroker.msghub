...
- `app`, when present, is forwarded from the plugin manifest for text/url/display metadata; the current AdminTab installability/head path resolves plugin panel icons from the generic host set `icons/pluginUI/*`
