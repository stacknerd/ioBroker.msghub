...
| `web.pluginUi.discover` | `ioPlugins.getAdminUiContributions()`, `ioPlugins.computeAdminUiBundleHash(...)`, `ioPlugins.readAdminUiTranslations(...)` | Requires a valid web token via `payload.token`. Exclusive shared-safe discovery path. Normalizes optional `lang` to a safe base tag or `'en'`. Best-effort per-panel hash computation. Hash failures degrade to `bundle.hash = ''` with a warning log. Plugin-owned Admin-UI i18n is read once per plugin type and attached as `i18n`; read failures degrade to `null` with a warning log. | `{ ok, data: PluginUiContribution[] }` | `lib/IoWebUi.js`, `lib/IoPlugins.js` |
| `web.pluginUi.bundle.get` | `ioPlugins.getAdminUiContributions()`, `ioPlugins.computeAdminUiBundleHash(...)`, `ioPlugins.readAdminUiBundle(...)` | Requires a valid web token via `payload.token`. Exclusive shared-safe bundle path. Validates `pluginType` and `panelId`, normalizes `instanceId` to integer or `0`, normalizes `lang` to a safe base tag or `'en'`, checks that the plugin panel is currently running, caches by `(pluginType, instanceId, panelId, hash, lang)`, enforces JS size `<= 512 KiB` and CSS size `<= 64 KiB`. | `{ ok, data: { apiVersion, moduleFormat: 'esm', hash, js, css?, i18n } }` | `lib/IoWebUi.js`, `lib/IoPlugins.js` |
| `web.pluginUi.rpc` | `IoPluginUiRpc.handleWebRpc(payload)` | Requires a valid web token via `payload.token`. Thin web-host facade over the shared PluginUi RPC dispatcher. Validates identity, payload size, active panel contribution, and dispatches to `handleWebUiRpc`. | `{ ok, data|error }` | `lib/IoWebUi.js`, `lib/IoPluginUiRpc.js` |
