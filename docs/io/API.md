...
| Source of truth | `main.js`, `web.js`, `lib/index.js`, `lib/IoAdminCapabilities.js`, `lib/IoAdminTab.js`, `lib/IoWebUi.js`, `lib/IoUiCatalog.js`, `lib/IoAdminConfig.js`, `lib/IoCoreConnection.js`, `lib/IoSystemBaseline.js`, `lib/IoPlugins.js`, `lib/IoPluginResources.js`, `lib/IoManagedMeta.js`, `lib/IngestStates/manifest.js`, `src/MsgStore.js`, `src/MsgStats.js`. |
...
| IO <> UI | `main.js`, `lib/IoAdminCapabilities.js`, `lib/IoAdminTab.js`, `lib/IoWebUi.js`, `lib/IoAdminConfig.js` | `ui.bootstrap`, `admin.*`, `web.*`, `config.*`, and the bootstrap-owned `about` payload. |
...
| `IoSystemBaseline.ensure()` | Ensures the canonical MsgHub-owned user/group baseline: user, group, canonical descriptions, exact membership, canonical ACL, icon, enabled flags, and a fresh password via `setPasswordAsync(...)`. It does not mutate `web.*` config. | Adapter startup baseline | IO runtime | `lib/IoSystemBaseline.js`, `main.js` |
...
| `config.idcatalog.get` | `adapter.getForeignObjectsAsync(filter || '*', 'state')` or callback equivalent | Requires a valid config token via `payload.token`. Optional payload `filter` is treated as plain ioBroker object pattern with fallback `'*'`. Returns a hard-whitelisted picker projection of ioBroker `state` objects: `_id` plus `common.{name,type,role,unit}` only. | `{ ok, data: { objects } }` | `lib/IoAdminConfig.js` |
| `config.idcatalog.openTree` | `adapter.getForeignObjectsAsync('*', 'state')` at root, otherwise `adapter.getForeignObjectsAsync(entry + '.*', 'state')`, or callback equivalent | Requires a valid config token via `payload.token`. Returns a tree-oriented catalog slice for the requested `entry` and `depth`, plus `meta.backendDurationMs`. Current `depth` limits the response shape only; it does not make the underlying ioBroker object read depth-aware. | `{ ok, data: { entry, depth, nodes, meta } }` | `lib/IoAdminConfig.js` |
