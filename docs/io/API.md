...
| Source of truth | `main.js`, `web.js`, `lib/index.js`, `lib/IoAdminCapabilities.js`, `lib/IoAdminTab.js`, `lib/IoWebUi.js`, `lib/IoUiCatalog.js`, `lib/IoAdminConfig.js`, `lib/IoIdCatalog.js`, `lib/IoCoreConnection.js`, `lib/IoSystemBaseline.js`, `lib/IoPlugins.js`, `lib/IoPluginResources.js`, `lib/IoManagedMeta.js`, `lib/IngestStates/manifest.js`, `src/MsgStore.js`. |

| Area | Owned by | Use this file for |
| --- | --- | --- |
| IO <> Core | `main.js`, `lib/IoCoreConnection.js`, `lib/IoPlugins.js`, `lib/IoAdminCapabilities.js`, `lib/IoAdminTab.js`, `lib/IoWebUi.js`, `lib/IoAdminConfig.js`, `lib/IoIdCatalog.js` | Adapter/runtime bridge entry points and the composition-root routing boundary. |
...
| IO <> UI | `main.js`, `lib/IoAdminCapabilities.js`, `lib/IoAdminTab.js`, `lib/IoWebUi.js`, `lib/IoAdminConfig.js`, `lib/IoIdCatalog.js` | `ui.bootstrap`, `admin.*`, `web.*`, `config.*`, and the bootstrap-owned `about` payload. |
...
| `IoCoreConnection.getBootstrapAboutConnection()` | Returns the minimal `ui.bootstrap.about.connection` fragment `{ scope: 'core-link', connected, mode: 'local' }`. | `ui.bootstrap` payload builder | IO runtime | `lib/IoCoreConnection.js`, `lib/IoAdminCapabilities.js` |
| `new IoSystemBaseline({ adapter })` | Requires `adapter.namespace`. Owns the MsgHub-managed public-web reference baseline in `system.user.*` and `system.group.*`. | Adapter startup baseline | IO runtime | `lib/IoSystemBaseline.js` |
| `IoSystemBaseline.ensure()` | Ensures the canonical MsgHub-owned user/group baseline: user, group, exact membership, canonical ACL, icon, enabled flags, and a fresh password via `setPasswordAsync(...)`. It does not mutate `web.*` config. | Adapter startup baseline | IO runtime | `lib/IoSystemBaseline.js`, `main.js` |
...
| `new IoWebExtension({ instanceObject, ... })` | Installs the public Single-WebApp host under `/MessageHub/<instance>/<panelId>/...` when the surrounding `web`-extension entry is active. The productive host lives in `web.js`, runs inside the `web` adapter process, and therefore cannot consume the live MsgHub `main.js` service graph or a `MsgConfig` instance directly. Its activation gate intentionally reads the hosting-boundary flags `instanceObject.native.publicWebEnabled` and `instanceObject.native.webInstance` directly, while panel eligibility is resolved only through `sendTo('<msghub.x>', 'internal.uiCatalog.getApp', { mode: 'panel', targetId })`, which `main.js` proxies 1:1 to the running `IoUiCatalog.getApp(...)`. `null` becomes pre-flight `404`. The host reuses `admin/tab.html`, forwards canonical bootstrap args through `msghub-forwarded-args`, exposes only `admin/tab.css`, `admin/tab.js`, `admin/tab/**`, and `admin/i18n/**` under the panel route, blocks `/MessageHub/<instance>/tab.html`, does not expose a server-side public manifest route, and serves icon assets under the host-root path `/MessageHub/<instance>/icons/...` from `admin/icons/**`. Manifest generation stays frontend-owned in `admin/tab/layout.js` as a blob manifest. | ioBroker web-extension host | IO runtime | `web.js` |
...
| `IoAdminCapabilities.buildBootstrapAbout()` | Returns the shared bootstrap `about` payload `{ title, version, time, lang, connection }`. | `ui.bootstrap.about` payload builder | IO runtime | `lib/IoAdminCapabilities.js` |
...
| `new IoAdminConfig(adapter, { ai?, msgStore?, archiveProbeNative?, adminCapabilities?, idCatalog? })` | Requires `adapter.namespace`. Binds the config command facade to optional AI and archive/status services plus the shared `IoAdminCapabilities` authority for canonical payload-token validation. ID-catalog commands delegate to the injected or default `IoIdCatalog` instance. | Config command router | IO runtime | `lib/IoAdminConfig.js` |
...
| `new IoIdCatalog(adapter, { ttlMs? })` | Requires `adapter.namespace`. Owns the shared reduced backend full-cache for config-facing ID-catalog commands, including TTL, reset, and the shared `meta` block. | ID-catalog backend owner | IO runtime | `lib/IoIdCatalog.js` |

### `main.js` composition-root routing boundary

| Entry | Contract | Owner | Reference |
| --- | --- | --- | --- |
| `onReady()` IO bootstrap | Creates `IoSystemBaseline` early and calls `ensure()` as a best-effort startup hardening step, then creates `IoCoreConnection`, initializes `MsgStore`, marks core health, creates `IoPlugins`, and finally creates `IoAdminTab`, `IoWebUi`, and `IoAdminConfig`. Baseline failures are logged as errors but do not stop startup. `main.js` is composition root only, not a separate API owner. | Composition root | `main.js` |
...
| `onMessage(obj)` routing | `internal.*` routes to `_handleInternalCommand(...)`; `admin.*` routes to `IoAdminTab`; `web.*` routes to `IoWebUi`; `config.*` routes to `IoAdminConfig`; `ui.bootstrap` delegates to `IoAdminCapabilities`; all other commands are passed to `IoPlugins.dispatchMessagebox(obj)`. The internal router sorts by exact command name. | Composition root | `main.js` |
...
| `IoPlugins.readAdminUiBundle({ type, panelId, lang })` | Reads the plugin-owned Admin UI JS bundle, optional companion CSS, and optional i18n payload with safe-language fallback to `en`. When `i18n` is present, it has shape `{ lang, translations }`. | IO runtime | `lib/IoPlugins.js` |
...
| --- | --- | --- | --- | --- |
| `internal.uiCatalog.getApp` | `IoUiCatalog.getApp(payload)` via `main.js` | Internal-only adapter bridge for the public web host. Expects the same request shape as `IoUiCatalog.getApp(...)` and returns either the bare app record or `null`. `main.js` adds no host-specific enrichment, filtering, or fallback semantics beyond routing the call to the existing runtime `IoUiCatalog` instance. | Composition root / IO runtime | `main.js`, `web.js`, `lib/IoUiCatalog.js` |
| `internal.IoPlugins.readAdminUiBundle` | `IoPlugins.readAdminUiBundle({ type, panelId, lang, parts: ['i18n'] })` via `main.js` | Internal-only bridge with an i18n-only payload cut. `main.js` always enforces `parts: ['i18n']` server-side, so this path is not a generic JS/CSS bundle bridge. On success it returns the bare `{ i18n }` payload from `IoPlugins.readAdminUiBundle(...)`; when present, `i18n` has shape `{ lang, translations }`. | Composition root / IO runtime | `main.js`, `lib/IoPlugins.js` |
| `ui.bootstrap` | `IoAdminCapabilities.buildBootstrap({ host })` | Neutral bootstrap endpoint. `main.js` sets the host hint server-side with fallback `admin`. Admin-host response is `{ ok: true, data: { capabilities, about } }` with `admin`, `config`, and `web` grants; `webExtension` response carries only `capabilities.web`. Each grant is `{ token, expiresAt }` with a fixed `2h` TTL. | Composition root / IO runtime | `main.js`, `lib/IoAdminCapabilities.js` |
| `POST /MessageHub/<instance>/query` | Public web-host bridge for `ui.bootstrap`, `ui.*`, and `web.*`. Request body is `{ cmd, payload }`; invalid JSON returns `400`, disallowed commands return `403`. The bridge overwrites `payload.host='webExtension'` server-side for every forwarded request. For `ui.bootstrap`, the web host still filters the response to `capabilities.web` before forwarding it to the browser runtime. | Web-host boundary / IO runtime bridge | `web.js`, `admin/tab/runtime.js` |
...
| `admin.*` | `IoAdminTab.handleCommand(cmd, payload, { host })` | Requires `_adminTab`; otherwise returns `NOT_READY`. `main.js` injects the server-side host hint and the facade validates `payload.token` against that host through `IoAdminCapabilities`. | Composition root / IO runtime | `main.js`, `lib/IoAdminTab.js` |
| `web.*` | `IoWebUi.handleCommand(cmd, payload, { host })` | Requires `_webUi`; otherwise returns `NOT_READY`. `main.js` injects the server-side host hint and the facade validates `payload.token` against that host through `IoAdminCapabilities`. | Composition root / IO runtime | `main.js`, `lib/IoWebUi.js` |
| `config.*` | `IoAdminConfig.handleCommand(cmd, payload, { host })` | Requires `_adminConfig`; otherwise returns `NOT_READY`. `main.js` injects the server-side host hint and the facade validates `payload.token` against that host through `IoAdminCapabilities`. | Composition root / IO runtime | `main.js`, `lib/IoAdminConfig.js` |
...
| Entry | Contract | Owner | Reference |
| --- | --- | --- | --- |
| `IoUiCatalog.getView({ mode, targetId? })` | Resolves the canonical backend view payload `{ composition, corePanels, pluginPanels, request }`. `corePanels[*]` and `pluginPanels[*]` may carry `resolvedAppIcons`, which is the catalog-owned effective icon-slot map for panel-app consumers. Each slot value is a host-relative asset path such as `icons/<panel-or-plugin-scope>/<file>`, not an `admin/...` repo path. Plugin-owned Admin-UI i18n is intentionally absent from `pluginPanels[*]`. | IO runtime | `lib/IoUiCatalog.js` |
| `IoUiCatalog.getApp({ mode: 'panel', targetId })` | Panel-only helper. Returns the existing canonical core-panel entry or resolved plugin-panel entry enriched with `resolvedAppIcons`, or `null` when the target is formally valid but unknown, currently unavailable, or has no valid `app` block. Each `resolvedAppIcons[*]` value is a host-relative asset path such as `icons/...`. `composition` mode and invalid targets are rejected with `BAD_REQUEST`. | IO runtime | `lib/IoUiCatalog.js` |
...
| `config.idcatalog.get` | `IoIdCatalog.get(payload)` | Requires a valid config token via `payload.token`. Returns a flat reduced state-object view from the shared backend full-cache. The projection is hard-limited to `_id` plus `common.{name,type,role,unit}`. `data.meta` is shared with `config.idcatalog.openTree` and contains `{ backendDurationMs, createdAt, ttlMs }`. | `{ ok, data: { objects, meta } }` | `lib/IoAdminConfig.js`, `lib/IoIdCatalog.js` |
| `config.idcatalog.openTree` | `IoIdCatalog.openTree(payload)` | Requires a valid config token via `payload.token`. Returns a tree-oriented slice from the same shared backend full-cache used by `config.idcatalog.get`. For non-root `entry`, the response also carries `data.ancestors`, the ordered ancestor path from root to `entry` without auto-loading siblings on those levels. `depth` continues to limit only the downward response shape below `entry`. `data.meta` is the same shared block as for `config.idcatalog.get`. | `{ ok, data: { entry, depth, ancestors, nodes, meta } }` | `lib/IoAdminConfig.js`, `lib/IoIdCatalog.js` |
| `config.idcatalog.reset` | `IoIdCatalog.reset()` | Requires a valid config token via `payload.token`. Clears the shared backend ID-catalog cache completely so the next `get` or `openTree` request rebuilds it from scratch. | `{ ok, data: { reset: true, hadCache } }` | `lib/IoAdminConfig.js`, `lib/IoIdCatalog.js` |
...
| `IoWebUi` owns the migrated web-safe command set | `web.ping`, `web.constants.get`, `web.messages.query`, `web.messages.action`, `web.pluginUi.bundle.get`, and `web.pluginUi.rpc` no longer live under `admin.*` except for the intentionally separate `admin.pluginUi.rpc`. | IO runtime | `lib/IoWebUi.js`, `lib/IoAdminTab.js` |
...
| `IoAdminCapabilities` is the single bootstrap/about authority | `main.js` delegates `ui.bootstrap` to `IoAdminCapabilities`, which owns both capability grants and the `about` sub-payload for every supported host. | Composition root / IO runtime | `main.js`, `lib/IoAdminCapabilities.js` |
