...
| `config.idcatalog.get` | `adapter.getForeignObjectsAsync(filter || '*', 'state')` or callback equivalent | Requires a valid config token via `payload.token`. Optional payload `filter` is treated as plain ioBroker object pattern with fallback `'*'`. Returns a reduced projection of ioBroker `state` objects for config-facing ID pickers and strips `native` from each object to reduce transport payload. | `{ ok, data: { objects } }` | `lib/IoAdminConfig.js` |
