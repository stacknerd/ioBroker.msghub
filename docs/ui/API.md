...
| `pluginPanels[*].resolvedAppIcons` | Optional effective icon-slot map resolved by `IoUiCatalog`. Plugin panels always resolve to the fixed host-owned generic set under `admin/icons/pluginUI/*`; plugin-owned `app.icons` are not part of this consumer contract. | Backend view runtime | `lib/IoUiCatalog.js` |
