...
| `ensureAdminI18nLoaded()` | Loads `i18n/en.json` plus `i18n/<lang>.json` (admin-relative URLs; served from `admin/i18n/` in the repo — unrelated to the backend runtime catalog at `i18n/`) and merges them as `en` fallback overridden by the active language. Returns one shared promise per active language. | Browser runtime | `admin/tab/runtime.js` |
