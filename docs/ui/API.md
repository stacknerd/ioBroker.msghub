...
| `ctx.api.bootstrap.get()` | Calls `ui.bootstrap` and returns the full bootstrap payload `{ capabilities, about }`. | Browser API layer | `admin/tab/api.js`, `admin/tab/runtime.js`, `main.js`, `lib/IoAdminCapabilities.js` |
...
| `ctx.api.bootstrap.get()` | `ui.bootstrap` | No business payload fields are required. The browser runtime owns the bootstrap cache and token refresh path; `ctx.api` returns the full bootstrap payload so consumers read `.about` and `.capabilities` from one canonical entry. | `{ capabilities, about }` | Neutral bootstrap command | `admin/tab/api.js`, `admin/tab/runtime.js`, `main.js`, `lib/IoAdminCapabilities.js` |
...
| `panel.description` | Optional string surfaced through the backend-resolved view payload. Built-in plugin manifests currently use plugin-owned i18n keys here; the AdminTab shell uses the translated value as the tab tooltip after plugin i18n hydration. | Plugin-owned | `lib/IngestStates/manifest.js`, `lib/IoPlugins.js`, `admin/tab/boot.js` |
| `panel.category` | Optional plugin-owned primary access/capability marker (`web`, `admin`, `config`) surfaced through the backend-resolved view payload. It is declared manually by the panel owner and is not derived from individual backend commands. | Plugin-owned | `lib/IngestStates/manifest.js`, `lib/IoPlugins.js` |
