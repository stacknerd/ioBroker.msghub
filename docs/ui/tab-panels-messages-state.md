...
- [`./tab-panels-messages-index.md`](./tab-panels-messages-index.md) creates the state object and injects the active timestamp formatter.
...
3. Provide timestamp formatting and expert-mode detection.
   ...
   - `detectExpertMode()` checks session storage first and then `_system.expertMode` on the current or top window.
...
- request bookkeeping: `requestSeq`, `hasLoadedOnce`, `lastError`
...
- Implementation: `admin/tab/panels/messages/state.js`
- Test: `admin/tab/panels/messages/state.test.js`
