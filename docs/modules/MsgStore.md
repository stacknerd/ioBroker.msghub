...
3. **Reading as a view (not as mutation)**
   - `getMessageByRef`, `getMessagesByLevel`, `getMessages`
...
If a message stays “due”, it will be dispatched again on the next tick – until something updates/removes the message
or moves `notifyAt` into the future.
...
- `addMessage(msg)`
  - If `timing.notifyAt` is missing/not finite: dispatch `"due"` immediately (message is “due now”).
- `updateMessage(...)`
...
- `_pruneOldMessages()` removes expired messages from `fullList` (throttled via `pruneIntervalMs`).
- Side effects when removing expired messages:
  - persist the list
  - dispatch `"expired"` via `MsgNotify` (as an array of removed messages)
  - archive a delete per message with `{ event: "expired" }`
...
- Triggers: archive delete + `"deleted"` notification + persist.

### Read APIs
- `getMessageByRef(ref)`
- `getMessagesByLevel(level)` (also matches numeric strings from imported data via `==`)
- `getMessages()`
