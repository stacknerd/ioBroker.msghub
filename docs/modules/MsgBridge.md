...
Bidirectional integrations need two handlers:
...
3. A bridge integration provides two handlers (`ingest` + `notify`).
4. The adapter calls `MsgBridge.registerBridge(...)` once to register both.
...
### `MsgBridge.registerBridge(options)`
...
- `ingest`: the ingest handler (producer plugin)
- `notify`: the notify handler (notifier plugin)
- plugin ids:
  - `id` (used for both sides), or
  - `ingestId` + `notifyId` (separate ids)
...
- Create a shared context object and pass it to both `ingest` and `notify`
...
- If needed, use different ids (`ingestId` / `notifyId`) to separate concerns in logs and tests
