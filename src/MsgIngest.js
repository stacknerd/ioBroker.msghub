...
class MsgIngest {
	...
	constructor(adapter, msgConstants, msgFactory, msgStore) {
		...
		const store = Object.freeze({
			addMessage: msg => this.msgStore.addMessage(msg),
			updateMessage: (msgOrRef, patch) => this.msgStore.updateMessage(msgOrRef, patch),
			addOrUpdateMessage: msg => this.msgStore.addOrUpdateMessage(msg),
			removeMessage: ref => this.msgStore.removeMessage(ref),
			getMessageByRef: ref => this.msgStore.getMessageByRef(ref),
			getMessages: () => this.msgStore.getMessages(),
		});

		// Factory access is intentionally limited:
		// - createMessage() is for "create" paths only (it always sets timing.createdAt = now).
		const factory = Object.freeze({
			createMessage: data => this.msgFactory.createMessage(data),
		});
