...
class MsgPlugins {
	...
	async _ensurePluginEnabledState({ category, type, instanceId, initialEnabled, defaultOptions }) {
		...
		const existingNative = existingObject && this._isObject(existingObject.native) ? existingObject.native : null;
		...
		await this._setObjectNotExistsAsync(enabledStateId, {
			...
			native: this._isObject(defaultOptions) ? defaultOptions : {},
		});
			...
			try {
				await this._extendObjectAsync(enabledStateId, {
					...
					native: existingNative
						? { ...(this._isObject(defaultOptions) ? defaultOptions : {}), ...existingNative }
						: undefined,
				});
			} catch {
				// swallow (best-effort)
	...
	async _loadPluginOptions({ type, instanceId, defaultOptions }) {
		...
		return this._isObject(native) ? native : this._isObject(defaultOptions) ? defaultOptions : {};
	}
	 ...
	 * @returns {boolean} True when the value is an object (and not an array).
	 */
	_isObject(value) {
