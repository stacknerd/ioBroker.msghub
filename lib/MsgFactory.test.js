...
describe('MsgFactory.createMessage', () => {
	...
	describe('applyPatch', () => {
		it('preserves createdAt and sets updatedAt on updates', () => {
			const { factory } = makeFactory();
		...
		it('sets updatedAt to the current timestamp when title changes', () => {
			const { factory } = makeFactory();
			const originalNow = Date.now;
			Date.now = () => 1111;
			const msg = factory.createMessage(buildBase());
		...
		it('sets updatedAt when only timing.endAt changes', () => {
			const { factory } = makeFactory();
			const originalNow = Date.now;
			Date.now = () => 3000;
		...
		it('rejects ref changes but allows the same ref', () => {
		...
		it('does not set updatedAt when only metrics change', () => {
			const { factory } = makeFactory();
			const originalNow = Date.now;
			Date.now = () => 1000;
			const msg = factory.createMessage(buildBase());
		...
		it('merges timing patches and preserves existing fields', () => {
			const { factory } = makeFactory();
			const originalNow = Date.now;
			Date.now = () => 1000;
		...
		it('removes timing fields when patch sets them to null', () => {
			const { factory } = makeFactory();
			const msg = factory.createMessage(
				buildBase({ timing: { notifyAt: Date.UTC(2025, 0, 1) } }),
			);
		...
		it('clears attachments when patch sets them to null', () => {
			const { factory } = makeFactory();
		...
		it('returns null when called with a string', () => {
			const { factory, logs } = makeFactory();
		...
		it('returns null for invalid message objects', () => {
			const { factory, logs } = makeFactory();
	...
	describe('applySystemPatch', () => {
		it('updates only metrics and keeps updatedAt unchanged', () => {
			const { factory } = makeFactory();
			const originalNow = Date.now;
			Date.now = () => 1000;
			const msg = factory.createMessage(buildBase());

			const metrics = new Map([['temp', { val: 2, unit: 'C', ts: Date.UTC(2025, 0, 1) }]]);
			Date.now = () => 2000;
			const updated = factory.applySystemPatch(msg, { metrics });
			Date.now = originalNow;

			expect(updated.metrics).to.be.instanceOf(Map);
			expect(updated.metrics.get('temp')).to.deep.equal({ val: 2, unit: 'C', ts: Date.UTC(2025, 0, 1) });
			expect(updated.timing.updatedAt).to.equal(undefined);
			expect(updated.timing.createdAt).to.equal(1000);
		});

		it('rejects non-metrics updates', () => {
			const { factory, logs } = makeFactory();
			const msg = factory.createMessage(buildBase());

			const updated = factory.applySystemPatch(msg, { title: 'nope' });
			expect(updated).to.equal(null);
			expect(logs.error.length).to.be.greaterThan(0);
		});
	});
