...
class MsghubStore {
	...
	constructor(adapter, options = {}) {
		if (!adapter) throw new Error("MsghubStore: adapter is required");

		this.adapter = adapter;
		this.metaId = options.metaId || adapter.namespace;
		this.fileName = options.fileName || "messages.json";
		this.writeIntervalMs = typeof options.writeIntervalMs === "number" && Number.isFinite(options.writeIntervalMs)
	...
	async init() {
		...
		if (this.adapter?.log?.debug) {
			this.adapter.log.debug(`MsghubStore initialized: ${this.fileName}, Interval: ${this.writeIntervalMs}ms`);
		}
	}
	 ...
	 */
	async flushPending() {
		if (!this._flushPromise) return this._op;
