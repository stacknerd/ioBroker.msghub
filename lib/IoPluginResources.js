 ...
 *
 * Docs: ../docs/io/IoPluginResources.md
 *
 * Scope (v1)
...
class IoPluginResources {
	...
	setTimeout(fn, delayMs, ...args) {
		// Wrap one-shot timeouts so we can forget the handle after it fires.
		// This prevents unbounded growth of `_timerHandles` for plugins that schedule recurring one-shot timeouts.
		let handle = null;
		const wrapped = (...callArgs) => {
			// Forget first so even a throwing callback can't leak tracking entries.
			if (handle != null) {
		...
		handle = this._timers.setTimeout(wrapped, delayMs, ...args);
