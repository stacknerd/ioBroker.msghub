...
class IoPlugins {
	 ...
	 */
	_wrapIngestHandlerWithManagedMeta(handler, reporter, pluginMeta, optionsApi) {
		const injectCtx = ctx => {
			const base = ctx && typeof ctx === 'object' ? ctx : {};
			const meta = base.meta && typeof base.meta === 'object' ? base.meta : {};
			return { ...base, meta: { ...meta, managedObjects: reporter, plugin: pluginMeta, options: optionsApi } };
		};
	 ...
	 * @returns {any} Wrapped handler in the same "shape" (function remains a function).
	 */
	_wrapNotifyHandlerWithPluginMeta(handler, pluginMeta, optionsApi) {
		const injectCtx = ctx => {
			const base = ctx && typeof ctx === 'object' ? ctx : {};
			const meta = base.meta && typeof base.meta === 'object' ? base.meta : {};
			return { ...base, meta: { ...meta, plugin: pluginMeta, options: optionsApi } };
		};
	 ...
	 */
	_wrapBridgeOrEngageHandlerWithPluginMeta(handler, pluginMeta, optionsApi) {
		const injectCtx = ctx => {
			const base = ctx && typeof ctx === 'object' ? ctx : {};
			const meta = base.meta && typeof base.meta === 'object' ? base.meta : {};
			return { ...base, meta: { ...meta, plugin: pluginMeta, options: optionsApi } };
	...
	async _registerOne({ category, type, instanceId }) {
		if (category === IoPluginsCategories.ingest) {
			...
			try {
				...
				this.msgStore.msgIngest.registerPlugin(
					regId,
					this._wrapIngestHandlerWithManagedMeta(handler, reporter, pluginMeta, optionsApi),
				);
		...
		if (category === IoPluginsCategories.notify) {
			...
			try {
				...
				this.msgStore.msgNotify.registerPlugin(
					regId,
					this._wrapNotifyHandlerWithPluginMeta(handler, pluginMeta, optionsApi),
				);
				this.adapter?.log?.info?.(`IoPlugins: registered '${category}/${regId}'`);
				await this._setPluginStatus({ type, instanceId }, 'running');
			} catch (e) {
		...
		if (category === IoPluginsCategories.bridge) {
			...
			try {
				...
				const handler = plugin.create(optionsWithBase);
				const decoratedHandler = this._wrapBridgeOrEngageHandlerWithPluginMeta(handler, pluginMeta, optionsApi);
