...
const { createOpQueue, isObject, shouldDispatchByAudienceChannels } = require(`${__dirname}/../src/MsgUtils`);
...
const { IoPluginsCategories, IoPluginsCatalog, IoPluginsDescriptors } = require('./index');
...
class IoPlugins {
	 ...
	 *
	 * @param {import('@iobroker/adapter-core').AdapterInstance & { namespace: string, i18n?: ({ t?: Function, getTranslatedObject?: Function } | null), i18nBackend?: ({ t?: Function, getTranslatedObject?: Function } | null) }} adapter Adapter instance (ioBroker).
	 ...
	 * @param {Map<string, { packageRoot?: string }>} [options.pluginDescriptors]
	 *   Resolved plugin descriptor override keyed by type. `packageRoot` is used for plugin-owned assets.
	 * @param {object|null} [options.i18nRegistry] IoRuntimeI18n instance for plugin i18n lifecycle hooks
	 */
	constructor(
		adapter,
		msgStore,
		{ instanceId = 0, catalog = IoPluginsCatalog, pluginDescriptors, i18nRegistry = null } = {},
	) {
		...
		// Private map of type -> resolved plugin package descriptor.
		this._pluginDescriptorsByType = pluginDescriptors instanceof Map ? pluginDescriptors : IoPluginsDescriptors;
		...
		this._runtimeHandlersByRegId = new Map();
		this._resourcesByRegId = new Map();
		this._gateWatchersByStateId = new Map();
		this._gateWatchersByRegId = new Map();
		...
		// Content-hash cache for admin UI bundles (per-process; cleared on restart).
		// Key: `${type}:${panelId}`. Value: 'sha256-<hex>'.
		// Intentionally never invalidated: artifacts are deployment-time and cannot change mid-process.
		this._adminUiBundleHashCache = new Map();

		// Backend i18n registry
		this._i18nRegistry = i18nRegistry;
	}
	 ...
	 * @param {object} [options] Options forwarded to the constructor (advanced/testing). Pass i18nRegistry for plugin i18n lifecycle hooks
	 ...
	 *
	 * @returns {Array<{ category: string, type: string, label?: string, defaultEnabled?: boolean, supportsMultiple?: boolean, supportsChannelRouting?: boolean, defaultOptions?: object, title?: any, description?: any, options?: object }>}
		...
		for (const category of Object.values(IoPluginsCategories)) {
			for (const plugin of this.catalog?.[category] || []) {
				...
				out.push({
					...
					supportsChannelRouting: plugin.supportsChannelRouting === true,
					defaultOptions,
					title: plugin.title,
	...
	/**
	 * Call an optional read-only runtime method on a currently registered plugin handler.
	 *
	 * This is an adapter-internal bridge used by Admin/runtime facades for plugin-owned
	 * diagnostics or snapshots. It intentionally does not know any plugin-specific method names.
	 *
	 * @param {{ type?: string, instanceId?: number, method?: string, args?: any[] }} [options] Call options.
	 * @returns {any|null} Method result, or null when the plugin/method is unavailable.
	 */
	callPluginRuntime(options = {}) {
		...
		const instanceId =
			typeof options?.instanceId === 'number' && Number.isFinite(options.instanceId)
				? Math.trunc(options.instanceId)
				: 0;
		const regId = this._makeRegistrationId({ type, instanceId });
		const handler = this._runtimeHandlersByRegId.get(regId);
	...
	/**
	 * Resolve a plugin package descriptor by type.
	 *
	 * @param {string} type Plugin type.
	 * @returns {{ packageRoot?: string } | null} Resolved descriptor or null.
	 */
	_getPluginDescriptorByType(type) {
		const t = typeof type === 'string' ? type.trim() : '';
		if (!t) {
			return null;
		}
		const descriptor = this._pluginDescriptorsByType.get(t);
		return descriptor && typeof descriptor === 'object' ? descriptor : null;
	}

	/**
	 * Resolve the package root from a plugin descriptor.
	 *
	 * @param {string} type Plugin type.
	 * @returns {string|undefined} Absolute package root or undefined.
	 */
	_getPluginPackageRoot(type) {
		const descriptor = this._getPluginDescriptorByType(type);
		const packageRoot = typeof descriptor?.packageRoot === 'string' ? descriptor.packageRoot.trim() : '';
	...
	async updateInstanceNative(info) {
		return await this._queue(async () => {
			...
			for (const [k, v] of Object.entries(nativePatch)) {
				// Special case: `channel` is a first-class (UI-visible) routing feature, but stored in `native`.
				// `extendObject` merges may not delete keys reliably, so translate "unset" to an explicit empty string.
				if (k === 'channel') {
					nextNative.channel = typeof v === 'string' ? v.trim() : '';
					continue;
				}
	...
	/**
	 * Resolve an i18n key to a translated ioBroker name object via the backend translator.
	 *
	 * @param {string} key i18n key with optional `%s` placeholders.
	 * @param {...any} args Placeholder arguments.
	 * @returns {ioBroker.StringOrTranslated} Translated name object (all available languages).
	 */
	_getTranslatedName(key, ...args) {
		const i18n = this.adapter?.i18nBackend;
	...
	async _ensurePluginEnabledState({ category, type, instanceId, initialEnabled, defaultOptions }) {
		...
		const baseName = this._getTranslatedName('msghub.i18n.core.IoPlugins.base.name', category, type, instanceId);
		const enableName = this._getTranslatedName(
			'msghub.i18n.core.IoPlugins.enable.name',
			category,
			type,
			instanceId,
		);
		const statusName = this._getTranslatedName(
			'msghub.i18n.core.IoPlugins.status.name',
			category,
			type,
			instanceId,
		);
	 ...
	 *
	 * @param {{ category: string, type: string, instanceId: number, regId: string, pluginBaseObjectId: string, manifest: object, channel?: string }} info
	 *   Plugin identity.
	 * @returns {{ category: string, type: string, instanceId: number, regId: string, baseFullId: string, baseOwnId: string, channel: string, manifest: object }}
	 *   Stable meta injected into `ctx.meta.plugin`.
	 */
	_buildPluginMeta({ category, type, instanceId, regId, pluginBaseObjectId, manifest, channel }) {
	...
	/**
	 * Create a gate helper API exposed as `ctx.meta.gates`.
	 *
	 * @param {object} options Options.
	 * @param {object} [options.pluginMeta] Plugin meta (for labels/ownership).
	 * @param {import('./IoPluginResources').IoPluginResources|null} [options.resources] Resource tracker.
	 * @param {{ warn?: (msg: string) => void }} [options.log] Optional logger.
	 * @returns {{ register: (options: object) => ({ dispose: () => void } | null) }} Gates API.
	 */
	/**
	 * Build the gate helper API injected into `ctx.meta.gates`.
	 *
	 * @param {{ pluginMeta?: { regId?: string, baseOwnId?: string }, resources?: import('./IoPluginResources').IoPluginResources|null, log?: { warn?: Function } | null }} [options] Context for the gates API.
	 * @returns {{ register: (options: object) => ({ dispose: () => void } | null) }} Gates API.
	 */
	_createGatesApi({ pluginMeta, resources, log } = {}) {
		...
		const label = baseOwnId || regId || 'Plugin';
		const logger = log && typeof log === 'object' ? log : undefined;

		return Object.freeze({
			register: options => this._registerGateWatcher(options, { regId, label, resources, log: logger }),
		});
	}

	/**
	 * Register a gate watcher and return a disposable handle.
	 *
	 * @param {object} options Gate options.
	 * @param {{ regId?: string, label?: string, resources?: import('./IoPluginResources').IoPluginResources|null, log?: { warn?: Function } | null }} [meta] Gate registration context.
	 * @returns {{ dispose: () => void } | null} Disposable handle (or null on invalid input).
	 */
	_registerGateWatcher(options, { regId, label, resources, log } = {}) {
		const opts = options && typeof options === 'object' ? options : {};
		const id = typeof opts.id === 'string' ? opts.id.trim() : '';
		const op = typeof opts.op === 'string' ? opts.op.trim() : '';

		if (!id || !op) {
			log?.warn?.(`${label}: ctx.meta.gates.register(...) requires id and op`);
			return null;
		}
		...
		const watcher = {
			id,
			op,
			value: Object.prototype.hasOwnProperty.call(opts, 'value') ? opts.value : undefined,
			onOpen: typeof opts.onOpen === 'function' ? opts.onOpen : null,
			onClose: typeof opts.onClose === 'function' ? opts.onClose : null,
			onChange: typeof opts.onChange === 'function' ? opts.onChange : null,
			fireOnInit: opts.fireOnInit === true,
			hasValue: false,
			lastOpen: false,
			regId,
			label,
		...
		this._addGateWatcher(watcher);
		const dispose = () => this._removeGateWatcher(watcher);
		resources?.add?.(dispose);

		this._primeGateWatcher(watcher);

		return Object.freeze({ dispose });
	}

	/**
	 * Add a watcher to the internal registry and subscribe to its state id.
	 *
	 * @param {object} watcher Watcher object.
	 * @returns {void} No return value.
	 */
	_addGateWatcher(watcher) {
		const id = watcher?.id;
		if (!id) {
			return;
		}
		let set = this._gateWatchersByStateId.get(id);
		if (!set) {
			set = new Set();
			this._gateWatchersByStateId.set(id, set);
			this._subscribeGateStateId(id);
		}
		set.add(watcher);

		if (watcher.regId) {
			let byReg = this._gateWatchersByRegId.get(watcher.regId);
			if (!byReg) {
				byReg = new Set();
				this._gateWatchersByRegId.set(watcher.regId, byReg);
			}
			byReg.add(watcher);
		}
	}
	...
	/**
	 * Remove a watcher from the registry and unsubscribe when last watcher is removed.
	 *
	 * @param {object} watcher Watcher object.
	 * @returns {void} No return value.
	 */
	_removeGateWatcher(watcher) {
		const id = watcher?.id;
		if (!id) {
			return;
		}
		const set = this._gateWatchersByStateId.get(id);
		if (!set) {
			return;
		}
		set.delete(watcher);
		if (set.size === 0) {
			this._gateWatchersByStateId.delete(id);
			this._unsubscribeGateStateId(id);
		}
		if (watcher.regId) {
			const byReg = this._gateWatchersByRegId.get(watcher.regId);
			if (byReg) {
				byReg.delete(watcher);
				if (byReg.size === 0) {
					this._gateWatchersByRegId.delete(watcher.regId);
				}
			}
		}
	}
	...
	/**
	 * Subscribe to a gate state id (own or foreign).
	 *
	 * @param {string} id State id.
	 * @returns {void} No return value.
	 */
	_subscribeGateStateId(id) {
		const raw = typeof id === 'string' ? id.trim() : '';
		if (!raw) {
			return;
		}
		const ns = typeof this.adapter?.namespace === 'string' ? this.adapter.namespace.trim() : '';
		const isOwn = !!(ns && (raw === ns || raw.startsWith(`${ns}.`)));
		const ownId = isOwn && raw.startsWith(`${ns}.`) ? raw.slice(ns.length + 1) : raw;

		try {
			if (isOwn && typeof this.adapter?.subscribeStates === 'function') {
				this.adapter.subscribeStates(ownId);
				return;
			}
			if (typeof this.adapter?.subscribeForeignStates === 'function') {
				this.adapter.subscribeForeignStates(raw);
				return;
			}
		} catch (e) {
			this.adapter?.log?.warn?.(`IoPlugins: gate subscribe failed for '${raw}' (${e?.message || e})`);
		}
	}

	/**
	 * Unsubscribe from a gate state id (own or foreign).
	 *
	 * @param {string} id State id.
	 * @returns {void} No return value.
	 */
	_unsubscribeGateStateId(id) {
		const raw = typeof id === 'string' ? id.trim() : '';
		if (!raw) {
			return;
		}
		...
		try {
			if (isOwn && typeof this.adapter?.unsubscribeStates === 'function') {
				this.adapter.unsubscribeStates(ownId);
				return;
			}
			if (typeof this.adapter?.unsubscribeForeignStates === 'function') {
				this.adapter.unsubscribeForeignStates(raw);
				return;
			}
		} catch (e) {
			this.adapter?.log?.warn?.(`IoPlugins: gate unsubscribe failed for '${raw}' (${e?.message || e})`);
		}
	}

	/**
	 * Prime a watcher with the current state value (best-effort).
	 *
	 * @param {object} watcher Watcher object.
	 * @returns {void} No return value.
	 */
	_primeGateWatcher(watcher) {
		const id = watcher?.id;
		if (!id) {
			return;
		}
		this._readGateState(id)
			.then(state => {
				if (state && typeof state === 'object') {
					this._applyGateState(watcher, state, { initial: true });
				}
			})
			.catch(e => {
				watcher.hasValue = true;
				watcher.lastOpen = true;
				watcher?.log?.warn?.(`${watcher.label}: gate getForeignState failed (${e?.message || e})`);
			});
	}

	/**
	 * Read a gate state (own or foreign).
	 *
	 * @param {string} id State id.
	 * @returns {Promise<ioBroker.State | null>} State value or null when not available.
	 */
	_readGateState(id) {
		const raw = typeof id === 'string' ? id.trim() : '';
		if (!raw) {
		...
		if (isOwn && typeof this.adapter?.getStateAsync === 'function') {
			return this.adapter.getStateAsync(ownId).then(state => (state == null ? null : state));
		}
		if (!isOwn && typeof this.adapter?.getForeignStateAsync === 'function') {
			return this.adapter.getForeignStateAsync(raw).then(state => (state == null ? null : state));
		}
		if (isOwn && typeof this.adapter?.getState === 'function') {
			return new Promise((resolve, reject) => {
				this.adapter.getState(ownId, (err, state) => (err ? reject(err) : resolve(state ?? null)));
			});
		}
		if (typeof this.adapter?.getForeignState === 'function') {
			return new Promise((resolve, reject) => {
				this.adapter.getForeignState(raw, (err, state) => (err ? reject(err) : resolve(state ?? null)));
			});
		}
		return Promise.resolve(null);
	}
	...
	/**
	 * Evaluate a gate value against an operator/compare value.
	 *
	 * @param {any} val State value.
	 * @param {string} op Operator.
	 * @param {any} cmp Compare value.
	 * @returns {boolean} True when the gate evaluates as open.
	 */
	_evaluateGateValue(val, op, cmp) {
		if (op === 'true') {
			return val === true;
		}
		if (op === 'false') {
		...
		if (op === '=') {
			...
			return String(val).trim() === String(cmp).trim();
		}
	...
	/**
	 * Apply a new state value to a watcher and emit transition callbacks.
	 *
	 * @param {object} watcher Watcher object.
	 * @param {ioBroker.State} state State object.
	 * @param {{ initial?: boolean }} [options] Options.
	 * @returns {void} No return value.
	 */
	_applyGateState(watcher, state, { initial = false } = {}) {
		if (!watcher || !state || typeof state !== 'object') {
			return;
		}
		const open = this._evaluateGateValue(state.val, watcher.op, watcher.value);
		if (!watcher.hasValue) {
			watcher.lastOpen = open;
			watcher.hasValue = true;
			if (initial && watcher.fireOnInit) {
				this._emitGateChange(watcher, open, undefined, state);
			}
			return;
		}
		if (open === watcher.lastOpen) {
			return;
		}
		const prevOpen = watcher.lastOpen;
		watcher.lastOpen = open;
		this._emitGateChange(watcher, open, prevOpen, state);
	}

	/**
	 * Emit gate callbacks for a watcher.
	 *
	 * @param {object} watcher Watcher object.
	 * @param {boolean} open Current state.
	 * @param {boolean|undefined} prevOpen Previous state.
	 * @param {ioBroker.State} state State object.
	 * @returns {void} No return value.
	 */
	_emitGateChange(watcher, open, prevOpen, state) {
		...
		try {
			...
			watcher?.log?.warn?.(`${watcher.label}: gate onChange failed (${e?.message || e})`);
		}
		const fn = open ? watcher.onOpen : watcher.onClose;
		...
		try {
			...
			if (res && typeof res.then === 'function') {
				res.catch(err => {
					watcher?.log?.warn?.(`${watcher.label}: gate callback failed (${err?.message || err})`);
				});
			}
		} catch (e) {
			watcher?.log?.warn?.(`${watcher.label}: gate callback failed (${e?.message || e})`);
		}
	}

	/**
	 * Dispatch a state change to registered gate watchers.
	 *
	 * @param {string} id State id.
	 * @param {ioBroker.State | null | undefined} state State object.
	 * @returns {boolean} True when any watcher was notified.
	 */
	/**
	 * Dispatch a state change to registered gate watchers.
	 *
	...
	handleGateStateChange(id, state) {
		...
		const watchers = this._gateWatchersByStateId.get(key);
		if (!watchers || watchers.size === 0) {
			return false;
		}
		for (const watcher of watchers) {
			this._applyGateState(watcher, state, { initial: false });
		}
		return true;
	}
	 ...
	 *
	 * - injects `ctx.meta.plugin` (identity), `ctx.meta.options` (manifest-backed resolvers), `ctx.meta.resources`, `ctx.meta.gates`
	...
	_decorateCtxForPlugin(
		...
		// Optional: bind per-plugin identity into ctx.api.log (prefix every log line with baseOwnId).
		if (nextApi?.log && typeof nextApi.log === 'object' && typeof nextApi.log.__bindCaller === 'function') {
			try {
				const boundLog = nextApi.log.__bindCaller(pluginMeta);
				if (boundLog && boundLog !== nextApi.log) {
					nextApi = Object.freeze({ ...nextApi, log: boundLog });
				}
			} catch (e) {
				this.adapter?.log?.warn?.(`IoPlugins: failed to bind ctx.api.log (${e?.message || e})`);
			}
		}

		// Optional: add simple state-template rendering helpers (without changing host APIs).
		//
		// This is intentionally wired here (IoPlugins runtime), not in MsgHostApi, so hosts stay minimal.
		if (
			!nextApi?.templates &&
			nextApi?.iobroker &&
			typeof nextApi.iobroker === 'object' &&
			nextApi.iobroker.states &&
			typeof nextApi.iobroker.states === 'object' &&
			typeof nextApi.iobroker.states.getForeignState === 'function'
		) {
			const baseOwnId = typeof pluginMeta?.baseOwnId === 'string' ? pluginMeta.baseOwnId.trim() : '';
			const label = baseOwnId || 'Plugin';
			const getForeignState = nextApi.iobroker.states.getForeignState;
			const log = nextApi?.log && typeof nextApi.log === 'object' ? nextApi.log : null;

			const templates = Object.freeze({
				/**
				 * Replace `{id}` placeholders in a string with current ioBroker foreign state values.
				 *
				 * - `{id}` can appear anywhere and multiple times.
				 * - Missing/unreadable states resolve to an empty string.
				 *
				 * @param {string} text Template text.
				 * @returns {Promise<string>} Resolved text.
				 */
				renderStates: async text => {
					if (typeof text !== 'string') {
						throw new TypeError(`${label}: ctx.api.templates.renderStates(text) expects a string`);
					}
					if (!text) {
						return '';
					}
					...
					await Promise.all(
						Array.from(ids).map(async id => {
							try {
								const state = await getForeignState(id);
								const val =
									state && Object.prototype.hasOwnProperty.call(state, 'val') ? state.val : '';
								values.set(id, val === null || val === undefined ? '' : String(val));
							} catch (e) {
								values.set(id, '');
								try {
									log?.warn?.(
										`templates.renderStates: getForeignState('${id}') failed: ${e?.message || e}`,
									);
								} catch {
									// ignore
								}
					...
					return text.replace(/\{([^}]+)\}/g, (m, idRaw) => {
						const id = typeof idRaw === 'string' ? idRaw.trim() : '';
						if (!id) {
							return '';
						}
						return values.has(id) ? values.get(id) : '';
					});
				},
			});

			nextApi = Object.freeze({ ...nextApi, templates });
		}

		const gates = this._createGatesApi({ pluginMeta, resources, log: nextApi?.log });
	...
	_wrapIngestHandlerWithManagedMeta(handler, reporter, pluginMeta, optionsApi, resources) {
		...
		const onAction = typeof handler?.onAction === 'function' ? handler.onAction.bind(handler) : null;

		const hasAny = !!(start || stop || onStateChange || onObjectChange || onAction);
		if (!hasAny) {
			return handler;
		}
		...
		return Object.freeze({
			...
			...(onAction
				? {
						onAction: (actionInfo, ctx) => onAction(actionInfo, inject(ctx)),
					}
				: {}),
		});
	}
	...
	_wrapNotifyHandlerWithPluginMeta(handler, pluginMeta, optionsApi, resources) {
		...
		return Object.freeze({
			...
			onNotifications: (event, notifications, ctx) => {
				...
				const supportsRouting = pluginMeta?.manifest?.supportsChannelRouting === true;
				if (!supportsRouting) {
					return onNotifications?.(event, list, inject(ctx));
				}
				const channel = pluginMeta?.channel;
				const filtered = list.filter(n => shouldDispatchByAudienceChannels(n, channel));
				if (filtered.length === 0) {
					return;
				}
				return onNotifications?.(event, filtered, inject(ctx));
			},
		});
	}
	...
	async _registerOne({ category, type, instanceId }) {
		if (category === IoPluginsCategories.ingest) {
			...
			try {
				...
				this._runtimeHandlersByRegId.set(regId, handler);
				this._tryRegisterPluginI18nSource({ category, type, regId });
				...
				this._runtimeHandlersByRegId.delete(regId);
				this._removePluginI18nSource(regId);
	...
	/**
	 * Best-effort plugin i18n registration before host-side plugin start hooks may run.
	 *
	 * MsgIngest/MsgNotify start hooks can execute during `registerPlugin(...)`.
	 * Registering the source first ensures `ctx.api.i18n.t(...)` already sees plugin-owned texts in `start(ctx)`.
	 *
	 * @param {{ category: string, type: string, regId: string }} info Plugin registration info.
	 * @returns {void}
	 */
	_tryRegisterPluginI18nSource({ category, type, regId }) {
		if (!this._i18nRegistry) {
			return;
		}
		try {
			const words = this._loadPluginI18n(this._getPluginPackageRoot(type), type);
			if (words) {
				this._i18nRegistry.addSource(`plugin:${regId}`, 'plugin-runtime', words);
			}
		} catch (i18nErr) {
			this.adapter?.log?.warn?.(
				`IoPlugins: backend i18n load failed for '${category}/${regId}' — plugin stays registered: ${i18nErr instanceof Error ? i18nErr.message : String(i18nErr)}`,
			);
		}
	}

	/**
	 * Best-effort removal of a plugin i18n source.
	 *
	 * Safe for unregister and for register-error rollback.
	 *
	 * @param {string} regId Plugin registration id.
	 * @returns {void}
	 */
	_removePluginI18nSource(regId) {
		if (!this._i18nRegistry) {
			return;
		}
		this._i18nRegistry.removeSource(`plugin:${regId}`);
	}

	/**
	 * Load backend i18n for a plugin from its plugin-owned i18n directories.
	 *
	 * Uses fs.readFileSync + JSON.parse (not require()) to avoid CJS module cache issues during
	 * disable→enable cycles (A10).
	 *
	 * Source order:
	 * - `admin-ui/i18n/` (UI + backend-visible shared plugin texts)
	 * - `i18n/` (backend-only plugin texts)
	 *
	 * Duplicate handling:
	 * - When the same language+key appears twice across these directories, the later occurrence
	 *   is rejected with warn and the first value is kept.
	 *
	 * Failure model (degraded success):
	 * - Missing both plugin i18n directories → return null (no error; plugin runs without i18n).
	 * - Missing individual language file → adapter.log.warn + skip.
	 * - JSON parse error for a language → adapter.log.warn + skip.
	 * - Non-plain-object JSON root → adapter.log.warn + skip.
	 * - Key outside allowed namespace (Namespace Guard ADR-P1) → adapter.log.warn + key dropped.
	 * - Duplicate key across `admin-ui/i18n` and `i18n` → adapter.log.warn + later key dropped.
	 * - No valid keys remain after filtering → return null.
	 *
	 * @param {string|undefined} packageRoot Absolute path to the plugin package root.
	 * @param {string} pluginType Plugin type name from manifest.type (e.g. 'IngestStates').
	 * @returns {{[lang: string]: {[key: string]: string}}|null} Filtered wordsByLang, or null.
	 */
	_loadPluginI18n(packageRoot, pluginType) {
		if (!packageRoot) {
			return null;
		}

		const i18nDirs = [
			{ label: 'admin-ui/i18n', dir: path.join(packageRoot, 'admin-ui', 'i18n') },
			{ label: 'i18n', dir: path.join(packageRoot, 'i18n') },
		].filter(entry => fs.existsSync(entry.dir));
		if (i18nDirs.length === 0) {
			return null;
		}

		const allowedPrefix = `msghub.i18n.${pluginType}.`;
		const langs = ['de', 'en', 'es', 'fr', 'it', 'nl', 'pl', 'pt', 'ru', 'uk', 'zh-cn'];
		const wordsByLang = Object.create(null);

		for (const source of i18nDirs) {
			for (const lang of langs) {
				const filePath = path.join(source.dir, `${lang}.json`);
				let data;
				try {
					data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
				} catch (e) {
					if (e && e.code === 'ENOENT') {
						this.adapter?.log?.warn?.(`Plugin i18n: missing language file, skipping: ${filePath}`);
						continue;
					}
					this.adapter?.log?.warn?.(
						`Plugin i18n: JSON parse error in ${filePath}: ${e instanceof Error ? e.message : String(e)}`,
					);
					continue;
				}

				if (!data || typeof data !== 'object' || Array.isArray(data)) {
					this.adapter?.log?.warn?.(`Plugin i18n: expected plain object in ${filePath}, skipping`);
					continue;
				}

				// Namespace guard (ADR-P1): drop keys outside the plugin's own prefix.
				const filtered = Object.create(null);
				for (const key of Object.keys(data)) {
					if (!key.startsWith(allowedPrefix)) {
						this.adapter?.log?.warn?.(
							`Plugin i18n key outside allowed namespace: ${key} (expected prefix: ${allowedPrefix})`,
						);
						continue;
					}
					if (wordsByLang[lang] && Object.prototype.hasOwnProperty.call(wordsByLang[lang], key)) {
						this.adapter?.log?.warn?.(
							`Plugin i18n duplicate key ignored: ${key} (${source.label}, lang=${lang})`,
						);
						continue;
					}
				...
				if (!wordsByLang[lang]) {
					wordsByLang[lang] = Object.create(null);
				}
				Object.assign(wordsByLang[lang], filtered);
			}
		}

		// Return null when no valid keys survived filtering.
		if (Object.keys(wordsByLang).length === 0) {
			return null;
		}
	...
	async _unregisterOne({ category, type, instanceId }) {
		...
		// i18n hook — remove plugin i18n source on unregister.
		this._removePluginI18nSource(regId);
		this._runtimeHandlersByRegId.delete(regId);
		this._disposeResources(regId);
	...
	createOptionsApi(manifest) {
		...
		const resolveString = (key, val) => {
			const spec = getSpec(key) || {};
			const keepWhitespace = spec.trim === false;
			const def = typeof spec.default === 'string' ? spec.default : '';
			if (val === undefined || val === null) {
				return keepWhitespace ? String(def) : String(def).trim();
			}
			if (typeof val !== 'string') {
				return keepWhitespace ? String(def) : String(def).trim();
			}
			// Always trim by default (prevents trivial whitespace tricks; no further normalization by design).
			return keepWhitespace ? val : val.trim();
		};
	...
	/**
	 * Compute (and cache) the SHA-256 content hash for a plugin admin UI bundle.
	 *
	 * Hash inputs (in order, NUL-separated):
	 *   1. JS bundle file content
	 *   2. CSS companion file content (empty string if absent)
	 *   3. All `admin-ui/i18n/*.json` files, sorted alphabetically by filename,
	 *      each contributed as `<filename>\0<content>`, joined with `\0`
	 *
	 * The hash is lang-independent: all i18n files are included, not just one language.
	 * Filenames are included in the i18n portion so that renames change the hash (per D2).
	 * Result is cached per `${type}:${panelId}` for the process lifetime (per D1).
	 *
	 * No file size limits are applied here — this is an internal hash computation path,
	 * not a payload-serving path. Size enforcement lives in `_pluginUiBundleGet` (per D4).
	 *
	 * @param {{ type?: string, panelId?: string }} [options] Lookup options.
	 * @returns {Promise<string>} Hash string in the form `sha256-<64 hex chars>`.
	 * @throws {Error} With `code: 'NOT_FOUND'` when plugin or panel is not found.
	 * @throws {Error} With `code: 'FORBIDDEN'` when bundle.entry escapes the plugin package root.
	 * @throws {Error} With `code: 'INTERNAL'` when bundle.entry is missing or the JS file is unreadable.
	 */
	async computeAdminUiBundleHash({ type, panelId } = {}) {
		const t = typeof type === 'string' ? type.trim() : '';
		const p = typeof panelId === 'string' ? panelId.trim() : '';

		const cacheKey = `${t}:${p}`;
		if (this._adminUiBundleHashCache.has(cacheKey)) {
			return this._adminUiBundleHashCache.get(cacheKey);
		}

		const entry = this._findCatalogEntryByType(t);
		if (!entry) {
			throw Object.assign(new Error(`IoPlugins.computeAdminUiBundleHash: plugin '${t}' not found`), {
				code: 'NOT_FOUND',
			});
		}

		const packageRoot = this._getPluginPackageRoot(t);
		if (!packageRoot) {
			throw Object.assign(
				new Error(
					`IoPlugins.computeAdminUiBundleHash: no plugin descriptor with packageRoot registered for '${t}'`,
				),
				{ code: 'NOT_FOUND' },
			);
		}

		const panels = entry.plugin?.adminUi?.panels;
		const panel = Array.isArray(panels) ? panels.find(x => typeof x?.id === 'string' && x.id === p) : null;
		if (!panel) {
			throw Object.assign(new Error(`IoPlugins.computeAdminUiBundleHash: panel '${p}' not found in '${t}'`), {
				code: 'NOT_FOUND',
			});
		}

		const bundleEntry = typeof panel?.bundle?.entry === 'string' ? panel.bundle.entry.trim() : '';
		if (!bundleEntry) {
			throw Object.assign(
				new Error(`IoPlugins.computeAdminUiBundleHash: panel '${p}' in '${t}' has no bundle.entry`),
				{ code: 'INTERNAL' },
			);
		}

		// Path traversal guard — same pattern as readAdminUiBundle.
		const resolvedBase = path.resolve(packageRoot);
		const jsPath = path.resolve(packageRoot, bundleEntry);
		if (!jsPath.startsWith(resolvedBase + path.sep)) {
			throw Object.assign(
				new Error(
					`IoPlugins.computeAdminUiBundleHash: bundle.entry '${bundleEntry}' escapes plugin package root`,
				),
				{ code: 'FORBIDDEN' },
			);
		}

		// Read JS content (mandatory).
		let jsContent;
		try {
			...
			throw Object.assign(
				new Error(
					`IoPlugins.computeAdminUiBundleHash: JS bundle not readable for '${t}/${p}': ${e?.message || e}`,
				),
				{ code: 'INTERNAL' },
			);
		}

		// Read CSS companion (optional; empty string if absent).
		const cssPath = jsPath.replace(/\.js$/, '.css');
		let cssContent = '';
		try {
			cssContent = await fs.promises.readFile(cssPath, 'utf8');
		} catch {
			// CSS companion is optional — absence is not an error.
		}

		// Read all admin-ui/i18n/*.json files sorted alphabetically.
		// Each file contributes `<filename>\0<content>` to the hash (per D2: filenames are semantic).
		// ENOENT on readdir → no i18n dir, use empty string. Individual file read errors → warn + '' (per D5).
		const i18nDir = path.resolve(packageRoot, 'admin-ui', 'i18n');
		let i18nContents = '';
		try {
			const filenames = (await fs.promises.readdir(i18nDir)).filter(f => f.endsWith('.json')).sort();
			const parts = await Promise.all(
				filenames.map(async filename => {
					const filePath = path.join(i18nDir, filename);
					try {
						const content = await fs.promises.readFile(filePath, 'utf8');
						return `${filename}\0${content}`;
					} catch (e) {
						if (e?.code === 'ENOENT') {
							return null; // File vanished between readdir and readFile — skip.
						}
						this.adapter?.log?.warn?.(
							`IoPlugins: plugin '${t}' i18n hash — '${filename}' read error, skipped: ${e?.message || e}`,
						);
						return `${filename}\0`;
					}
				}),
			);
			i18nContents = parts.filter(x => x !== null).join('\0');
		} catch (e) {
			if (e?.code !== 'ENOENT') {
				this.adapter?.log?.warn?.(
					`IoPlugins: plugin '${t}' i18n hash — cannot read i18n directory, skipped: ${e?.message || e}`,
				);
			}
			// i18n dir absent or unreadable — hash without i18n content.
		}
		...
		const hexHash = crypto
			...
			.update(i18nContents)
			.digest('hex');
		const hash = `sha256-${hexHash}`;

		this._adminUiBundleHashCache.set(cacheKey, hash);
		return hash;
	}

	/**
	 * Get all admin UI contribution descriptors from currently registered plugin instances.
	 *
	 * Only running (registered) plugin instances that declare `adminUi` in their manifest
	 * are included. Plugins that are configured but not yet started are excluded.
	 *
	 * `bundle.hash` is always `''` in the returned contributions — callers must use
	 * `computeAdminUiBundleHash()` to obtain the real content hash (per D3).
	 *
	 * @returns {Array<{ pluginType: string, instanceId: number, panelId: string, label: string, description: string, category?: string, app?: object, apiVersion: string, bundle: { hash: string } }>}
	 *   Flat list of admin UI panel contributions. `bundle.hash` is always empty string.
	 */
	getAdminUiContributions() {
		const out = [];
		for (const [regId] of this._runtimeHandlersByRegId) {
			const colonIdx = regId.indexOf(':');
			if (colonIdx < 0) {
				continue;
			}
			const type = regId.slice(0, colonIdx);
			const instanceId = parseInt(regId.slice(colonIdx + 1), 10);
			if (!type || !Number.isFinite(instanceId)) {
				continue;
			}
			...
			const adminUi = entry.plugin?.adminUi;
			const panels = adminUi?.panels;
			if (!Array.isArray(panels) || panels.length === 0) {
				continue;
			}

			const apiVersion = typeof adminUi.apiVersion === 'string' ? adminUi.apiVersion : '1';

			for (const panel of panels) {
				const panelId = typeof panel?.id === 'string' ? panel.id.trim() : '';
				const bundleEntry = typeof panel?.bundle?.entry === 'string' ? panel.bundle.entry.trim() : '';
				if (!panelId || !bundleEntry) {
					continue;
				}
				out.push({
					pluginType: type,
	...
	/**
	 * Read plugin-owned Admin UI translations for one plugin package.
	 *
	 * Reads `admin-ui/i18n/<lang>.json` with `en` fallback and applies the same soft-failure
	 * semantics as `readAdminUiBundle()`: missing files yield `null`, malformed or oversized
	 * files are skipped with a warning, and path traversal is rejected.
	 *
	 * @param {{ type?: string, lang?: string }} [options] Translation lookup options.
	 * @returns {Promise<{ lang: string, translations: object }|null>} Parsed translations or null.
	 * @throws {Error} When the plugin cannot be resolved or the i18n path escapes the plugin package root.
	 */
	async readAdminUiTranslations({ type, lang } = {}) {
		const t = typeof type === 'string' ? type.trim() : '';
		if (!t) {
			throw Object.assign(new Error('IoPlugins.readAdminUiTranslations: type is required'), {
				code: 'BAD_REQUEST',
			});
		}
		...
		const entry = this._findCatalogEntryByType(t);
		if (!entry) {
			throw Object.assign(new Error(`IoPlugins.readAdminUiTranslations: plugin '${t}' not found`), {
				code: 'NOT_FOUND',
			});
		}
		...
		if (!packageRoot) {
			throw Object.assign(
				new Error(
					`IoPlugins.readAdminUiTranslations: no plugin descriptor with packageRoot registered for '${t}'`,
				),
				{ code: 'NOT_FOUND' },
			);
		}

		return this._readAdminUiTranslationsFromPackageRoot({ type: t, packageRoot, lang });
	}

	/**
	 * Reads plugin-owned Admin UI translations from a resolved plugin package root.
	 *
	 * @param {{ type?: string, packageRoot?: string, lang?: string }} options Translation lookup options.
	 * @returns {Promise<{ lang: string, translations: object }|null>} Parsed translations or null.
	 * @throws {Error} When the i18n path escapes the plugin package root.
	 */
	async _readAdminUiTranslationsFromPackageRoot(options = {}) {
		const { type, packageRoot, lang } = options || {};
		const t = typeof type === 'string' ? type.trim() : '';
		const resolvedPackageRoot = typeof packageRoot === 'string' ? packageRoot : '';
		const resolvedBase = path.resolve(resolvedPackageRoot);
		const rawLang = typeof lang === 'string' ? lang.trim().toLowerCase() : 'en';
		const safeLang = /^[a-z]{2}(-[a-z]{2,4})?$/.test(rawLang) ? rawLang : 'en';
		const I18N_LIMIT = 64 * 1024;

		/**
		 * Attempts to read and parse one i18n language file.
		 * Returns { lang, translations } on success, null on absence or soft failure.
		 * Throws on path traversal (FORBIDDEN).
		 *
		 * @param {string} targetLang - Language code to try (e.g. 'de', 'en').
		 * @returns {Promise<{ lang: string, translations: object }|null>} Parsed i18n entry or null.
		 */
		const tryReadI18n = async targetLang => {
			const i18nPath = path.resolve(resolvedPackageRoot, 'admin-ui', 'i18n', `${targetLang}.json`);
			if (!i18nPath.startsWith(resolvedBase + path.sep)) {
				throw Object.assign(
					new Error(`IoPlugins.readAdminUiTranslations: i18n path escapes plugin package root`),
					{ code: 'FORBIDDEN' },
				);
			}
			let content;
			try {
				content = await fs.promises.readFile(i18nPath, 'utf8');
			} catch (e) {
				if (e?.code === 'ENOENT') {
					return null;
				}
				this.adapter?.log?.warn?.(
					`IoPlugins: plugin '${t}' i18n '${targetLang}.json' read error — skipped: ${e?.message || e}`,
				);
				return null;
			}
			if (Buffer.byteLength(content, 'utf8') > I18N_LIMIT) {
				this.adapter?.log?.warn?.(
					`IoPlugins: plugin '${t}' i18n '${targetLang}.json' exceeds ${I18N_LIMIT} byte limit — skipped`,
				);
				return null;
			}
			let parsed;
			try {
				...
				this.adapter?.log?.warn?.(
					`IoPlugins: plugin '${t}' i18n '${targetLang}.json' JSON parse error — skipped`,
				);
				return null;
			}
			if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
				this.adapter?.log?.warn?.(
					`IoPlugins: plugin '${t}' i18n '${targetLang}.json' is not a plain object — skipped`,
				);
				return null;
			}
			return { lang: targetLang, translations: parsed };
		};

		let i18n = await tryReadI18n(safeLang);
		if (i18n === null && safeLang !== 'en') {
			i18n = await tryReadI18n('en');
		}
		return i18n;
	}

	/**
	 * Read the admin UI bundle files for a specific plugin panel.
	 *
	 * Resolves the plugin package root from the descriptor (not from the type string alone)
	 * to avoid brittle repo-path construction. Reads the JS bundle and optional companion CSS file.
	 *
	 * @param {{ type?: string, instanceId?: number, panelId?: string, lang?: string, parts?: string[]|null }} [options]
	 *   Bundle lookup options. `parts === null` means the full bundle.
	 * @returns {Promise<{ js?: string, css?: string|null, i18n?: { lang: string, translations: object }|null }>}
	 *   Bundle content for the requested parts.
	 * @throws {Error} When plugin or panel not found, or bundle file cannot be read.
	 */
	async readAdminUiBundle({ type, panelId, lang, parts = null } = {}) {
		const t = typeof type === 'string' ? type.trim() : '';
		const p = typeof panelId === 'string' ? panelId.trim() : '';
		if (!t || !p) {
			throw Object.assign(new Error('IoPlugins.readAdminUiBundle: type and panelId are required'), {
				code: 'BAD_REQUEST',
			});
		}
		...
		const entry = this._findCatalogEntryByType(t);
		if (!entry) {
			throw Object.assign(new Error(`IoPlugins.readAdminUiBundle: plugin '${t}' not found`), {
				code: 'NOT_FOUND',
			});
		}
		...
		if (!packageRoot) {
			throw Object.assign(
				new Error(`IoPlugins.readAdminUiBundle: no plugin descriptor with packageRoot registered for '${t}'`),
				{
					code: 'NOT_FOUND',
				},
			);
		...
		const panel = Array.isArray(panels) ? panels.find(x => typeof x?.id === 'string' && x.id === p) : null;
		if (!panel) {
			throw Object.assign(new Error(`IoPlugins.readAdminUiBundle: panel '${p}' not found in '${t}'`), {
				code: 'NOT_FOUND',
			});
		}
		...
		const bundleEntry = typeof panel?.bundle?.entry === 'string' ? panel.bundle.entry.trim() : '';
		if (!bundleEntry) {
			throw Object.assign(new Error(`IoPlugins.readAdminUiBundle: panel '${p}' in '${t}' has no bundle.entry`), {
				code: 'INTERNAL',
			});
		}

		// Resolve and constrain to packageRoot to prevent path traversal via crafted bundle.entry values.
		const resolvedBase = path.resolve(packageRoot);
		const jsPath = path.resolve(packageRoot, bundleEntry);
		if (!jsPath.startsWith(resolvedBase + path.sep)) {
			throw Object.assign(
				new Error(`IoPlugins.readAdminUiBundle: bundle.entry '${bundleEntry}' escapes plugin package root`),
				{ code: 'FORBIDDEN' },
			);
		}
		const cssPath = jsPath.replace(/\.js$/, '.css');
		const requestedParts = Array.isArray(parts) ? parts : null;
		const includeJs = !requestedParts || requestedParts.includes('js');
		const includeCss = !requestedParts || requestedParts.includes('css');
		const includeI18n = !requestedParts || requestedParts.includes('i18n');
		const out = {};

		if (includeJs) {
			out.js = await fs.promises.readFile(jsPath, 'utf8');
		}

		if (includeCss) {
			try {
				out.css = await fs.promises.readFile(cssPath, 'utf8');
			} catch {
				// Companion CSS is optional — absence is not an error.
				out.css = null;
			}
		}

		if (includeI18n) {
			out.i18n = await this._readAdminUiTranslationsFromPackageRoot({ type: t, packageRoot, lang });
		}
