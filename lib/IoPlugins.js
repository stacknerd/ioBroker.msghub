...
class IoPlugins {
	 ...
	 */
	_wrapIngestHandlerWithManagedMeta(handler, reporter) {
		const injectCtx = ctx => {
			const base = ctx && typeof ctx === 'object' ? ctx : {};
			const meta = base.meta && typeof base.meta === 'object' ? base.meta : {};
			return { ...base, meta: { ...meta, managedObjects: reporter } };
		};
		...
		if (category === IoPluginsCategories.ingest) {
			...
			try {
				...
				this.msgStore.msgIngest.registerPlugin(
					regId,
					this._wrapIngestHandlerWithManagedMeta(handler, reporter),
				);
