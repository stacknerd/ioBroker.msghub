...
class IoPlugins {
	...
	async _registerOne({ category, type, instanceId }) {
		if (category === IoPluginsCategories.ingest) {
			...
			// i18n hook — decoupled from plugin registration (degraded success model).
			// Errors during i18n load or source registration are logged as warnings only;
			// plugin registration is never rolled back due to i18n errors.
