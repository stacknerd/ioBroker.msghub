...
class IoPlugins {
	 ...
	 */
	_normalizeRoutingChannel(value) {
	 ...
	 */
	_normalizeRoutingList(list) {
		if (!Array.isArray(list)) {
			return [];
		}
		const out = [];
		for (const item of list) {
			const s = this._normalizeRoutingChannel(item);
			if (s) {
				out.push(s);
			}
		}
	...
	/**
	 * Decide whether a notification message should be dispatched to a plugin channel.
	 *
	 * Semantics:
	 * - plugin channel empty => only unscoped messages (include empty)
	 * - plugin channel set => exclude blocks, include restricts
	 *
	 * @param {any} message Notification payload (message object).
	 * @param {any} pluginChannel Plugin channel value.
	 * @returns {boolean} True when the message should be dispatched.
	 */
	_shouldDispatchByAudienceChannels(message, pluginChannel) {
		const channel = this._normalizeRoutingChannel(pluginChannel);
		...
		const include = this._normalizeRoutingList(channels && typeof channels === 'object' ? channels.include : null);
		const exclude = this._normalizeRoutingList(channels && typeof channels === 'object' ? channels.exclude : null);
	...
	_wrapNotifyHandlerWithPluginMeta(handler, pluginMeta, optionsApi, resources) {
		...
		return Object.freeze({
			...
			onNotifications: (event, notifications, ctx) => {
				...
				const filtered = list.filter(n => this._shouldDispatchByAudienceChannels(n, channel));
