...
class IoPlugins {
	 ...
	 * @param {object|null} [options.i18nRegistry] IoRuntimeI18n instance for plugin i18n lifecycle hooks (Etappe 4).
	 */
	constructor(
		adapter,
		msgStore,
		...
		// Backend i18n registry — used in Etappe 4 for plugin i18n lifecycle hooks.
	 ...
	 * @param {object} [options] Options forwarded to the constructor (advanced/testing). Pass i18nRegistry for plugin i18n lifecycle hooks (Etappe 4).
	 ...
	 *
	 * @returns {Array<{ pluginType: string, instanceId: number, panelId: string, title: object, description: object, apiVersion: string, bundle: { hash: string } }>}
		...
		for (const [regId] of this._runtimeHandlersByRegId) {
			...
			for (const panel of panels) {
				...
				out.push({
					...
					instanceId,
					panelId,
					title: panel.title && typeof panel.title === 'object' ? panel.title : {},
					description: panel.description && typeof panel.description === 'object' ? panel.description : {},
					apiVersion,
					bundle: { hash: '' },
				});
			}
	...
	async readAdminUiBundle({ type, panelId, lang } = {}) {
		...
		// Read plugin-owned i18n for the requested language.
		// Normalize lang to a safe base code; IoAdminTab pre-validates, this is defense-in-depth.
		...
		const tryReadI18n = async targetLang => {
			const i18nPath = path.resolve(pluginDir, 'admin-ui', 'i18n', `${targetLang}.json`);
			// Defense-in-depth: prevent traversal even if lang was not pre-sanitized by caller.
			if (!i18nPath.startsWith(resolvedBase + path.sep)) {
				throw Object.assign(new Error(`IoPlugins.readAdminUiBundle: i18n path escapes plugin directory`), {
					code: 'FORBIDDEN',
				});
			}
			let content;
			try {
				...
				if (e?.code === 'ENOENT') {
					return null; // File absent — not an error.
				}
				this.adapter?.log?.warn?.(
