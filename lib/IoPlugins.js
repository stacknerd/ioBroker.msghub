...
const { buildIoBrokerApi, buildActionApi } = require(`${__dirname}/../src/MsgHostApi`);
...
class IoPlugins {
	...
	async _ensureManagedMetaAsync({ id, managedBy, managedText }) {
		...
		const obj = await this._metaIoBroker.objects.getForeignObject(id);
		if (!obj) {
			return;
		}
		...
		const nativeNeedsUpdate =
			(hasManagedBy && existingNativeMeta.managedBy !== managedBy) ||
			(hasManagedText &&
				(typeof existingNativeMeta.managedText !== 'string' ||
					existingNativeMeta.managedText !== managedText)) ||
			typeof existingNativeMeta.managedSince !== 'string';

		const customKey =
			typeof this._metaIoBroker?.ids?.namespace === 'string' ? this._metaIoBroker.ids.namespace : '';
		const existingCustom =
			customKey && isObject(obj.common) && isObject(obj.common.custom) ? obj.common.custom[customKey] : null;
		...
		const customNeedsUpdate =
			existingCustom &&
			(!existingCustomMeta ||
				(hasManagedBy && existingCustomMeta.managedBy !== managedBy) ||
				(hasManagedText && existingCustomMeta.managedText !== managedText) ||
		...
		if (nativeNeedsUpdate) {
			patch.native = {
				meta: {
					...existingNativeMeta,
					managedBy: hasManagedBy ? managedBy : existingNativeMeta.managedBy,
					managedText: hasManagedText ? managedText : existingNativeMeta.managedText,
					managedSince:
				...
				custom: {
					[customKey]: {
						meta: {
							...
							managedBy: hasManagedBy ? managedBy : existingCustomMeta?.managedBy,
							managedText: hasManagedText ? managedText : existingCustomMeta?.managedText,
							managedSince:
		...
		try {
			await this._metaIoBroker.objects.extendForeignObject(id, patch);
		} catch (e) {
			this.adapter?.log?.warn?.(`IoPlugins: failed to set managed meta on '${id}': ${e?.message || e}`);
		}
	}
	...
	constructor(adapter, msgStore, { instanceId = 0, catalog = IoPluginsCatalog } = {}) {
		...
		this._metaIoBroker = buildIoBrokerApi(this.adapter, { hostName: 'IoPlugins' });
		this._watchlists = new Map(); // `${type}.${instanceId}` -> { ids:Set<string>, timer:any }
		this._ingestMeta = Object.freeze({
			managedObjects: Object.freeze({
				report: async (ids, meta) => {
					const safeMeta = meta && typeof meta === 'object' && !Array.isArray(meta) ? meta : null;
					const managedBy = safeMeta ? Reflect.get(safeMeta, 'managedBy') : undefined;
					const managedText = safeMeta ? Reflect.get(safeMeta, 'managedText') : undefined;
					const list = Array.isArray(ids) ? ids : [ids];
					const parsed = this._parsePluginBaseObjectIdFromManagedBy(managedBy);
					if (parsed) {
						this._addToWatchlist(parsed, list);
					}
	...
	/**
	 * Meta bundle passed to `MsgIngest.start(...)` so ingest plugins can report managed object ids.
	 *
	 * @returns {{ managedObjects: { report: Function } }} Meta object (contains functions).
	 */
	getIngestMeta() {
		return this._ingestMeta;
	}
	...
	_getPluginWatchlistOwnId({ type, instanceId }) {
		return `${type}.${instanceId}.status.watchlist`;
	}

	/**
	 * Check whether a plugin type exists in the catalog (any category).
	 *
	 * @param {string} type Plugin type.
	 * @returns {boolean} True when the type exists in the catalog.
	 */
	_isKnownPluginType(type) {
		return (
			this._ingestByType.has(type) ||
			this._notifyByType.has(type) ||
			this._bridgeByType.has(type) ||
			this._engageByType.has(type)
		);
	}
	 ...
	 *
	 * This is intentionally a "plausibility check" only:
	 * - it must match `"<namespace>.<PluginType>.<instanceId>"`
	 * - the type must exist in the current catalog
	 *
	 * @param {any} managedBy Candidate managedBy value.
	 * @returns {{ type: string, instanceId: number } | null} Parsed identity or null when not plausible.
	 */
	_parsePluginBaseObjectIdFromManagedBy(managedBy) {
		...
		const re = new RegExp(`^${IoPlugins._escapeRegExp(ns)}\\.([A-Za-z][A-Za-z0-9_]*)\\.(\\d+)$`);
		...
		if (!this._isKnownPluginType(type)) {
			return null;
		}
		return { type, instanceId };
	}
	...
	/**
	 * Add ids to the watchlist for a plugin instance and schedule a best-effort state write.
	 *
	 * @param {{ type: string, instanceId: number }} identity Plugin identity.
	 * @param {string|string[]} ids ioBroker ids to add.
	 * @returns {void}
	 */
	_addToWatchlist({ type, instanceId }, ids) {
		const key = `${type}.${instanceId}`;
		let entry = this._watchlists.get(key);
		if (!entry) {
			entry = { ids: new Set(), timer: null };
			this._watchlists.set(key, entry);
		}
		const list = Array.isArray(ids) ? ids : [ids];
		for (const id of list) {
		...
		this._scheduleWatchlistWrite({ type, instanceId }, entry);
	}

	/**
	 * Throttle watchlist writes into the plugin status state.
	 *
	 * @param {{ type: string, instanceId: number }} identity Plugin identity.
	 * @param {{ ids: Set<string>, timer: any }} entry Watchlist entry.
	 * @returns {void}
	 */
	_scheduleWatchlistWrite({ type, instanceId }, entry) {
		if (entry.timer) {
			return;
		}
		entry.timer = setTimeout(async () => {
			...
			try {
				...
				arr.sort();
				await this._setStateAckAsync(ownId, JSON.stringify(arr));
			} catch {
				// swallow (best-effort)
			}
		}, 500);
	...
	/**
	 * Clear cached watchlist and reset the watchlist status state to `[]`.
	 *
	 * @param {{ type: string, instanceId: number }} identity Plugin identity.
	 * @returns {Promise<void>} Resolves after best-effort reset.
	 */
	async _clearWatchlist({ type, instanceId }) {
		const key = `${type}.${instanceId}`;
		const entry = this._watchlists.get(key);
		if (entry?.timer) {
			clearTimeout(entry.timer);
		}
		if (entry?.ids && typeof entry.ids.clear === 'function') {
			entry.ids.clear();
		}
		this._watchlists.delete(key);
		try {
			await this._setStateAckAsync(this._getPluginWatchlistOwnId({ type, instanceId }), '[]');
		} catch {
			// swallow (best-effort)
		}
	}
	 ...
	 * @returns {Promise<{ baseObjectId: string, baseObjectIdFull: string, enabledStateId: string, enabledStateIdFull: string, statusStateId: string, statusStateIdFull: string, watchlistStateId: string, watchlistStateIdFull: string, enabled: boolean }>}
	 *   Enable/status info.
	 */
	async _ensurePluginEnabledState({ category, type, instanceId, initialEnabled, defaultOptions }) {
		...
		const watchlistStateIdFull = this._toFullId(watchlistStateId);
		...
		// Ensure watchlist state exists (read-only).
	...
	async _registerOne({ category, type, instanceId }) {
		if (category === IoPluginsCategories.ingest) {
			...
			try {
				...
				this.msgStore.msgIngest.registerPlugin(regId, plugin.create(optionsWithBase));
		...
		if (category === IoPluginsCategories.notify) {
			...
			try {
				...
				this.msgStore.msgNotify.registerPlugin(regId, plugin.create(optionsWithBase));
				this.adapter?.log?.info?.(`IoPlugins: registered '${category}/${regId}'`);
				await this._setPluginStatus({ type, instanceId }, 'running');
			} catch (e) {
				this._registered[category].delete(regId);
		...
		if (category === IoPluginsCategories.bridge) {
			...
			try {
				...
				const handle = MsgBridge.registerBridge(regId, handler, {
			...
			try {
				...
				const wrappedAction =
					baseAction && typeof baseAction.execute === 'function'
						...
						: null;
				const handle = MsgEngage.registerEngage(regId, handler, {
					msgIngest: this.msgStore.msgIngest,
					msgNotify: this.msgStore.msgNotify,
	...
	async _unregisterOne({ category, type, instanceId }) {
		...
		await this._clearWatchlist({ type, instanceId });
		this._registered[category].delete(regId);
