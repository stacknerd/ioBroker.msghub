...
class IoPlugins {
	 ...
	 *
	 * @param {{ type?: string, instanceId?: number, panelId?: string, lang?: string }} [options] Bundle lookup options.
	 * @returns {Promise<{ js: string, css: string|null, i18n: { lang: string, translations: object }|null }>} Bundle content.
	 * @throws {Error} When plugin or panel not found, or bundle file cannot be read.
	 */
	async readAdminUiBundle({ type, panelId, lang } = {}) {
		const t = typeof type === 'string' ? type.trim() : '';
		const p = typeof panelId === 'string' ? panelId.trim() : '';
		if (!t || !p) {
		...
		const i18n = await this._readAdminUiTranslationsFromPackageRoot({ type: t, packageRoot, lang });
