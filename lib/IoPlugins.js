...
const { IoPluginsCategories, IoPluginsCatalog, IoPluginsPluginDirs } = require('./index');
...
class IoPlugins {
	 ...
	 * @param {Map<string,string>} [options.pluginDirs] Plugin directory map override (type -> absolute path; defaults to `IoPluginsPluginDirs`).
	 * @param {object|null} [options.i18nRegistry] IoRuntimeI18n instance for plugin i18n lifecycle hooks
	 */
	constructor(
		adapter,
		msgStore,
		{ instanceId = 0, catalog = IoPluginsCatalog, pluginDirs, i18nRegistry = null } = {},
	) {
		...
		// Private map of type -> absolute plugin directory path (filesystem detail kept out of catalog).
		this._pluginDirsByType = pluginDirs instanceof Map ? pluginDirs : IoPluginsPluginDirs;
	...
	_tryRegisterPluginI18nSource({ category, type, regId }) {
		...
		try {
			const words = this._loadPluginI18n(this._pluginDirsByType.get(type), type);
			if (words) {
	 ...
	 *
	 * @param {string|undefined} pluginDir Absolute path to the plugin directory (undefined when not in pluginDirs map).
	 ...
	 */
	_loadPluginI18n(pluginDir, pluginType) {
		if (!pluginDir) {
			return null;
		}

		const i18nDirs = [
			{ label: 'admin-ui/i18n', dir: path.join(pluginDir, 'admin-ui', 'i18n') },
			{ label: 'i18n', dir: path.join(pluginDir, 'i18n') },
	 ...
	 * @throws {Error} With `code: 'FORBIDDEN'` when bundle.entry escapes the plugin directory.
	...
	async computeAdminUiBundleHash({ type, panelId } = {}) {
		...
		const pluginDir = this._pluginDirsByType.get(t);
		if (!pluginDir) {
			throw Object.assign(
				new Error(`IoPlugins.computeAdminUiBundleHash: no plugin directory registered for '${t}'`),
				{ code: 'NOT_FOUND' },
			);
		}
		...
		const jsPath = path.resolve(pluginDir, bundleEntry);
		if (!jsPath.startsWith(resolvedBase + path.sep)) {
			throw Object.assign(
				new Error(`IoPlugins.computeAdminUiBundleHash: bundle.entry '${bundleEntry}' escapes plugin directory`),
				{ code: 'FORBIDDEN' },
			);
		}
		...
		const i18nDir = path.resolve(pluginDir, 'admin-ui', 'i18n');
		let i18nContents = '';
		try {
	 ...
	 *
	 * @returns {Array<{ pluginType: string, instanceId: number, panelId: string, label: string, description: string, surface?: string, category?: string, app?: object, apiVersion: string, bundle: { hash: string } }>}
		...
		for (const [regId] of this._runtimeHandlersByRegId) {
			...
			for (const panel of panels) {
				...
				out.push({
					...
					...(typeof panel.surface === 'string' ? { surface: panel.surface } : {}),
	 ...
	 * @throws {Error} When the plugin cannot be resolved or the i18n path escapes the plugin directory.
	 */
	async readAdminUiTranslations({ type, lang } = {}) {
		...
		if (!pluginDir) {
			throw Object.assign(
				new Error(`IoPlugins.readAdminUiTranslations: no plugin directory registered for '${t}'`),
				{ code: 'NOT_FOUND' },
			);
		}

		return this._readAdminUiTranslationsFromPluginDir({ type: t, pluginDir, lang });
	}

	/**
	 * Reads plugin-owned Admin UI translations from a resolved plugin directory.
	 *
	 * @param {{ type?: string, pluginDir?: string, lang?: string }} options Translation lookup options.
	 * @returns {Promise<{ lang: string, translations: object }|null>} Parsed translations or null.
	 * @throws {Error} When the i18n path escapes the plugin directory.
	 */
	async _readAdminUiTranslationsFromPluginDir(options = {}) {
		const { type, pluginDir, lang } = options || {};
		const t = typeof type === 'string' ? type.trim() : '';
		const resolvedPluginDir = typeof pluginDir === 'string' ? pluginDir : '';
		const resolvedBase = path.resolve(resolvedPluginDir);
		...
		const tryReadI18n = async targetLang => {
			const i18nPath = path.resolve(resolvedPluginDir, 'admin-ui', 'i18n', `${targetLang}.json`);
			if (!i18nPath.startsWith(resolvedBase + path.sep)) {
				throw Object.assign(
					new Error(`IoPlugins.readAdminUiTranslations: i18n path escapes plugin directory`),
					{ code: 'FORBIDDEN' },
				);
			}
			let content;
	 ...
	 *
	 * Resolves the plugin directory from the catalog entry (not from the type string) to
	 * avoid brittle path construction. Reads the JS bundle and optional companion CSS file.
	 *
	...
	async readAdminUiBundle({ type, panelId, lang } = {}) {
		...
		const pluginDir = this._pluginDirsByType.get(t);
		if (!pluginDir) {
			throw Object.assign(new Error(`IoPlugins.readAdminUiBundle: no plugin directory registered for '${t}'`), {
				code: 'NOT_FOUND',
			});
		}
		...
		// Resolve and constrain to pluginDir to prevent path traversal via crafted bundle.entry values.
		const resolvedBase = path.resolve(pluginDir);
		const jsPath = path.resolve(pluginDir, bundleEntry);
		if (!jsPath.startsWith(resolvedBase + path.sep)) {
			throw Object.assign(
				new Error(`IoPlugins.readAdminUiBundle: bundle.entry '${bundleEntry}' escapes plugin directory`),
				{ code: 'FORBIDDEN' },
			);
		}
		const cssPath = jsPath.replace(/\.js$/, '.css');
		...
		const i18n = await this._readAdminUiTranslationsFromPluginDir({ type: t, pluginDir, lang });
