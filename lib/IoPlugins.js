...
class IoPlugins {
	 ...
	 *
	 * @returns {Array<{ pluginType: string, instanceId: number, panelId: string, title: object, description: object, placement: string, order: number, apiVersion: string, bundle: { hash: string } }>}
		...
		for (const [regId] of this._runtimeHandlersByRegId) {
			...
			for (const panel of panels) {
				...
				out.push({
					...
					placement: typeof panel.placement === 'string' ? panel.placement : 'plugin-instance',
					order: typeof panel.order === 'number' ? panel.order : 999,
					apiVersion,
					bundle: { hash },
				});
			}
	 ...
	 *
	 * @param {{ type?: string, instanceId?: number, panelId?: string }} [options] Bundle lookup options.
	 * @returns {Promise<{ js: string, css: string|null }>} Bundle content.
	 * @throws {Error} When plugin or panel not found, or bundle file cannot be read.
	 */
	async readAdminUiBundle({ type, panelId } = {}) {
		const t = typeof type === 'string' ? type.trim() : '';
