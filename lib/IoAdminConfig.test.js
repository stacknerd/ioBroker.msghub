...
describe('IoAdminConfig id catalog routing', () => {
	function createAdapter() {
		return {
			namespace: 'msghub.0',
			log: { warn: () => undefined, info: () => undefined, debug: () => undefined, error: () => undefined },
	...
	it('delegates get and openTree with the cleaned payload to IoIdCatalog', async () => {
		const adapter = createAdapter();
		const capabilities = new IoAdminCapabilities(adapter);
		const calls = [];
		const idCatalog = {
			async get(payload) {
				...
				return { ok: true, data: { objects: {}, meta: { backendDurationMs: 1, createdAt: 123, ttlMs: 456 } } };
			},
			async openTree(payload) {
				calls.push(['openTree', payload]);
				return { ok: true, data: { entry: '', depth: 1, nodes: [], meta: { backendDurationMs: 1, createdAt: 123, ttlMs: 456 } } };
			},
			reset() {
				calls.push(['reset', undefined]);
				return { ok: true, data: { reset: true, hadCache: false } };
			},
		};
		const config = new IoAdminConfig(adapter, { adminCapabilities: capabilities, idCatalog });

		const token = capabilities.mintToken({ host: 'admin', capability: 'config' }).token;
		const getRes = await config.handleCommand('config.idcatalog.get', { token, filter: 'zigbee.0.*' });
		const treeRes = await config.handleCommand('config.idcatalog.openTree', { token, entry: 'javascript.0', depth: 2 });
		const resetRes = await config.handleCommand('config.idcatalog.reset', { token });

		expect(getRes.ok).to.equal(true);
		expect(treeRes.ok).to.equal(true);
		expect(resetRes.ok).to.equal(true);
		expect(calls).to.deep.equal([
			['get', { filter: 'zigbee.0.*' }],
			['openTree', { entry: 'javascript.0', depth: 2 }],
			['reset', undefined],
		]);
	});
});
