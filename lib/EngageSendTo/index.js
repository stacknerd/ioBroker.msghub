 ...
 * - Implements a command API: create/createIfAbsent/patch/patchIfPresent/upsert/remove/get/list/action.
 */

'use strict';

const { manifest } = require('./manifest');
const { isObject, normalizeCreateLikePayload, normalizePatchLikePayload, toJsonSafe } = require('./transport');
...
function EngageSendTo(options = {}) {
	...
	if (!messagebox || typeof messagebox.register !== 'function' || typeof messagebox.unregister !== 'function') {
		throw new Error('options.__messagebox.register/unregister are required (IoPlugins wiring required)');
	}
	...
	const ok = data => {
		log.debug(`ok (${summarize(data)})`);
		return { ok: true, data };
	};
	const err = (code, message, details = undefined) => {
		log.warn(`err ${String(code)}: ${String(message)}${details !== undefined ? ` (${summarize(details)})` : ''}`);
		return {
			ok: false,
			error: {
				code,
	...
	const createMessage = payload => {
		const normalized = normalizeCreateLikePayload(payload);
		if (!isObject(normalized)) {
			return null;
		}
		return factory.createMessage(normalized);
	};

	async function onMessage(obj) {
		log.silly(`onMessage ${summarize(obj)})`);
		...
		if (command === 'create' || command === 'createIfAbsent') {
			if (!isObject(payload)) {
				return err('BAD_REQUEST', `${command}: payload must be an object`);
			}
			const msg = createMessage(payload);
			if (!msg) {
				return err('VALIDATION_FAILED', `${command}: invalid message payload`);
			}
			if (command === 'createIfAbsent') {
				const existing = store.getMessageByRef(msg.ref, 'quasiOpen');
				if (existing) {
					return ok({ ref: msg.ref, created: false });
				}
			}
			const added = store.addMessage(msg);
			if (!added) {
				return err('CONFLICT', `${command}: message '${msg.ref}' could not be added`);
			}
			const created = store.getMessageByRef(msg.ref, 'quasiOpen');
			return ok({
				ref: msg.ref,
				...(command === 'createIfAbsent' ? { created: true } : {}),
				message: toJsonSafe(created),
			});
		}

		if (command === 'patch' || command === 'patchIfPresent') {
			if (!isObject(payload)) {
				return err('BAD_REQUEST', `${command}: payload must be an object`);
			}
			const normalized = normalizePatchLikePayload(payload);
			const ref = getRef(normalized);
			if (!ref) {
				return err('BAD_REQUEST', `${command}: ref is required`);
			}
			const existing = store.getMessageByRef(ref, 'all');
			if (!existing) {
				if (command === 'patchIfPresent') {
					return ok({ ref, patched: false });
				}
				return err('NOT_FOUND', `patch: message '${ref}' not found`);
			}

			const patch = isObject(normalized.patch) ? normalized.patch : stripControlKeys(normalized);
			const okUpdate = store.updateMessage(ref, patch);
			if (!okUpdate) {
				return err('VALIDATION_FAILED', `${command}: message '${ref}' could not be updated`);
			}

			const updated = store.getMessageByRef(ref, 'all');
			return ok({
				ref,
				...(command === 'patchIfPresent' ? { patched: true } : {}),
				message: toJsonSafe(updated),
			});
		}
		...
		if (command === 'upsert') {
			...
			const ref = getRef(normalized);
			const exists = ref ? store.getMessageByRef(ref, 'quasiOpen') != null : false;
			if (exists) {
				const okUpdate = store.updateMessage(ref, stripControlKeys(normalized));
				if (!okUpdate) {
					return err('VALIDATION_FAILED', `upsert: message '${ref}' could not be updated`);
				}
				const updated = store.getMessageByRef(ref, 'quasiOpen');
				return ok({ ref, message: toJsonSafe(updated) });
			}

			const msg = createMessage(normalized);
			if (!msg) {
		...
		if (command === 'remove') {
			...
			const removed = store.getMessageByRef(ref, 'all');
