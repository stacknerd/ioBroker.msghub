	...
	async function onMessage(obj) {
		...
		if (command === 'create' || command === 'createIfAbsent') {
			...
			if (command === 'createIfAbsent') {
				const existing = store.getMessageByRef(msg.ref, 'all');
				if (existing) {
			...
			const created = store.getMessageByRef(msg.ref, 'all');
			return ok({
				ref: msg.ref,
		...
		if (command === 'upsert') {
			...
			const ref = getRef(normalized);
			const exists = ref ? store.getMessageByRef(ref, 'all') != null : false;
			if (exists) {
