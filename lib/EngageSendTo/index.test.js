...
describe('EngageSendTo', () => {
	...
	it('handles create/get/remove with structured responses', async () => {
		...
		const created = await h.onMessage({ command: 'create', message: { ref: 'm1', title: 't1', text: 'x' } });
	...
	it('handles patch/upsert and strips control keys when needed', async () => {
		...
		const patched = await h.onMessage({
			command: 'patch',
			message: { ref: 'm1', title: 'new', command: 'patch', from: 'x', silent: true, actor: 'x' },
		});
		...
		expect(patchArg).to.deep.equal({ ref: 'm1', title: 'new' });
