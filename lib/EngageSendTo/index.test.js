...
function makeStore(seed = []) {
	...
	const calls = { addMessage: [], updateMessage: [], removeMessage: [], queryMessages: [], getMessageByRef: [] };
	const isQuasiDeletedState = state => state === 'deleted' || state === 'closed' || state === 'expired';
	const isQuasiOpenState = state => !isQuasiDeletedState(state || 'open');
	...
	return {
		...
		addMessage(msg) {
			...
			const existing = messages.get(msg.ref);
			if (existing && !isQuasiDeletedState(existing?.lifecycle?.state)) {
				return false;
			}
		...
		getMessageByRef(ref, filter = 'all') {
			...
			const msg = messages.get(ref);
			if (!msg) {
				return undefined;
			}
			if (filter === 'quasiOpen') {
				return isQuasiOpenState(msg?.lifecycle?.state) ? msg : undefined;
			}
			if (filter === 'quasiDeleted') {
				return isQuasiDeletedState(msg?.lifecycle?.state) ? msg : undefined;
			}
			return msg;
		},
		removeMessage(ref) {
...
describe('EngageSendTo', () => {
	...
	it('handles create/get/remove with structured responses', async () => {
		...
		expect(store.calls.getMessageByRef).to.deep.include.members([
			['m1', 'quasiOpen'],
		]);
	...
	it('supports createIfAbsent and accepts plain-object metrics on create paths', async () => {
		const messagebox = { register() {}, unregister() {} };
		const { log } = makeLog();
		const store = makeStore();
		const factory = makeFactory();
		...
		expect(created.ok).to.equal(true);
		expect(created.data).to.deep.include({ ref: 'm-metrics', created: true });
		expect(factory.calls.createMessage).to.have.length(1);
		expect(store.calls.getMessageByRef).to.deep.include.members([
			['m-metrics', 'quasiOpen'],
		]);
		expect(factory.calls.createMessage[0].metrics).to.be.instanceOf(Map);
		expect(factory.calls.createMessage[0].metrics.get('battTime')).to.deep.equal({
			val: 12,
			unit: 's',
			ts: Date.UTC(2026, 0, 1),
		});
		expect(created.data.message.metrics).to.deep.equal({
			__msghubType: 'Map',
			value: [['battTime', { val: 12, unit: 's', ts: Date.UTC(2026, 0, 1) }]],
		});

		const noop = await h.onMessage({
			command: 'createIfAbsent',
			message: { ref: 'm-metrics', title: 't2', text: 'y' },
		});
		expect(noop.ok).to.equal(true);
		expect(noop.data).to.deep.equal({ ref: 'm-metrics', created: false });
		expect(store.calls.addMessage).to.have.length(1);
		expect(factory.calls.createMessage).to.have.length(2);
	});

	it('recreates a soft-deleted message via createIfAbsent', async () => {
		const messagebox = { register() {}, unregister() {} };
		const { log } = makeLog();
		const store = makeStore([
			{
				...
				listItems: [{ id: 'a', name: 'Alpha', checked: false }],
			},
		]);
		const factory = makeFactory();
		const action = makeAction(store);
		...
		const recreated = await h.onMessage({
			command: 'createIfAbsent',
			message: {
				ref: 'm-deleted',
				title: 'new',
				text: 'second',
				listItems: [{ id: 'b', name: 'Beta', checked: false }],
			},
		});

		expect(recreated.ok).to.equal(true);
		expect(recreated.data).to.deep.include({ ref: 'm-deleted', created: true });
		expect(store.calls.addMessage).to.have.length(1);
		expect(store.calls.getMessageByRef).to.deep.include.members([
			['m-deleted', 'quasiOpen'],
		]);
		expect(store.getMessageByRef('m-deleted')).to.have.property('text', 'second');
		expect(store.getMessageByRef('m-deleted')).to.have.nested.property('listItems[0].id', 'b');
	});
	...
	it('recreates a soft-deleted message via upsert', async () => {
		const messagebox = { register() {}, unregister() {} };
		const { log } = makeLog();
		const store = makeStore([
			{
				ref: 'm-upsert-deleted',
				title: 'old',
				text: 'first',
				lifecycle: { state: 'deleted' },
				listItems: [{ id: 'a', name: 'Alpha', checked: false }],
		...
		expect(recreated.ok).to.equal(true);
		expect(recreated.data).to.have.property('ref', 'm-upsert-deleted');
		expect(store.calls.addMessage).to.have.length(1);
		expect(store.calls.updateMessage).to.have.length(0);
		expect(store.calls.getMessageByRef).to.deep.include.members([
			['m-upsert-deleted', 'quasiOpen'],
		]);
		expect(store.getMessageByRef('m-upsert-deleted')).to.have.property('text', 'second');
		expect(store.getMessageByRef('m-upsert-deleted')).to.have.nested.property('listItems[0].id', 'b');
	});

	it('supports patchIfPresent and revives encoded map payloads', async () => {
		const messagebox = { register() {}, unregister() {} };
		const { log } = makeLog();
		const store = makeStore([{ ref: 'm1', title: 'old', text: 't' }]);
		const factory = makeFactory();
		...
		expect(patched.ok).to.equal(true);
		expect(patched.data).to.deep.include({ ref: 'm1', patched: true });
		expect(store.calls.updateMessage[0][1].metrics).to.be.instanceOf(Map);
		expect(store.calls.updateMessage[0][1].metrics.get('remainingTime')).to.deep.equal({
			val: 17,
			unit: 'min',
		...
		const noop = await h.onMessage({
			command: 'patchIfPresent',
			message: { ref: 'missing', patch: { title: 'x' } },
		});
		expect(noop.ok).to.equal(true);
		expect(noop.data).to.deep.equal({ ref: 'missing', patched: false });
	});
