...
// Private filesystem metadata: type -> absolute plugin directory path.
// Kept separate from the catalog to avoid leaking filesystem details into the public catalog contract.
const IoPluginsPluginDirs = new Map(discoveredPlugins.map(p => [p.type, path.join(__dirname, p.dir)]));
