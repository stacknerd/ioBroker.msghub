 ...
 * ---------------------------------------
 * Builtin plugin packages are currently discovered under `lib/<PluginType>/` and must export:
 ...
 * - `sourceKind`: plugin source identifier (`builtin` for repo-owned built-ins).
 * - `sourceId`: stable source-local package identity.
 * - `packageRoot`: absolute package root used for plugin-owned assets.
...
function discoverPlugins() {
	...
	for (const entry of entries) {
		...
		plugins.push({
			...
			type,
			sourceKind: 'builtin',
			sourceId: `builtin:${dir}`,
			packageRoot: path.join(__dirname, dir),
			manifest,
			create,
		});
	}
...
// Resolved plugin package descriptors used by host-side asset consumers.
const IoPluginsDescriptors = new Map(
