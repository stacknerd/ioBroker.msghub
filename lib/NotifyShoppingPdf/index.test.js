...
function makeCtx({ optionsValues, pluginChannel = '', onQuery } = {}) {
	...
	return {
		ctx: {
			api: { log, i18n, format: { formatlocale: 'en' }, iobroker, store, constants: MsgConstants },
			meta: {
...
describe('NotifyShoppingPdf', () => {
	...
	it('filters notifications by audience.tags (any)', () => {
		const { ctx, resCalls } = makeCtx({
			optionsValues: {
				renderDebounceMs: 0,
				pdfTitle: 'Shopping',
				...
				printRoomLabelsFromItems: 0,
				uncategorizedLabel: 'x',
				audienceTagsAnyCsv: 'Bio,Supermarkt',
			},
		});
		...
		h.onNotifications(
			MsgConstants.notfication.events.added,
			[{ kind: MsgConstants.kind.shoppinglist, ref: 'list:1', audience: { tags: ['Baumarkt'] } }],
			ctx,
		);
		expect(resCalls.setTimeout).to.have.length(1);
		...
		h.onNotifications(
			MsgConstants.notfication.events.added,
			[{ kind: MsgConstants.kind.shoppinglist, ref: 'list:2', audience: { tags: ['Bio'] } }],
			ctx,
		);
		expect(resCalls.setTimeout).to.have.length(2);
		...
		h.onNotifications(
			MsgConstants.notfication.events.added,
			[{ kind: MsgConstants.kind.shoppinglist, ref: 'list:3', audience: { tags: ['Supermarkt'] } }],
			ctx,
		);
		expect(resCalls.setTimeout).to.have.length(3);
	});
	...
	it('filters the pull query by plugin channel (audience.channels.routeTo)', async () => {
		...
		expect(queries[0]).to.deep.equal({
			where: {
				...
				timing: { startAt: { max: 1767772826126, orMissing: true } },
