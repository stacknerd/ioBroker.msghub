...
function NotifyShoppingPdf(options = {}) {
	...
	const ensurePdflatex = () => {
		const r = spawnSync('sh', ['-lc', 'command -v pdflatex'], { encoding: 'utf8' });
		if (r.status !== 0) {
			throw new Error('NotifyShoppingPdf: pdflatex is not available (PATH). Please install texlive.');
		}
	};
	...
	const ensureInitialized = ctx => {
		...
		const refsWhitelist = new Set(toCsvList(o.resolveString('refsWhitelistCsv', options.refsWhitelistCsv)));
		const refsBlacklist = new Set(toCsvList(o.resolveString('refsBlacklistCsv', options.refsBlacklistCsv)));
	...
	const ensureTextState = (id, name, role = 'text') => {
		...
		const p = iobroker.objects
			...
			.catch(err => {
				...
				log.warn(`NotifyShoppingPdf: failed to create state "${id}": ${err?.message || err}`);
			});
	...
	const renderLatex = model => {
		...
		add('}');
		add(
			`\\fancyfoot[L]{\\small\\textcolor{${theme.metacol}}{${latexEscape(`Summe Artikel: ${model.stats.totalTasks} / gedruckt: ${totalPrinted}`)}}}`,
		);
		add(`\\fancyfoot[C]{\\small\\textcolor{${theme.metacol}}{Seite \\thepage\\ von \\pageref{LastPage}}}`);
		...
		for (const cat of model.categories || []) {
			...
			const printRoomLabels = totalTasks > 6;
			...
			for (let i = 0; i < rooms.length; i++) {
				...
				for (const task of room.tasks || []) {
					...
					const cb = task.completed ? '\\ensuremath{\\blacksquare}' : '\\ensuremath{\\square}';
					const text = task.completed ? `\\textcolor{black!55}{${main}}` : main;
					add(`${cb} & ${text} \\\\[1pt]`);
				}
				add('\\end{tabularx}');
			}
		...
		if (Number(cfg.notesLines) > 0) {
			add('\\begin{CategoryBlock}');
			add(`\\begin{CategoryCard}{${latexEscape('NOTIZEN')}}{ }`);
			add('\\MetaBlock{\\textbf{Platz für Ergänzungen:}}');
	...
	const compileLatexToPdf = (texPath, { runs = 2 } = {}) => {
		...
		for (let i = 0; i < Math.max(1, runs); i++) {
			const r = spawnSync('pdflatex', args, { cwd: texDir, encoding: 'utf8' });
			if (r.status !== 0) {
				const out = `${r.stdout || ''}\n${r.stderr || ''}`;
				throw new Error(`NotifyShoppingPdf: pdflatex failed (run ${i + 1}/${runs}).\n${out}`);
			}
		}
	...
	const buildModel = () => {
		...
		const whitelistEnabled = cfg.refsWhitelist.size > 0;
		...
		for (const msg of messages) {
			...
			if (whitelistEnabled && !cfg.refsWhitelist.has(ref)) {
				continue;
			}
			if (cfg.refsBlacklist.has(ref)) {
				continue;
			}

			const lifecycle = msg.lifecycle && typeof msg.lifecycle === 'object' ? msg.lifecycle : null;
			const st = typeof lifecycle?.state === 'string' ? lifecycle.state : '';
			if (st === constants.lifecycle.state.deleted || st === constants.lifecycle.state.expired) {
				continue;
			}
		...
		return {
			meta: {
				...
				subtitle: `Shoppinglisten: ${cats.length}`,
				generatedLabel: new Date().toISOString().replace('T', ' ').slice(0, 19),
			},
			stats: { totalTasks },
			categories: cats,
		};
	...
	const renderNow = async () => {
		...
		try {
			...
			try {
				...
				log.debug(`NotifyShoppingPdf: LaTeX written: ${texPath}`);
				const pdfPath = compileLatexToPdf(texPath, { runs: 2 });
				log.debug(`NotifyShoppingPdf: PDF created: ${pdfPath}`);

				const metaId = iobroker.ids.namespace;
				const filePath = `documents/${plugin.type}.${plugin.instanceId}.pdf`;
				...
				log.info(`NotifyShoppingPdf: PDF updated: ${metaId}/${filePath}`);
			} finally {
				try {
	...
	const scheduleRender = _ctx => {
		...
		renderTimer = resources.setTimeout(() => {
			renderTimer = null;
			renderNow().catch(e => log.warn(`NotifyShoppingPdf: render failed: ${e?.message || e}`));
		}, delay);
	};
	...
	return {
		start(ctx) {
			...
			ensurePdflatex();
			ensureOwnStates().catch(e => log.warn(`NotifyShoppingPdf: failed to ensure states: ${e?.message || e}`));
			scheduleRender(ctx);
		},
		stop(ctx) {
			ensureInitialized(ctx);
			...
			log.info('NotifyShoppingPdf: stopped');
		},
