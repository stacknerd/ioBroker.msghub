...
function NotifyShoppingPdf(options = {}) {
	...
	const buildModel = ({ subtitle }) => {
		...
		for (const msg of messages) {
			...
			// Only render active lists:
			// - missing startAt: unscheduled -> include
			// - startAt in the past: started -> include
			// - startAt in the future: scheduled -> skip
			const rawStartAt = msg?.timing?.startAt;
			if (rawStartAt !== undefined && rawStartAt !== null) {
				const startAt = typeof rawStartAt === 'number' ? rawStartAt : Number(rawStartAt);
