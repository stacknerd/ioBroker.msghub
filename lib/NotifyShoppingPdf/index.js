...
function NotifyShoppingPdf(options = {}) {
	...
	const ensureInitialized = ctx => {
		...
		ensureCtxAvailability('NotifyShoppingPdf', ctx, {
			...
			stringNonEmpty: ['ctx.api.format.formatlocale'],
			fn: [
				'api.log.info',
				'api.log.warn',
				'api.i18n.t',
		...
		log = ctx.api.log;
		i18n = ctx.api.i18n;
		locale = ctx.api.format.formatlocale;
	...
	const renderLatex = model => {
		...
		add('}');
		add(
			`\\fancyfoot[L]{\\small\\textcolor{${theme.metacol}}{${latexEscape(t('msghub.i18n.NotifyShoppingPdf.footer.itemsPrinted.format', model.stats.totalTasks, totalPrinted))}}}`,
		);

		const footerCenterRaw = t('msghub.i18n.NotifyShoppingPdf.footer.pageOf.format', 'PAGENUM', 'PAGETOTAL');
		...
		if (Number(cfg.notesLines) > 0) {
			add('\\begin{CategoryBlock}');
			add(
				`\\begin{CategoryCard}{${latexEscape(String(t('msghub.i18n.NotifyShoppingPdf.notes.title.label')).toUpperCase())}}{ }`,
			);
			add(
				`\\MetaBlock{\\textbf{${latexEscape(t('msghub.i18n.NotifyShoppingPdf.notes.spaceForAdditions.label'))}}}`,
			);
	...
	return {
		...
		onNotifications(event, notifications, ctx) {
			...
			if (
				...
				ev !== constants.notfication.events.recreated &&
				ev !== constants.notfication.events.recovered
			) {
				return;
			}
