...
class IoAdminTab {
	...
	async _ingestStatesReadCustom(payload) {
		...
		return this._ok({ id, customKey, custom: entry && isObject(entry) ? this._stripManagedMeta(entry) : null });
	}
	...
	async _ingestStatesBulkApplyPreview(payload) {
		...
		const customPatch = this._stripManagedMeta(payload.custom);
		...
		for (const [id, obj] of Object.entries(objects || {})) {
			...
			const existingNoMeta = existing && isObject(existing) ? this._stripManagedMeta(existing) : null;
