...
class IoAdminTab {
					...
					const rawName =
						typeof c.obj?.common?.name === 'string'
							...
							: c.obj?.common?.name && typeof c.obj.common.name === 'object'
								...
								: '';
					const name = this._capitalizeFirstChar(rawName) || c.presetId;
					return {
						value: c.presetId,
						label,
						hasOwner,
						...
						kind: parsed?.message?.kind || null,
						level: typeof parsed?.message?.level === 'number' ? parsed.message.level : null,
						name,
					};
				} catch {
					return null;
	...
	async _ingestStatesPresetsUpsert(payload) {
		...
		if (source !== 'user') {
			return this._err(
				'FORBIDDEN',
				`Preset cannot be created/updated with source '${source || 'missing'}' via admin`,
			);
		}
		const nextPreset = this._cloneJson(preset);
		nextPreset.source = 'user';
