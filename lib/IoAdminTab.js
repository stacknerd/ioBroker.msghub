...
class IoAdminTab {
	...
	async _archiveRetryNative() {
		...
		if (typeof archive._probeNativeStorage !== 'function') {
			return this._err('NOT_SUPPORTED', 'Archive runtime does not support native probe');
		}

		const probe = await archive._probeNativeStorage();
