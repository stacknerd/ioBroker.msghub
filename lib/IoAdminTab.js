...
class IoAdminTab {
	...
	_presetLabel({ presetId, obj, preset }) {
		...
		const ownerText = this._t(`msghub.i18n.IngestStates.admin.jsonCustom.rules.${ownerKey}.header.text`);
	...
	async _ingestStatesPresetsList(payload) {
		...
		this.adapter?.log?.debug(`_ingestStatesPresetsList ${JSON.stringify(payload, null, 2)}`);
		...
		return this._ok(out.map(({ value, label }) => ({ value, label })));
	}
