...
class IoAdminTab {
	...
	constructor(
		adapter,
		ioPlugins,
		{ msgStore = null, pluginPanelResolver = null, pluginUiRpc = null, adminCapabilities = null } = {},
	) {
	 ...
	 */
	_getAuthorizedPayload(cmd, payload, host = 'admin') {
		...
		if (cmd.startsWith('admin.ingestStates.presets.selectOptions')) {
			if (
				...
				typeof payloadWithoutHost.token === 'string' &&
				payloadWithoutHost.token.trim()
			) {
				try {
					return {
						ok: true,
			...
			const { token: _ignoredToken, ...restPayload } = payloadWithoutHost;
	...
	async handleCommand(cmd, payload, { host = 'admin' } = {}) {
		...
		const authorized = this._getAuthorizedPayload(c, payload, host);
