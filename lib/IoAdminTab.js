...
class IoAdminTab {
	 ...
	 */
	async _ingestStatesPresetsList() {
		...
		const reads = await Promise.all(
			candidates.map(async c => {
				try {
					...
					const name =
						typeof c?.obj?.common?.name === 'string'
							? c.obj.common.name
							: c?.obj?.common?.name && typeof c.obj.common.name === 'object'
								? c.obj.common.name.en || c.obj.common.name.de || ''
								: '';
					const label = typeof name === 'string' && name.trim() ? name.trim() : c.presetId;
					return { value: c.presetId, label };
				} catch {
					return null;
				}
			}),
	...
	async _ingestStatesPresetsUpsert(payload) {
		...
		if (existing) {
			...
			if (existingPreset?.immutable === true) {
				return this._err('FORBIDDEN', 'Preset is immutable');
			}
		}

		if (preset.immutable === true) {
			return this._err('FORBIDDEN', 'Preset cannot be created/updated as immutable via admin');
		}
	...
	async handleCommand(cmd, payload) {
		...
		if (c === 'admin.ingestStates.presets.list') {
			return await this._ingestStatesPresetsList();
		}
