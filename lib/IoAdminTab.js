...
const { ruleTemplateCatalog } = require('./IngestStates/constants');
const { extractPresetName, parsePresetState } = require('./IngestStates/AdminPresets');
...
class IoAdminTab {
	...
	async _pluginUiDiscover() {
		...
		return this._ok(contribs.map((c, i) => ({ ...c, bundle: { hash: hashes[i] } })));
	}
	...
	/**
	 * Internal helper: load and filter preset options for selectSendTo.
	 *
	 * When `payload.rule` is set, presets are returned only when `preset.ownedBy` matches
	 * the rule or when `preset.ownedBy` is empty (global presets).
	 *
	 * @param {any} payload Filter payload (`rule`, `subset`).
	 * @returns {Promise<{ ok: boolean, data?: any, error?: any }>} Filtered options wrapper.
	 */
	...
	/**
	 * Internal helper: load a single preset by id.
	 *
	...
	async handleCommand(cmd, payload) {
		...
		if (c === 'admin.pluginUi.discover') {
			return await this._pluginUiDiscover();
		}
