...
describe('IngestDwd', () => {
	...
	it('patches only on relevant changes and silences due-on-update when notifyAt is missing', async () => {
		...
		// Change end + description => should patch and set notifyAt to end+1h (silence due-on-update).
		store.calls.updateMessage.length = 0;
		const warning2 = { ...warning1, end: warning1.end + 60000, description: 'B' };
		foreignStates['dwd.0.warning.object'] = { val: warning2 };
		plugin.onStateChange('dwd.0.warning.object', { val: warning2 }, ctx);
		await flush();
		await flush();
	...
	it('defers sync when warning set is unstable (stability gate)', async () => {
		const now = Date.now();
		const warning1 = {
			state: 'Baden-Württemberg',
			type: 4,
			...
			instruction: 'Handlungsempfehlungen: Verhalten im Straßenverkehr anpassen',
			stateShort: 'BW',
			altitudeStart: null,
			altitudeEnd: null,
		};
		const warning2 = { ...warning1, description: 'B' };

		const foreignStates = {
			'dwd.0.numberOfWarnings': { val: 1 },
			'dwd.0.warning.object': { val: warning1 },
			'msghub.0.IngestDwd.0.aiCache': { val: '' },
		};
		const { iobroker } = makeIoBroker({ foreignStates });
		const store = makeStore();
		const managedObjects = makeManagedObjects();
		...
		timers.shift().fn(); // start initial sync
		await flush();
		await flush();

		// Stability gate schedules its internal delay.
		expect(timers.length).to.equal(1);

		// Simulate a warning change between the two reads.
		foreignStates['dwd.0.warning.object'] = { val: warning2 };
		timers.shift().fn(); // finish stability gate delay
		await flush();
		await flush();

		// Unstable => no writes, but schedules retry.
		expect(store.calls.addMessage.length).to.equal(0);
		expect(store.calls.updateMessage.length).to.equal(0);
		expect(timers.length).to.equal(1);
	});
});
