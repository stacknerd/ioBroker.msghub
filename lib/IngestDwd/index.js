...
function IngestDwd(options = {}) {
	...
	const ensureValidUntilInText = (text, endTs) => {
		...
		if (raw.includes(formatted) || /\bgültig\s+bis\b/iu.test(raw)) {
			return raw;
		}
		return `${raw.trim()}\n\nWarnung gültig bis: ${formatted}`.trim();
	};
			...
			messages: [
				{
					...
					content:
						...
						'- aiText: 2 gut lesbare Sätze auf Deutsch. Keine unnötigen Zahlen/Einheiten; wenn Werte wichtig sind, vereinheitliche Einheiten und reduziere Wiederholungen. Wenn "validUntil" vorhanden ist: erwähne, bis wann die Warnung gilt.\n' +
	...
	const upsertWarning = async (warning, { cache }) => {
		...
		const textBaseCandidate = normalizeText(cachedAi?.aiText) || description || titleBaseCandidate;
		...
		const textCandidate = ensureValidUntilInText(textBaseCandidate, end);
		...
		const textRaw = normalizeText(aiOut?.aiText) || textBaseCandidate;
		const text = ensureValidUntilInText(textRaw, end);
