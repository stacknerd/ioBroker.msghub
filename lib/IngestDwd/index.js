 ...
 * IngestDwd
 * =========
 * Producer plugin for DWD-based weather alerts and message generation.
 *
 * Docs: ../../docs/plugins/IngestDwd.md
 */
...
function IngestDwd(options = {}) {
	...
	// DWD warning type icons (derived from warning.type numeric codes).
	// Reference mapping (DWD adapter):
	// 0 Thunderstorm, 1 Wind/Storm, 2 Rain, 3 Snow, 4 Fog, 5 Frost, 6 Ice, 7 Thawing, 8 Heat, 9 UV warning
	...
	const computeIconByType = warningType => {
		const n = normalizeNumber(warningType);
		if (n === null) {
			return '';
		}
		const key = Math.trunc(n);
		return Object.prototype.hasOwnProperty.call(typeIcons, key) ? typeIcons[key] : '';
	};
	...
	const computeStableHash = warning => {
		const stable = Object.freeze({
			...
			description: typeof warning?.description === 'string' ? normalizeText(warning.description) : '',
			event: typeof warning?.event === 'string' ? warning.event : '',
			headline: typeof warning?.headline === 'string' ? warning.headline : '',
			instruction: typeof warning?.instruction === 'string' ? normalizeText(warning.instruction) : '',
	...
	const matchesAltitude = warning => {
		...
		if (start === null) {
			return end !== null && a <= end;
		}
		if (end === null) {
			return start !== null && start <= a;
		}
	...
	const formatWarningEnd = ts => {
		...
		const locale = typeof formatLocale === 'string' && formatLocale.trim() ? formatLocale.trim() : 'de-DE';
	...
	const upsertWarning = async (warning, { cache }) => {
		...
		// Identity:
		// We derive a stable ref from a warning subset. If that subset changes, we intentionally
		// create a new message (new ref) instead of patching the existing one.
		const hash = computeStableHash(warning);
		const ref = warningRef(hash);

		// Raw input fields (kept separate so we can:
		// - build readable fallbacks when AI is off
		// - store the raw-ish values as metrics for debugging/auditing
		// - keep the upsert logic linear and easy to follow
		...
		// Lookup the currently "active" message for this ref (ignores terminal states like deleted/expired).
		const existing = store.getMessageByRef(ref, 'quasiOpen');

		// Read existing timing fields once; used for "is patch needed?" checks and timing patch creation.
		...
		// AI input block:
		// Create a compact warning representation for the AI model and compute a content hash.
		// This allows caching (local + ai.json cache key) and lets us detect changes cheaply.
		const textBlockJson = computeTextBlockJson(warning);
		const textBlockHash = computeTextBlockHash(textBlockJson);

		// Local cache hit:
		// The per-plugin cache is keyed by ref and only stores one variant per ref.
		// The `ai.json()` call has its own cache key (see maybeAiEnhance).
		...
		// Candidate content:
		// We compute "what the message should look like" based on (AI or fallback) + current warning data.
		// These candidates are used both for create and for "do we need to patch?" decisions.
		...
		// Decide whether we need to create or patch.
		// Note: metrics are handled separately below so we can patch them even when everything else is unchanged.
		...
		// Metrics:
		// Store raw-ish input values as metrics (debug/audit). We patch only when values differ/missing.
		const unit = '';
		const desiredMetrics = {
			descriptionRaw: { val: normalizeText(description), unit, ts: now },
			eventRaw: { val: normalizeText(event), unit, ts: now },
			headlineRaw: { val: normalizeText(headline), unit, ts: now },
			instructionRaw: { val: normalizeText(instruction), unit, ts: now },
			altitudeStartRaw: { val: normalizeNumber(warning?.altitudeStart), unit, ts: now },
			altitudeEndRaw: { val: normalizeNumber(warning?.altitudeEnd), unit, ts: now },
		};
		...
		if (existing) {
			...
			const metricsSet = {};
			for (const [key, metric] of Object.entries(desiredMetrics)) {
				const prev = existingMetrics instanceof Map ? existingMetrics.get(key) : null;
				if (!prev || prev.val !== metric.val || prev.unit !== metric.unit) {
					metricsSet[key] = metric;
				}
			}
			metricsPatch = Object.keys(metricsSet).length > 0 ? { set: metricsSet } : null;
			if (metricsPatch) {
				shouldPatch = true;
			}
		}

		// Nothing to do: no field changes and metrics already present/consistent.
		if (existing && !shouldPatch) {
			return ref;
		}

		// AI enhancement:
		// Only run AI when enabled and available; otherwise we use the previously computed candidates.
		let aiOut = null;
		...
		const icon = computeIconByType(warning?.type);

		// Create path: new message for new ref (no patching possible).
		if (!existing) {
			const msg = factory.createMessage({
				...
				details,
				actions,
				metrics: new Map(Object.entries(desiredMetrics)),
				audience,
			});
			if (!msg) {
				return null;
		...
		// Patch path: update only the relevant parts of the message.
		const timingPatch = {};
		if (end !== null && existingExpiresAt !== end) {
	...
	const syncNow = async () => {
		...
		log.debug(`syncing ${warnings.length} warnings.`);
		log.silly(JSON.stringify(warnings, null, 2));
	...
	const start = ctx => {
		...
		formatLocale =
			typeof ctx?.api?.format?.formatlocale === 'string' && ctx.api.format.formatlocale.trim()
				? ctx.api.format.formatlocale.trim()
				: 'de-DE';
	...
	const onStateChange = (id, _state, _ctx) => {
		...
		if (fullId === `${cfg.dwdInstance}.numberOfWarnings`) {
			scheduleSync();
		}
	};
