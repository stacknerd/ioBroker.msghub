...
function IngestDwd(options = {}) {
	...
	const formatWarningEnd = ts => {
		...
		try {
			...
			if (isSoon) {
				return new Intl.DateTimeFormat(locale, { weekday: 'long', timeStyle: 'short' }).format(new Date(ts));
			}
			...
			messages: [
				{
					...
					content:
						...
						'- aiText: 2 bis 3 gut lesbare Sätze auf Deutsch (bevorzugt 3), natürlich im Fließtext. Keine relativen Zeitangaben ("heute/morgen/übermorgen/nächste Woche"). Wenn "validUntil" vorhanden ist: baue das Ende natürlich ein (z.B. "… bis Mittwoch um 11:00 Uhr …"). Verwende dafür die Information aus "validUntil" (kein Rechnen/kein anderes Datum erfinden). Keine zusätzliche Zeile wie "Warnung gültig bis: ...".\n' +
	...
	const upsertWarning = async (warning, { cache }) => {
		...
		let existing = store.getMessageByRef(ref);
		const state = existing?.lifecycle?.state;
		existing =
			state === constants.lifecycle.state.deleted || state === constants.lifecycle.state.expired
				? null
				: existing;
