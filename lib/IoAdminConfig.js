 ...
 * - Delegate shared ID-catalog commands to `IoIdCatalog`.
...
const { IoIdCatalog } = require(`${__dirname}/IoIdCatalog`);
...
class IoAdminConfig {
	 ...
	 * @param {import('./IoIdCatalog').IoIdCatalog|null} [options.idCatalog]
	 *   Optional shared ID-catalog backend facade. Defaults to one adapter-bound instance.
	 */
	constructor(
		adapter,
		{ ai = null, msgStore = null, archiveProbeNative, adminCapabilities = null, idCatalog = null } = {},
	) {
		...
		this.idCatalog = idCatalog && typeof idCatalog === 'object' ? idCatalog : new IoIdCatalog(adapter);
	}
	...
	async handleCommand(cmd, payload, { host = 'admin' } = {}) {
		...
		// Shared ID-catalog backend commands.
		if (c === 'config.idcatalog.get') {
			return await this.idCatalog.get(cleanPayload);
		}
		if (c === 'config.idcatalog.openTree') {
			return await this.idCatalog.openTree(cleanPayload);
		}
		if (c === 'config.idcatalog.reset') {
			return this.idCatalog.reset();
		}
