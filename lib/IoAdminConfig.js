...
class IoAdminConfig {
	 ...
	 */
	constructor(adapter, { ai = null, msgStore = null, archiveProbeNative, adminCapabilities = null } = {}) {
	...
	/**
	 * Validate the canonical `payload.token` for `config.*` and strip it before
	 * the business payload reaches config command handlers.
	 *
	 * @param {any} payload Raw command payload.
	 * @param {string} [host] Trusted server-side host hint for token validation.
	 ...
	 */
	_getAuthorizedPayload(payload, host = 'admin') {
		...
		const normalizedHost = typeof host === 'string' && host.trim() ? host.trim() : 'admin';
		const { host: _ignoredHost, ...payloadWithoutHost } = safePayload;
		try {
			return {
				ok: true,
	...
	/**
	 * Tokenize one full ioBroker state id for root-tree grouping.
	 *
	 * Root grouping uses the full instance prefix up to and including the first
	 * purely numeric segment, e.g. `javascript.0` or `system.adapter.web.1`.
	 *
	 * @param {string} id Full state id.
	 * @returns {string[]} Token list for root-tree traversal.
	 */
	_tokenizeIdCatalogRootId(id) {
	...
	/**
	 * Return a config-facing catalog projection of ioBroker state objects.
	 *
	 * The projection intentionally keeps only the picker-relevant subset:
	 * `_id` plus a tiny whitelist from `common`.
	 *
	 * @param {any} payload Optional business payload.
	 * @returns {Promise<{ok: true, data: { objects: Record<string, any> }} | {ok: false, error: {code: string, message: string}}>}
	 *   Reduced state-object catalog or structured backend error.
	 */
	async _idCatalogGet(payload) {
		const filter = typeof payload?.filter === 'string' && payload.filter.trim() ? payload.filter.trim() : '*';
		const startedAt = performance.now();
		let objects;
		try {
			objects = await this._getForeignObjectsAsync(filter, 'state');
		} catch (e) {
		...
		for (const [id, obj] of Object.entries(objects || {})) {
			...
			const out = { _id: typeof obj._id === 'string' && obj._id.trim() ? obj._id : id };
			const commonOut = this._projectIdCatalogCommon(obj.common);
			if (commonOut) {
				out.common = commonOut;
			}
			reduced[id] = out;
		}
		const backendDurationMs = Number((performance.now() - startedAt).toFixed(1));
		return this._ok({
			objects: reduced,
			meta: {
				backendDurationMs,
	...
	/**
	 * Return one tree-oriented catalog slice for the ID picker.
	 *
	 * The current implementation intentionally keeps the transport tree-oriented
	 * while still relying on `getForeignObjects(...)` for backend reads.
	 * Without a dedicated backend-side depth-aware query, `depth` limits the
	 * response shape, not the initial object fetch cost.
	 *
	 * @param {any} payload Optional business payload.
	 * @returns {Promise<{ok: true, data: { entry: string, depth: number, nodes: any[], meta: Record<string, any> }} | {ok: false, error: {code: string, message: string}}>}
	 *   Tree slice or structured backend error.
	 */
	async _idCatalogOpenTree(payload) {
		const entry = this._normalizeIdCatalogEntry(payload?.entry);
		const depth = this._normalizeIdCatalogDepth(payload?.depth);
		const pattern = entry ? `${entry}.*` : '*';
		const startedAt = performance.now();
		let objects;
		try {
			objects = await this._getForeignObjectsAsync(pattern, 'state');
		} catch (e) {
			return this._err('NOT_READY', String(e?.message || e));
		}
		...
		for (const [id, obj] of Object.entries(objects || {})) {
			...
			let relativeTokens;
			if (!entry) {
				relativeTokens = this._tokenizeIdCatalogRootId(id);
			} else {
				const prefix = `${entry}.`;
				if (!String(id).startsWith(prefix)) {
					continue;
				}
				relativeTokens = String(id)
					.slice(prefix.length)
					.split('.')
					.filter(Boolean);
			}
			...
			for (let level = 1; level <= maxLevel; level += 1) {
				...
				const parentPathTokens = pathTokens.slice(0, -1);
				const parentEntry = !entry
					? parentPathTokens.join('.')
					: parentPathTokens.length > 0
						? `${entry}.${parentPathTokens.join('.')}`
						: entry;
				...
				if (isExactState) {
					node._id = typeof obj._id === 'string' && obj._id.trim() ? obj._id : id;
					const commonOut = this._projectIdCatalogCommon(obj.common);
					if (commonOut) {
						node.common = commonOut;
					}
				}
			}
		}

		const nodes = Array.from(nodesByEntry.values()).sort((a, b) => {
			if (a.level !== b.level) {
	 ...
	 * @param {{ host?: string }} [options] Trusted server-side host hint for token validation.
	 * @returns {Promise<any>} Command response.
	 */
	async handleCommand(cmd, payload, { host = 'admin' } = {}) {
		const c = typeof cmd === 'string' ? cmd.trim() : '';
		if (!c) {
			return this._err('BAD_REQUEST', 'Missing command');
		}
		const authorized = this._getAuthorizedPayload(payload, host);
		...
		if (c === 'config.ai.test') {
			return this._applyNativeAllowlist(await this._aiTest(cleanPayload));
		}
		if (c === 'config.idcatalog.get') {
			return await this._idCatalogGet(cleanPayload);
		}
		if (c === 'config.idcatalog.openTree') {
			return await this._idCatalogOpenTree(cleanPayload);
		}
