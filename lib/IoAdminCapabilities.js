 ...
 * - `buildBootstrapAbout()` returns the shared `about` sub-payload for `ui.bootstrap`.
...
class IoAdminCapabilities {
	...
	/**
	 * Build the shared `ui.bootstrap.about` payload.
	 *
	 * @returns {{
	 *   title: string,
	 *   version: string,
	...
	buildBootstrapAbout() {
		...
		return {
			...
			connection: this.adapter._coreConnection?.getBootstrapAboutConnection?.() || {
				scope: 'core-link',
				connected: false,
				mode: 'local',
			},
	 ...
	 *   about: ReturnType<IoAdminCapabilities['buildBootstrapAbout']>
	...
	buildBootstrap({ host = 'admin' } = {}) {
		...
		return {
			...
			about: this.buildBootstrapAbout(),
		};
	}
}
