 ...
 * - Normalize UI-language requests for plugin-owned Admin-UI i18n.
...
/**
 * Normalize a plugin UI language code to the shared backend-safe form.
 *
 * @param {any} lang Requested language.
 * @returns {string} Safe language or `en`.
 */
function normalizePluginUiLang(lang) {
...
class IoPluginPanelResolver {
	 ...
	 *
	 * @param {{ lang?: string }} [options] Resolver options.
	...
	async getPanelsByRuntimeId({ lang } = {}) {
		...
		const safeLang = normalizePluginUiLang(lang);
		...
		await Promise.all(
			Array.from(
				new Set(contributions.map(c => c?.pluginType).filter(type => typeof type === 'string' && type.trim())),
			).map(async pluginType => {
				translationsByType.set(pluginType, await this._readTranslations(pluginType, safeLang));
			}),
		);

		const out = {};
		for (const [index, contribution] of contributions.entries()) {
			const panel = this._toResolvedPanel(contribution, {
				hash: hashes[index],
				i18n: translationsByType.get(contribution.pluginType) ?? null,
			});
			out[panel.id] = panel;
		}
		return out;
	 ...
	 *
	 * @param {{ runtimePanelId?: string, lang?: string }} [options] Resolver options.
	 * @returns {Promise<object|null>} Resolved panel or null.
	 */
	async getPanelByRuntimeId({ runtimePanelId, lang } = {}) {
	 ...
	 *
	 * @param {{ pluginType?: string, instanceId?: number, panelId?: string, lang?: string }} [options]
	 *   Resolver options.
	 * @returns {Promise<object|null>} Resolved panel or null.
	 */
	async getPanelByRef({ pluginType, instanceId = 0, panelId, lang } = {}) {
		this._assertReady();
		const ioPlugins = this._getIoPlugins();
		...
		return this._toResolvedPanel(contribution, {
			hash: await this._computeBundleHash(contribution),
			i18n: await this._readTranslations(type, normalizePluginUiLang(lang)),
		});
	}
	 ...
	 * @param {{ hash?: string, i18n?: any }} [options] Enriched runtime metadata.
	 * @returns {object} Resolved panel.
	 */
	_toResolvedPanel(contribution, { hash = '', i18n = null } = {}) {
	...
	/**
	 * Read plugin-owned Admin UI translations for one plugin type, degrading softly on failure.
	 *
	 * @param {string} pluginType Plugin type.
	 * @param {string} lang Safe UI language.
	 * @returns {Promise<{ lang: string, translations: object }|null>} Resolved i18n payload or null.
	 */
	async _readTranslations(pluginType, lang) {
		const ioPlugins = this._getIoPlugins();
		try {
			return await ioPlugins.readAdminUiTranslations({ type: pluginType, lang });
		} catch (e) {
			this.log?.warn?.(
				`PluginPanelResolver: failed to read Admin UI i18n for '${pluginType}': ${e?.message || e}`,
			);
			return null;
		}
	}
...
module.exports = { IoPluginPanelResolver, normalizePluginUiLang };
