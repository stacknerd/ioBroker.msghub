...
function NotifyDebug(options = {}) {
	...
	const ensureInitialized = ctx => {
		...
		ensureCtxAvailability('NotifyDebug', ctx, {
			...
			fn: ['api.log.debug', 'api.log.info', 'api.i18n.t'],
		});
		log = ctx.api.log;
		i18n = ctx.api.i18n;
	...
	return {
		start(ctx) {
			...
			log.debug(`ctx.api.constants.kind=${JSON.stringify(constants.kind)}`);
			log.debug(`ctx.api.constants.level=${JSON.stringify(constants.level)}`);

			const translated = i18n.t('msghub.i18n.NotifyDebug.demo.translation.text');
			log.debug(`i18n='${translated}'`);

			log.debug(
				`plugin regId='${plugin.regId}' baseFullId='${plugin.baseFullId}' baseOwnId='${plugin.baseOwnId}'`,
			);
		},
		stop(ctx) {
			if (!trace) {
		...
		onNotifications(event, notifications, ctx) {
			if (trace) {
				...
				for (const msg of notifications) {
					log.info(`'${msg.ref}' ${event}: ${msg?.title} - ${msg.text}`);
				}
			}
		},
	};
