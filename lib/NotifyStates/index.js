...
function NotifyStates(options = {}) {
	...
	const writeBlob = () => {
		...
		const filtered = supportsChannelRouting ? list.filter(m => shouldDispatchByAudienceChannels(m, pluginChannel)) : list;
	...
	const computeStats = () => {
		const all = store.getMessages();
		const list = Array.isArray(all) ? all : [];
		const messages = supportsChannelRouting ? list.filter(m => shouldDispatchByAudienceChannels(m, pluginChannel)) : list;
		const now = Date.now();
