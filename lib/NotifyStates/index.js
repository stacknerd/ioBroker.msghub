...
function NotifyStates(options = {}) {
	...
	const ensureInitialized = (ctx = {}) => {
		...
		if (!opts || typeof opts.resolveInt !== 'function' || typeof opts.resolveString !== 'function') {
			throw new Error('NotifyStates: missing ctx.meta.options (IoPlugins wiring required)');
		}
		blobTimerIntervalMs = opts.resolveInt('blobIntervalMs', options?.blobIntervalMs);
		statsMinIntervalMs = opts.resolveInt('statsMinIntervalMs', options?.statsMinIntervalMs);
		statsMaxIntervalMs = opts.resolveInt('statsMaxIntervalMs', options?.statsMaxIntervalMs);
		mapTypeMarker = opts.resolveString('mapTypeMarker', options?.mapTypeMarker);
	};
