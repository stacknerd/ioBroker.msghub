...
const { serializeWithMaps, shouldDispatchByAudienceChannels } = require(`${__dirname}/../../src/MsgUtils`);
...
function NotifyStates(options = {}) {
	...
	let resources = null;
	let pluginChannel = '';
	let supportsChannelRouting = false;
	...
	const ensureAllStates = async () => {
		if (!started) {
			throw new Error('not started (start(ctx) not called yet)');
		}

		const promises = [];
		const eventValueList = eventEntries.map(([, value]) => value);

		ensureStateName(blobStateId, 'msghub.i18n.NotifyStates.state.blob.label.format', blobTimerIntervalMs / 1000);
		promises.push(ensureState(blobStateId, stateNames.get(blobStateId)));

		ensureStateName(statsIds.total, 'msghub.i18n.NotifyStates.state.stats.total.label');
		promises.push(ensureNumberState(statsIds.total, stateNames.get(statsIds.total)));
		ensureStateName(statsIds.open, 'msghub.i18n.NotifyStates.state.stats.open.label');
		promises.push(ensureNumberState(statsIds.open, stateNames.get(statsIds.open)));
		ensureStateName(statsIds.dueNow, 'msghub.i18n.NotifyStates.state.stats.dueNow.label');
		promises.push(ensureNumberState(statsIds.dueNow, stateNames.get(statsIds.dueNow)));
		ensureStateName(statsIds.deleted, 'msghub.i18n.NotifyStates.state.stats.deleted.label');
		promises.push(ensureNumberState(statsIds.deleted, stateNames.get(statsIds.deleted)));
		ensureStateName(statsIds.expired, 'msghub.i18n.NotifyStates.state.stats.expired.label');
		...
		for (const eventValue of eventValueList) {
			...
			ensureStateName(id, 'msghub.i18n.NotifyStates.state.latest.event.label.format', eventValue);
		...
		for (const [kindKey] of kindEntries) {
			for (const eventValue of eventValueList) {
				...
				ensureStateName(id, 'msghub.i18n.NotifyStates.state.latest.kind.label.format', kindKey, eventValue);
				promises.push(ensureState(id, stateNames.get(id)));
			}
		}
		...
		for (const [levelKey] of levelEntries) {
			for (const eventValue of eventValueList) {
				...
				ensureStateName(id, 'msghub.i18n.NotifyStates.state.latest.level.label.format', levelKey, eventValue);
				promises.push(ensureState(id, stateNames.get(id)));
			}
		}
	...
	const writeState = (id, value) => {
		...
		ensureState(id, stateNames.get(id) || id)
			...
			.catch(err => log.warn(`failed to write state "${id}": ${err?.message || err}`));
	};
	...
	const getRoutedMessages = () => {
		const lifecycleStates = constants?.lifecycle?.state ? Object.values(constants.lifecycle.state) : [];

		if (store?.queryMessages && typeof store.queryMessages === 'function') {
			const where = {
				// `queryMessages` hides deleted/expired by default, so request all lifecycle states explicitly.
				lifecycle: { state: { in: lifecycleStates } },
				...(supportsChannelRouting ? { audience: { channels: { routeTo: pluginChannel } } } : {}),
			};
			try {
				const result = store.queryMessages({ where });
				return Array.isArray(result?.items) ? result.items : [];
			} catch (e) {
				log.warn(`queryMessages failed: ${e?.message || e}`);
				return [];
			}
		}
		...
		return supportsChannelRouting ? list.filter(m => shouldDispatchByAudienceChannels(m, pluginChannel)) : list;
	};

	const writeBlob = () => {
		const messages = getRoutedMessages();
	...
	const start = ctx => {
		...
		supportsChannelRouting = ctx?.meta?.plugin?.manifest?.supportsChannelRouting === true;
		pluginChannel = typeof ctx?.meta?.plugin?.channel === 'string' ? ctx.meta.plugin.channel.trim() : '';
		...
		ensureAllStates().catch(err => log.warn(`ensureAllStates failed: ${err?.message || err}`));
	...
	const onNotifications = (event, notifications) => {
		if (!started) {
			throw new Error('onNotifications called before start(ctx)');
		}
