 ...
 *
 * @param {{ blobIntervalMs?: number, pluginBaseObjectId?: string , mapTypeMarker?}} [options] Plugin options (from ioBroker `native`).
...
function NotifyStates(options = {}) {
	...
	const ensureInitialized = (ctx = {}) => {
		...
		if (baseOwnId && latestPrefix && kindPrefix && levelPrefix) {
			return;
		}
