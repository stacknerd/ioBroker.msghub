...
describe('IoCoreConnection', () => {
	...
	it('initializes official info.connection state with boolean indicator contract', async () => {
		...
		expect(coreConnection.getBootstrapAboutConnection()).to.deep.equal({
			scope: 'core-link',
			connected: false,
			mode: 'local',
		});
	...
	it('marks connected from local health snapshot and exposes the bootstrap about fragment', async () => {
