...
function createMenuRuntime(deps) {
	...
	const sendNewImagesForRecord = async (msg, existing, nowMs = Date.now()) => {
		...
		for (const value of values) {
			...
			const { chatMessages } = await transport.sendImageBroadcast({
				image: value,
				caption,
	...
	const onDue = async (msg, generateShortId) => {
		...
		const { chatMessages } = await transport.sendBroadcast({
			html: text.html,
			replyMarkup,
			disableNotification,
		});
		...
		if (!chatMessages || Object.keys(chatMessages).length === 0) {
			debug(`due: sent ref='${ref}' shortId='${shortId}' mapping=skipped (no send response mapping)`);
			return;
		}
