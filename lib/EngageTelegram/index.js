...
function EngageTelegram(options = {}) {
	...
	const ensureInitialized = ctx => {
		...
		cfg = Object.freeze({
			...
			deleteOldNotificationOnEnd: o.resolveBool(
				'deleteOldNotificationOnEnd',
				options.deleteOldNotificationOnEnd !== undefined ? options.deleteOldNotificationOnEnd : false,
			),
	...
	const MAPPING_RETENTION_MS = 90 * 24 * 60 * 60 * 1000;

	const pruneMappings = (now = Date.now()) => {
		const cutoff = now - MAPPING_RETENTION_MS;
		const deleteRefs = [];
		for (const [ref, entry] of Object.entries(mappingByRef)) {
			if (!entry || typeof entry !== 'object') {
				continue;
			}
			if (entry.shouldHaveButtons !== false) {
				continue;
			}
			const updatedAt = typeof entry.updatedAt === 'number' ? entry.updatedAt : Number(entry.updatedAt);
		...
		if (deleteRefs.length > 0) {
			debug(`mappings: pruned stale entries count=${deleteRefs.length} retentionDays=90`);
		}
	};
	...
	const editMessageSetButtons = ({ chatId, messageId, text, replyMarkup }) => {
		const safeChatId = typeof chatId === 'string' || typeof chatId === 'number' ? chatId : '';
		const safeMessageId = typeof messageId === 'number' ? messageId : Number(messageId);
		if (!safeChatId || !Number.isFinite(safeMessageId)) {
			return Promise.resolve();
	...
	const buildCleanupText = ({ baseHtml, reason }) => {
		const safeBase = typeof baseHtml === 'string' ? baseHtml : '';
		const safeReason = typeof reason === 'string' ? reason.trim() : '';
		if (!safeReason) {
			return safeBase;
		}
		const translated = String(t('Removed buttons because message has been %s', safeReason) || '').trim();
		if (!translated) {
			return safeBase;
		}
		return `${safeBase}\n\n<i>(${escapeHtml(translated)})</i>`;
	};

	const cleanupButtonsForRef = async (ref, reason = '') => {
		const current = mappingByRef[ref];
		if (!current || typeof current !== 'object') {
			return;
		}
		if (current.shouldHaveButtons === false) {
			debug(`cleanup: ref='${ref}' skipped (shouldHaveButtons=false)`);
			return;
		}

		const baseText = typeof current.textHtml === 'string' ? current.textHtml : '';
		const text = buildCleanupText({ baseHtml: baseText, reason });

		const chatMessages = isObject(current.chatMessages) ? current.chatMessages : {};
		debug(`cleanup: ref='${ref}' chats=${Object.keys(chatMessages).length} mode=buttons`);
		const tasks = Object.entries(chatMessages).map(([chatId, messageId]) =>
			editMessageSetButtons({
				chatId,
		...
		await saveMappings();
		debug(`cleanup: ref='${ref}' done buttons=removed mapping=kept`);
	};

	const cleanupOnResend = async ref => {
		...
		debug(`cleanup(resend): ref='${ref}' chats=${Object.keys(chatMessages).length} mode=${mode}`);
		...
		await saveMappings();
		debug(`cleanup(resend): ref='${ref}' done mapping=kept`);
	};

	const cleanupOnEnded = async (ref, lifecycle) => {
		const entry = mappingByRef[ref];
		if (!entry || typeof entry !== 'object') {
			return;
		}
		...
		const chatMessages = isObject(entry.chatMessages) ? entry.chatMessages : {};
		const mode = cfg.deleteOldNotificationOnEnd ? 'delete' : 'buttons';
		debug(`cleanup(end): ref='${ref}' chats=${Object.keys(chatMessages).length} lifecycle='${lifecycle}' mode=${mode}`);

		if (cfg.deleteOldNotificationOnEnd) {
			const tasks = Object.entries(chatMessages).map(([chatId, messageId]) =>
				deleteTelegramMessage({ chatId, messageId: Number(messageId) }),
			);
			await Promise.allSettled(tasks);
		} else if (entry.shouldHaveButtons !== false) {
			const baseText = typeof entry.textHtml === 'string' ? entry.textHtml : '';
			const text = buildCleanupText({ baseHtml: baseText, reason: lifecycle });
			const tasks = Object.entries(chatMessages).map(([chatId, messageId]) =>
				editMessageSetButtons({
					chatId,
					messageId: Number(messageId),
		...
		deleteRefMapping(ref);
		await saveMappings();
		debug(`cleanup(end): ref='${ref}' done mapping=removed`);
	};
	...
	const setRefShouldHaveButtons = (ref, shouldHaveButtons) => {
		const key = typeof ref === 'string' ? ref : '';
		if (!key) {
			return false;
		}
		...
		const current = entry.shouldHaveButtons !== false;
		const desired = shouldHaveButtons !== false;
		if (current === desired) {
			return true;
		}
		mappingByRef[key] = Object.freeze({ ...entry, shouldHaveButtons: desired });
		return true;
	};
	...
	const sendNotification = async msg => {
		...
		if (mappingByRef[ref]) {
			debug(`notify: ref='${ref}' has active mapping -> cleaning up before send`);
			await cleanupOnResend(ref);
		}
	...
	const handleCallback = async ({ chatId, messageId, userLabel, payload }) => {
		...
		try {
			...
			mappingByRef[ref] = Object.freeze({
				...(entry || {}),
				ref,
				shortId: typeof entry?.shortId === 'string' ? entry.shortId : shortId,
				textHtml: text,
				shouldHaveButtons: false,
				updatedAt: Date.now(),
			});
	...
	const syncButtonsOnUpdated = async msg => {
		const ref = typeof msg?.ref === 'string' ? msg.ref.trim() : '';
		if (!ref) {
			return;
		}
		...
		const lifecycle = typeof msg?.lifecycle?.state === 'string' ? msg.lifecycle.state.trim().toLowerCase() : '';

		if (lifecycle === 'deleted' || lifecycle === 'expired') {
			await cleanupOnEnded(ref, lifecycle);
			return;
		}
		...
		if (lifecycle === 'closed' || lifecycle === 'deleted' || lifecycle === 'expired') {
			await cleanupButtonsForRef(ref, lifecycle || 'done');
			return;
		}

		const activeStates = new Set(['open', 'acked', 'snoozed']);
		if (lifecycle && !activeStates.has(lifecycle)) {
			return;
		}
		...
		const replyMarkup = buildButtons({ msg, shortId });
		if (!replyMarkup) {
			await cleanupButtonsForRef(ref, 'no actions');
			return;
		}

		const chatMessages = isObject(entry.chatMessages) ? entry.chatMessages : {};
		const tasks = Object.entries(chatMessages).map(([chatId, messageId]) =>
			editMessageSetButtons({ chatId, messageId: Number(messageId), text: baseText, replyMarkup }),
		);
		await Promise.all(tasks);
		debug(`updated: synced buttons ref='${ref}' chats=${Object.keys(chatMessages).length}`);
	};
	...
	return Object.freeze({
		...
		onNotifications: (event, notifications, ctx) => {
			...
			const isCleanupEvent = ev === 'deleted' || ev === 'expired';
			const isUpdatedEvent = ev === 'updated' || ev === 'update';
			...
			for (const msg of list) {
				...
				if (isCleanupEvent) {
					const reason = ev === 'deleted' ? 'deleted' : 'expired';
					cleanupOnEnded(ref, reason).catch(e => log.warn(`cleanup(end) failed: ${e?.message || e}`));
					continue;
				}

				if (isUpdatedEvent) {
					syncButtonsOnUpdated(msg).catch(e => log.warn(`updated sync failed: ${e?.message || e}`));
					continue;
				}
