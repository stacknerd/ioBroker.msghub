...
function EngageTelegram(options = {}) {
	...
	const cleanupButtonsForRef = async (ref, reason = '') => {
		...
		if (current.shouldHaveButtons === false) {
			...
			await saveMappings();
			debug(`cleanup: ref='${ref}' done mapping=removed (no telegram edits; shouldHaveButtons=false)`);
			return;
		}
		...
		const tasks = Object.entries(chatMessages).map(([chatId, messageId]) =>
			editMessageSetButtons({ chatId, messageId: Number(messageId), text, replyMarkup: { inline_keyboard: [] } }),
		);
	...
	const cleanupOnResend = async ref => {
		...
		deleteRefMapping(ref);
		await saveMappings();
		debug(`cleanup(resend): ref='${ref}' done mapping=removed`);
	};
