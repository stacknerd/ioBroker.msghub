...
function EngageTelegram(options = {}) {
	...
	const ensureInitialized = ctx => {
		...
		const formatDateTime = (() => {
			...
			const fmtDateTime = makeFormatter(
				Object.freeze({
					day: '2-digit',
					month: '2-digit',
					hour: '2-digit',
			...
			const fmtTime = makeFormatter(Object.freeze({ hour: '2-digit', minute: '2-digit' }));
			...
			return ts => {
				...
				const d = new Date(ms);
				const now = new Date();
				const fmt = isSameLocalDay(d, now) ? fmtTime : fmtDateTime;
				try {
					return String(fmt.format(d)).replace(/,/g, '').trim();
				} catch {
					return '';
				}
			};
	...
	const sendGateMessage = async text => {
		...
		// Gate check-in/out messages are informational and should follow the same recipient rules
		// as due notifications:
		// - only registered chats (private + group)
		// - per-chat muted state respected
		//
		// We intentionally do *not* use telegram's broadcast mode (`chatId:''`) here, because:
		// - broadcast only targets telegram adapter's own recipients (typically private users)
		// - it does not cover enrolled group chats
		// - it bypasses our local mute preference
		const recipientChats =
			chatRegistry && typeof chatRegistry.listChats === 'function'
				? chatRegistry.listChats(c => c && c.muted !== true)
				: [];
	...
	// Outgoing: send Telegram notification and persist mapping metadata (createdAt)
	...
	// Inbound: accept actions (commands + callback buttons)
	...
	const handleRequestStateChange = async (_id, state) => {
		...
		// Resolve the effective chat id for callbacks.
		//
		// Why this exists:
		// - For callbacks, the Telegram adapter may provide either:
		//   - the chat id where the inline button was clicked (best case), OR
		//   - the user id of the clicker (depends on adapter implementation), OR
		//   - an empty/missing id (transient / adapter quirks).
		// - We want to operate on *the message that was clicked*, and therefore need the
		//   actual target chat id to edit that exact message.
		//
		// Strategy:
		// - Prefer the chatId state when present.
		// - For menu callbacks (opt_<shortId>:...), also use requestMessageId to resolve the
		//   originating chat by looking it up in the MappingStore record.
		let effectiveChatId = typeof chatId === 'string' || typeof chatId === 'number' ? String(chatId).trim() : '';
		if (trimmed.startsWith(CALLBACK_PREFIX) && Number.isFinite(messageId) && messageId > 0) {
			const parsedCb = parseCallbackPayload(trimmed);
			// Only resolve via mapping for menu callbacks. Enrollment callbacks are private-chat-only
			// and should keep using the adapter-provided chat id.
			if (parsedCb && parsedCb.kind !== 'enroll') {
				const ref = mappingStore.getRefByShortId(parsedCb.shortId);
				if (ref) {
					const record = mappingStore.getByRef(ref);
					const chatMessages =
						record?.chatMessages && typeof record.chatMessages === 'object' ? record.chatMessages : {};
					const matchedChatId = Object.entries(chatMessages).find(
						([_cid, mid]) => Number(mid) === messageId,
					)?.[0];
					if (matchedChatId) {
						effectiveChatId = String(matchedChatId).trim();
					}
				}
			}
		}
		...
		log.debug(
			`inbound: request raw='${String(raw)}' user='${parsed.userLabel}' chatId='${String(chatIdStr || '')}' messageId='${String(messageId || '')}'`,
		);

		if (trimmed.startsWith(CALLBACK_PREFIX)) {
			await handleCallback({ chatId: chatIdStr, messageId, userLabel: parsed.userLabel, payload: trimmed });
			return;
		}

		await handleCommand({ chatId: chatIdStr, userLabel: parsed.userLabel, payload });
	};
	 ...
	 * @param {number} [params.messageId] Telegram message id from the callback context.
	 * @param {string} params.userLabel Telegram user label.
	...
	const handleCallback = async ({ chatId, messageId, userLabel, payload }) => {
		...
		if (parsed.kind === 'menu') {
			...
			if (menuModel.hasAny) {
				await menuRuntime.showMenuRootInChat(msgAll, chatId, Number(messageId));
			} else {
				await menuRuntime.showMenuEntryInChat(ref, chatId, Number(messageId));
			}
			return;
		}
		...
		if (parsed.kind === 'nav') {
			...
			if (target === 'snooze') {
				...
				if (menu.snooze) {
					await menuRuntime.showSnoozeMenuInChat(msgAll, chatId, Number(messageId));
				} else {
					await menuRuntime.showMenuRootInChat(msgAll, chatId, Number(messageId));
				}
				return;
			...
			if ((target === 'open' || target === 'link') && parsed.parts[1]) {
				...
				// causes a race: removing menu if all actions are removed due to execution of the action
				// could get overwritten by collapse atempt. intentionally disabled.
				// await menuRuntime.showMenuEntryInChat(ref, chatId, Number(messageId));
			}
			return;
		}
