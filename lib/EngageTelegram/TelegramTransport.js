...
function createTelegramTransport({ iobroker, log, telegramInstance }) {
	...
	const parseSendResponse = res => {
		// Telegram API message object (or adapter passthrough): normalize to `{ [chatId]: messageId }`.
		if (
			res &&
			typeof res === 'object' &&
			!Array.isArray(res) &&
			Number.isFinite(Number(res?.message_id)) &&
			res?.chat &&
			typeof res.chat === 'object' &&
			res.chat.id !== undefined &&
			res.chat.id !== null
		) {
			return { [String(res.chat.id)]: Number(res.message_id) };
		}

		// Some adapters wrap telegram results in `{ ok: true, result: {...} }`.
		if (res && typeof res === 'object' && !Array.isArray(res) && res?.result) {
			return parseSendResponse(res.result);
		}
	...
	/**
	 * Send an image to a specific chat (best effort).
	 *
	 * @param {object} params Params.
	 ...
	 * @returns {Promise<{ chatMessages: object, raw: any }>} Parsed chat-id -> message-id mapping and raw adapter response.
	 */
	const sendImageToChat = async ({ chatId, image, caption = '', disableNotification = true }) => {
		const safeChatId = typeof chatId === 'string' || typeof chatId === 'number' ? chatId : '';
		if (!safeChatId) {
			return { raw: null, chatMessages: {} };
		}
	...
	const sendToChat = ({ chatId, text, parseMode = '', replyMarkup = null, disableNotification = false }) => {
		const safeChatId = typeof chatId === 'string' || typeof chatId === 'number' ? chatId : '';
		if (!safeChatId) {
			return Promise.resolve({ raw: null, chatMessages: {} });
		}
		...
		return send(payload).then(raw => ({ raw, chatMessages: parseSendResponse(raw) }));
	};
	...
	const editMessage = ({ chatId, messageId, html, replyMarkup = null }) => {
		...
		const effectiveReplyMarkup = replyMarkup || { inline_keyboard: [] };
		...
		return send({
			...
			editMessageText: {
				options: {
					...
					parse_mode: 'HTML',
					reply_markup: effectiveReplyMarkup,
				},
				// Keep for compatibility with adapter versions that read it outside of `options`.
				reply_markup: effectiveReplyMarkup,
			},
		});
	};
