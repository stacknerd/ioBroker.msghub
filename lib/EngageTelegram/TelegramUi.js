 ...
 * @param {Function} [deps.formatDateTime] Optional formatter `(epochMs) => string` for rendering timestamps (API locale).
 * @returns {object} UI API.
 */
function createTelegramUi({ callbackPrefix, t, formatDateTime }) {
	...
	const formatTs =
		typeof formatDateTime === 'function'
			...
			: ts => {
					...
					try {
						// Default only; EngageTelegram should inject a locale-aware formatter.
	...
	const renderNotificationText = msg => {
		...
		const titleFullPrefix = typeof display?.titleFullPrefix === 'string' ? display.titleFullPrefix.trim() : '';
		const textFullPrefix = typeof display?.textFullPrefix === 'string' ? display.textFullPrefix.trim() : '';

		const title = String(msg?.title || '').trim();
		const text = String(msg?.text || '').trim();

		const hasTitle = Boolean(title);
		const rawTitle = normalizeTelegramText(
			hasTitle ? `${titleFullPrefix} ${title}`.trim() : `${textFullPrefix} ${text}`.trim(),
		);
		const rawBody = hasTitle ? normalizeTelegramText(text) : '';

		// Meta line: show lifecycle state + "since" timestamp in a friendly way.
		//
		// Goal:
		// - Always show one short status line for all lifecycle states (`open` .. `deleted`).
		// - Use centralized lifecycle label keys from runtime i18n:
		//   `msghub.i18n.core.common.MsgConstants.lifecycle.state.<state>.label`
		// - Render the timestamp in the API locale (injected formatter).
		const stateRaw = msg?.lifecycle?.state;
		const state = typeof stateRaw === 'string' && stateRaw.trim() ? stateRaw.trim() : 'open';
		const labelKey = `msghub.i18n.core.common.MsgConstants.lifecycle.state.${state}.label`;
		const stateLabelRaw = String(translate(labelKey)).trim();
		const stateLabel = stateLabelRaw && stateLabelRaw !== labelKey ? stateLabelRaw : state;

		const stateChangedAt = Number(msg?.lifecycle?.stateChangedAt);
		const createdAt = Number(msg?.timing?.createdAt);
		const updatedAt = Number(msg?.timing?.updatedAt);
		const renderedDataTs = Number(msg?.display?.renderedDataTs);
		const sinceTs = Number.isFinite(stateChangedAt)
			? stateChangedAt
			: Number.isFinite(createdAt)
				...
				: Number.isFinite(updatedAt)
					? updatedAt
					: 0;
		const sinceText = sinceTs ? String(formatTs(sinceTs) || '').trim() : '';

		const dataTs = Math.max(
			Number.isFinite(updatedAt) ? updatedAt : 0,
			Number.isFinite(renderedDataTs) ? renderedDataTs : 0,
		);
		const standText = dataTs ? String(formatTs(dataTs) || '').trim() : '';

		const rawMeta = sinceText
			? standText && sinceText && standText !== sinceText
				? String(
						translate(
							'msghub.i18n.EngageTelegram.notification.meta.stateSinceStand.text',
							stateLabel,
							sinceText,
							standText,
						),
				...
				: String(
						translate(
							'msghub.i18n.EngageTelegram.notification.meta.stateSince.text',
							stateLabel,
							sinceText,
						),
					)
			: String(translate('msghub.i18n.EngageTelegram.notification.meta.state.text', stateLabel));
		...
		const metaHtml = escapeHtml(rawMeta);
	...
	const buildMenuRootKeyboard = ({ shortId, msg, opts }) => {
		...
		if (menu.open) {
			if (menu.open.url) {
				rows.push([
					{ text: String(translate('msghub.i18n.EngageTelegram.ui.action.open.label')), url: menu.open.url },
				]);
			} else {
				rows.push([
					{
			...
			if (menu.link.url) {
				rows.push([
					{ text: String(translate('msghub.i18n.EngageTelegram.ui.action.open.label')), url: menu.link.url },
				]);
			} else {
				rows.push([
					{
