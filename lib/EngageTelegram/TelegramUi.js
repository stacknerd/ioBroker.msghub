...
function createTelegramUi({ callbackPrefix, t, formatDateTime }) {
	...
	const renderNotificationText = msg => {
		...
		const rawTitleMsg = String(msg?.title || '').trim();
		const rawTextMsg = String(msg?.text || '').trim();

		const title = displayTitle || [rawIcon, rawTitleMsg].filter(Boolean).join(' ').trim();
		const text = displayText || rawTextMsg;

		const rawTitle = normalizeTelegramText(title);
		const rawBody = normalizeTelegramText(text);
