...
function createTelegramUi({ callbackPrefix, t, iconByLevel, iconByKind }) {
	...
	const buildMenuEntryKeyboard = shortId => {
		...
		return {
			inline_keyboard: [
				[
					{
						text: String(translate('Menu') || 'Menu'),
	...
	const buildMenuRootKeyboard = ({ shortId, msg, opts }) => {
		...
		if (menu.ack) {
			rowMain.push({
				text: String(translate('Ack') || 'Ack'),
				callback_data: `${prefix}${sid}:act:${menu.ack.id}`,
			});
		}
		if (menu.close) {
			rowMain.push({
				text: String(translate('Close') || 'Close'),
		...
		if (menu.open) {
			if (menu.open.url) {
				navRow.push({ text: String(translate('Open') || 'Open'), url: menu.open.url });
			} else {
				navRow.push({
					text: String(translate('Open') || 'Open'),
					callback_data: `${prefix}${sid}:nav:open:${menu.open.id}`,
				});
			}
		} else if (menu.link) {
			if (menu.link.url) {
				navRow.push({ text: String(translate('Open') || 'Open'), url: menu.link.url });
			} else {
				navRow.push({
					text: String(translate('Open') || 'Open'),
		...
		if (menu.snooze) {
			rows.push([
				{
					text: String(translate('Snooze') || 'Snooze'),
		...
		rows.push([
			{
				text: String(translate('<<< back') || '<<< back'),
	...
	const buildSnoozeKeyboard = ({ shortId, msg, opts }) => {
		...
		for (const ms of menu.snooze.choicesMs || []) {
			...
			rows.push([
				{
					text: `${String(translate('Snooze') || 'Snooze')} ${hours}h`,
		...
		rows.push([
			{
				text: String(translate('<<< message') || '<<< message'),
				callback_data: `${prefix}${sid}:nav:back`,
			},
		]);
