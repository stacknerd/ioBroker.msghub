...
function createConfigCommand() {
	...
	return Object.freeze({
		...
		run: async ({ chatId, telegramInstance, sendTo, i18n, cfg, chatRegistry, constants, gateOpen, coreConfig }) => {
			...
			// Show the local mute state in a human-friendly way.
			// This helps avoid the most common UX pitfall:
			// - user enrolled a group and now receives duplicate notifications (private + group)
			// - user wants to silence one of the chats without touching the core config
			const currentChat = typeof chatRegistry?.getChat === 'function' ? chatRegistry.getChat(id) : null;
			const isMutedHere = currentChat?.muted === true;
			lines.push(
				`<u>${escapeHtml(String(t('msghub.i18n.EngageTelegram.command.config.muteStatus.title')).trim())}</u>`,
			);
			lines.push(
				String(
					t(
						isMutedHere
							? 'msghub.i18n.EngageTelegram.command.config.muteStatus.muted.text'
							: 'msghub.i18n.EngageTelegram.command.config.muteStatus.unmuted.text',
					),
				).trim(),
			);
			lines.push('');
