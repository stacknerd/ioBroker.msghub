...
describe('EngageTelegram TelegramUi', () => {
	const ui = createTelegramUi({
		callbackPrefix: 'opt_',
			t: (key, ...args) => {
				if (key === 'msghub.i18n.core.common.MsgConstants.lifecycle.state.snoozed.label') {
					return 'Snoozed';
				}
				if (key === 'msghub.i18n.EngageTelegram.notification.meta.stateSince.text') {
					return `(${args[0]} since ${args[1]})`;
				}
				if (key === 'msghub.i18n.EngageTelegram.notification.meta.stateSinceStand.text') {
					return `(${args[0]} since ${args[1]} - as of ${args[2]})`;
				}
				if (key === 'msghub.i18n.EngageTelegram.notification.meta.state.text') {
					return `(${args[0]})`;
				}
				return key;
			},
			formatDateTime: ts => (Number(ts) === 2 ? '14.01. 14:45' : '14.01. 12:34'),
		});
	...
	it('escapes html special chars', () => {
		...
		it('renders notification text with escaped HTML', () => {
			const msg = {
				...
				kind: 'task',
				level: 10,
				lifecycle: { state: 'snoozed', stateChangedAt: 1 },
				timing: { createdAt: 1, updatedAt: 2 },
			};
			...
			expect(out.html).to.contain('(Snoozed since 14.01. 12:34 - as of 14.01. 14:45)');
		});
