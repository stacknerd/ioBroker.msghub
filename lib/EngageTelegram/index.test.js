...
describe('EngageTelegram', () => {
	...
	it('does not delete messages on expired when deleteOldNotificationOnResend is enabled', async () => {
		const calls = { sendPayloads: [] };
		const sendTo = (_instance, _cmd, payload) => {
			calls.sendPayloads.push(payload);
			if (payload?.editMessageText) {
		...
		h.onNotifications('expired', [{ ref: 'm1' }], ctx);
		await flush();
		await flush();

		const deletes = calls.sendPayloads.filter(p => !!p?.deleteMessage);
		const edits = calls.sendPayloads.filter(p => !!p?.editMessageText);
		expect(deletes).to.have.length(0);
		expect(edits.length).to.be.greaterThan(0);
	});

	it('syncs buttons on updated and cleans up on closed', async () => {
		const calls = { sendPayloads: [] };
		const sendTo = (_instance, _cmd, payload) => {
			calls.sendPayloads.push(payload);
			if (payload?.editMessageText) {
		...
		calls.sendPayloads.length = 0;

		// Updated with different actions should edit existing message buttons
		h.onNotifications(
			'updated',
			[
				{
		...
		const edited = calls.sendPayloads.filter(p => !!p?.editMessageText);
		expect(edited.length).to.be.greaterThan(0);

		calls.sendPayloads.length = 0;

		// Updated closed should only remove buttons (no deleteMessage)
		h.onNotifications('updated', [{ ...msg, lifecycle: { state: 'closed' } }], ctx);
		await flush();
		await flush();

		const deletes = calls.sendPayloads.filter(p => !!p?.deleteMessage);
		const closeEdits = calls.sendPayloads.filter(p => !!p?.editMessageText);
		expect(deletes).to.have.length(0);
		expect(closeEdits.length).to.be.greaterThan(0);
	});
