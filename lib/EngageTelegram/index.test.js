	...
	const ctx = {
		api: {
			...
			iobroker: {
				...
				states: {
					getForeignState: id => {
						const raw = getForeignState(id);
						if (typeof id === 'string' && id.endsWith('.communicate.users')) {
							return Promise.resolve(raw).then(st => {
								const val = typeof st?.val === 'string' ? st.val.trim() : '';
								if (val) {
									return st;
								}
								return { val: JSON.stringify({ '1': { firstName: 'Test', sysMessages: false } }) };
							});
						}
						return raw;
					},
...
describe('EngageTelegram', () => {
	...
	it('uses gate helper for send gating and check-in/out templates', async () => {
		const sendPayloads = [];
		const sendTo = (_instance, _cmd, payload) => {
			sendPayloads.push(payload);
			return Promise.resolve({ '1': 10 });
		...
		const getForeignState = id => {
			if (id === 'telegram.0.communicate.users') {
				return Promise.resolve({ val: '{"1":{"firstName":"Test","sysMessages":false}}' });
			}
		...
		const options = makeOptionsResolver({
			...
			gateOp: 'true',
			gateValue: '',
			gateBypassFromLevel: MsgConstants.level.critical,
		...
		expect(sendPayloads.filter(p => p?.text)).to.have.length(0);
			...
			[
				{
					ref: 'm2',
					kind: 'task',
					level: MsgConstants.level.critical,
					title: 'Hello',
					text: 'World',
					actions: [{ type: 'ack', id: 'ack' }],
				},
		...
		expect(sendPayloads.filter(p => p?.text)).to.have.length(1);

		await registerCalls[0].onOpen({ prevOpen: false });
		await registerCalls[0].onClose({ prevOpen: true });

		const gateTexts = sendPayloads.filter(p => p?.text).map(p => p.text);
		expect(gateTexts).to.include('open 1');
		expect(gateTexts).to.include('closed 1');
	});

	it('sends gate check-in/out messages to all unmuted registered chats (private + group)', async () => {
		const sendPayloads = [];
		const sendTo = (_instance, _cmd, payload) => {
			sendPayloads.push(payload);
			return Promise.resolve({ [String(payload?.chatId || '')]: 10 });
		};
		const getForeignState = id => {
			if (id === 'telegram.0.communicate.users') {
				return Promise.resolve({ val: '{"1":{"firstName":"Test","sysMessages":false},"2":{"firstName":"Muted","sysMessages":false}}' });
			}
			if (id === 'msghub.0.EngageTelegram.0.chatRegistry') {
				return Promise.resolve({
					val: JSON.stringify({
						chats: {
							'1': { chatId: '1', type: 'private', muted: false, createdAt: 1, updatedAt: 1 },
							'2': { chatId: '2', type: 'private', muted: true, createdAt: 1, updatedAt: 1 },
							'-100': { chatId: '-100', type: 'group', muted: false, createdAt: 1, updatedAt: 1 },
							'-200': { chatId: '-200', type: 'group', muted: true, createdAt: 1, updatedAt: 1 },
						},
						pending: {},
					}),
				});
		...
		const texts = sendPayloads.filter(p => p?.text).map(p => ({ chatId: String(p.chatId), text: p.text }));
		expect(texts.map(t => t.chatId)).to.include('1');
		expect(texts.map(t => t.chatId)).to.include('-100');
		expect(texts.map(t => t.chatId)).to.not.include('2');
		expect(texts.map(t => t.chatId)).to.not.include('-200');
		expect(texts.map(t => t.text)).to.include('open 1');
	});
	...
	it('opens menu on callback and executes actions', async () => {
		const sendToCalls = [];
		const sentIds = Object.freeze({ '1': 101, '-100': 202 });
		const sendTo = (_instance, _cmd, payload) => {
			sendToCalls.push(payload);
			if (payload?.editMessageText) {
				return Promise.resolve(null);
			...
			const chatId = typeof payload?.chatId === 'string' || typeof payload?.chatId === 'number' ? String(payload.chatId) : '';
			const messageId = sentIds[chatId] || 10;
			return Promise.resolve({ [chatId]: messageId });
		};
		...
		const getForeignState = id => {
			...
			if (id === 'telegram.0.communicate.requestMessageId') {
				return Promise.resolve({ val: 101 });
			}
			if (id === 'msghub.0.EngageTelegram.0.chatRegistry') {
				return Promise.resolve({
					val: JSON.stringify({
		...
		await flush();
		await flush();
		const menuEdits = sendToCalls.filter(p => !!p?.editMessageText);
		expect(menuEdits).to.have.length(1);
		expect(menuEdits[0]).to.deep.include({ chatId: '1' });
		...
		const afterEdits = sendToCalls.filter(p => !!p?.editMessageText);
		// No auto-collapse after actions: UI should be driven by the subsequent store update + sync.
		// This avoids races where a collapse edit re-adds a stale menu entry button.
		expect(afterEdits).to.have.length(0);
	});
	...
	it('handles /config', async () => {
		...
		h.onStateChange('telegram.0.communicate.request', { val: '[Ben]/config' }, ctx);
		await flush();
		await flush();
	...
	it('ignores inbound traffic from unregistered group chats', async () => {
		const sendToCalls = [];
		const sendTo = (_instance, _cmd, payload) => {
			sendToCalls.push(payload);
			return Promise.resolve(null);
		...
		const getForeignState = id => {
			if (id === 'telegram.0.communicate.requestChatId') {
				return Promise.resolve({ val: '-123' });
			}
			if (id === 'telegram.0.communicate.requestMessageId') {
				return Promise.resolve({ val: 33 });
			}
		...
		expect(sendToCalls).to.have.length(0);
	});

	it('mutes and unmutes the current chat', async () => {
		const sendToCalls = [];
		const sendTo = (_instance, _cmd, payload) => {
			sendToCalls.push(payload);
			return Promise.resolve(null);
		...
		const getForeignState = id => {
			...
			if (id === 'telegram.0.communicate.users') {
				return Promise.resolve({ val: JSON.stringify({ '1': { firstName: 'Test', sysMessages: false } }) });
			}
			return Promise.resolve({ val: '' });
		};
		...
		h.onStateChange('telegram.0.communicate.request', { val: '[Ben]/mute' }, ctx);
		await flush();
		await flush();

		const registryWrites1 = setStateCalls.filter(([id]) => id.endsWith('.chatRegistry'));
		expect(registryWrites1.length).to.be.greaterThan(0);
		const reg1 = JSON.parse(registryWrites1[registryWrites1.length - 1][1].val);
		expect(reg1.chats['1']).to.deep.include({ chatId: '1', type: 'private', muted: true });

		h.onStateChange('telegram.0.communicate.request', { val: '[Ben]/unmute' }, ctx);
		await flush();
		await flush();

		const registryWrites2 = setStateCalls.filter(([id]) => id.endsWith('.chatRegistry'));
		const reg2 = JSON.parse(registryWrites2[registryWrites2.length - 1][1].val);
		expect(reg2.chats['1']).to.deep.include({ chatId: '1', type: 'private', muted: false });

		// Both commands respond to the current chat.
		expect(sendToCalls.some(p => p?.chatId === '1' && typeof p?.text === 'string')).to.equal(true);
	});

	it('enrolls a group via /startbot and allow button', async () => {
		let currentChatId = '-123';

		const sendPayloads = [];
		let msgId = 100;
		const sendTo = (_instance, _cmd, payload) => {
			sendPayloads.push(payload);
			if (payload?.deleteMessage || payload?.editMessageText) {
				return Promise.resolve(null);
			}
			if (payload?.chatId !== undefined) {
				msgId++;
				return Promise.resolve({ chat: { id: String(payload.chatId) }, message_id: msgId });
			}
			return Promise.resolve(null);
		};
		...
		const getForeignState = id => {
			if (id === 'telegram.0.communicate.requestChatId') {
				return Promise.resolve({ val: currentChatId });
			}
			if (id === 'telegram.0.communicate.requestMessageId') {
				return Promise.resolve({ val: 33 });
			}
			if (id === 'telegram.0.communicate.users') {
				return Promise.resolve({ val: JSON.stringify({ '765': { firstName: 'Ben', sysMessages: false } }) });
			}
			return Promise.resolve({ val: '' });
		};
		...
		h.onStateChange('telegram.0.communicate.request', { val: '[Ben]/startbot' }, ctx);
		await flush();
		await flush();

		// Approval prompt is sent to the requesting user's private chat id (765) with buttons.
		const approval = sendPayloads.find(p => p?.chatId === '765' && p?.reply_markup?.inline_keyboard);
		expect(approval).to.exist;
		const allowCb = approval.reply_markup.inline_keyboard[0][0].callback_data;
		expect(allowCb).to.match(/^opt_[A-Za-z0-9]+:enroll:allow$/);
		const requestId = allowCb.split(':')[0].slice('opt_'.length);

		// Pending request is persisted.
		const latestRegistry = () => {
			const writes = setStateCalls.filter(([id]) => id.endsWith('.chatRegistry'));
			return JSON.parse(writes[writes.length - 1][1].val);
		};

		const reg = latestRegistry();
		expect(reg.pending).to.have.property(requestId);

		// Allow callback from private chat registers the group.
		currentChatId = '765';
		h.onStateChange('telegram.0.communicate.request', { val: `[Ben]opt_${requestId}:enroll:allow` }, ctx);
		await flush();
		await flush();

		const reg2 = latestRegistry();
		expect(reg2.chats).to.have.property('-123');
		expect(reg2.chats['-123']).to.deep.include({ chatId: '-123', type: 'group', muted: false });

		// Group status message is edited (neutral, no buttons needed).
		expect(sendPayloads.some(p => p?.editMessageText?.options?.chat_id === '-123')).to.equal(true);
	});

	it('unenrolls a group via /stopbot and allow button', async () => {
		let currentChatId = '-123';
		...
		const approval = sendPayloads.find(p => p?.chatId === '765' && p?.reply_markup?.inline_keyboard);
		const allowCb = approval.reply_markup.inline_keyboard[0][0].callback_data;
		const enrollId = allowCb.split(':')[0].slice('opt_'.length);

		currentChatId = '765';
		h.onStateChange('telegram.0.communicate.request', { val: `[Ben]opt_${enrollId}:enroll:allow` }, ctx);
		await flush();
		await flush();

		expect(latestRegistry().chats).to.have.property('-123');

		// Now: stopbot from group creates a new request.
		currentChatId = '-123';
		h.onStateChange('telegram.0.communicate.request', { val: '[Ben]/stopbot' }, ctx);
		await flush();
		await flush();

		const stopApproval = sendPayloads
			.filter(p => p?.chatId === '765' && p?.reply_markup?.inline_keyboard)
			.slice(-1)[0];
		expect(stopApproval).to.exist;
		const stopAllowCb = stopApproval.reply_markup.inline_keyboard[0][0].callback_data;
		const unenrollId = stopAllowCb.split(':')[0].slice('opt_'.length);

		currentChatId = '765';
		h.onStateChange('telegram.0.communicate.request', { val: `[Ben]opt_${unenrollId}:enroll:allow` }, ctx);
		await flush();
		await flush();

		expect(latestRegistry().chats).to.not.have.property('-123');
	});
});
