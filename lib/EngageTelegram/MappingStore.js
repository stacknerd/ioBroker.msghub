...
function createMappingStore({ iobroker, log, baseFullId, retentionMs = DEFAULT_RETENTION_MS }) {
	...
	/**
	 * Build the derived short-id index from `mappingByRef`.
	 *
	 * `mappingShortToRef` is an optimization (callback routing); the source of truth is `mappingByRef`.
	 * Because state writes are not transactional (3 separate states), the stored index can drift
	 * from the stored ref records over time (e.g. restart/crash between writes).
	 *
	 * To keep the system stable, we rebuild the index on `load()` and `save()`.
	 *
	 * @param {object} byRef Mapping by ref.
	 * @returns {object} Fresh `{ [shortId]: ref }` index.
	 */
	const buildShortIndex = byRef => {
		const src = byRef && typeof byRef === 'object' ? byRef : {};
		const out = Object.create(null);
		for (const [ref, entry] of Object.entries(src)) {
			if (!entry || typeof entry !== 'object') {
				continue;
			}
			const key = typeof ref === 'string' ? ref.trim() : '';
			const shortId = typeof entry.shortId === 'string' ? entry.shortId.trim() : '';
			if (key && shortId) {
				out[shortId] = key;
			}
		}
	...
	const load = async () => {
		...
		// Always rebuild the short-id index from `mappingByRef` to prevent unbounded drift.
		const derived = buildShortIndex(mappingByRef);
		const stored = shortToRef && typeof shortToRef === 'object' ? shortToRef : null;
		if (stored) {
			const storedCount = Object.keys(stored).length;
			const derivedCount = Object.keys(derived).length;
			if (storedCount !== derivedCount) {
				debug(`mappings: rebuilt shortId index stored=${storedCount} derived=${derivedCount}`);
			}
		}
		mappingShortToRef = derived;
	};
	...
	const save = async (opts = {}) => {
		...
		// Persist a fresh derived index to guarantee consistency on disk.
		mappingShortToRef = buildShortIndex(mappingByRef);
