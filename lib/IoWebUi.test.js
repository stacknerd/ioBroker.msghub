...
describe('IoWebUi handleCommand', () => {
	...
	it('webExtension host accepts web tokens minted for webExtension', async () => {
		const capabilities = createCapabilities();
		const webUi = new IoWebUi(createAdapter(), { adminCapabilities: capabilities });
		const res = await webUi.handleCommand(
			'web.ping',
			{
				token: capabilities.mintToken({ host: 'webExtension', capability: 'web' }).token,
				host: 'ignored-client',
			},
			{ host: 'webExtension' },
		);
		expect(res).to.deep.equal({ ok: true, data: 'pong' });
	});

	it('webExtension host rejects admin-host web tokens', async () => {
		const capabilities = createCapabilities();
		const webUi = new IoWebUi(createAdapter(), { adminCapabilities: capabilities });
		const res = await webUi.handleCommand(
			'web.ping',
		...
		expect(res.error.message).to.match(/Token host mismatch/);
	});
	...
	it('strips payload.token before web command execution', async () => {
		...
		const webUi = new IoWebUi(createAdapter(), {
			adminCapabilities: capabilities,
			msgStore: {
				queryMessages: payload => {
					seenPayload = payload;
					return { total: 0, pages: 1, items: [] };
				},
			},
		});
		const res = await webUi.handleCommand('web.messages.query', {
		...
		expect(res.ok).to.equal(true);
		expect(seenPayload).to.deep.equal({ where: { kind: 'task' } });
	});
});
