...
function IngestRandomChaos(options = {}) {
	...
	const applyResolvedOptions = ctx => {
		const opts = ctx?.meta?.options;
		if (!opts || typeof opts.resolveInt !== 'function') {
			throw new Error('IngestRandomChaos: missing ctx.meta.options.resolveInt (IoPlugins wiring required)');
		}
		intervalMinMs = opts.resolveInt('intervalMinMs', options.intervalMinMs);
		intervalMaxMs = opts.resolveInt('intervalMaxMs', options.intervalMaxMs);
		intervalMaxMs = Math.max(intervalMinMs, intervalMaxMs);
		maxPool = opts.resolveInt('maxPool', options.maxPool);
	};
	...
	return {
		start(ctx) {
			...
			ctxRef = ctx;
			const pluginBaseObjectId =
				typeof options?.pluginBaseObjectId === 'string' ? options.pluginBaseObjectId.trim() : '';
			const idsApi = ctx?.api?.iobroker?.ids;
			const namespace =
				(typeof idsApi?.namespace === 'string' && idsApi.namespace) ||
				(typeof ctx?.api?.iobroker?.namespace === 'string' && ctx.api.iobroker.namespace) ||
				'';
			const computedOwnId =
				pluginBaseObjectId && typeof idsApi?.toOwnId === 'function'
					? idsApi.toOwnId(pluginBaseObjectId)
					: pluginBaseObjectId && namespace
						? pluginBaseObjectId.startsWith(`${namespace}.`)
							? pluginBaseObjectId.slice(`${namespace}.`.length)
							: pluginBaseObjectId
						: pluginBaseObjectId;
			if (typeof computedOwnId === 'string' && computedOwnId.trim()) {
				refOwnId = computedOwnId.trim();
			}
			running = true;
			stopTimer();
			scheduleNext();
