...
function IngestRandomChaos(options = {}) {
	...
	const updateOne = () => {
		...
		if (currentState === stateOpen) {
			...
			if (isTask && roll < 0.25) {
				patch.lifecycle = { state: stateSnoozed, stateChangedAt: now, stateChangedBy: ACTOR };
				patch.timing = { notifyAt: now + randomInt(1000 * 15, 1000 * 90) };
			} else if (roll < 0.6) {
				patch.lifecycle = { state: stateAcked, stateChangedAt: now, stateChangedBy: ACTOR };
			} else if (roll < 0.75) {
				patch.lifecycle = { state: stateClosed, stateChangedAt: now, stateChangedBy: ACTOR };
			}
		} else if (currentState === stateSnoozed) {
			patch.lifecycle = { state: stateOpen, stateChangedAt: now, stateChangedBy: ACTOR };
	...
	return {
		start(ctx) {
			...
			stopTimer();
			scheduleNext();
			log.info(`IngestRandomChaos: started (interval=${intervalMinMs}..${intervalMaxMs}ms, maxPool=${maxPool})`);
		},
		stop(_ctx) {
			running = false;
			stopTimer();
			...
			pool.clear();
			log.info('IngestRandomChaos: stopped');
		},
	};
}
