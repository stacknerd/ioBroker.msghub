...
describe('IngestRandomChaos', () => {
	...
	it('removes stale messages from previous runs on stop', () => {
		const { log } = makeLog();
		const { resources } = makeResources();
		const store = makeStore();
		const factory = makeFactory();
		const options = makeOptionsResolver({ intervalMinMs: 99999, intervalMaxMs: 99999, maxPool: 1 });

		const staleRef = 'IngestRandomChaos.0.stale.task.1';
		store.addMessage({
			ref: staleRef,
			origin: { system: 'IngestRandomChaos', id: 'IngestRandomChaos', type: MsgConstants.origin.type.automation },
			lifecycle: { state: MsgConstants.lifecycle.state.open },
		});

		const h = IngestRandomChaos({ intervalMinMs: 99999, intervalMaxMs: 99999, maxPool: 1 });
		h.start(makeCtx({ log, store, factory, resources, options }));
		h.stop();

		expect(store.calls.removeMessage).to.include(staleRef);
	});
