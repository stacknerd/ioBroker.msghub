...
class IoManagedMeta {
	...
	async _ensureManagedMetaAsync({ id, managedBy, managedText }) {
		...
		const existingCustom =
			isObject(obj.common) && isObject(obj.common.custom) && isObject(obj.common.custom[customKey])
				...
				: {};
		const existingCustomManagedMeta = isObject(existingCustom.managedMeta)
			? existingCustom.managedMeta
			: isObject(existingCustom.meta)
				? existingCustom.meta
				: {};

		const desiredManagedBy = hasManagedBy ? managedBy : existingCustomManagedMeta.managedBy;
		const desiredManagedText = hasManagedText ? managedText : existingCustomManagedMeta.managedText;
		const desiredManagedSince =
			typeof existingCustomManagedMeta.managedSince === 'string'
				? existingCustomManagedMeta.managedSince
				: nowIso;

		const needsUpdate =
			(hasManagedBy && existingCustomManagedMeta.managedBy !== managedBy) ||
			(hasManagedText &&
				(typeof existingCustomManagedMeta.managedText !== 'string' ||
					existingCustomManagedMeta.managedText !== managedText)) ||
			typeof existingCustomManagedMeta.managedSince !== 'string' ||
			existingCustomManagedMeta.managedMessage !== true ||
			...
			common: {
				custom: {
					[customKey]: {
						...
						managedMeta: {
							...(isObject(existingCustomManagedMeta) ? existingCustomManagedMeta : {}),
							managedBy: desiredManagedBy,
							managedText: desiredManagedText,
							managedSince: desiredManagedSince,
							managedMessage: true,
						},
						enabled: true,
					},
	...
	async _runJanitorOnce() {
		...
		try {
			...
			for (const obj of objects) {
				...
				const managedMeta = isObject(custom.managedMeta)
					? custom.managedMeta
					: isObject(custom.meta)
						? custom.meta
						: null;
				const managedBy = typeof managedMeta?.managedBy === 'string' ? managedMeta.managedBy.trim() : '';
				if (!managedBy) {
					continue;
				}
				...
				await this._applyOrphanPolicy({ id, customKey, custom, managedMeta });
			}
		} catch (e) {
	 ...
	 * @param {object|null} options.managedMeta Existing managed meta entry.
	 * @returns {Promise<void>} Resolves after best-effort patch.
	 */
	async _applyOrphanPolicy({ id, customKey, custom, managedMeta }) {
		const currentCustom = isObject(custom) ? custom : {};
		const currentManagedMeta = isObject(currentCustom.managedMeta)
			? currentCustom.managedMeta
			: isObject(managedMeta)
				? managedMeta
				: {};
		...
		const nextCustom = {
			...currentCustom,
			managedMeta: {
				...(isObject(currentManagedMeta) ? currentManagedMeta : {}),
		...
		for (const id of list) {
			...
			const managedMeta = isObject(custom.managedMeta)
				...
				: isObject(custom.meta)
					? custom.meta
					: null;
			const currentManagedBy = typeof managedMeta?.managedBy === 'string' ? managedMeta.managedBy.trim() : '';
			if (currentManagedBy !== managedBy) {
				continue;
			}

			await this._applyOrphanPolicy({ id: id.trim(), customKey, custom, managedMeta });
