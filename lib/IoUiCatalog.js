...
const { normalizePluginUiLang } = require('./IoPluginPanelResolver');
...
class IoUiCatalog {
	 ...
	 *
	 * @param {any} request Raw request payload (`{ mode, targetId?, lang? }`).
	 * @returns {Promise<{ composition: object, corePanels: Record<string, object>, pluginPanels: Record<string, object>, request: { mode: string, targetId: string, lang: string } }>}
	 *   Canonical backend view payload.
	 */
	async getView(request) {
		const normalizedRequest = this._normalizeRequest(request);
		const runtimePluginPanels = await this._getRuntimePluginPanels(normalizedRequest.lang);
	 ...
	 * @returns {{ mode: 'composition', targetId: string, lang: string } | { mode: 'panel', targetId: string, lang: string }}
	...
	_normalizeRequest(request) {
		...
		const lang = normalizePluginUiLang(safeRequest?.lang);
		...
		return { mode, targetId: targetIdRaw, lang };
	}

	/**
	 * Build the view payload for a composition request.
	 *
	 * @param {{ mode: string, targetId: string, lang: string }} request Normalized composition request.
	 * @param {Record<string, object>} runtimePluginPanels Plugin panels keyed by runtime panel id.
	 * @returns {{ composition: object, corePanels: Record<string, object>, pluginPanels: Record<string, object>, request: { mode: string, targetId: string, lang: string } }}
	 ...
	 *
	 * @param {{ mode: string, targetId: string, lang: string }} request Normalized panel request.
	 * @param {Record<string, object>} runtimePluginPanels Plugin panels keyed by runtime panel id.
	 * @returns {{ composition: object, corePanels: Record<string, object>, pluginPanels: Record<string, object>, request: { mode: string, targetId: string, lang: string } }}
