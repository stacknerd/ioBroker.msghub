...
const APP_ICON_SLOTS = Object.freeze(['any192', 'any512', 'maskable192', 'maskable512', 'apple180']);
const GENERIC_PLUGIN_APP_ICONS = Object.freeze({
...
class IoUiCatalog {
	...
	/**
	 * Resolve one normalized app payload for a panel target.
	 *
	 * Only panel targets are supported. The returned entry keeps the canonical panel shape
	 * and is enriched with the effective host-ready icon slots.
	 *
	 * @param {any} request Raw request payload (`{ mode: 'panel', targetId }`).
	 * @returns {Promise<object|null>} Canonical panel entry with `resolvedAppIcons`, or null.
	 */
	async getApp(request) {
		const normalizedRequest = this._normalizeAppRequest(request);
		const panelTarget = this._parsePanelTarget(normalizedRequest.targetId);
		if (panelTarget.kind === 'plugin') {
			const runtimePluginPanel = await this._getRuntimePluginPanel(panelTarget);
			if (!this._hasValidAppBlock(runtimePluginPanel?.app)) {
				return null;
			}
			return this._toViewPluginPanel(runtimePluginPanel);
		}

		const panelDef = this.registry.panels[panelTarget.panelId];
		if (!panelDef || typeof panelDef !== 'object' || !this._hasValidAppBlock(panelDef.app)) {
			return null;
		}
		return this._toViewCorePanel(panelDef, panelTarget.panelId);
	}
	...
	/**
	 * Validate and normalize the incoming app request.
	 *
	 * `getApp(...)` is panel-only and rejects composition mode explicitly.
	 *
	 * @param {any} request Raw request payload.
	 * @returns {{ mode: 'panel', targetId: string }} Normalized panel request.
	 */
	_normalizeAppRequest(request) {
		const safeRequest = request && typeof request === 'object' ? request : null;
		const mode = typeof safeRequest?.mode === 'string' ? safeRequest.mode.trim() : '';
		const targetIdRaw = typeof safeRequest?.targetId === 'string' ? safeRequest.targetId.trim() : '';
		if (mode !== 'panel') {
			throw this._badRequest("mode must be 'panel'");
		}
		if (!targetIdRaw) {
			throw this._badRequest('targetId is required for panel mode');
		}
	...
	/**
	 * Resolve one runtime plugin panel through the canonical backend resolver.
	 *
	 * `getApp(...)` treats an unavailable runtime as "not currently resolvable" and returns null.
	 *
	 * @param {{ pluginType: string, instanceId: number, panelId: string }} panelTarget Parsed plugin target.
	 * @returns {Promise<object|null>} Resolved runtime panel or null.
	 */
	async _getRuntimePluginPanel(panelTarget) {
		if (!this.pluginPanelResolver || typeof this.pluginPanelResolver !== 'object') {
			return null;
		}
		if (typeof this.pluginPanelResolver.getPanelByRef === 'function') {
			try {
				return await this.pluginPanelResolver.getPanelByRef({
		...
		const runtimePluginPanels = await this._getRuntimePluginPanels();
		return runtimePluginPanels[this._toRuntimePanelId(panelTarget)] || null;
	}

	/**
	 * Create the synthetic single composition used for `panel=` requests.
	 *
	 * Both core and plugin targets become single structured refs so the delivered
	 * `composition.panels[]` contract stays symmetric.
	 *
	...
	_buildSyntheticComposition(targetId, panelTarget) {
		...
		return {
			id: compositionId,
			layout: 'single',
			panels: [this._buildCoreRef(panelTarget.panelId)],
	...
	/**
	 * Materialize wildcard compositions backend-side and keep explicit panel refs stable.
	 *
	...
	_materializeComposition(composition, runtimePluginPanels) {
		...
		if (isWildcard) {
			for (const panelId of Object.keys(this.registry.panels)) {
				panels.push(this._buildCoreRef(panelId));
			}
			...
			for (const entry of sourcePanels) {
				if (
					entry &&
					typeof entry === 'object' &&
					entry.type === 'corePanel' &&
					typeof entry.panelId === 'string' &&
					entry.panelId
				) {
					panels.push(this._buildCoreRef(entry.panelId));
					continue;
				}
				if (entry && typeof entry === 'object' && entry.type === 'pluginPanel') {
					panels.push(this._buildPluginRef(entry));
				}
			}
		}
	...
	_extractCorePanels(composition) {
		...
		for (const entry of panelEntries) {
			...
			const panelDef = this.registry.panels[panelId];
			if (panelDef && typeof panelDef === 'object') {
				corePanels[panelId] = this._toViewCorePanel(panelDef, panelId);
			}
		}
		return corePanels;
	}
	...
	_toViewPluginPanel(panel) {
		return {
			...
			...(panel.app && typeof panel.app === 'object' ? { app: this._toPluginAppPayload(panel.app) } : {}),
			resolvedAppIcons: this._resolveAppIcons(panel, { kind: 'plugin' }),
		};
	}

	/**
	 * Convert one plugin-owned app block into the catalog-safe payload.
	 *
	 * Plugin panel consumers must not receive plugin-owned `app.icons`.
	 * The effective icon policy is exposed only through `resolvedAppIcons`.
	 *
	 * @param {object} app Plugin-owned app block.
	 * @returns {object} Plugin app payload without `icons`.
	 */
	_toPluginAppPayload(app) {
		const { icons, ...safeApp } = app || {};
		void icons;
		return safeApp;
	}

	/**
	 * Convert one core panel definition into the view-safe `corePanels[*]` DTO.
	 *
	 * @param {object} panelDef Core panel definition from the registry.
	 * @param {string} panelId Owner-local core panel id.
	 * @returns {object} View-safe core panel payload.
	 */
	_toViewCorePanel(panelDef, panelId) {
		return {
			...panelDef,
			id: typeof panelDef?.id === 'string' && panelDef.id.trim() ? panelDef.id : panelId,
			resolvedAppIcons: this._resolveAppIcons(panelDef, { kind: 'core', panelId }),
		};
	}

	/**
	 * Return whether the given app block is formally valid for catalog app resolution.
	 *
	 * The current minimal validity contract is a non-empty object with non-empty `name` and `url`.
	 *
	 * @param {any} app Candidate app block.
	 * @returns {boolean} True when the block is valid enough for `getApp(...)`.
	 */
	_hasValidAppBlock(app) {
		return !!(
			app &&
			typeof app === 'object' &&
	...
	/**
	 * Resolve the effective host-ready app icon slots for one panel entry.
	 *
	 * Core panels keep their owner-local filenames under the host-relative `icons/<panelId>/...` asset root.
	 * Plugin panels always use the fixed host-owned generic plugin icon set.
	 *
	 * @param {object} panelEntry Core or resolved plugin panel entry.
	 * @param {{ kind: 'core', panelId: string } | { kind: 'plugin' }} context Resolution context.
	 * @returns {Record<string, string>} Effective icon slots.
	 */
	_resolveAppIcons(panelEntry, context) {
		if (!this._hasValidAppBlock(panelEntry?.app)) {
			return {};
		}
		if (context.kind === 'plugin') {
			return { ...GENERIC_PLUGIN_APP_ICONS };
		}

		const resolvedIcons = Object.fromEntries([]);
		const rawIcons =
			panelEntry?.app?.icons && typeof panelEntry.app.icons === 'object' ? panelEntry.app.icons : null;
		const panelId = typeof context.panelId === 'string' ? context.panelId.trim() : '';
		if (!rawIcons || !panelId) {
			return resolvedIcons;
		}
		for (const slot of APP_ICON_SLOTS) {
			const fileName = typeof rawIcons[slot] === 'string' ? rawIcons[slot].trim() : '';
			if (fileName) {
				resolvedIcons[slot] = `icons/${panelId}/${fileName}`;
			}
		}
		return resolvedIcons;
	}
	...
	/**
	 * Build a structured core-panel ref from one owner-local panel id.
	 *
	 * @param {string} panelId Owner-local core panel id.
	 * @returns {{ type: 'corePanel', panelId: string }} Structured core-panel ref.
	 */
	_buildCoreRef(panelId) {
		return {
			type: 'corePanel',
			panelId,
		};
