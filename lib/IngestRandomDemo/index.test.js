...
describe('IngestRandomDemo', () => {
	...
	it('creates a first message immediately and schedules interval', () => {
		...
		expect(createdInput.ref).to.equal('msghub.0.IngestRandomDemo.0_ref01');
	...
	it('updates an existing ref on subsequent ticks', () => {
		...
		expect(messages.has('msghub.0.IngestRandomDemo.0_ref01')).to.equal(true);
		...
		expect(calls.updateMessage[0][0]).to.equal('msghub.0.IngestRandomDemo.0_ref01');
