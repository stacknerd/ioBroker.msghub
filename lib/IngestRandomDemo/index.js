 ...
 */
function IngestRandomDemo(adapter, { intervalMs = 15000, ttlMs = 120000, ttlJitter = 0.5, refPoolSize = 15 } = {}) {
	if (!adapter) {
		throw new Error('IngestRandomDemo: adapter is required');
	}

	const refPrefix = `${adapter.name}.${adapter.instance}.ingestRandomDemo`;
