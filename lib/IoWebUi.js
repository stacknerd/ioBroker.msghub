...
/**
 * Normalize bundle-language requests to the backend-safe form used by `readAdminUiBundle(...)`.
 *
 * @param {any} lang Requested language.
 * @returns {string} Safe language or `en`.
 */
function normalizeBundleLang(lang) {
	const rawLang = typeof lang === 'string' ? lang.trim().toLowerCase() : '';
	return /^[a-z]{2}(-[a-z]{2,4})?$/.test(rawLang) ? rawLang : 'en';
}

const BUNDLE_PARTS = Object.freeze(['js', 'css', 'i18n']);

/**
 * Normalize bundle projection requests to a canonical part list.
 *
 * Rules:
 * - valid parts: `js`, `css`, `i18n`
 * - empty `include` / `exclude` behaves like `null`
 * - `exclude` wins over `include`
 * - when neither list is supplied, `null` means "full bundle"
 *
 * @param {any} payload Raw request payload.
 * @returns {string[]|null} Canonical part list or `null` for the full bundle.
 */
function normalizeBundleParts(payload) {
	const normalizePartList = value => {
		...
		for (const part of value) {
			...
			if (!BUNDLE_PARTS.includes(normalized) || out.includes(normalized)) {
				continue;
			}
			out.push(normalized);
		}
	...
	const include = normalizePartList(payload?.include);
	const exclude = normalizePartList(payload?.exclude);
	if (!include && !exclude) {
		return null;
	}

	const selected = include ? include.slice() : BUNDLE_PARTS.slice();
	if (!exclude) {
		return selected;
	}
	return selected.filter(part => !exclude.includes(part));
}
...
class IoWebUi {
	...
	constructor(
		...
		this.pluginPanelResolver =
			pluginPanelResolver && typeof pluginPanelResolver === 'object'
				? pluginPanelResolver
				: new IoPluginPanelResolver({ ioPlugins: this.ioPlugins, log: adapter?.log || null });
		this.pluginUiRpc =
			pluginUiRpc && typeof pluginUiRpc === 'object'
				? pluginUiRpc
				: new IoPluginUiRpc(adapter, this.ioPlugins, { pluginPanelResolver: this.pluginPanelResolver });
		this.uiCatalog =
			uiCatalog && typeof uiCatalog === 'object'
				? uiCatalog
				: new IoUiCatalog({ pluginPanelResolver: this.pluginPanelResolver });
	 ...
	 *
	 * @param {any} payload Request payload: `{ pluginType, instanceId?, panelId, lang?, include?, exclude? }`.
	...
	async _pluginUiBundleGet(payload) {
		if (
			...
			!this.pluginPanelResolver ||
			typeof this.pluginPanelResolver.getPanelByRef !== 'function'
		) {
		...
		let panel;
		try {
			panel = await this.pluginPanelResolver.getPanelByRef({
				pluginType,
				instanceId,
				panelId,
			});
		...
		const hash = typeof panel.ui?.bundle?.hash === 'string' ? panel.ui.bundle.hash : '';
		const lang = normalizeBundleLang(payload?.lang);
		const parts = normalizeBundleParts(payload);
		const partsKey = Array.isArray(parts) ? parts.join(',') : 'all';
		const cacheKey = `${pluginType}:${instanceId}:${panelId}:${hash}:${lang}:${partsKey}`;
		...
		let bundle;
		try {
			bundle = await this.ioPlugins.readAdminUiBundle({ type: pluginType, instanceId, panelId, lang, parts });
		} catch (e) {
			if (e?.code === 'NOT_FOUND') {
		...
		if (typeof bundle.js === 'string' && Buffer.byteLength(bundle.js, 'utf8') > JS_LIMIT) {
			return this._err('INTERNAL', `Bundle JS exceeds ${JS_LIMIT} byte limit`);
		}
		if (typeof bundle.css === 'string' && Buffer.byteLength(bundle.css, 'utf8') > CSS_LIMIT) {
		...
		const data = {
			...
			moduleFormat: 'esm',
			hash,
			...(typeof bundle.js === 'string' ? { js: bundle.js } : {}),
			...(bundle.css != null ? { css: bundle.css } : {}),
			...(Object.prototype.hasOwnProperty.call(bundle, 'i18n') ? { i18n: bundle.i18n ?? null } : {}),
		};
	...
	async _viewGet(payload) {
		...
		try {
			return this._ok(await this.uiCatalog.getView(payload));
		} catch (e) {
