...
describe('IngestRandomDemo', () => {
	...
	it('creates a first message immediately and schedules interval', () => {
		...
		const plugin = IngestRandomDemo(adapter, { intervalMs: 1234, ttlMs: 60000, refPoolSize: 1 });
		...
		expect(createdInput.ref).to.equal('IngestRandomDemo.01');
	...
	it('updates an existing ref on subsequent ticks', () => {
		...
		const plugin = IngestRandomDemo(adapter, { intervalMs: 10, ttlMs: 5000, refPoolSize: 1 });
		...
		expect(messages.has('IngestRandomDemo.01')).to.equal(true);
		...
		try {
			...
			expect(calls.updateMessage[0][0]).to.equal('IngestRandomDemo.01');
