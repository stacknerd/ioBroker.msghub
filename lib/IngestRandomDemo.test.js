...
describe('IngestRandomDemo', () => {
	...
	it('creates a first message immediately and schedules interval', () => {
		...
		const plugin = IngestRandomDemo(adapter, { intervalMs: 1234, ttlMs: 60000, ttlJitter: 0, refPoolSize: 1 });
		...
		expect(createdInput.ref).to.equal('msghub.0.ingestRandomDemo.01');
	...
	it('updates an existing ref on subsequent ticks', () => {
		...
		const plugin = IngestRandomDemo(adapter, { intervalMs: 10, ttlMs: 5000, ttlJitter: 0, refPoolSize: 1 });
		...
		expect(messages.has('msghub.0.ingestRandomDemo.01')).to.equal(true);
		...
		try {
			...
			expect(calls.updateMessage[0][0]).to.equal('msghub.0.ingestRandomDemo.01');
	...
	it('logs and continues when tick throws', () => {
		...
		const plugin = IngestRandomDemo(adapter, { intervalMs: 10 });
	...
	it('does not schedule twice and stop is idempotent', () => {
		const { adapter } = makeAdapter();
		const { ctx } = makeCtx();
		const plugin = IngestRandomDemo(adapter, { intervalMs: 10, refPoolSize: 1 });
