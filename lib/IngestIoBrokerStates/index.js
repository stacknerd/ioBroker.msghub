 ...
 *   (e.g. `msghub.0.IngestIoBrokerStates.0`).
 * @param {number} [options.rescanIntervalMs] Polling interval to discover new customs (default: 180000, 0 = off).
 * @param {boolean} [options.traceEvents] Log per-event routing/custom payloads (default: false).
 */
function IngestIoBrokerStates(adapter, options = {}) {
	if (!adapter) {
		throw new Error('IngestIoBrokerStates: adapter is required');
	}

	const { rescanIntervalMs = 180000, traceEvents = false, pluginBaseObjectId } = options;

	const baseFullId = typeof pluginBaseObjectId === 'string' ? pluginBaseObjectId.trim() : '';
	if (!baseFullId) {
		throw new Error('IngestIoBrokerStates: options.pluginBaseObjectId is required');
	}

	const engine = new IngestIoBrokerStatesEngine(adapter, {
		rescanIntervalMs,
		traceEvents,
		pluginBaseObjectId: baseFullId,
	});
	return {
		start: () => engine.start(),
		stop: () => engine.stop(),
