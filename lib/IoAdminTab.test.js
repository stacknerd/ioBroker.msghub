...
describe('IoAdminTab IngestStates presets', () => {
	...
	it('allows user presets with ownedBy to be created via admin', async () => {
		const tab = createAdminTab();
		const preset = makePresetValue({ presetId: 'sessionStartCustom', ownedBy: 'Session', subset: 'start' });
		const up = await tab.handleCommand('admin.ingestStates.presets.upsert', { preset });
		expect(up.ok).to.equal(true);

		const get = await tab.handleCommand('admin.ingestStates.presets.get', { presetId: 'sessionStartCustom' });
		expect(get.ok).to.equal(true);
	...
	it('rejects builtin presets on upsert', async () => {
		const tab = createAdminTab();
		const preset = makePresetValue({ presetId: 'builtinClone', source: 'builtin', ownedBy: 'Threshold' });
		const up = await tab.handleCommand('admin.ingestStates.presets.upsert', { preset });
		expect(up.ok).to.equal(false);
		expect(up.error.code).to.equal('FORBIDDEN');
	});
