...
function NotifyPushover(options = {}) {
	...
	const ensureInitialized = ctx => {
		...
		ensureCtxAvailability('NotifyPushover', ctx, {
			plainObject: ['api', 'meta', 'meta.options', 'api.log', 'api.i18n', 'api.iobroker'],
			fn: [
				'api.log.info',
				'api.log.warn',
				'api.i18n.t',
	...
	const matchFilters = msg => {
		...
		if (cfg.priorityByLevel[level] === undefined) {
			return false;
		}
	...
	const sendMessage = msg => {
		...
		const title = `${iconKind}${iconLevel} ${String(msg.title || '').trim()}`.trim();
