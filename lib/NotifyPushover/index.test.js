...
function makeCtx({ options, sendTo, getForeignState, templates, gates } = {}) {
	const { log } = makeLog();
	return {
		ctx: {
			api: {
...
describe('NotifyPushover', () => {
	...
	it('respects gate and bypass-from-level', async () => {
		...
		h.onNotifications(
			'due',
			[
				{ ref: 'm1', kind: 'task', level: MsgConstants.level.warning, title: 'a', text: 'x', display: { title: 'w a', text: 'x' } },
				{ ref: 'm2', kind: 'task', level: MsgConstants.level.error, title: 'b', text: 'y', display: { title: 'e b', text: 'y' } },
			],
			ctx,
		);
	...
	it('applies kind/tag filters', async () => {
		...
		h.onNotifications(
			'due',
			[
				{ ref: 'm1', kind: 'status', level: MsgConstants.level.warning, title: 'a', text: 'x', display: { title: 'w a', text: 'x' } },
				{ ref: 'm2', kind: 'task', level: MsgConstants.level.warning, title: 'b', text: 'y', audience: { tags: ['other'] }, display: { title: 'w b', text: 'y' } },
				{
					ref: 'm3',
					kind: 'task',
					level: MsgConstants.level.warning,
