...
function BridgeAlexaShopping(options = {}) {
	...
	const parseAlexaItems = raw => {
		...
		try {
			...
			return { items: Array.isArray(items) ? items : [], fingerprint: v };
		} catch {
			return { items: [], fingerprint: v };
		}
	};
			...
			if (!internalId) {
				if (isTildeValue) {
					if (!tildeOrphanWarned) {
						...
						ctx.api.log.warn(
							`ignoring tilde-prefixed Alexa item without pending match (extId=${extId}, value=${rawValue})`,
						);
					}
					continue;
				}
