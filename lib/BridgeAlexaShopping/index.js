 ...
 *
 * Docs: ../../docs/plugins/BridgeAlexaShopping.md
 *
...
function BridgeAlexaShopping(options = {}) {
	...
	const applyResolvedOptions = ctx => {
		...
		cfg = Object.freeze({
			...
			messageIcon: o.resolveString('messageIcon', options.messageIcon),
	...
	// Parser language follows i18n text language; format locale is intentionally not the primary source.
	const getLocale = ctx => normalizeLocale(ctx?.api?.i18n?.i18nlocale || 'en');
	...
	const formatItemValue = (item, ctx) => {
		...
		const amountText = (val, unitId) => {
			...
			const unitKey = unit ? `msghub.i18n.core.msg.listItem.unit.${unit}.unit` : '';
			const unitLabel = unitKey ? t(unitKey) : '';
			const unitLabelResolved = unitLabel && unitLabel !== unitKey ? unitLabel : unit;
			return t('msghub.i18n.core.msg.listItem.amount.format', String(val), unitLabelResolved);
		};
		...
		const details =
			showQty && showPer
				? t('msghub.i18n.core.msg.listItem.qtyEach.format', qtyText, perText)
				: showQty
					? qtyText
					: perText;
		return t('msghub.i18n.core.msg.listItem.nameAndDetails.format', `~${name}`, details);
	};
	...
	const normalizeItemKey = (name, packagingUnit) => {
		const base = String(name || '')
			...
			.replace(/\s+/g, ' ')
			.trim();
		const pkg = packagingUnit && packagingUnit !== 'pcs' ? String(packagingUnit).trim() : '';
	...
	const categorizeNow = async ctx => {
		...
		for (const it of msg.listItems || []) {
			...
			const packagingUnit = it.quantity?.unit && it.quantity.unit !== 'pcs' ? it.quantity.unit : '';
			const key = normalizeItemKey(name, packagingUnit);
			if (!key) {
				continue;
			}
			...
			messages: [
				{
					...
					content:
						...
						'Each item has a key and text (item name). ' +
						'Optional fields: packaging (container type, e.g. "can", "bottle", "bag") and ' +
						'unit (measurement dimension, e.g. "g", "ml", "l"). ' +
						'When present, use packaging and unit as additional context alongside the item name ' +
						'to determine the most appropriate category — the same item name may belong to ' +
						'different categories depending on its packaging or unit. ' +
				...
				},
				{
					...
					content: JSON.stringify({
						...
						fallbackCategory,
						items: batch.map(b => {
							const item = { key: b.key, text: b.name };
							const pkgUnit = b.quantity?.unit;
							if (pkgUnit && pkgUnit !== 'pcs') {
								item.packaging = pkgUnit;
							}
							if (b.perUnit?.unit) {
								item.unit = b.perUnit.unit;
							}
							return item;
						}),
					}),
	...
	const ensureMessage = ctx => {
		...
		const desiredIcon = (() => {
			const raw = typeof cfg.messageIcon === 'string' ? cfg.messageIcon.trim() : '';
			return raw ? raw : null;
		})();
		const base = {
			ref,
			...(desiredIcon ? { icon: desiredIcon } : {}),
			title: cfg.listTitle,
			text: t(ctx, 'msghub.i18n.BridgeAlexaShopping.msg.syncInfo.text', cfg.listTitle),
		...
		const normalizeStringList = v => (Array.isArray(v) ? v : []).map(s => normalizeString(s)).filter(Boolean);
		const normalizeAudience = raw => {
			if (!raw || typeof raw !== 'object') {
				return null;
			}
			const tags = normalizeStringList(raw.tags);
			const c = raw.channels && typeof raw.channels === 'object' ? raw.channels : null;
			const include = normalizeStringList(c?.include);
			const exclude = normalizeStringList(c?.exclude);
			const channels = include.length || exclude.length ? { include, exclude } : null;
			return tags.length || channels ? { tags, channels } : null;
		};
		const isSameStringList = (a, b) => {
		...
		const isSameAudience = (aRaw, bRaw) => {
			const a = normalizeAudience(aRaw);
			const b = normalizeAudience(bRaw);
			if (!a && !b) {
				return true;
			}
			if (!a || !b) {
			...
			if (!isSameStringList(a.tags || [], b.tags || [])) {
				return false;
			}
			const ac = a.channels;
			const bc = b.channels;
			...
			if (!isSameStringList(ac.include || [], bc.include || [])) {
				return false;
			}
			if (!isSameStringList(ac.exclude || [], bc.exclude || [])) {
				return false;
			}
			return true;
		};
		...
		const isSameDetails = (aRaw, bRaw) => {
			const a = normalizeDetails(aRaw);
			const b = normalizeDetails(bRaw);
			if (!a && !b) {
				return true;
			}
			if (!a || !b) {
		...
		const desiredTitle = normalizeString(cfg.listTitle);
		const desiredText = normalizeString(base.text);
		const existingIcon = typeof existing.icon === 'string' ? existing.icon.trim() : '';
		const desiredIconNorm = typeof desiredIcon === 'string' ? desiredIcon : null;
		const iconMismatch = desiredIconNorm === null ? existingIcon !== '' : existingIcon !== desiredIconNorm;
		const isNoop =
			!timing &&
			!iconMismatch &&
			normalizeString(existing.title) === desiredTitle &&
			normalizeString(existing.text) === desiredText &&
			isSameAudience(existing.audience, base.audience) &&
			isSameDetails(existing.details, base.details);

		if (!isNoop) {
			const iconPatch = iconMismatch
				? desiredIconNorm === null
					? { icon: null }
					: { icon: desiredIconNorm }
				: {};
	...
	const stop = ctx => {
		...
		(async () => {
			...
			ctx.api.store.updateMessage(ref, {
				title: `${cfg.listTitle} (${t(ctx, 'msghub.i18n.BridgeAlexaShopping.status.connectionLost.label')})`,
			});
