...
function BridgeAlexaShopping(options = {}) {
	...
	const resolveCreateCmdId = async ctx => {
		...
		if (!createCmdWarned) {
			...
			ctx.api.log.warn(
				`BridgeAlexaShopping: no create command state found for '${cfg.jsonStateId}' (tried: ${candidates.join(
					', ',
				)})`,
			);
		}
	...
	const reportManagedState = async ctx => {
		...
		try {
			...
			ctx.api.log.warn(`BridgeAlexaShopping: reportManagedState failed: ${e?.message || e}`);
		}
	};
	...
	const syncFromAlexa = async (raw, ctx) => {
		...
		for (const it of items) {
			...
			if (keepMs > 0 && checked && Number.isFinite(updated) && Date.now() - updated >= keepMs) {
				patchDelete.push(internalId);
				continue;
			}
			...
			if (desired) {
				...
				if (!valueMismatch && !completedMismatch) {
					continue;
				}
				const conflict =
	...
	const enforceOne = async (extId, ctx) => {
		...
		const keepMs = Number(cfg.keepCompleted);
		const completedAt = ext?.updatedDateTime;
		if (keepMs > 0 && item.checked && Number.isFinite(completedAt) && Date.now() - completedAt >= keepMs) {
	...
	const syncToAlexa = async (msg, ctx) => {
		...
		for (const it of items) {
			...
			const ext = lastExternal.get(extId);
			const completedAt = ext?.updatedDateTime;
			if (keepMs > 0 && checked && Number.isFinite(completedAt) && Date.now() - completedAt >= keepMs) {
