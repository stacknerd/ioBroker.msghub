...
describe('BridgeAlexaShopping parser', () => {
	...
	it('parses composite scale numbers (de)', () => {
		const p = new ShoppingItemParser({ locale: 'de' });

		expect(p.parse('Hundert milliliter alnatura limette direktsaft')).to.deep.include({
			name: 'Alnatura limette direktsaft',
			perUnit: { val: 100, unit: 'ml' },
		});

		expect(p.parse('Zwei hundert milliliter alnatura limette direktsaft')).to.deep.include({
			name: 'Alnatura limette direktsaft',
			perUnit: { val: 200, unit: 'ml' },
		});

		expect(p.parse('Ein hundert fünfzig gramm mais')).to.deep.include({
			name: 'Mais',
			perUnit: { val: 150, unit: 'g' },
		});
	});

	it('parses composite scale numbers (en)', () => {
		const p = new ShoppingItemParser({ locale: 'en' });
		const out = p.parse('Two hundred milliliters milk');
