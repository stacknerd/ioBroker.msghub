...
const LEXICON_DE = {
	...
	units: [
		...
		{ id: 'carton', type: 'count', aliases: ['karton', 'kartons', 'kiste', 'kisten'] },
...
class ShoppingItemParser {
	...
	_buildIndex() {
		const make = lx => {
			...
			return { aliasToUnit, multipliers, connectors, numberWords, countUnitIds, measureUnitIds };
		};
	...
	_tryCountWithPackaging({ lowerTokens }) {
		for (let i = 0; i < lowerTokens.length - 1; i++) {
			const count = this._parseCountToken(lowerTokens[i]);
			if (count == null || !(count > 0)) {
				continue;
			}

			const pkg = this._parseUnitToken(lowerTokens[i + 1]);
			if (!pkg || pkg.type !== 'count') {
				continue;
			}

			const removeIdx = new Set([i, i + 1]);
			const measure = this._findFirstMeasure({ lowerTokens, from: i + 2 });
			if (measure) {
				for (const idx of measure.removeIdx) {
					removeIdx.add(idx);
				}
	...
	_tryCountMeasurePackaging({ lowerTokens }) {
		for (let i = 0; i < lowerTokens.length - 2; i++) {
			...
			const measure = this._parseMeasureAt(lowerTokens, i + 1);
			if (!measure) {
				continue;
			}

			const pkgTok = lowerTokens[i + 1 + measure.len];
			const pkg = pkgTok ? this._parseUnitToken(pkgTok) : null;
			if (!pkg || pkg.type !== 'count') {
				continue;
			}

			const removeIdx = new Set([i]);
			for (let k = 0; k < measure.len; k++) {
				removeIdx.add(i + 1 + k);
			}
			removeIdx.add(i + 1 + measure.len);
	...
	_tryCountOnly({ lowerTokens }) {
		for (let i = 0; i < lowerTokens.length; i++) {
			...
			const pkg = lowerTokens[i + 1] ? this._parseUnitToken(lowerTokens[i + 1]) : null;
			if (pkg && pkg.type === 'count') {
				unitId = pkg.id;
				removeIdx.add(i + 1);
			}
	...
	_parseMeasureAt(tokens, i) {
		...
		// Separated: "<number> <unit>"
		const val = this._parseMeasureNumberToken(t);
		if (val == null || !(val > 0)) {
			return null;
		}
		const unit = tokens[i + 1] ? this._parseUnitToken(tokens[i + 1]) : null;
		if (unit && (unit.type === 'mass' || unit.type === 'volume')) {
			// optional connector token right after unit ("2 l of milk")
			const connector = tokens[i + 2] && this._isConnector(tokens[i + 2]) ? 1 : 0;
			return { val, unitId: unit.id, len: 2 + connector };
		}
