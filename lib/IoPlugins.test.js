...
describe('IoPlugins', () => {
	it('seeds enable states from catalog defaults and subscribes to them', async () => {
		...
		expect(objects.get('IngestDemo.0.status.watchlist')?.type).to.equal('state');
		...
		expect(states.get('IngestDemo.0.status.watchlist')).to.deep.equal({ val: '[]', ack: true });
		expect(states.get('NotifyDemo.0.status.watchlist')).to.deep.equal({ val: '[]', ack: true });
		expect(states.get('BridgeDemo.0.status.watchlist')).to.deep.equal({ val: '[]', ack: true });
		expect(calls.setState).to.have.length(9);
	});
	...
	it('toggles plugins on ack:false state changes and ignores ack:true writes', async () => {
		...
		// Seed a non-empty watchlist state to verify it is cleared on stop.
		states.set('IngestDemo.0.status.watchlist', { val: '["x"]', ack: true });
		mgr._watchlists.set('IngestDemo.0', { ids: new Set(['x']), timer: null });
