...
describe('IoPlugins', () => {
	...
	it('passes pluginBaseObjectId in options when registering', async () => {
		...
		it('filters notify dispatch by message audience.channels and plugin native.channel', async () => {
			const { adapter } = makeAdapter();
			const store = makeMsgStore();
			const calls = [];
			...
			// Default: plugin channel empty => deliver only when message include is empty.
			let notifyHandler = store.notifyHandlers.get('NotifyDemo:0');
			expect(notifyHandler).to.be.ok;

			notifyHandler.onNotifications(
				'due',
			...
			notifyHandler.onNotifications('due', [{ ref: 'm2' }], { api: {}, meta: {} });
			expect(calls).to.have.length(1);
			expect(calls[0]).to.deep.equal({ event: 'due', ref: 'm2', channel: '' });

			// Exclude is ignored for empty plugin channels.
			notifyHandler.onNotifications('due', [{ ref: 'm3', audience: { channels: { exclude: ['x'] } } }], { api: {}, meta: {} });
			expect(calls).to.have.length(2);
			expect(calls[1]).to.deep.equal({ event: 'due', ref: 'm3', channel: '' });

			// Set plugin channel (update native => restart enabled instance).
			await mgr.updateInstanceNative({ type: 'NotifyDemo', instanceId: 0, nativePatch: { channel: 'Push ' } });
			notifyHandler = store.notifyHandlers.get('NotifyDemo:0');
			expect(notifyHandler).to.be.ok;
			...
			expect(calls[2]).to.deep.equal({ event: 'due', ref: 'm4', channel: 'Push' });
