...
describe('IoPlugins', () => {
	it('stores i18nRegistry from options on the instance', () => {
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		const registry = new IoRuntimeI18n();

		const mgr = new IoPlugins(adapter, store, { catalog: {}, i18nRegistry: registry });
		expect(mgr._i18nRegistry).to.equal(registry);
	});

	it('stores null for i18nRegistry when option is omitted', () => {
		const { adapter } = makeAdapter();
		const store = makeMsgStore();

		const mgr = new IoPlugins(adapter, store, { catalog: {} });
		expect(mgr._i18nRegistry).to.be.null;
	});
	...
	describe('getAdminUiContributions', () => {
			...
			if (withAdminUi) {
				entry.adminUi = {
					...
					panels: [
						{
							id: 'presets',
							label: 'msghub.i18n.IngestAdminTest.ui.panels.presets.label',
							description: 'msghub.i18n.IngestAdminTest.ui.panels.presets.description.text',
							category: 'admin',
							app: {
								name: 'msghub.i18n.IngestAdminTest.ui.panels.presets.app.name',
								shortName: 'msghub.i18n.IngestAdminTest.ui.panels.presets.app.shortName',
								url: '?panel=tab-plugin-IngestAdminTest-0-presets',
								display: 'standalone',
								themeColor: '#1f6a53',
		...
		it('returns panel contributions for a running plugin with adminUi', () => {
			...
			expect(c.label).to.equal('msghub.i18n.IngestAdminTest.ui.panels.presets.label');
			expect(c.description).to.equal('msghub.i18n.IngestAdminTest.ui.panels.presets.description.text');
			expect(c).to.not.have.property('surface');
			expect(c.category).to.equal('admin');
			expect(c.app).to.be.an('object');
			expect(c.app.url).to.equal('?panel=tab-plugin-IngestAdminTest-0-presets');
			expect(c.apiVersion).to.equal('1');
			expect(c.bundle).to.deep.equal({ hash: '' });
		});
			...
			const entry = {
				...
				adminUi: {
					...
					panels: [
						...
						{ id: '', label: 'x', description: 'y', bundle: { entry: 'a.js' } },
					],
				},
			};
		...
		it('skips panels with empty bundle.entry', () => {
			const { adapter } = makeAdapter();
			const store = makeMsgStore();
			const entry = {
				type: 'IngestNoEntry',
				...
				adminUi: {
					apiVersion: '1',
					panels: [
						// bundle.entry is empty — must be skipped
						{ id: 'presets', label: 'x', description: 'y', bundle: { entry: '' } },
					],
				},
			};
			const mgr = new IoPlugins(adapter, store, {
			...
			mgr._runtimeHandlersByRegId.set('IngestNoEntry:0', {});
			expect(mgr.getAdminUiContributions()).to.deep.equal([]);
		});
	});
	...
	describe('readAdminUiTranslations', () => {
		const os = require('os');
		...
		beforeEach(() => {
			tmpDir = fsSync.mkdtempSync(pathMod.join(os.tmpdir(), 'msghub-ioplugins-i18n-test-'));
		});
		...
		function makeMgr() {
				...
				[IoPluginsCategories.ingest]: [
					{
						...
						adminUi: {
							apiVersion: '1',
							panels: [{ id: 'presets', label: 'x', description: 'y', bundle: { entry: 'presets.esm.js' } }],
						},
					},
				],
				[IoPluginsCategories.notify]: [],
			...
			return new IoPlugins(adapter, store, {
				catalog,
				pluginDescriptors: makePluginDescriptors('IngestBundleTest', tmpDir),
			});
		}

		it('returns translations for the requested language when the file exists', async () => {
			...
			const mgr = makeMgr();
			const result = await mgr.readAdminUiTranslations({ type: 'IngestBundleTest', lang: 'de' });

			expect(result).to.deep.equal({ lang: 'de', translations });
		});

		it('falls back to en translations when the requested language file is absent', async () => {
			const i18nDir = pathMod.join(tmpDir, 'admin-ui', 'i18n');
			fsSync.mkdirSync(i18nDir, { recursive: true });
			...
			expect(result).to.deep.equal({ lang: 'en', translations });
		});
	});
	...
	describe('readAdminUiBundle', () => {
				...
				[IoPluginsCategories.ingest]: [
					{
						...
						adminUi: {
							...
							panels: [
								{
									id: 'presets',
									label: 'msghub.i18n.IngestBundleTest.ui.panels.presets.label',
									description: 'msghub.i18n.IngestBundleTest.ui.panels.presets.description.text',
		...
		it('throws FORBIDDEN when bundle.entry escapes the plugin package root', async () => {
		...
		it('throws NOT_FOUND when no plugin descriptor with packageRoot is registered for the type', async () => {
			const { adapter } = makeAdapter();
			const store = makeMsgStore();
			const catalog = {
				[IoPluginsCategories.ingest]: [
					{
						...
						adminUi: {
							apiVersion: '1',
							panels: [{ id: 'p', label: 'x', description: 'y', bundle: { entry: 'a.js' } }],
						},
					},
				],
				[IoPluginsCategories.notify]: [],
			...
			const mgr = new IoPlugins(adapter, store, { catalog, pluginDescriptors: new Map() });
		...
		it('supports i18n-only bundle projection on the existing bundle reader', async () => {
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.js'), 'export function mount(ctx) {}', 'utf8');
			const i18nDir = pathMod.join(tmpDir, 'admin-ui', 'i18n');
			fsSync.mkdirSync(i18nDir, { recursive: true });
			const translations = { 'msghub.i18n.IngestBundleTest.ui.foo': 'Foo' };
			...
			const mgr = makeMgrForBundle('presets.esm.js');
			const result = await mgr.readAdminUiBundle({
				type: 'IngestBundleTest',
				panelId: 'presets',
				lang: 'en',
				parts: ['i18n'],
			...
			expect(Object.prototype.hasOwnProperty.call(result, 'js')).to.equal(false);
			expect(Object.prototype.hasOwnProperty.call(result, 'css')).to.equal(false);
		});

		it('supports bundle projection without i18n so JS/CSS stay on the mount path', async () => {
			const jsContent = 'export function mount(ctx) {}';
			const cssContent = ':host { color: var(--msghub-color-text); }';
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.js'), jsContent, 'utf8');
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.css'), cssContent, 'utf8');
			...
			fsSync.writeFileSync(
				pathMod.join(i18nDir, 'en.json'),
				JSON.stringify({ 'msghub.i18n.IngestBundleTest.ui.foo': 'Foo' }),
				'utf8',
			);
			...
			expect(Object.prototype.hasOwnProperty.call(result, 'i18n')).to.equal(false);
		});
	});
	...
	describe('computeAdminUiBundleHash', () => {
		const os = require('os');
		const fsSync = require('node:fs');
		const pathMod = require('node:path');
		...
		beforeEach(() => {
			tmpDir = fsSync.mkdtempSync(pathMod.join(os.tmpdir(), 'msghub-hash-test-'));
		});
		 ...
		 * @returns {IoPlugins} Manager.
		 */
		function makeMgrForHash(bundleEntry = 'presets.esm.js') {
			const { adapter } = makeAdapter();
			const store = makeMsgStore();
			const catalog = {
				[IoPluginsCategories.ingest]: [
		...
		it('returns a sha256-<hex> hash when JS, CSS, and i18n files are present', async () => {
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.js'), 'export function mount(ctx) {}', 'utf8');
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.css'), ':host { color: red; }', 'utf8');
			const i18nDir = pathMod.join(tmpDir, 'admin-ui', 'i18n');
			fsSync.mkdirSync(i18nDir, { recursive: true });
			fsSync.writeFileSync(pathMod.join(i18nDir, 'en.json'), '{"k":"v"}', 'utf8');
			fsSync.writeFileSync(pathMod.join(i18nDir, 'de.json'), '{"k":"w"}', 'utf8');

			const mgr = makeMgrForHash('presets.esm.js');
			const hash = await mgr.computeAdminUiBundleHash({ type: 'IngestBundleTest', panelId: 'presets' });

			expect(hash).to.match(/^sha256-[0-9a-f]{64}$/);
		});

		it('returns a valid hash when CSS file is absent', async () => {
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.js'), 'export function mount(ctx) {}', 'utf8');
			// No CSS file.
		...
		it('returns a valid hash when i18n directory is absent', async () => {
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.js'), 'export function mount(ctx) {}', 'utf8');
			// No admin-ui/i18n directory.
		...
		it('returns the cached hash on second call even when file content changes', async () => {
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.js'), 'version-A', 'utf8');

			const mgr = makeMgrForHash('presets.esm.js');
			const hash1 = await mgr.computeAdminUiBundleHash({ type: 'IngestBundleTest', panelId: 'presets' });

			// Overwrite JS with different content — cache must shield the result.
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.js'), 'version-B', 'utf8');
			const hash2 = await mgr.computeAdminUiBundleHash({ type: 'IngestBundleTest', panelId: 'presets' });

			expect(hash2).to.equal(hash1);
		});

		it('produces a different hash when JS content changes', async () => {
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.js'), 'version-A', 'utf8');
			const mgrA = makeMgrForHash('presets.esm.js');
			const hashA = await mgrA.computeAdminUiBundleHash({ type: 'IngestBundleTest', panelId: 'presets' });

			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.js'), 'version-B', 'utf8');
			const mgrB = makeMgrForHash('presets.esm.js');
			const hashB = await mgrB.computeAdminUiBundleHash({ type: 'IngestBundleTest', panelId: 'presets' });

			expect(hashA).to.not.equal(hashB);
		});

		it('produces a different hash when i18n file content changes', async () => {
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.js'), 'export function mount(){}', 'utf8');
			const i18nDir = pathMod.join(tmpDir, 'admin-ui', 'i18n');
			fsSync.mkdirSync(i18nDir, { recursive: true });

			fsSync.writeFileSync(pathMod.join(i18nDir, 'en.json'), '{"k":"v1"}', 'utf8');
			const mgrA = makeMgrForHash('presets.esm.js');
			const hashA = await mgrA.computeAdminUiBundleHash({ type: 'IngestBundleTest', panelId: 'presets' });

			fsSync.writeFileSync(pathMod.join(i18nDir, 'en.json'), '{"k":"v2"}', 'utf8');
			const mgrB = makeMgrForHash('presets.esm.js');
			const hashB = await mgrB.computeAdminUiBundleHash({ type: 'IngestBundleTest', panelId: 'presets' });
		...
		it('produces a different hash when i18n filename changes (per D2)', async () => {
			fsSync.writeFileSync(pathMod.join(tmpDir, 'presets.esm.js'), 'export function mount(){}', 'utf8');
			const i18nDir = pathMod.join(tmpDir, 'admin-ui', 'i18n');
			fsSync.mkdirSync(i18nDir, { recursive: true });

			// Both instances have a file with identical content but different filename.
			fsSync.writeFileSync(pathMod.join(i18nDir, 'en.json'), '{"k":"v"}', 'utf8');
			const mgrA = makeMgrForHash('presets.esm.js');
			const hashA = await mgrA.computeAdminUiBundleHash({ type: 'IngestBundleTest', panelId: 'presets' });

			fsSync.unlinkSync(pathMod.join(i18nDir, 'en.json'));
			fsSync.writeFileSync(pathMod.join(i18nDir, 'xx.json'), '{"k":"v"}', 'utf8');
			const mgrB = makeMgrForHash('presets.esm.js');
			const hashB = await mgrB.computeAdminUiBundleHash({ type: 'IngestBundleTest', panelId: 'presets' });
		...
		it('throws NOT_FOUND for an unknown plugin type', async () => {
			const mgr = makeMgrForHash('presets.esm.js');

			let err;
			try {
				await mgr.computeAdminUiBundleHash({ type: 'UnknownType', panelId: 'presets' });
			} catch (e) {
				err = e;
			}
		...
		it('throws NOT_FOUND for an unknown panelId', async () => {
			const mgr = makeMgrForHash('presets.esm.js');

			let err;
			try {
				await mgr.computeAdminUiBundleHash({ type: 'IngestBundleTest', panelId: 'nonexistent' });
			} catch (e) {
				err = e;
			}
...
// ---------------------------------------------------------------------------
// _loadPluginI18n unit tests
// ---------------------------------------------------------------------------

/**
 * Creates a temp plugin dir with an `i18n/` subdir containing the given lang→data files.
 *
 * @param {{ [lang: string]: object }} langData Map of language code to translation object.
 * @returns {string} Path to the temp plugin root directory (caller must clean up).
 */
function makeTempPluginDir(langData) {
	const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'msghub-plugini18n-'));
	const i18nDir = path.join(tmpDir, 'i18n');
	fs.mkdirSync(i18nDir);
	for (const [lang, data] of Object.entries(langData)) {
		fs.writeFileSync(path.join(i18nDir, `${lang}.json`), JSON.stringify(data), 'utf8');
	}
	return tmpDir;
}

/**
 * Creates a temp plugin dir with optional `admin-ui/i18n/` and `i18n/` subdirs.
 *
 * @param {{ adminUi?: { [lang: string]: object }, runtime?: { [lang: string]: object } }} sources Source maps by lang.
 * @returns {string} Path to the temp plugin root directory (caller must clean up).
 */
function makeTempPluginDirWithSources({ adminUi, runtime } = {}) {
	const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'msghub-plugini18n-'));

	if (adminUi && Object.keys(adminUi).length > 0) {
		const adminUiDir = path.join(tmpDir, 'admin-ui', 'i18n');
		fs.mkdirSync(adminUiDir, { recursive: true });
		for (const [lang, data] of Object.entries(adminUi)) {
			fs.writeFileSync(path.join(adminUiDir, `${lang}.json`), JSON.stringify(data), 'utf8');
		}
	}

	if (runtime && Object.keys(runtime).length > 0) {
		const runtimeDir = path.join(tmpDir, 'i18n');
		fs.mkdirSync(runtimeDir, { recursive: true });
		for (const [lang, data] of Object.entries(runtime)) {
			fs.writeFileSync(path.join(runtimeDir, `${lang}.json`), JSON.stringify(data), 'utf8');
		}
	}
...
/**
 * Builds a minimal IoPlugins instance for unit-testing internal methods.
 *
 * @param {object} [opts] Optional overrides.
 * @param {Map<string, { packageRoot?: string }>} [opts.pluginDescriptors] Plugin descriptor map.
 * @param {object|null} [opts.i18nRegistry] i18n registry instance.
 * @returns {{ mgr: IoPlugins, adapter: object, logs: { warn: string[], error: string[] } }}
 */
function makeMgrForI18n({ pluginDescriptors, i18nRegistry } = {}) {
	const { adapter, logs } = makeAdapter();
	const store = makeMsgStore();
	const mgr = new IoPlugins(adapter, store, {
		catalog: {},
		pluginDescriptors: pluginDescriptors ?? new Map(),
		i18nRegistry: i18nRegistry ?? null,
	});
	return { mgr, adapter, logs };
}

/**
 * Build a minimal resolved descriptor map for tests.
 *
 * @param {string} type Plugin type.
 * @param {string} packageRoot Absolute package root.
 * @returns {Map<string, { packageRoot: string }>} Descriptor map.
 */
function makePluginDescriptors(type, packageRoot) {
	return new Map([[type, { packageRoot }]]);
}

/**
 * Builds a minimal catalog for a single ingest plugin type (defaultEnabled).
 *
 * @param {string} type Plugin type name.
 * @returns {object} Catalog object.
 */
function makeIngestCatalog(type) {
	return {
		[IoPluginsCategories.ingest]: [
			{ type, label: type, defaultEnabled: true, supportsMultiple: true, defaultOptions: {}, create: () => () => {} },
		],
		[IoPluginsCategories.notify]: [],
		[IoPluginsCategories.bridge]: [],
		[IoPluginsCategories.engage]: [],
...
describe('IoPlugins — _loadPluginI18n', () => {
	let tmpDir = null;
	afterEach(() => {
		if (tmpDir) {
	...
	it('returns null when packageRoot is undefined', () => {
		const { mgr } = makeMgrForI18n();
		expect(mgr._loadPluginI18n(undefined, 'IngestDemo')).to.be.null;
	});

	it('returns null when neither admin-ui/i18n nor i18n exists', () => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'msghub-plugini18n-'));
		const { mgr } = makeMgrForI18n();
		expect(mgr._loadPluginI18n(tmpDir, 'IngestDemo')).to.be.null;
	});

	it('returns wordsByLang with valid language files', () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.IngestDemo.msg.title': 'Hello' }, de: { 'msghub.i18n.IngestDemo.msg.title': 'Hallo' } });
		const { mgr } = makeMgrForI18n();
		const result = mgr._loadPluginI18n(tmpDir, 'IngestDemo');
		expect(result).to.be.an('object');
		expect(result['en']).to.deep.equal({ 'msghub.i18n.IngestDemo.msg.title': 'Hello' });
		expect(result['de']).to.deep.equal({ 'msghub.i18n.IngestDemo.msg.title': 'Hallo' });
	});

	it('returns wordsByLang from admin-ui/i18n when no backend-only i18n exists', () => {
		tmpDir = makeTempPluginDirWithSources({
			adminUi: {
				en: { 'msghub.i18n.IngestDemo.ui.title': 'Hello UI' },
				de: { 'msghub.i18n.IngestDemo.ui.title': 'Hallo UI' },
			},
		});
		const { mgr } = makeMgrForI18n();
		const result = mgr._loadPluginI18n(tmpDir, 'IngestDemo');
		expect(result).to.be.an('object');
		expect(result['en']).to.deep.equal({ 'msghub.i18n.IngestDemo.ui.title': 'Hello UI' });
		expect(result['de']).to.deep.equal({ 'msghub.i18n.IngestDemo.ui.title': 'Hallo UI' });
	});

	it('merges admin-ui/i18n and backend-only i18n into one plugin runtime source', () => {
		tmpDir = makeTempPluginDirWithSources({
			adminUi: {
				en: { 'msghub.i18n.IngestDemo.ui.title': 'Hello UI' },
			},
			runtime: {
				en: { 'msghub.i18n.IngestDemo.msg.title': 'Hello Backend' },
			},
		});
		const { mgr } = makeMgrForI18n();
		const result = mgr._loadPluginI18n(tmpDir, 'IngestDemo');
		expect(result).to.be.an('object');
		expect(result['en']).to.deep.equal({
			'msghub.i18n.IngestDemo.msg.title': 'Hello Backend',
			'msghub.i18n.IngestDemo.ui.title': 'Hello UI',
		});
	});

	it('duplicate key across admin-ui/i18n and i18n is warned and later key is ignored', () => {
		tmpDir = makeTempPluginDirWithSources({
			adminUi: {
				en: { 'msghub.i18n.IngestDemo.ui.title': 'Hello UI' },
			},
			runtime: {
				en: { 'msghub.i18n.IngestDemo.ui.title': 'Hello Backend Duplicate' },
			},
		});
		const { mgr, logs } = makeMgrForI18n();
		const result = mgr._loadPluginI18n(tmpDir, 'IngestDemo');
		expect(result).to.be.an('object');
		expect(result['en']).to.deep.equal({ 'msghub.i18n.IngestDemo.ui.title': 'Hello UI' });
		expect(logs.warn.some(w => w.includes('duplicate key ignored'))).to.equal(true);
		expect(logs.warn.some(w => w.includes('msghub.i18n.IngestDemo.ui.title'))).to.equal(true);
	});

	it('warns and skips language with JSON parse error; other langs returned', () => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'msghub-plugini18n-'));
		const i18nDir = path.join(tmpDir, 'i18n');
		fs.mkdirSync(i18nDir);
		fs.writeFileSync(path.join(i18nDir, 'en.json'), JSON.stringify({ 'msghub.i18n.IngestDemo.msg.ok': 'OK' }), 'utf8');
		fs.writeFileSync(path.join(i18nDir, 'de.json'), 'NOT VALID JSON {', 'utf8');
		const { mgr, logs } = makeMgrForI18n();
		const result = mgr._loadPluginI18n(tmpDir, 'IngestDemo');
		expect(result).to.be.an('object');
		expect(result['en']).to.be.an('object');
		expect(result['de']).to.be.undefined;
		expect(logs.warn.some(w => w.includes('de.json'))).to.equal(true);
	});

	it('returns null when no valid language file survives (all parse errors)', () => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'msghub-plugini18n-'));
		const i18nDir = path.join(tmpDir, 'i18n');
		fs.mkdirSync(i18nDir);
		fs.writeFileSync(path.join(i18nDir, 'en.json'), 'BAD JSON', 'utf8');
		const { mgr, logs } = makeMgrForI18n();
		const result = mgr._loadPluginI18n(tmpDir, 'IngestDemo');
		expect(result).to.be.null;
		expect(logs.warn.some(w => w.includes('en.json'))).to.equal(true);
	});

	it('namespace guard — all valid keys pass without namespace-violation warn', () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.IngestDemo.msg.title': 'Hello', 'msghub.i18n.IngestDemo.error.foo': 'Err' } });
		const { mgr, logs } = makeMgrForI18n();
		const result = mgr._loadPluginI18n(tmpDir, 'IngestDemo');
		expect(result['en']).to.have.all.keys('msghub.i18n.IngestDemo.msg.title', 'msghub.i18n.IngestDemo.error.foo');
		// Missing-file warns for other languages are expected; only namespace-violation warns must be absent.
		expect(logs.warn.some(w => w.includes('outside allowed namespace'))).to.equal(false);
	});

	it('namespace guard — foreign keys are dropped with warn; own keys retained', () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.IngestDemo.msg.own': 'OK', 'msghub.i18n.core.admin.stolen': 'X' } });
		const { mgr, logs } = makeMgrForI18n();
		const result = mgr._loadPluginI18n(tmpDir, 'IngestDemo');
		expect(result).to.be.an('object');
		expect(result['en']).to.deep.equal({ 'msghub.i18n.IngestDemo.msg.own': 'OK' });
		expect(logs.warn.some(w => w.includes('msghub.i18n.core.admin.stolen'))).to.equal(true);
		expect(logs.warn.some(w => w.includes('msghub.i18n.IngestDemo.'))).to.equal(true);
	});

	it('namespace guard — exclusively foreign keys returns null', () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.core.admin.stolen': 'X' } });
		const { mgr, logs } = makeMgrForI18n();
		const result = mgr._loadPluginI18n(tmpDir, 'IngestDemo');
		expect(result).to.be.null;
		expect(logs.warn.some(w => w.includes('msghub.i18n.core.admin.stolen'))).to.equal(true);
...
// ---------------------------------------------------------------------------
// Plugin i18n lifecycle — register/unregister hooks
// ---------------------------------------------------------------------------

describe('IoPlugins — plugin i18n lifecycle', () => {
	let tmpDir = null;
	let tmpDir2 = null;
	afterEach(() => {
		if (tmpDir) {
	...
	it('register with no i18nRegistry → succeeds without error (registry-null path)', async () => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'msghub-plugini18n-'));
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		const catalog = makeIngestCatalog('IngestI18nTest');
		const pluginDescriptors = makePluginDescriptors('IngestI18nTest', tmpDir);
		const mgr = new IoPlugins(adapter, store, { catalog, pluginDescriptors });
		// _i18nRegistry is null — hooks skip silently.
		await mgr._registerOne({ category: IoPluginsCategories.ingest, type: 'IngestI18nTest', instanceId: 0 });
		expect(store.ingestRegistered.has('IngestI18nTest:0')).to.equal(true);
	});

	it('register with registry but no i18n dir → addSource not called', async () => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'msghub-plugini18n-'));
		// tmpDir has no i18n/ subdir.
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		const registry = new IoRuntimeI18n();
		const catalog = makeIngestCatalog('IngestI18nTest');
		const pluginDescriptors = makePluginDescriptors('IngestI18nTest', tmpDir);
		const mgr = new IoPlugins(adapter, store, { catalog, pluginDescriptors, i18nRegistry: registry });
		await mgr._registerOne({ category: IoPluginsCategories.ingest, type: 'IngestI18nTest', instanceId: 0 });
		expect(registry.getSourceIds()).to.not.include('plugin:IngestI18nTest:0');
	});

	it('register with registry and i18n dir → addSource called with correct source ID', async () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.IngestI18nTest.msg.hello': 'Hello' } });
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		const registry = new IoRuntimeI18n();
		const catalog = makeIngestCatalog('IngestI18nTest');
		...
		const mgr = new IoPlugins(adapter, store, { catalog, pluginDescriptors, i18nRegistry: registry });
		await mgr._registerOne({ category: IoPluginsCategories.ingest, type: 'IngestI18nTest', instanceId: 0 });
		expect(registry.getSourceIds()).to.include('plugin:IngestI18nTest:0');
		const { t } = registry.createTranslator('en');
		expect(t('msghub.i18n.IngestI18nTest.msg.hello')).to.equal('Hello');
	});

	it('unregister → source ID removed; key falls back to key string', async () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.IngestI18nTest.msg.hello': 'Hello' } });
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		const registry = new IoRuntimeI18n();
		...
		await mgr._registerOne({ category: IoPluginsCategories.ingest, type: 'IngestI18nTest', instanceId: 0 });
		expect(registry.getSourceIds()).to.include('plugin:IngestI18nTest:0');
		await mgr._unregisterOne({ category: IoPluginsCategories.ingest, type: 'IngestI18nTest', instanceId: 0 });
		expect(registry.getSourceIds()).to.not.include('plugin:IngestI18nTest:0');
		const { t } = registry.createTranslator('en');
		// Key was removed — falls back to key string.
		expect(t('msghub.i18n.IngestI18nTest.msg.hello')).to.equal('msghub.i18n.IngestI18nTest.msg.hello');
	});

	it('disable→enable cycle → source removed then re-added; fresh fs.readFileSync picks up updated content', async () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.IngestI18nTest.msg.hello': 'Hello-v1' } });
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		const registry = new IoRuntimeI18n();
		const catalog = makeIngestCatalog('IngestI18nTest');
		...
		// Overwrite the file between disable and enable — fs.readFileSync (A10) picks up fresh content.
		fs.writeFileSync(path.join(tmpDir, 'i18n', 'en.json'), JSON.stringify({ 'msghub.i18n.IngestI18nTest.msg.hello': 'Hello-v2' }), 'utf8');

		await mgr._registerOne({ category: IoPluginsCategories.ingest, type: 'IngestI18nTest', instanceId: 0 });
		expect(registry.getSourceIds()).to.include('plugin:IngestI18nTest:0');
		const { t } = registry.createTranslator('en');
		expect(t('msghub.i18n.IngestI18nTest.msg.hello')).to.equal('Hello-v2');
	});

	it('two instances of same type, same key same value → both source IDs registered, no collision warn', async () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.IngestMulti.msg.hello': 'Hello' } });
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		const registryWarns = [];
		const registry = new IoRuntimeI18n({ log: { warn: msg => registryWarns.push(msg) } });
		const catalog = makeIngestCatalog('IngestMulti');
		const pluginDescriptors = makePluginDescriptors('IngestMulti', tmpDir);
		const mgr = new IoPlugins(adapter, store, { catalog, pluginDescriptors, i18nRegistry: registry });

		await mgr._registerOne({ category: IoPluginsCategories.ingest, type: 'IngestMulti', instanceId: 0 });
		await mgr._registerOne({ category: IoPluginsCategories.ingest, type: 'IngestMulti', instanceId: 1 });

		expect(registry.getSourceIds()).to.include('plugin:IngestMulti:0');
		expect(registry.getSourceIds()).to.include('plugin:IngestMulti:1');
		// Trigger materialization — same value means no collision warn.
		const { t } = registry.createTranslator('en');
		t('msghub.i18n.IngestMulti.msg.hello');
		expect(registryWarns).to.have.length(0);
	});

	// -----------------------------------------------------------------------
	// Degraded-success contract — i18n errors must not roll back registration
	// -----------------------------------------------------------------------

	it('register hook is error-tolerant: addSource() throws → plugin stays registered, warn logged', async () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.IngestI18nTest.msg.hello': 'Hello' } });
		const { adapter, logs } = makeAdapter();
		const store = makeMsgStore();
		const registry = new IoRuntimeI18n();
		// Force addSource to throw to simulate an unexpected registry error.
		registry.addSource = () => {
			throw new Error('simulated addSource failure');
		};
		const catalog = makeIngestCatalog('IngestI18nTest');
		const pluginDescriptors = makePluginDescriptors('IngestI18nTest', tmpDir);
		const mgr = new IoPlugins(adapter, store, { catalog, pluginDescriptors, i18nRegistry: registry });
		await mgr._registerOne({ category: IoPluginsCategories.ingest, type: 'IngestI18nTest', instanceId: 0 });
		// Plugin must be registered despite the i18n failure.
		expect(store.ingestRegistered.has('IngestI18nTest:0')).to.equal(true);
		// A warn must be logged identifying the failing plugin.
		expect(logs.warn.some(w => w.includes('IngestI18nTest:0'))).to.equal(true);
		// No source must have been registered (addSource threw before completing).
		expect(registry.getSourceIds()).to.not.include('plugin:IngestI18nTest:0');
	});

	it('register hook is error-tolerant: _loadPluginI18n() throws → plugin stays registered, warn logged', async () => {
		const { adapter, logs } = makeAdapter();
		const store = makeMsgStore();
		const registry = new IoRuntimeI18n();
		const catalog = makeIngestCatalog('IngestI18nTest');
		const mgr = new IoPlugins(adapter, store, { catalog, i18nRegistry: registry });
		// Simulate an unexpected error inside _loadPluginI18n.
		mgr._loadPluginI18n = () => {
	...
	it('register order regression: notify start(ctx) already sees plugin-owned i18n', async () => {
		tmpDir = makeTempPluginDirWithSources({
			adminUi: {
				en: { 'msghub.i18n.NotifyI18nTest.ui.hello': 'Hello from admin-ui runtime' },
			},
		});
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		const registry = new IoRuntimeI18n();
		store.msgNotify.registerPlugin = (id, handler) => {
			store.notifyRegistered.add(id);
			store.notifyHandlers.set(id, handler);
			handler?.start?.({ api: { i18n: registry.createTranslator('en') }, meta: {} });
		};
		let translatedAtStart = null;
		const catalog = {
			[IoPluginsCategories.ingest]: [],
			[IoPluginsCategories.notify]: [
				{
					type: 'NotifyI18nTest',
					label: 'NotifyI18nTest',
					defaultEnabled: true,
					supportsMultiple: false,
					defaultOptions: {},
					create: () => ({
						start(ctx) {
							translatedAtStart = ctx.api.i18n.t('msghub.i18n.NotifyI18nTest.ui.hello');
						},
						onNotifications() {},
					}),
				},
		...
		const pluginDescriptors = makePluginDescriptors('NotifyI18nTest', tmpDir);
		const mgr = new IoPlugins(adapter, store, { catalog, pluginDescriptors, i18nRegistry: registry });

		await mgr._registerOne({ category: IoPluginsCategories.notify, type: 'NotifyI18nTest', instanceId: 0 });

		expect(translatedAtStart).to.equal('Hello from admin-ui runtime');
		expect(registry.getSourceIds()).to.include('plugin:NotifyI18nTest:0');
	});

	it('register rollback removes pre-registered i18n source when host registration throws', async () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.IngestI18nTest.msg.hello': 'Hello' } });
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		store.msgIngest.registerPlugin = () => {
		...
		expect(thrown?.message).to.equal('simulated register failure');
		expect(registry.getSourceIds()).to.not.include('plugin:IngestI18nTest:0');
	});

	// -----------------------------------------------------------------------
	// All-4-categories hook coverage
	// -----------------------------------------------------------------------

	it('register+unregister hooks fire for notify category', async () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.NotifyI18nTest.msg.hello': 'Hello' } });
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		const registry = new IoRuntimeI18n();
		const catalog = {
			[IoPluginsCategories.ingest]: [],
			[IoPluginsCategories.notify]: [
				{ type: 'NotifyI18nTest', label: 'NotifyI18nTest', defaultEnabled: true, supportsMultiple: false, defaultOptions: {}, create: () => () => {} },
			],
			[IoPluginsCategories.bridge]: [],
			[IoPluginsCategories.engage]: [],
		};
		...
		expect(registry.getSourceIds()).to.include('plugin:NotifyI18nTest:0');
		const { t } = registry.createTranslator('en');
		expect(t('msghub.i18n.NotifyI18nTest.msg.hello')).to.equal('Hello');
		await mgr._unregisterOne({ category: IoPluginsCategories.notify, type: 'NotifyI18nTest', instanceId: 0 });
		expect(registry.getSourceIds()).to.not.include('plugin:NotifyI18nTest:0');
	});

	it('register+unregister hooks fire for bridge category', async () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.BridgeI18nTest.msg.hello': 'Hello' } });
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		const registry = new IoRuntimeI18n();
		const bridgeHandler = { onNotifications: function () {}, start: function () {} };
		const catalog = {
			[IoPluginsCategories.ingest]: [],
			[IoPluginsCategories.notify]: [],
			[IoPluginsCategories.bridge]: [
				{ type: 'BridgeI18nTest', label: 'BridgeI18nTest', defaultEnabled: true, supportsMultiple: false, defaultOptions: {}, create: () => bridgeHandler },
			],
			[IoPluginsCategories.engage]: [],
		};
		const pluginDescriptors = makePluginDescriptors('BridgeI18nTest', tmpDir);
		const mgr = new IoPlugins(adapter, store, { catalog, pluginDescriptors, i18nRegistry: registry });
		await mgr._registerOne({ category: IoPluginsCategories.bridge, type: 'BridgeI18nTest', instanceId: 0 });
		expect(registry.getSourceIds()).to.include('plugin:BridgeI18nTest:0');
		const { t } = registry.createTranslator('en');
		expect(t('msghub.i18n.BridgeI18nTest.msg.hello')).to.equal('Hello');
		await mgr._unregisterOne({ category: IoPluginsCategories.bridge, type: 'BridgeI18nTest', instanceId: 0 });
		expect(registry.getSourceIds()).to.not.include('plugin:BridgeI18nTest:0');
	});

	it('register+unregister hooks fire for engage category', async () => {
		tmpDir = makeTempPluginDir({ en: { 'msghub.i18n.EngageI18nTest.msg.hello': 'Hello' } });
		const { adapter } = makeAdapter();
		const store = makeMsgStore();
		const registry = new IoRuntimeI18n();
		const engageHandler = { onNotifications: function () {}, start: function () {} };
		const catalog = {
			[IoPluginsCategories.ingest]: [],
			[IoPluginsCategories.notify]: [],
			[IoPluginsCategories.bridge]: [],
			[IoPluginsCategories.engage]: [
				{ type: 'EngageI18nTest', label: 'EngageI18nTest', defaultEnabled: true, supportsMultiple: false, defaultOptions: {}, create: () => engageHandler },
			],
		};
		const pluginDescriptors = makePluginDescriptors('EngageI18nTest', tmpDir);
		const mgr = new IoPlugins(adapter, store, { catalog, pluginDescriptors, i18nRegistry: registry });
		await mgr._registerOne({ category: IoPluginsCategories.engage, type: 'EngageI18nTest', instanceId: 0 });
		expect(registry.getSourceIds()).to.include('plugin:EngageI18nTest:0');
		const { t } = registry.createTranslator('en');
		expect(t('msghub.i18n.EngageI18nTest.msg.hello')).to.equal('Hello');
		await mgr._unregisterOne({ category: IoPluginsCategories.engage, type: 'EngageI18nTest', instanceId: 0 });
		expect(registry.getSourceIds()).to.not.include('plugin:EngageI18nTest:0');
	});

	it('two instances of same type, same key different values → last-registered-wins; collision warn from registry', async () => {
		// Instance 0 loaded with v1, instance 1 loaded with v2 (file overwritten between registers).
		// fs.readFileSync (A10) ensures each register call reads fresh content.
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'msghub-plugini18n-'));
		const i18nDir = path.join(tmpDir, 'i18n');
		fs.mkdirSync(i18nDir);
		fs.writeFileSync(path.join(i18nDir, 'en.json'), JSON.stringify({ 'msghub.i18n.IngestMulti.msg.hello': 'v1' }), 'utf8');
		...
		// Overwrite file before second register — demonstrates fs.readFileSync picks up fresh content.
		fs.writeFileSync(path.join(i18nDir, 'en.json'), JSON.stringify({ 'msghub.i18n.IngestMulti.msg.hello': 'v2' }), 'utf8');
		await mgr._registerOne({ category: IoPluginsCategories.ingest, type: 'IngestMulti', instanceId: 1 });

		// Trigger materialization — both sources have the same key with different values; last (instance:1) wins.
		const { t } = registry.createTranslator('en');
		expect(t('msghub.i18n.IngestMulti.msg.hello')).to.equal('v2');
		// Registry logs a collision warn (from IoRuntimeI18n, not adapter.log).
		expect(registryWarns.some(w => w.includes('msghub.i18n.IngestMulti.msg.hello'))).to.equal(true);
	});
});
