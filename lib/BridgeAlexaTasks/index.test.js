...
function createCtx({ addOk = true } = {}) {
	...
	const store = {
		...
		getMessageByRef: sinon.stub().returns(null),
		updateMessage: sinon.stub().returns(true),
	...
	it('uses queryMessages with audience.channels.routeTo for outbound selection', async () => {
		const { ctx, store, started } = createCtx({ addOk: true });
		store.queryMessages = sinon.stub().returns({
			items: [
				{
					ref: 'a',
					kind: 'task',
		...
		ctx.api.iobroker.objects.getForeignObject = async () => ({ _id: 'alexa2.0.Lists.TODO.#New' });
		...
		expect(arg.where).to.have.property('timing');
		expect(arg.where.timing).to.deep.equal({ startAt: { max: 1000, orMissing: true } });

		const createCalls = ctx.api.iobroker.states.setForeignState
			.getCalls()
			.filter(c => String(c.args[0]).endsWith('.#New'))
			.map(c => c.args[1]?.val);
			expect(createCalls).to.deep.equal(['~A', '~C']);
		});

		it('deletes tilde-prefixed Alexa items that are not desired (tilde orphan cleanup)', async () => {
			const { ctx, setForeignState, started } = createCtx({ addOk: true });
			ctx.api.iobroker.states.getForeignState = async () => ({
				val: JSON.stringify([{ id: 'o', value: '~Orphan', completed: false }]),
			});
			...
			const calls = setForeignState.getCalls().map(c => c.args[0]);
			expect(calls.some(id => id === 'alexa2.0.Lists.TODO.items.o.#delete')).to.equal(true);
		});

		it('patches icon on existing tasks when messageIcon option changes', async () => {
			const { ctx, store, started } = createCtx({ addOk: true });

			// existing open message => syncInbound uses updateMessage path
			store.getMessageByRef = sinon.stub().returns({ icon: '📝', lifecycle: { state: 'open' } });
			ctx.api.iobroker.states.getForeignState = async () => ({
				val: JSON.stringify([{ id: 'x', value: 'Do thing', completed: false, createdDateTime: Date.now() }]),
			});
			...
			expect(store.updateMessage).to.have.been.called;
			const [, patch] = store.updateMessage.getCalls().map(c => c.args).find(Boolean);
			expect(patch).to.have.property('icon', '✍️');
		});

		it('adopts matching tilde-prefixed Alexa items into the outbound mapping', async () => {
			const { ctx, store, started } = createCtx({ addOk: true });
			const setState = sinon.spy(async () => {});
			ctx.api.iobroker.states.setState = setState;
			...
			ctx.api.iobroker.states.getForeignState = async () => ({
				val: JSON.stringify([{ id: 'x', value: '~A', completed: false }]),
			});
			...
			const persisted = lastWrite ? JSON.parse(lastWrite) : null;
			expect(persisted).to.have.property('out');
			expect(persisted.out.messageRefToExternal).to.have.property('a', 'x');
			expect(persisted.out.externalToMessageRef).to.have.property('x', 'a');
		});

		it('skips import and outbound when Alexa connection is false', async () => {
			const { ctx, store, setForeignState, started } = createCtx({ addOk: true });
			ctx.api.iobroker.states.getForeignState = async id => {
				if (id === 'alexa2.0.info.connection') {
					return { val: false };
				}
				return { val: JSON.stringify([{ id: 'x', value: 'Do thing', completed: false, createdDateTime: Date.now() }]) };
			};
			...
			expect(store.addOrUpdateMessage).to.not.have.been.called;
