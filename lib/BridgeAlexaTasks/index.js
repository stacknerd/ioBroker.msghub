...
function BridgeAlexaTasks(options = {}) {
	...
	const stripLeadingTildes = value => String(value || '').trim().replace(/^~+/, '').trim();
	...
	const syncInbound = async (ctx, items) => {
		...
		for (const it of items) {
			...
			if (existingState === 'closed') {
				ctx.api.log.debug?.(`BridgeAlexaTasks inbound: ref='${ref}' exists with lifecycle.state=closed -> update`);
			}
			// hier endet der entfernbare debug-block.
