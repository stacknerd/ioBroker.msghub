...
function BridgeAlexaTasks(options = {}) {
	...
	const resolveCreateCmdId = async ctx => {
		...
		if (!createCmdWarned) {
			...
			ctx.api.log.warn(
				`BridgeAlexaTasks: no create command state found for '${cfg.jsonStateId}' (tried: ${candidates.join(
					', ',
				)})`,
			);
		}
	...
	const reportManagedState = async ctx => {
		...
		try {
			...
			ctx.api.log.warn(`BridgeAlexaTasks: reportManagedState failed: ${e?.message || e}`);
		}
	};
	...
	const writeCmd = (ctx, id, val) =>
		ctx.api.iobroker.states
			...
			.catch(e => ctx.api.log.warn(`BridgeAlexaTasks: setForeignState failed: ${e?.message || e}`));
	...
	const syncOutbound = async (ctx, items) => {
		...
		const desiredMessages = ctx.api.store.getMessages().filter(m => matchOutboundFilter(m));
	...
	const syncInbound = async (ctx, items) => {
		...
		if (!ctx?.api?.factory || typeof ctx.api.factory.createMessage !== 'function') {
			ctx.api.log.warn('BridgeAlexaTasks: inbound sync skipped (ctx.api.factory is not available)');
			return;
		}
	...
	const scheduleFullSync = (ctx, delayMs = 250) => {
		...
		fullSyncTimer = ctx.meta.resources.setTimeout(
			() => {
				fullSyncTimer = null;
				fullSync(ctxIngestRef || ctx).catch(e =>
					ctx.api.log.warn(`BridgeAlexaTasks: fullSync failed: ${e?.message || e}`),
				);
			},
			Math.max(0, delayMs),
		);
		...
		outboundTimer = ctx.meta.resources.setTimeout(
			() => {
				...
				outboundSyncOnly(ctxNotifyRef || ctx).catch(e =>
					ctx.api.log.warn(`BridgeAlexaTasks: outbound sync failed: ${e?.message || e}`),
				);
			},
			Math.max(0, delayMs),
		);
	...
	const start = ctx => {
		...
		(async () => {
			...
			if (!mapping.out || typeof mapping.out !== 'object') {
				mapping.out = { messageRefToExternal: {}, externalToMessageRef: {}, pendingCreates: {} };
			}
			if (!mapping.out.messageRefToExternal) {
				mapping.out.messageRefToExternal = {};
			}
			if (!mapping.out.externalToMessageRef) {
				mapping.out.externalToMessageRef = {};
			}
			if (!mapping.out.pendingCreates) {
				mapping.out.pendingCreates = {};
			}
			...
			if (cfg.fullSyncIntervalMs > 0) {
				ctx.meta.resources.setInterval(
					() =>
						fullSync(ctxIngestRef).catch(e =>
							ctx.api.log.warn(`BridgeAlexaTasks: fullSync failed: ${e?.message || e}`),
						),
					cfg.fullSyncIntervalMs,
				);
			}
		})().catch(e => ctx.api.log.warn(`BridgeAlexaTasks: start failed: ${e?.message || e}`));
	};
