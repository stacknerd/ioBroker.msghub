 ...
 *
 * Docs: ../../docs/plugins/BridgeAlexaTasks.md
 *
 * Semantics:
...
function BridgeAlexaTasks(options = {}) {
	...
	const syncOutbound = async (ctx, items, opts) => {
		...
		// Treat all tilde-prefixed Alexa items as plugin-owned projections.
		// - If they match a desired projected value, adopt them into the mapping (e.g. after mapping reset).
		// - If they do not match any desired value, delete them as orphans.
		const desiredRefsByValue = new Map();
		for (const [ref, value] of desiredByRef.entries()) {
			if (!desiredRefsByValue.has(value)) {
				desiredRefsByValue.set(value, []);
			}
			desiredRefsByValue.get(value).push(ref);
		}

		const claimedDesiredRefs = new Set();
		for (const [ref, extId] of Object.entries(mapping.out.messageRefToExternal || {})) {
			if (typeof extId === 'string' && extId.trim() && currentById.has(extId)) {
				claimedDesiredRefs.add(ref);
			}
		}
		...
		for (const it of items) {
			...
			const desiredRefs = desiredRefsByValue.get(raw);
			const adoptRef = Array.isArray(desiredRefs) ? desiredRefs.find(r => !claimedDesiredRefs.has(r)) : null;
			if (adoptRef) {
				const oldExtId = mapping.out.messageRefToExternal?.[adoptRef];
				if (typeof oldExtId === 'string' && oldExtId.trim()) {
					delete mapping.out.externalToMessageRef[oldExtId];
				}
				mapping.out.messageRefToExternal[adoptRef] = extId;
				mapping.out.externalToMessageRef[extId] = adoptRef;
				claimedDesiredRefs.add(adoptRef);
				delete mapping.out.pendingCreates?.[adoptRef];
				delete mapping.out.missingByRef?.[adoptRef];
				continue;
			}
	...
	const syncInbound = async (ctx, items) => {
		...
		for (const it of items) {
			...
			// removable debug block starts here
			const existingOpen = ctx.api.store.getMessageByRef(ref, 'quasiOpen');
			const existingQuasiDeleted = ctx.api.store.getMessageByRef(ref, 'quasiDeleted');
			const closed = ctx.api.constants.lifecycle.state.closed;
			if (existingQuasiDeleted && existingQuasiDeleted.lifecycle?.state === closed) {
				ctx.api.log.debug?.(`BridgeAlexaTasks inbound: ref='${ref}' is closed -> recreate`);
			}
			// removable debug block ends here.
			...
			if (existingOpen) {
				const existingIcon = typeof existingOpen.icon === 'string' ? existingOpen.icon.trim() : '';
				const iconMismatch = desiredIcon === null ? existingIcon !== '' : existingIcon !== desiredIcon;
				const iconPatch = iconMismatch ? (desiredIcon === null ? { icon: null } : { icon: desiredIcon }) : {};
				ok = ctx.api.store.updateMessage(ref, {
					...iconPatch,
					title,
					text: raw,
					origin: {
						type: ctx.api.constants.origin.type.import,
						system: sysString,
						id: cfg.jsonStateId,
					},
					audience,
