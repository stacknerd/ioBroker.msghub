...
function BridgeAlexaTasks(options = {}) {
	...
	const matchOutboundFilter = (msg, now = Date.now()) => {
		if (!cfg.outEnabled) {
			return false;
		}
		...
		if (kinds.size > 0) {
			...
			if (k === 'task') {
				const raw = msg?.timing?.startAt;
				if (raw !== undefined && raw !== null) {
					const startAt = typeof raw === 'number' ? raw : Number(raw);
	...
	const syncOutbound = async (ctx, items) => {
		...
		const desiredMessages = (queried?.items || []).filter(m => matchOutboundFilter(m, now));
