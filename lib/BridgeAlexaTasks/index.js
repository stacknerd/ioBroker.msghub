...
function BridgeAlexaTasks(options = {}) {
	...
	const syncOutbound = async (ctx, items) => {
		...
		// If a user deleted an item on Alexa-side, the mapping can become stale and would block re-creation.
		// Treat the Alexa list (`items`) as source of truth for existence and drop stale mappings.
		for (const [ref, extId] of Object.entries(mapping.out.messageRefToExternal || {})) {
			if (!desiredByRef.has(ref)) {
				continue;
			}
		...
		for (const [extId, ref] of Object.entries(mapping.out.externalToMessageRef || {})) {
			if (typeof extId !== 'string' || !extId.trim()) {
				continue;
			}
			if (currentById.has(extId)) {
			...
			if (typeof ref === 'string' && ref.trim() && mapping.out.messageRefToExternal?.[ref] === extId) {
				delete mapping.out.messageRefToExternal[ref];
				delete mapping.out.pendingCreates?.[ref];
			}
			delete mapping.out.externalToMessageRef[extId];
	...
	const syncInbound = async (ctx, items) => {
		...
		for (const it of items) {
			...
			const existingState = existing?.lifecycle?.state ? String(existing.lifecycle.state) : '';
			const rawHasPrefix = raw.startsWith('~');
			if (existingState === 'closed' || rawHasPrefix) {
				const rawShort = raw.length > 120 ? `${raw.slice(0, 119)}…` : raw;
				ctx.api.log.debug?.(
					`BridgeAlexaTasks inbound: extId='${extId}' ref='${ref}' raw='${rawShort}' rawPrefixed=${rawHasPrefix}${existingState ? ` existingState=${existingState}` : ''}`,
				);
			}
			// hier endet der entfernbare debug-block.
