...
const { MsghubStorage } = require('./MsghubStorage');
...
describe('MsghubStorage', () => {
	...
	it('writes JSON via rename when supported', async () => {
		...
		const storage = new MsghubStorage(adapter, { writeIntervalMs: 0 });
		await storage.init();
	...
	it('throttles writes and persists only the latest value', async () => {
		const { adapter, files } = createAdapter();
		const storage = new MsghubStorage(adapter, { writeIntervalMs: 50 });
		await storage.init();
	...
	it('does not write immediately when throttled, but flushPending persists latest value', async () => {
		const { adapter, files } = createAdapter();
		const storage = new MsghubStorage(adapter, { writeIntervalMs: 100 });
		await storage.init();
	...
	it('writes after the throttle interval without flushPending', async () => {
		const { adapter, files } = createAdapter();
		const storage = new MsghubStorage(adapter, { writeIntervalMs: 20 });
		await storage.init();
