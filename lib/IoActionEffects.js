...
class IoActionEffects {
	...
	/**
	 * Best-effort side-effect runner for `payload.effects`.
	 *
	 * @param {any} payload Raw payload passed to ctx.api.action.execute({ payload }).
	 * @param {{ adapter?: import('@iobroker/adapter-core').AdapterInstance, ref?: string, actionId?: string, actor?: string|null }} [meta]
	 *   Optional metadata for future dispatching/logging.
	 * @returns {void}
	 */
	handleActionPayload(payload, meta = {}) {
		const adapter = meta?.adapter || this.adapter;
		...
		Promise.resolve()
					...
					try {
						if (typeof adapter?.setForeignStateAsync === 'function') {
							...
							await new Promise((resolve, reject) => {
								adapter.setForeignState(id, { val, ack }, err =>
			...
			.catch(e => {
				adapter?.log?.warn?.(`IoActionEffects: failed (${e?.message || e})`);
			});
	}

	/**
	 * Convenience wrapper to tap `ctx.api.action.execute` calls.
	 *
	 * @param {{ payload?: any, ref?: string, actionId?: string, actor?: string|null }} execOptions Execute options.
	 * @returns {void}
	 */
	tapActionExecute(execOptions) {
		try {
			this.handleActionPayload(execOptions?.payload, {
			...
			this.adapter?.log?.warn?.(`IoActionEffects.tapActionExecute: failed (${e?.message || e})`);
		}
	}

	/**
	 * Static wrapper for `handleActionPayload` (useful for functional call sites).
	 *
	 * @param {any} payload Raw payload passed to ctx.api.action.execute({ payload }).
	 * @param {{ adapter?: import('@iobroker/adapter-core').AdapterInstance, ref?: string, actionId?: string, actor?: string|null }} [meta]
	 *   Optional metadata for future dispatching/logging.
	 * @returns {void}
	 */
	static handleActionPayload(payload, meta = {}) {
		const adapter = meta?.adapter;
		if (!adapter) {
			return;
		}
		new IoActionEffects(adapter).handleActionPayload(payload, meta);
	}

	/**
	 * Static wrapper for `tapActionExecute` (useful for functional call sites).
	 *
	 * @param {import('@iobroker/adapter-core').AdapterInstance} adapter Adapter instance.
	 * @param {{ payload?: any, ref?: string, actionId?: string, actor?: string|null }} execOptions Execute options.
	 * @returns {void}
	 */
	static tapActionExecute(adapter, execOptions) {
		new IoActionEffects(adapter).tapActionExecute(execOptions);
	}
}
