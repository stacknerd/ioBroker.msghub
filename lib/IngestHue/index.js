 ...
 * @property {string[]|string|null} [reachableAllowRoles] Allow-list of parent object roles for reachable states.
 *   - `[]` means "allow all roles" (no filtering).
 *   - When omitted or invalid, defaults to `["ZLLSwitch","ZLLPresence"]`.
 * @property {number|string} [batteryCreateBelow=7] Create/update when battery is below this threshold.
 * @property {number|string} [batteryRemoveAbove=30] Remove when battery is at/above this threshold.
 ...
 * @property {{ store: import('../../src/MsgStore').MsgStore, factory: import('../../src/MsgFactory').MsgFactory, constants: import('../../src/MsgConstants').MsgConstants }} api
 *
...
function IngestHue(adapter, options = {}) {
	...
	/** @type {Map<string, IngestHueWatchedState>} */
	...
	const emitBattery = (id, val) => {
		...
		if (num < cfg.batteryCreateBelow) {
			...
			const deviceLabel =
				pickI18n(modelInfo.label || I18N.unknownDeviceLabel, localeRef) || pickI18n(I18N.unknownDeviceLabel, localeRef);
	...
	const emitReachable = (id, val) => {
		...
		if (reachable === false) {
			const label =
				pickI18n(I18N.reachableRoleLabels[info.parentRole], localeRef) || pickI18n(I18N.unknownDeviceLabel, localeRef);
