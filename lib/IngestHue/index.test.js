...
describe('IngestHue', () => {
	it('discovers configured Hue states, subscribes, reports metadata, and emits messages', async () => {
		...
		expect(batteryMsg.actions).to.deep.equal([
			{ id: 'ack', type: MsgConstants.actions.type.ack },
			{ id: 'snooze-4h', type: MsgConstants.actions.type.snooze, payload: { forMs: 4 * 60 * 60 * 1000 } },
		]);
		...
		expect(reachableMsg.actions).to.deep.equal([
			{ id: 'ack', type: MsgConstants.actions.type.ack },
			{ id: 'snooze-4h', type: MsgConstants.actions.type.snooze, payload: { forMs: 4 * 60 * 60 * 1000 } },
		]);
