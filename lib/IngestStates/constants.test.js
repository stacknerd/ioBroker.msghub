...
describe('IngestStates constants', () => {
	...
	function collectEditorPresetPaths() {
		const pluginsDir = path.join(__dirname, '..', '..', 'admin', 'tab', 'panels', 'plugins');
		const source = ['index.js', 'overlay.presets.js']
			.map(name => fs.readFileSync(path.join(pluginsDir, name), 'utf8'))
			.join('\n');
		const paths = new Set();
