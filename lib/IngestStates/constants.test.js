...
describe('IngestStates constants', () => {
	...
	function collectEditorPresetPaths() {
		const source = fs.readFileSync(
			path.join(__dirname, 'admin-ui', 'dist', 'presets.esm.js'),
			'utf8',
		);
		const paths = new Set();
	...
	it('provides a frozen preset template', () => {
		...
		expect(presetTemplateV1.source).to.equal('user');
		expect(presetTemplateV1.message.textRecovered).to.equal('');
		expect(Object.prototype.hasOwnProperty.call(presetTemplateV1, 'ui')).to.equal(false);
	});
		...
		it('creates an internal fallback preset (no iobroker state)', () => {
			...
			expect(p.source).to.equal('builtin');
			...
			expect(p.message.timing.expiresInMs).to.equal(0);
			expect(Object.prototype.hasOwnProperty.call(p, 'ui')).to.equal(false);
		});
	...
	it('preset editor covers all preset template fields (no missed schema updates)', () => {
		const ignoredExact = new Set([
			'ui',
			// presetId is backend-owned on create and only shown read-only in the editor.
			'presetId',
			// source is set via direct assignment (preset.source = 'user'), not via updateDraft(),
			// so the regex-based scan cannot detect it.
			'source',
		]);
...
describe('presetBindingCatalog', () => {
	const { presetBindingCatalog, ruleTemplateCatalog } = require('./constants');
	...
	const EXPECTED_OWNERS = Object.freeze({
		...
		cycle: 'Cycle',
		triggered: 'Triggered',
		nonSettling: 'NonSettling',
		session: 'Session',
	});
	const EXPECTED_SUBSETS = Object.freeze({
		threshold: ['lt', 'gt', 'inside', 'outside', 'truthy', 'falsy'],
		freshness: ['lc', 'ts'],
		cycle: [],
		triggered: ['changed', 'deltaUp', 'deltaDown', 'thresholdGte', 'thresholdLte'],
		nonSettling: ['activity', 'trend'],
		session: ['start', 'end', 'end+summary'],
	});

	it('exports presetBindingCatalog for all six rules and keeps keys aligned with ruleTemplateCatalog', () => {
		expect(presetBindingCatalog).to.be.an('object');
		expect(Object.keys(presetBindingCatalog).sort()).to.deep.equal(EXPECTED_RULE_IDS);
		expect(Object.keys(presetBindingCatalog).sort()).to.deep.equal(Object.keys(ruleTemplateCatalog).sort());
	});

	it('exposes frozen rule entries with canonical owner names and subset metadata', () => {
		for (const ruleId of EXPECTED_RULE_IDS) {
			const entry = presetBindingCatalog[ruleId];
			expect(Object.isFrozen(entry), `${ruleId} entry not frozen`).to.equal(true);
			expect(entry.ownedBy, `${ruleId}.ownedBy mismatch`).to.equal(EXPECTED_OWNERS[ruleId]);
			expect(entry.headerKey, `${ruleId}.headerKey missing`).to.be.a('string');
			expect(entry.headerKey.length, `${ruleId}.headerKey empty`).to.be.greaterThan(0);
			if (entry.subsetFieldKey !== null) {
				expect(entry.subsetFieldKey, `${ruleId}.subsetFieldKey must be string|null`).to.be.a('string');
				expect(entry.subsetFieldKey.length, `${ruleId}.subsetFieldKey empty`).to.be.greaterThan(0);
			}
			expect(Array.isArray(entry.subsets), `${ruleId}.subsets must be an array`).to.equal(true);
			expect(Object.isFrozen(entry.subsets), `${ruleId}.subsets not frozen`).to.equal(true);
			expect(
				entry.subsets.map(item => item.value),
				`${ruleId}.subsets mismatch`,
			).to.deep.equal(EXPECTED_SUBSETS[ruleId]);
		}
	});

	it('stores subset option entries as frozen value/labelKey pairs', () => {
		for (const [ruleId, entry] of Object.entries(presetBindingCatalog)) {
			for (const subset of entry.subsets) {
				expect(Object.isFrozen(subset), `${ruleId}.${subset?.value}: subset entry not frozen`).to.equal(true);
				expect(subset.value, `${ruleId}: subset value missing`).to.be.a('string');
				expect(subset.value.length, `${ruleId}: subset value empty`).to.be.greaterThan(0);
				expect(subset.labelKey, `${ruleId}.${subset.value}: labelKey missing`).to.be.a('string');
				expect(subset.labelKey.length, `${ruleId}.${subset.value}: labelKey empty`).to.be.greaterThan(0);
				expect(
					subset.labelKey.startsWith('msghub.i18n.IngestStates.admin.jsonCustom.rules.'),
					`${ruleId}.${subset.value}: labelKey bad prefix`,
				).to.equal(true);
			}
		}
	});
	...
	it('each metric entry has the required fields with valid values', () => {
		for (const ruleId of EXPECTED_RULE_IDS) {
			...
			for (const [key, entry] of Object.entries(metrics)) {
				...
				expect(
					entry.labelKey.startsWith('msghub.i18n.IngestStates.ui.templateCatalog.'),
				...
				expect(
					entry.helpKey.startsWith('msghub.i18n.IngestStates.ui.templateCatalog.'),
