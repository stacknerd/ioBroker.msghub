...
describe('presets-service get', () => {
	it('returns builtin presets with raw payload plus Core-text editor translation', async () => {
		const rawTitle = 'msghub.i18n.IngestStates.msg.threshold.gt.title';
		const rawText = 'msghub.i18n.IngestStates.msg.threshold.gt.text';
		const rawRecovered = 'msghub.i18n.IngestStates.msg.threshold.gt.textRecovered';
		const ctx = makeCtxWithPresets({
			presets: [
				{
					presetId: 'pBuiltin',
					preset: makePresetVal({
						presetId: 'pBuiltin',
						source: 'builtin',
						ownedBy: 'threshold',
						subset: 'gt',
						message: {
							...
							title: rawTitle,
							text: rawText,
							textRecovered: rawRecovered,
							icon: 'msghub.i18n.icon.threshold.gt',
							timing: {},
							details: {},
							audience: {},
							actions: [],
			...
			i18nMap: {
				[rawTitle]: 'Zu hoch',
				[rawText]: 'Wert ist zu hoch',
				[rawRecovered]: 'Wert ist wieder im Soll',
				'msghub.i18n.icon.threshold.gt': 'thermometer',
			},
		});
		const service = createPresetsService(ctx, null);

		const res = await service.get({ presetId: 'pBuiltin' });

		assert.strictEqual(res.ok, true);
		assert.strictEqual(res.data.preset.message.title, rawTitle);
		assert.strictEqual(res.data.preset.message.text, rawText);
		assert.strictEqual(res.data.preset.message.textRecovered, rawRecovered);
		assert.strictEqual(res.data.editorPreset.message.title, 'Zu hoch');
		assert.strictEqual(res.data.editorPreset.message.text, 'Wert ist zu hoch');
		assert.strictEqual(res.data.editorPreset.message.textRecovered, 'Wert ist wieder im Soll');
		assert.strictEqual(res.data.editorPreset.message.icon, 'thermometer');
	});

	it('leaves user presets unchanged in editorPreset', async () => {
		const ctx = makeCtxWithPresets({
			presets: [
				{
					presetId: 'pUser',
			...
			i18nMap: {
				'msghub.i18n.IngestStates.msg.threshold.gt.title': 'Should not be used',
			},
		});
		const service = createPresetsService(ctx, null);

		const res = await service.get({ presetId: 'pUser' });

		assert.strictEqual(res.ok, true);
		assert.deepStrictEqual(res.data.editorPreset, res.data.preset);
	});
});
