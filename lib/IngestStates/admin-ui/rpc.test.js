...
const { createAdminRpcHandler, createWebRpcHandler } = require('./rpc');
...
function makeHandler(presetsOverrides = {}) {
	return createAdminRpcHandler({ presets: makePresets(presetsOverrides) });
}
...
function makeFullHandler(bulkApplyOverrides = {}) {
	return createAdminRpcHandler({ presets: makePresets(), bulkApply: makeBulkApply(bulkApplyOverrides) });
}
...
describe('web rpc skeleton', () => {
	it('returns BAD_REQUEST when panelId is missing', async () => {
		const { handleRpc } = createWebRpcHandler({ presets: makePresets(), bulkApply: makeBulkApply() });
		const res = await handleRpc({ command: 'presets.list' });
		assert.strictEqual(res.ok, false);
		assert.strictEqual(res.error.code, 'BAD_REQUEST');
	});

	it('returns UNSUPPORTED_COMMAND for existing admin-only commands', async () => {
		const { handleRpc } = createWebRpcHandler({ presets: makePresets(), bulkApply: makeBulkApply() });
		const res = await handleRpc({ panelId: 'presets', command: 'presets.list' });
		assert.strictEqual(res.ok, false);
		assert.strictEqual(res.error.code, 'UNSUPPORTED_COMMAND');
		assert.ok(res.error.message.includes('presets/presets.list'));
	});
});
