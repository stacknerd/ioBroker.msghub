	...
	const ctx = {
		...
		api: {
			request: async (command, payload) => {
				...
				if (command === 'admin.presets.bootstrap') {
					return {
						ok: true,
						data: {
							ingestConstants: {
				...
				if (command === 'admin.presets.list') {
					return {
						ok: true,
						data: listData,
					};
				}
				if (command === 'admin.presets.get') {
					return {
						ok: true,
						data: getData(payload),
					};
				}
				if (command === 'admin.presets.delete') {
...
describe('presets.esm.js', () => {
	it('mounts the toolbar above the grid and initializes the strip wrapper', async () => {
		...
		assert.deepEqual(
			...
			['admin.presets.bootstrap', 'admin.presets.list'],
		);
	...
	it('marks a clicked row as selected after loading the full preset', async () => {
		...
		assert.equal(calls[calls.length - 1].command, 'admin.presets.get');
	...
	it('reload button keeps using the spinner path instead of hard list replacement semantics', async () => {
		...
		assert.equal(
			calls.filter(call => call.command === 'admin.presets.list').length,
			2,
