...
export async function mount(ctx) {
	...
	function createPanelRuntime() {
		...
		const dataApi = Object.freeze({
			 ...
			 */
			bootstrap() {
				return call('admin.presets.bootstrap', null);
			},
			/**
			 ...
			 */
			listPresets(payload) {
				return call('admin.presets.list', payload);
			},
			/**
			 ...
			 */
			getPreset(payload) {
				return call('admin.presets.get', payload);
			},
			/**
			...
			createPreset(payload) {
				return call('admin.presets.create', payload);
			},
			/**
			...
			updatePreset(payload) {
				return call('admin.presets.update', payload);
			},
			/**
			 * Delete one preset.
			 *
			...
			deletePreset(payload) {
				return call('admin.presets.delete', payload);
			},
		});
	...
	function createPresetsState(bootstrapModel) {
		...
		function getSnapshot() {
			return { selectedId, original, rawOriginal, draft, isNew, presetLoading, saving, lastError };
		}
		 ...
		 *
		 * @param {any} preset Candidate preset.
		 * @param {any} rawPreset Raw stored preset payload.
		 * @returns {void}
		 */
		function setLoadedPreset(preset, rawPreset) {
			original = cloneJson(preset);
			rawOriginal = cloneJson(rawPreset || preset);
	 ...
	 * @returns {{ toolbar: any, node: any, loadList: Function, setSelectedId: Function, getRowById: Function }} List pane API.
	 */
	function createListPane(runtime, bootstrapModel, cfg = {}) {
		...
		/**
		 * Resolve the translated ownedBy label for one preset row.
		 *
		 * @param {any} ownedBy Binding ownedBy value.
		 * @returns {string} Visible ownedBy text.
		 */
		function getOwnedByText(ownedBy) {
			const entry = getBindingEntryByOwnedBy(ownedBy);
			return ownedBy
		...
		function sortPresetRows() {
			presetRows.sort((a, b) => {
				...
				const byOwnedBy = getOwnedByText(a?.ownedBy).localeCompare(getOwnedByText(b?.ownedBy));
				if (byOwnedBy !== 0) {
					return byOwnedBy;
				}
				const bySubset = getSubsetText(a?.ownedBy, a?.subset).localeCompare(
					getSubsetText(b?.ownedBy, b?.subset),
				);
				if (bySubset !== 0) {
					return bySubset;
				}
		...
		function renderPresetRow(row) {
			...
			const ownedByText = getOwnedByText(ownedBy);
			...
			const selectionCell = runtime.h(
				'td',
				{
					class: 'msghub-tools-presets-select msghub-colCell msghub-colCell--select msghub-col--select',
				},
				[
					runtime.h(
						'span',
						{ class: 'msghub-uicheckbox msghub-tools-presets-select-indicator', 'aria-hidden': 'true' },
						[
							runtime.h('input', {
							...
							runtime.h('span', { class: 'msghub-uicheckbox__box', text: '' }),
						],
					),
				],
			);
		...
		/**
		 * Build the shared preset toolbar shown above the grid.
		 *
		 * @returns {any} Toolbar DOM node.
		 */
		function createToolbar() {
			const refreshLabel = runtime.t('msghub.i18n.core.admin.ui.action.refresh');
			const addLabel = runtime.t('msghub.i18n.IngestStates.ui.presets.toolbar.add.action');
			const copyLabel = runtime.t('msghub.i18n.IngestStates.ui.presets.toolbar.copy.action');
			const deleteLabel = runtime.t('msghub.i18n.IngestStates.ui.presets.toolbar.delete.action');

			const refreshBtn = runtime.h('button', {
				type: 'button',
				class: 'msghub-uibutton-icon msghub-toolbarbutton-icon msghub-tools-presets-toolbar-refresh',
				title: refreshLabel,
			...
			const addBtn = runtime.h('button', {
				type: 'button',
				class: 'msghub-tools-presets-toolbar-add msghub-uibutton-iconandtext msghub-toolbarbutton-text',
				text: addLabel,
				title: addLabel,
				'aria-label': addLabel,
				onclick: () => cfg.onCreate?.(),
			});
			const copyBtn = runtime.h('button', {
				type: 'button',
				class: 'msghub-tools-presets-toolbar-copy msghub-uibutton-iconandtext msghub-toolbarbutton-text',
				text: copyLabel,
				title: copyLabel,
				'aria-label': copyLabel,
				onclick: () => cfg.onDuplicate?.(),
			});
			const deleteBtn = runtime.h('button', {
				type: 'button',
				class: 'msghub-tools-presets-toolbar-delete msghub-uibutton-iconandtext msghub-toolbarbutton-iconandtext is-danger',
				text: deleteLabel,
				title: deleteLabel,
				'aria-label': deleteLabel,
				onclick: () => cfg.onDelete?.(),
			});
			const toolbar = runtime.h('div', { class: 'msghub-toolbar msghub-tools-presets-toolbar' }, [
				runtime.h('div', { class: 'msghub-toolbar__group' }, [
					refreshBtn,
					runtime.h('span', { class: 'msghub-toolbar__separator', 'aria-hidden': 'true' }),
					addBtn,
					copyBtn,
					runtime.h('span', { class: 'msghub-toolbar__separator', 'aria-hidden': 'true' }),
					deleteBtn,
			...
			const stripApi = globalThis.MsghubScrollStrip || globalThis.window?.MsghubScrollStrip;
			stripApi?.initStrip?.(toolbar);
			return { toolbar, copyBtn, deleteBtn };
		}

		const toolbarState = createToolbar();
		const toolbar = toolbarState.toolbar;

		/**
		 * Sync the toolbar buttons that depend on the current table selection.
		 *
		 * @returns {void}
		 */
		function updateToolbarButtons() {
			const hasSelectedRow = presetRows.some(row => row?.presetId === selectedId);
			toolbarState.copyBtn.disabled = !hasSelectedRow;
			toolbarState.deleteBtn.disabled = !hasSelectedRow;
		}
				...
				items = runtime.h('div', { class: 'msghub-tools-presets-list-items' }, [
					runtime.h('table', { class: 'msghub-table msghub-presets-table' }, [
						runtime.h('colgroup', null, [
							runtime.h('col', { class: 'msghub-col--select' }),
		...
		return { node, toolbar, loadList, setSelectedId, getRowById };
	}
	...
	function createEditorPane(runtime, bootstrapModel, presetsState, fieldFactory, cfg = {}) {
		...
		function adoptBuiltField(field, { key, label, help = '', required = false, control = null }) {
			if (field?.input?.type === 'checkbox') {
				const checkboxId = field?.input?.id || '';
				const inlineCheckbox = runtime.h('label', { for: checkboxId }, [
					field.input,
					` ${String(label || '')}`,
				]);
		 ...
		 * @returns {{ wrapper: any, content: any, update: (nextOwnedByValue: any, nextSubsetValue: any) => void }}
	...
	function createFieldFactory(runtime, bootstrapModel) {
		const timeUnits = Object.freeze([
			...
			{
				key: 'd',
				label: 'msghub.i18n.core.admin.common.time.d.label',
		 ...
		 *
		 * @param {{ field?: { displayUnit?: string, label?: string } }} info Field metadata.
		 * @returns {string} Inferred unit key.
		 */
		function inferUnitFromFieldMetadata({ field }) {
			const displayUnit = normalizeUnit(field?.displayUnit);
			if (displayUnit) {
				return displayUnit;
			}
		...
		function createTimingField(cfg) {
			...
			const unitKey =
				inferUnitFromFieldMetadata({ field: { displayUnit: cfg?.displayUnit, label } }) ||
				pickDefaultTimeUnit(msNum);
			let displayUnitKey = unitKey;
			select.value = unitKey;
			input.value = Number.isFinite(msNum) ? String(msNum / getTimeFactor(unitKey)) : '';
			select.addEventListener('change', () => {
				const nextUnitKey = normalizeUnit(select.value) || 'ms';
				const currentValue = Number(input.value);
				if (input.value !== '' && Number.isFinite(currentValue)) {
					const currentMs = currentValue * getTimeFactor(displayUnitKey);
					input.value = String(currentMs / getTimeFactor(nextUnitKey));
				}
				displayUnitKey = nextUnitKey;
			});
			return {
				input,
				select,
	...
	function createEditorActions(runtime, bootstrapModel, presetsState, listPane, editorPane) {
		...
		async function loadPreset(presetId) {
			...
			const editorPreset = res?.editorPreset;
			if (!preset || typeof preset !== 'object') {
				return null;
			}
			return {
				preset,
				editorPreset: editorPreset && typeof editorPreset === 'object' ? editorPreset : preset,
			};
		}
		...
		async function setSelected(presetId, { skipDiscard = false } = {}) {
			...
			try {
				const loaded = await loadPreset(nextId);
				if (!loaded) {
				...
				presetsState.setLoadedPreset(loaded.editorPreset, loaded.preset);
		...
		async function duplicateSelected() {
			...
			const nextDraft = cloneJson(
				snapshot?.original?.source === 'builtin' && snapshot.rawOriginal
					? snapshot.rawOriginal
					: snapshot.draft || snapshot.original,
			);
			nextDraft.presetId = '';
