...
function makeCtx(overrides = {}) {
	...
	const defaultRequest = async (command, payload) => {
		...
		if (command === 'admin.bulkapply.bootstrap') {
			return {
				ok: true,
				data: {
		...
		if (command === 'admin.bulkapply.configRead') {
			return {
				ok: true,
				data: { custom: { myKey: 'myValue' } },
			};
		}
		if (command === 'admin.bulkapply.preview') {
			return {
				ok: true,
				data: {
		...
		if (command === 'admin.bulkapply.apply') {
...
describe('bulkapply.esm.js', () => {
	...
	it('bootstrap RPC is called on mount', async () => {
		...
		assert.equal(calls[0]?.command, 'admin.bulkapply.bootstrap');
	});
	...
	it('load calls configRead and fills textarea with the object custom config', async () => {
		...
		assert.ok(
			calls.some(c => c.command === 'admin.bulkapply.configRead' && c.payload?.id === 'my-object-id'),
		);
	...
	it('preview calls preview RPC and renders counts and sample ids', async () => {
		...
		assert.ok(calls.some(c => c.command === 'admin.bulkapply.preview'));
	...
	it('apply → confirm → apply RPC called → success toast shown', async () => {
		...
		assert.ok(calls.some(c => c.command === 'admin.bulkapply.apply'), 'apply RPC was called');
		assert.ok(
	...
	it('load with empty sourceId → configRead RPC not called', async () => {
		...
		assert.ok(
			!calls.some(c => c.command === 'admin.bulkapply.configRead'),
	...
	it('preview with empty pattern → preview RPC not called', async () => {
		...
		assert.ok(
			!calls.some(c => c.command === 'admin.bulkapply.preview'),
	...
	it('confirm cancel → apply RPC not called', async () => {
		...
		assert.ok(
			!calls.some(c => c.command === 'admin.bulkapply.apply'),
