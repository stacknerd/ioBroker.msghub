 ...
 * @returns {object} Presets service: { bootstrap, list, get, create, update, delete }.
 */
function createPresetsService(ctx, engine) {
	...
	const i18n = ctx.api.i18n;
	const msgConstants = ctx.api.constants;

	// ── Generic value helpers ─────────────────────────────────────────────────

	/**
	 * Deep-clone plain JSON payloads.
	 *
	...
	/**
	 * Normalize a value into trimmed text or null.
	 *
	 * @param {any} value Source value.
	...
	/**
	 * Normalize an array-like value into a trimmed string array.
	 *
	 * @param {any} value Source value.
	...
	/**
	 * Validate the preset identifier format.
	 *
	 * @param {any} presetId Candidate identifier.
	...
	/**
	 * Extract a translated display name from a common.name-like value.
	 *
	 * @param {any} value common.name value.
	 * @returns {string} Preferred translated name.
	 */
	...
	/**
	 * Upper-case the first character of a display string.
	 *
	 * @param {any} value Source text.
	...
	/**
	 * Derive the display name shown in preset summaries.
	 *
	 * @param {{ presetId?: string, obj?: any, preset?: any }} info Summary inputs.
	...
	/**
	 * Check whether a string looks like a MsgHub i18n key.
	 *
	 * @param {any} value Candidate value.
	 * @returns {boolean} True when the value looks like a MsgHub i18n key.
	 */
	function isMsghubI18nKey(value) {
		const text = typeof value === 'string' ? value.trim() : '';
		return /^msghub\.i18n\./.test(text);
	}

	/**
	 * Resolve one preset content string through the Core-text translator when it is an i18n key.
	 *
	 * This is intentionally display-only for admin read paths. Stored preset payloads remain raw.
	 *
	 * @param {any} value Raw preset field value.
	 * @returns {string} Display text.
	 */
	function resolvePresetDisplayText(value) {
		const text = normalizeText(value);
		const key = text.trim();
		if (!key || !isMsghubI18nKey(key) || typeof i18n?.t !== 'function') {
			return text;
		}
	...
	/**
	 * Build the editor-facing preset payload.
	 *
	 * Builtins keep their raw stored payload, but visible message fields are translated
	 * through the Core-text channel for the admin editor.
	 *
	 * @param {object|null} preset Normalized preset payload.
	 * @returns {object|null} Editor-facing preset payload.
	 */
	function buildEditorPreset(preset) {
		if (!isPlainObject(preset)) {
			return preset ?? null;
		}
		const next = cloneJson(preset);
		if (next.source !== 'builtin' || !isPlainObject(next.message)) {
			return next;
		}
		next.message = {
			...next.message,
			title: resolvePresetDisplayText(next.message.title),
			icon: resolvePresetDisplayText(next.message.icon),
			text: resolvePresetDisplayText(next.message.text),
			textRecovered: resolvePresetDisplayText(next.message.textRecovered),
		};
		return next;
	}

	/**
	 * Validate a normalized preset payload.
	 *
	...
	/**
	 * Parse and validate a preset JSON state value.
	 *
	 * @param {any} raw Raw state value.
	 * @param {{ presetId?: string }} [options] Parsing options.
	 * @returns {{ preset: object|null, error: string|null }} Parsed preset result.
	 */
	...
	/**
	 * Normalize a description into a preset id base.
	 *
	 * @param {any} description Source description.
	 * @returns {string} Identifier base.
	 */
	...
	/**
	 * Resolve the full ioBroker id for a preset state.
	 *
	 * @param {string} presetId Preset identifier.
	 * @returns {string} Full ioBroker id.
	 */
	...
	/**
	 * Ensure the preset root channel exists.
	 *
	...
	/**
	 * Collect all existing preset ids below the preset root.
	 *
	 * @returns {Promise<Set<string>>} Existing preset ids.
	 */
	...
	/**
	 * Generate a unique preset id from a description.
	 *
	 * @param {any} description Source description.
	 ...
	 */
	function extractMetricKeys(text) {
		const keys = new Set();
	 ...
	 * @returns {string} Lowercased kind token or empty string.
	 */
	...
	/**
	 * Resolve a MsgConstants level name from a numeric level value.
	 *
	 * @param {any} level Candidate level value.
	 * @returns {string} Level key or empty string when unknown.
	 */
	function levelKey(level) {
		...
		const map =
			msgConstants &&
			typeof msgConstants === 'object' &&
			msgConstants.level &&
			typeof msgConstants.level === 'object'
				? msgConstants.level
				: {};
	 ...
	 * Returns false (lenient) when the rule is unknown or the preset has no metric references.
	 *
	 * @param {object|null} preset Parsed preset object.
	 * @param {string} rule Rule name (case-insensitive).
	 * @param {string} subset Active subset or empty string for any.
	 * @returns {boolean} True when at least one referenced metric key is unavailable.
	 */
	function hasUnavailableMetrics(preset, rule, subset) {
		...
		const usedKeys = new Set([
			...extractMetricKeys(typeof msg?.title === 'string' ? msg.title : ''),
			...extractMetricKeys(typeof msg?.text === 'string' ? msg.text : ''),
			...extractMetricKeys(typeof msg?.textRecovered === 'string' ? msg.textRecovered : ''),
		]);
		if (usedKeys.size === 0) {
			return false;
		}
	 ...
	 * @param {string} rule Rule filter (ownedBy match) from command payload.
	 ...
	 */
	function isPresetIncompatibleWithFilter(preset, rule, subset) {
		...
		const metricMismatch = !!filterRule && hasUnavailableMetrics(preset, rule, subset);
		return ruleMismatch || subsetMismatch || metricMismatch;
	}

	/**
	 * Build a human-friendly preset label for jsonCustom select options.
	 *
	 * Format: `<ownerText> <kindText>[ (<levelText>)]: <name>`.
	 *
	 * @param {{ presetId: string, obj: any, preset: any }} info Label inputs.
	 * @returns {string} Display label.
	 */
	function buildSelectLabel({ presetId, obj, preset }) {
		const t = typeof i18n?.t === 'function' ? i18n.t.bind(i18n) : String;
		const name = capitalizeFirstChar(extractPresetName({ presetId, obj, preset })) || String(presetId || '').trim();
		...
		const ownerText = ownedBy
			? t(`msghub.i18n.IngestStates.admin.jsonCustom.rules.${ownerKey}.header.text`)
			: t(`msghub.i18n.IngestStates.admin.jsonCustom.preset.global.label`);
		const kKey = kindKey(preset?.message?.kind);
		const kindText = t(`msghub.i18n.core.admin.common.MsgConstants.kind.${kKey}.label`);
		const lKey = levelKey(preset?.message?.level);
		const levelText = lKey ? t(`msghub.i18n.core.admin.common.MsgConstants.level.${lKey}.label`) : '';
	...
	return {
		/**
		 * Return the static bootstrap payload required by the preset editor.
		 *
		 * @returns {Promise<{ ok: true, data: { ingestConstants: object, msgConstants: object } }>}
		 *   Bootstrap response.
		 */
		async bootstrap() {
			const apiConstants = ctx?.api?.constants && typeof ctx.api.constants === 'object' ? ctx.api.constants : {};
			const kind = apiConstants.kind && typeof apiConstants.kind === 'object' ? cloneJson(apiConstants.kind) : {};
			const level =
				apiConstants.level && typeof apiConstants.level === 'object' ? cloneJson(apiConstants.level) : {};
			return {
				ok: true,
				data: {
					ingestConstants: {
						presetSchema,
						presetTemplate: cloneJson(presetTemplateV1),
						presetBindingCatalog: cloneJson(presetBindingCatalog),
						ruleTemplateCatalog: cloneJson(ruleTemplateCatalog),
					},
					msgConstants: {
						kind,
						level,
		...
		async get(payload) {
			...
			return {
				...
				data: {
					presetId,
					preset: parsed,
					editorPreset: buildEditorPreset(parsed),
					object: cloneJson(obj),
		...
		/**
		 * Return select options for jsonCustom preset fields.
		 *
		 * Reads all presets, applies rule/subset/metric filters, builds {value, label} pairs,
		 * sorts (owned-first then alpha), and injects currentValue when it was filtered out.
		 *
		 * @param {{ rule?: string|null, subset?: string|null, currentValue?: string }} [payload] Filter options.
		 * @returns {Promise<Array<{value: string, label: string}>>} Ordered select options.
		 */
		async getPresetSelectOptions(payload) {
			const filterRuleRaw = typeof payload?.rule === 'string' ? payload.rule.trim() : '';
			const filterSubsetRaw = typeof payload?.subset === 'string' ? payload.subset.trim() : '';
			const currentValue = typeof payload?.currentValue === 'string' ? payload.currentValue.trim() : '';

			const presetIds = await getPresetIds();

			const reads = await Promise.all(
				Array.from(presetIds).map(async presetId => {
					try {
						const fullId = presetFullId(presetId);
						const obj = await ioObjects.getForeignObject(fullId);
						const st = await ioStates.getForeignState(fullId);
						const raw = typeof st?.val === 'string' ? st.val.trim() : '';
						const { preset, error: err } = parsePresetState(raw, { presetId });
						if (err || !preset) {
							return null;
						}
						if (isPresetIncompatibleWithFilter(preset, filterRuleRaw, filterSubsetRaw)) {
							return null;
						}
						const label = buildSelectLabel({ presetId, obj, preset });
						const ownedByRaw = typeof preset.ownedBy === 'string' ? preset.ownedBy.trim() : '';
						return { value: presetId, label, hasOwner: !!ownedByRaw };
					} catch {
						return null;
					}
				}),
			...
			const items = reads.filter(x => x !== null);
			items.sort((a, b) => {
				const aHasOwner = a?.hasOwner === true;
				const bHasOwner = b?.hasOwner === true;
				if (aHasOwner !== bHasOwner) {
			...
			if (!currentValue || !isValidPresetId(currentValue)) {
				return options;
			}
			...
			// Inject the currently saved preset even when it was filtered out.
			try {
				const fullId = presetFullId(currentValue);
				const obj = await ioObjects.getForeignObject(fullId);
				const st = await ioStates.getForeignState(fullId);
				const raw = typeof st?.val === 'string' ? st.val.trim() : '';
				const { preset } = parsePresetState(raw, { presetId: currentValue });
				const t = typeof i18n?.t === 'function' ? i18n.t.bind(i18n) : String;
				let label = preset ? buildSelectLabel({ presetId: currentValue, obj, preset }) : currentValue;
				if (preset && isPresetIncompatibleWithFilter(preset, filterRuleRaw, filterSubsetRaw)) {
					label = t('msghub.i18n.IngestStates.admin.jsonCustom.preset.incompatible.label', label);
				}
