...
const { CycleRule } = require('./rules/Cycle');
...
class IngestStatesEngine {
	...
	static _normalizeFlatKeys(input) {
		...
		const out = {};
		const groupedPrefixes = new Set(['thr', 'fresh', 'cyc', 'trg', 'nonset', 'sess', 'msg', 'managedMeta']);
	...
	/**
	 * Compute a stable fingerprint for a normalized rule config.
	 * Excludes managed metadata written by the engine to avoid false change-detection.
	 *
	 * @param {object|null} cfg Normalized rule config.
	 * @returns {string} JSON fingerprint (empty string when cfg is invalid).
	 */
	static _cfgFingerprint(cfg) {
	...
	/**
	 * Increment usage counts for valid preset references.
	 *
	 * Counts runtime bindings (`targetId + presetKey`) rather than persisted preset objects.
	 *
	 * @param {Map<string, number>} counts Mutable usage map.
	 * @param {Array<{ presetKey: string, presetId: string }>} presetRefs Extracted preset references.
	 * @returns {void}
	 */
	static _countPresetRefs(counts, presetRefs) {
		if (!(counts instanceof Map) || !Array.isArray(presetRefs)) {
			return;
		}
		for (const ref of presetRefs) {
			const presetId = typeof ref?.presetId === 'string' ? ref.presetId.trim() : '';
			if (!presetId) {
				continue;
			}
	...
	constructor(ctx, options = {}) {
		...
		this._presetUsageCounts = new Map(); // presetId -> active runtime binding count
		...
		this._rulesByTargetId = new Map(); // targetId -> Set<rule>
		...
		this._rulesByTargetIdCfgFingerprints = new Map(); // targetId -> config fingerprint for change detection
	}
	...
	/**
	 * Return the current preset usage snapshot of the running runtime model.
	 *
	 * Semantics:
	 * - counts active runtime bindings (`targetId + presetKey`)
	 * - only includes preset ids that are currently part of successfully active rule configs
	 *
	 * @returns {Array<{ presetId: string, usageCount: number }>} Stable, sorted snapshot.
	 */
	getPresetUsageSnapshot() {
		return Array.from(this._presetUsageCounts.entries())
			.sort((a, b) => String(a[0] || '').localeCompare(String(b[0] || '')))
			.map(([presetId, usageCount]) => ({ presetId, usageCount }));
	}
	...
	onStateChange(id, state, _ctx) {
		...
		if (this._traceEnabled()) {
			...
			this.ctx.api.log.silly(
				`stateChange('${id}' val=${v} ts=${ts} lc=${lc}) routes to ${rules.size} rule(s): ${Array.from(rules)
					.map(r => r?.constructor?.name || r?.targetId)
					.filter(Boolean)
					.join(', ')}`,
			);
		}
	...
	/**
	 * Route an executed action to interested rule instances.
	 *
	 * @param {object} actionInfo Executed action info.
	 * @param {object} _ctx Host ctx (unused; engine keeps its own ctx reference).
	 * @returns {void}
	 */
	onAction(actionInfo, _ctx) {
		if (!this._running) {
			return;
		}
		...
		// Fast-path routing for Cycle refs: `${baseOwnId}.cycle.<targetId>`
		const baseOwnId = this.ctx?.meta?.plugin?.baseOwnId;
		const prefix = typeof baseOwnId === 'string' && baseOwnId ? `${baseOwnId}.cycle.` : '';
		if (prefix && ref.startsWith(prefix)) {
			const targetId = ref.slice(prefix.length);
			const rules = this._rulesByTargetId.get(targetId);
			for (const rule of rules || []) {
				rule?.onAction?.(actionInfo);
			}
			return;
		}

		// Fallback: broadcast to all rules that implement `onAction` (best-effort).
		for (const rules of this._rulesByTargetId.values()) {
			for (const rule of rules || []) {
				rule?.onAction?.(actionInfo);
			}
	...
	async _rescan(reason) {
		...
		const nextCfgFingerprints = new Map();
		const nextPresetUsageCounts = new Map();
		...
		for (const row of res?.rows || []) {
			...
			if (!mode && cfg?.fresh?.enable !== true) {
				continue;
			}
			if (mode && !this._isValidMode(mode)) {
				this.ctx.api.log.warn(`unrecognized mode '${mode}' for '${targetId}'`);
				continue;
			}

			const presetRefs = this._extractPresetRefs(cfg.msg); // [{ presetKey: 'DefaultId', presetId: 'x' }, ...]
			...
			if (this._isValidMode(mode) && managedBy && !this._isManagedByUs(managedBy)) {
				this.ctx.api.log.warn(
					`skipping '${targetId}' because it is managed by '${managedBy}' (mode='${mode}')`,
				);
				continue;
			}

			// Config-diff: reuse existing rule instances when config is unchanged to preserve in-flight state.
			const fingerprint = IngestStatesEngine._cfgFingerprint(cfg);
			const existingFingerprint = this._rulesByTargetIdCfgFingerprints.get(targetId);
			const existingRules = this._rulesByTargetId.get(targetId);

			if (fingerprint === existingFingerprint && existingRules?.size) {
				// Config unchanged — reuse to keep active sessions (and other stateful rules) alive.
				nextRulesByTargetId.set(targetId, existingRules);
				nextCfgFingerprints.set(targetId, fingerprint);
				IngestStatesEngine._countPresetRefs(nextPresetUsageCounts, presetRefs);
				...
				if (traceEvents) {
					this.ctx.api.log.debug(`rule reused (config unchanged): targetId='${targetId}' mode='${mode}'`);
				}
				continue;
			}

			// Config changed or first scan — dispose existing rules before creating replacements.
			if (existingRules) {
			...
			try {
				if (traceEvents) {
					this.ctx.api.log.debug(
						`rule detected: targetId='${targetId}' mode='${mode}' cfg=${IngestStatesEngine._safeJson(cfg)}`,
					);
				}

				// Map: presetKey -> writer (each line gets its own writer, even when presetId is identical)
				const messageWritersByPresetKey = Object.fromEntries(
					presetRefs.map(({ presetKey, presetId }) => {
						const effectivePresetId = presetId || fallbackPresetId;
						return [
							presetKey,
							this._messageWriter.forTarget({ targetId, presetKey, presetId: effectivePresetId }),
						];
					}),
				);

				// Optional: global fallback (when the key is missing/unknown)
				...
				if (mode) {
					try {
						const rule = this._createRule({ ctx, targetId, cfg, messageWritersByPresetKey, mode });
						rules.push(rule);
						ruleModes.push(mode);
					} catch (e) {
						this.ctx.api.log.warn(
							`rule init failed for '${targetId}' (mode='${mode}'): ${e?.message || e}`,
						);
					}
				}

				if (cfg?.fresh?.enable === true && mode !== 'freshness') {
					try {
						const fresh = this._createRule({
							ctx,
							targetId,
							cfg,
						...
						rules.push(fresh);
						ruleModes.push('freshness');
					} catch (e) {
						this.ctx.api.log.warn(
							`rule init failed for '${targetId}' (mode='freshness'): ${e?.message || e}`,
						);
					}
				}
				...
				nextRulesByTargetId.set(targetId, new Set(rules));
				nextCfgFingerprints.set(targetId, fingerprint);
				IngestStatesEngine._countPresetRefs(nextPresetUsageCounts, presetRefs);
				...
				for (let i = 0; i < rules.length; i += 1) {
					const rule = rules[i];
					const ruleMode = ruleModes[i] || mode;
					const required = rule.requiredStateIds();
					const requiredList = Array.from(required);
					for (const stateId of required) {
						requiredAll.add(stateId);
						nextSubscribedStateIds.add(stateId);
						const set = nextRulesByStateId.get(stateId) || new Set();
						set.add(rule);
						nextRulesByStateId.set(stateId, set);
					...
					if (traceEvents) {
						this.ctx.api.log.debug(
							`rule started: targetId='${targetId}' mode='${mode}' rule='${ruleMode}' class='${rule?.constructor?.name || 'unknown'}' required=[${requiredList.join(', ')}]`,
						);
					}
				}

				if (traceEvents) {
					const requiredList = Array.from(requiredAll);
				...
				const managedModes = Array.from(new Set(ruleModes.filter(Boolean)));
				await this.ctx.meta.managedObjects.report(targetId, {
					managedText: `IngestStates (${managedModes.join(' + ')})`,
				});
			} catch (e) {
		...
		// Dispose rules for target IDs that are no longer configured.
		// Rules with changed configs are already disposed inline before replacement.
		for (const [id, rules] of this._rulesByTargetId.entries()) {
		...
		this._rulesByTargetIdCfgFingerprints = nextCfgFingerprints;
		this._presetUsageCounts = nextPresetUsageCounts;

		if (this.ctx.meta.options.resolveBool('traceEvents', this.options.traceEvents)) {
			this.ctx.api.log.debug(
				`rescan(${reason}) targets=${this._rulesByTargetId.size}, requiredStates=${this._subscribedStateIds.size}, watchedObjects=${this._watchedObjectIds.size}, presets=${this._presetSubscribedStateIds.size}`,
			);
		}
	}
	...
	_tracePresetCacheEvent({ event, presetId, reason, preset, objectId = null }) {
		...
		this.ctx.api.log.silly(`preset cache ${ev}: presetId='${presetId}' reason='${reason}'${src} preset=${payload}`);
	}
	 ...
	 * @param {string} [info.mode] Optional mode override (internal).
	 * @returns {object} Rule instance.
	 */
	_createRule({ ctx, targetId, cfg, messageWritersByPresetKey = {}, mode: modeOverride = undefined }) {
		const traceEvents = this._traceEnabled();
		let mode = typeof modeOverride === 'string' ? modeOverride.trim() : '';
		if (!mode) {
			mode = typeof cfg.mode === 'string' ? cfg.mode.trim() : '';
		}
