...
// jsonCustom coverage anchors (UI-only keys that are not wired yet).
// Kept as literal strings so `JsonCustomCoverage.test.js` can ensure these keys
// stay visible in code while the functional wiring is implemented later.
const JSONCUSTOM_KEYS_RESERVED = ['msg-DefaultId', 'msg-SessionStartId'];
...
class IngestStatesEngine {
	...
	constructor(ctx, options = {}) {
		...
		this._messageWriter = new MessageWriter(
			ctx,
			Object.assign({}, options, { presetProvider: this._presetProvider }),
		);
	...
	async _syncPresetSubscriptions(nextPresetIds) {
		...
		const diff = setDiff(this._presetSubscribedStateIds, nextStateIds);
 ...
 */
function normalizeFlatKeys(input) {
	if (!isObject(input)) {
		return null;
	}
...
function normalizeRuleCfg(cfg) {
	...
	return normalizeFlatKeys(cloned);
}
