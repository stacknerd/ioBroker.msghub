...
class IngestStatesEngine {
	...
	/**
	 * Warn when a config mixes nested blocks (`thr:{...}`) and dot keys (`"thr.mode": ...`) with different values.
	 *
	 * Dot keys are treated as source-of-truth because ioBroker's jsonCustom UI primarily writes dot keys.
	 *
	 * @param {object} info Info.
	 * @param {string} info.targetId Target object/state id.
	 * @param {object} info.raw Raw custom config payload (`common.custom.<ns>`).
	 * @returns {void}
	 */
	_warnDotKeyConflicts({ targetId, raw }) {
		if (!isObject(raw)) {
			return;
		}
		...
		for (const prefix of prefixes) {
			...
			for (const [k, nestedVal] of Object.entries(nested)) {
				const dotKey = `${prefix}.${k}`;
				if (!Object.prototype.hasOwnProperty.call(raw, dotKey)) {
					continue;
				}
				const dotVal = raw[dotKey];
				if (dotVal === nestedVal) {
					continue;
				}
				conflicts.push(
					`${dotKey} (dot=${JSON.stringify(dotVal ?? null)}, nested=${JSON.stringify(nestedVal ?? null)})`,
				);
			}
		}
		...
		const suffix = conflicts.length > shown.length ? ` (+${conflicts.length - shown.length} more)` : '';
		this.ctx.api.log.warn(
			`${this._prefix()}conflicting custom config on '${targetId}': do not mix dot keys and nested objects; nested objects win. ${shown.join(', ')}${suffix}`,
		);
	}
	...
	onStateChange(id, state, _ctx) {
		...
		if (this._traceEnabled()) {
			const v = formatStateVal(state?.val);
	...
	async _rescan(reason) {
		...
		for (const row of res?.rows || []) {
			...
			this._warnDotKeyConflicts({ targetId, raw });
			...
			try {
				if (traceEvents) {
					this.ctx.api.log.debug(
						`${this._prefix()}rule detected: targetId='${targetId}' mode='${mode}' cfg=${safeJson(cfg)}`,
					);
				}
				...
				if (traceEvents) {
					...
					if (typeof getState === 'function' && requiredList.length) {
						...
						this.ctx.api.log.debug(`${this._prefix()}rule bootstrap states: ${snapshots.join(' | ')}`);
					}
				}
	...
	_createRule({ targetId, mode, cfg, message }) {
		const traceEvents = this._traceEnabled();
		if (mode === 'freshness') {
			return new FreshnessRule({ targetId, ruleConfig: cfg.fresh, message, traceEvents });
		}
		if (mode === 'threshold') {
			return new ThresholdRule({ targetId, ruleConfig: cfg.thr, message, timers: this._timers, traceEvents });
		}
		if (mode === 'triggered') {
			return new TriggeredRule({ targetId, ruleConfig: cfg.trg, message, timers: this._timers, traceEvents });
		}
		if (mode === 'nonSettling') {
			return new NonSettlingRule({ targetId, ruleConfig: cfg.ns, message, timers: this._timers, traceEvents });
		}
		if (mode === 'session') {
			return new SessionRule({ targetId, ruleConfig: cfg.sess, message, timers: this._timers, traceEvents });
		}
...
function normalizeDotKeys(input) {
	...
	for (const [key, value] of Object.entries(input)) {
		...
		cur[leaf] = isObject(value) ? normalizeDotKeys(value) : value;
	}
