...
describe('IngestStates TargetMessageWriter', () => {
	...
	it('ignores quasi-deleted message on close', () => {
		const { ctx, calls, byRef } = createCtx();
		const writer = new TargetMessageWriter(ctx, {
			targetId: '0_userdata.0.s1',
			presetKey: 'DefaultId',
		...
		ctx.api.store.getMessageByRef = (id, filter) => {
			const msg = byRef[id] || null;
			if (!msg) {
				return null;
			}
			if (filter === 'quasiOpen' && msg?.lifecycle?.state === 'deleted') {
				return null;
			}
			return msg;
		};

		const ok = writer.onClose(ref);
