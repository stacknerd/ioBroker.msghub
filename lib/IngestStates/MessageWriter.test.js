...
describe('IngestStates TargetMessageWriter actions', () => {
	...
	it('injects close on create when resetOnNormal=false', () => {
		...
		expect(calls.createMessage[0].actions.map(a => a.type)).to.include.members(['ack', 'snooze', 'close']);
	});

	it('injects close even when actions are omitted', () => {
		const { ctx, calls } = createCtx();
		...
		expect(calls.createMessage).to.have.length(1);
		expect(calls.createMessage[0].actions.map(a => a.type)).to.deep.equal(['close']);
	});

	it('does not drop existing actions when adding close to an active message', () => {
...
describe('IngestStates TargetMessageWriter mappings', () => {
	...
	it('fills details.location from enum.rooms membership and preserves other details', () => {
		const { ctx, calls } = createCtx();
		ctx.meta.__ingestStatesRoomsCache = {
			byMember: new Map([['a.b', 'Kitchen']]),
			inFlight: null,
			loadedAt: 1,
		};
	...
	it('reopens silently during cooldown when reminders are off', () => {
		...
		it('reopens during cooldown from deleted status messages and delays notifyAt', () => {
			...
			const patch = reopenPatch;
			expect(patch.timing.remindEvery).to.equal(5 * 60 * 1000);
			expect(patch.timing.notifyAt).to.equal(deletedAt + cooldownMs);
		});
