...
describe('IngestStates TargetMessageWriter actions', () => {
	...
	it('unescapes \\\\n sequences in configured title/text', () => {
		const { ctx, calls } = createCtx();

		const writer = new TargetMessageWriter(ctx, {
			targetId: 'a.b.c',
			ruleType: 'freshness',
			messageConfig: { title: 'Line 1\\nLine 2', text: 'A\\r\\nB\\nC' },
			startMessageConfig: null,
		});

		writer.openActive({ now: 1 });

		expect(calls.createMessage).to.have.length(1);
		expect(calls.createMessage[0].title).to.equal('Line 1\nLine 2');
		expect(calls.createMessage[0].text).to.equal('A\nB\nC');
	});
});
...
describe('IngestStates TargetMessageWriter mappings', () => {
	...
	it('does not set dueAt/timeBudget on create when configured durations are 0', () => {
		...
		it('includes initial metrics in createMessage when provided', () => {
			const { ctx, calls } = createCtx();
			const writer = new TargetMessageWriter(ctx, {
				targetId: 'a.b.c',
				ruleType: 'threshold',
			...
			writer.openActive({
				...
				now: 1000,
				metrics: {
					'state-value': { val: 9, unit: 'W' },
					'state-min': { val: 10, unit: 'W' },
					'state-max': { val: 20, unit: 'W' },
				},
			});

			expect(calls.createMessage).to.have.length(1);
			expect(calls.createMessage[0].metrics).to.be.instanceOf(Map);
			expect(calls.createMessage[0].metrics.get('state-value')).to.deep.equal({ val: 9, unit: 'W', ts: 1000 });
			expect(calls.createMessage[0].metrics.get('state-min')).to.deep.equal({ val: 10, unit: 'W', ts: 1000 });
			expect(calls.createMessage[0].metrics.get('state-max')).to.deep.equal({ val: 20, unit: 'W', ts: 1000 });
		});

		it('does not patch actions/metrics on openActive when unchanged', () => {
			const { ctx, calls, byRef } = createCtx();
			const writer = new TargetMessageWriter(ctx, {
				targetId: 'a.b.c',
				ruleType: 'threshold',
			...
			byRef[ref] = {
				...
				kind: 'status',
				actions: [{ id: 'ack', type: 'ack' }],
				metrics: new Map([['state-max', { val: 20, unit: 'W', ts: 1000 }]]),
				lifecycle: { state: ctx.api.constants.lifecycle.state.open },
			};
			...
			writer.openActive({
				...
				now: 2000,
				actions: [{ id: 'ack', type: 'ack' }],
				metrics: { 'state-max': { val: 20, unit: 'W' } },
			});
			expect(calls.updateMessage).to.have.length(0);
		});

		it('filters unchanged metrics/actions in openActive updates', () => {
			const { ctx, calls, byRef } = createCtx();
			const writer = new TargetMessageWriter(ctx, {
				targetId: 'a.b.c',
				ruleType: 'threshold',
			...
			byRef[ref] = {
				...
				actions: [{ id: 'ack', type: 'ack' }],
				metrics: new Map([
					['state-value', { val: 1, unit: 'ppm', ts: 1 }],
					['state-max', { val: 1000, unit: 'ppm', ts: 1 }],
				]),
				lifecycle: { state: ctx.api.constants.lifecycle.state.open },
			};
			...
			writer.openActive({
				...
				actions: [{ id: 'ack', type: 'ack' }],
				metrics: {
					'state-value': { val: 2, unit: 'ppm' },
					'state-max': { val: 1000, unit: 'ppm' },
				},
			});

			expect(calls.updateMessage).to.have.length(1);
			expect(calls.updateMessage[0].ref).to.equal(ref);
			expect(calls.updateMessage[0].patch).to.have.property('metrics');
			expect(calls.updateMessage[0].patch.metrics.set).to.have.property('state-value');
			expect(calls.updateMessage[0].patch.metrics.set).to.not.have.property('state-max');
			expect(calls.updateMessage[0].patch).to.not.have.property('actions');
		});

		it('maps msg-consumables/tools/reason/task into details (end message only)', () => {
			const { ctx, calls } = createCtx();
			const writer = new TargetMessageWriter(ctx, {
				targetId: 'a.b.c',
				ruleType: 'threshold',
				messageConfig: {
					consumables: 'paper, ink, paper',
			...
			expect(calls.createMessage).to.have.length(1);
			expect(calls.createMessage[0].details).to.deep.equal({
			...
			writer.openStartActive({ defaultTitle: 'st', defaultText: 'sx', now: 1000 });
			expect(calls.createMessage).to.have.length(2);
			expect(calls.createMessage[1].details).to.equal(undefined);
		});

		it('ignores resetDelay config and closes immediately', () => {
			const { ctx, calls, byRef } = createCtx();
			const writer = new TargetMessageWriter(ctx, {
				targetId: 'a.b.c',
				ruleType: 'freshness',
			...
			expect(calls.setTimeout).to.have.length(0);
			expect(byRef[ref].metrics.size).to.equal(0);
		});
