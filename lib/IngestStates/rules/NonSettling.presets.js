...
function getNonSettlingPresets({ cloneTemplate, kindStatus, levelNotice }) {
	...
	}
	{
		...
		preset.message.textRecovered =
			'Der Wert war von {{t.startedAt.val|datetime}} bis {{m.state-recovered-at.val|datetime}} nicht stabil.' +
	...
	}
	{
		...
		preset.message.text =
			...
			'Start: {{t.startedAt|datetime}}';
		preset.message.textRecovered =
			'Der Wert hat von {{t.startedAt.val|datetime}} bis {{m.state-recovered-at.val|datetime}} in Richtung {{m.trendDir}} getrendet.' +
			'Diese Meldung kann geschlossen werden.';
		preset.policy.resetOnNormal = false;
		presets.push(preset);
	}
