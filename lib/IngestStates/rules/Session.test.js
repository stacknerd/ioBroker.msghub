...
describe('IngestStates SessionRule', () => {
	function createStubs({ startEnabled = true, foreignStates = {} } = {}) {
		...
		const ctx = {
			api: {
				...
				constants: {
					lifecycle: { state: { open: 'open', closed: 'closed', deleted: 'deleted', expired: 'expired' } },
				},
				store: {
					getMessageByRef: (ref, filter = 'all') => {
						const msg = byRef[ref] || null;
						if (!msg) {
							return null;
						}
						const state = msg?.lifecycle?.state || 'open';
						if (filter === 'quasiOpen') {
							return state === 'closed' || state === 'deleted' || state === 'expired' ? null : msg;
						}
						if (Array.isArray(filter)) {
							return filter.includes(state) ? msg : null;
						}
						return msg;
					},
					removeMessage: ref => {
					...
					completeAfterCauseEliminated: (ref, info) => {
						...
						if (byRef[ref]) {
							byRef[ref].lifecycle = { ...(byRef[ref].lifecycle || {}), state: 'closed' };
						}
						return true;
					},
				},
			...
			meta: {
				plugin: {
					baseOwnId: 'msghub.0.IngestStates.0',
	...
	it('creates start message and emits end message on stop', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createStubs({ startEnabled: true });
		...
		try {
			...
			expect(calls.endClose).to.have.length(0);
			...
			expect(calls.startUpsert[0].metrics['session-count']).to.deep.equal({ val: 0, unit: '' });
			expect(calls.startUpsert[0].metrics['session-counter-start']).to.deep.equal({ val: 100, unit: '' });
			...
			expect(calls.endUpsert[0].metrics['session-count']).to.deep.equal({ val: 1, unit: '' });
			expect(calls.endUpsert[0].metrics['session-counter-start']).to.deep.equal({ val: 100, unit: '' });
	...
	it('completes a still-open previous end message when a new session starts', () => {
		const { ctx, messageWritersByPresetKey, timers, calls, byRef } = createStubs({ startEnabled: true });
		const endRef = 'msghub.0.IngestStates.0.session.a.b.c';
		byRef[endRef] = {
			ref: endRef,
			kind: 'task',
		...
		expect(calls.completeAfterCauseEliminated).to.deep.equal([
			{ ref: endRef, info: { actor: 'IngestStates:0' } },
		]);
		expect(calls.endClose).to.have.length(0);
		expect(calls.startUpsert).to.have.length(1);
	});

	it('bootstraps the persistent session counter state and uses it in start/end metrics', async () => {
		const sessionCountId = 'msghub.0.IngestStates.0.session.a.b.c.sessionCount';
		const sessionActiveId = 'msghub.0.IngestStates.0.session.a.b.c.sessionActive';
		const { ctx, messageWritersByPresetKey, timers, calls } = createStubs({
			startEnabled: true,
			foreignStates: { [sessionCountId]: '2.9' },
		});
		...
		expect(calls.setObjectNotExists.some(call => call.id === sessionCountId)).to.equal(true);
		expect(calls.setObjectNotExists.some(call => call.id === sessionActiveId)).to.equal(true);
		expect(calls.setForeignState.some(call => call.id === sessionCountId && call.state.val === 2 && call.state.ack === true)).to.equal(true);
		expect(calls.setForeignState.some(call => call.id === sessionActiveId && call.state.val === false && call.state.ack === true)).to.equal(true);
		expect(Array.from(rule.requiredStateIds())).to.include(sessionCountId);
		expect(Array.from(rule.requiredStateIds())).to.not.include(sessionActiveId);

		rule.onStateChange('a.b.c', { val: 60 });
		expect(calls.startUpsert[0].metrics['session-count']).to.deep.equal({ val: 2, unit: '' });

		rule.onStateChange('a.b.c', { val: 10 });
		expect(calls.endUpsert[0].metrics['session-count']).to.deep.equal({ val: 3, unit: '' });
	});

	it('mirrors sessionActive while a session is running and clears it on dispose', async () => {
		const sessionActiveId = 'msghub.0.IngestStates.0.session.a.b.c.sessionActive';
		const { ctx, messageWritersByPresetKey, timers, calls } = createStubs({ startEnabled: true });
		...
		await flushAsync();
		await flushAsync();
		expect(calls.setForeignState.some(call => call.id === sessionActiveId && call.state.val === false)).to.equal(true);

		rule.onStateChange('a.b.c', { val: 60 });
		await flushAsync();
		expect(calls.setForeignState.some(call => call.id === sessionActiveId && call.state.val === true)).to.equal(true);

		rule.onStateChange('a.b.c', { val: 10 });
		await flushAsync();
		const activeWritesAfterEnd = calls.setForeignState.filter(call => call.id === sessionActiveId);
		expect(activeWritesAfterEnd[activeWritesAfterEnd.length - 1].state).to.deep.equal({ val: false, ack: true });

		rule.onStateChange('a.b.c', { val: 60 });
		await flushAsync();
		rule.dispose();
		await flushAsync();
		const activeWritesAfterDispose = calls.setForeignState.filter(call => call.id === sessionActiveId);
		expect(activeWritesAfterDispose[activeWritesAfterDispose.length - 1].state).to.deep.equal({ val: false, ack: true });
	});

	it('accepts manual session counter corrections through ack=false state writes', async () => {
		const sessionCountId = 'msghub.0.IngestStates.0.session.a.b.c.sessionCount';
		const { ctx, messageWritersByPresetKey, timers, calls } = createStubs({ startEnabled: true });
		...
		rule.onStateChange(sessionCountId, { val: '7.9', ack: false });
		await flushAsync();
		expect(calls.setForeignState.some(call => call.id === sessionCountId && call.state.val === 7 && call.state.ack === true)).to.equal(true);
		expect(calls.startMetrics[calls.startMetrics.length - 1].set['session-count']).to.deep.equal({ val: 7, unit: '' });

		rule.onStateChange(sessionCountId, { val: -1, ack: false });
		rule.onStateChange('a.b.c', { val: 10 });
		expect(calls.endUpsert[0].metrics['session-count']).to.deep.equal({ val: 8, unit: '' });
	});
	...
	it('patches all session metrics while active', () => {
		...
		try {
			...
			expect(set).to.have.property('session-count');
			expect(set).to.have.property('session-counter-start');
	...
	it('disables summary completely when enableSummary=false (no subscriptions, no summary metrics)', () => {
		...
		try {
			...
			expect(startKeys).to.deep.equal(['session-count', 'state-name']);

			rule.onTick(now + 1000);
			const patchKeys = Object.keys(calls.startMetrics[calls.startMetrics.length - 1].set).sort();
			expect(patchKeys).to.deep.equal(['session-count', 'state-name']);

			now += 2000;
			rule.onStateChange('a.b.c', { val: 10 });
			const endKeys = Object.keys(calls.endUpsert[0].metrics).sort();
			expect(endKeys).to.deep.equal(['session-count', 'state-name']);
		} finally {
			Date.now = originalNow;
		}
	});
	...
	it('enables summary metrics only when enableSummary=true (full metric set)', () => {
		...
		try {
			...
			const expectedMetricKeys = ['session-cost', 'session-count', 'session-counter', 'session-counter-start', 'state-name'];
	...
	it('latches config during active session and applies changed config only after restart', async () => {
		...
		try {
			...
			expect(Object.keys(calls2.startUpsert[0].metrics).sort()).to.deep.equal(['session-count', 'state-name']);
		} finally {
			Date.now = originalNow;
		}
	});
	...
	it('restores counterStartVal from an existing start message when called mid-session', () => {
		const { ctx, messageWritersByPresetKey, timers, calls, byRef } = createStubs({ startEnabled: true });

		// Simulate an existing start message already in the store from a previous rule instance.
		// Metrics are stored as Map objects (matching MessageWriter output format).
		byRef['msghub.0.IngestStates.0.session.a.b.c_start'] = {
			metrics: new Map([['session-counter-start', { val: 100, unit: '' }]]),
		};
		...
		// Counter has grown to 103 since the original session start (which captured 100).
		rule.onStateChange('counter', { val: 103 });
		rule.onStateChange('price', { val: 2 });

		// Power crosses threshold — triggers _startSession(). Must restore counterStartVal=100 from store.
		rule.onStateChange('a.b.c', { val: 60 });

		expect(calls.startUpsert).to.have.length(1);
		expect(calls.startUpsert[0].metrics['session-counter-start']).to.deep.equal({ val: 100, unit: '' });

		// Session-counter must be calculated relative to the restored start value, not the current counter.
		rule.onStateChange('counter', { val: 106 });
		rule.onStateChange('a.b.c', { val: 10 }); // stop

		expect(calls.endUpsert).to.have.length(1);
		expect(calls.endUpsert[0].metrics['session-counter-start']).to.deep.equal({ val: 100, unit: '' });
		expect(calls.endUpsert[0].metrics['session-counter']).to.deep.equal({ val: 6, unit: '' });
		expect(calls.endUpsert[0].metrics['session-cost']).to.deep.equal({ val: 12, unit: '' });
	});

	it('reschedules the stop timer when gate is unknown at stop-delay expiry', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createStubs({ startEnabled: true });
		...
		try {
			const rule = new SessionRule({
				...
				ruleConfig: {
					...
					stopThreshold: 15,
					stopDelayValue: 5,
					stopDelayUnit: 1, // 5 seconds
				},
				messageWritersByPresetKey,
				timers,
			});

			// Gate active → power high → session starts.
			rule.onStateChange('gate', { val: true });
			rule.onStateChange('a.b.c', { val: 60 });
			expect(calls.startUpsert).to.have.length(1);

			// Power drops → stop timer set (gate still truthy).
			now += 1_000;
			rule.onStateChange('a.b.c', { val: 10 });
			expect(calls.timerSet.some(([id]) => id === 'sess:stopDelay:a.b.c')).to.equal(true);

			// Gate becomes unknown before the stop timer fires.
			rule._gateVal = null;

			// Spy on _refreshGateValueBestEffort to verify it is called alongside the reschedule.
			let refreshCalled = false;
			const origRefresh = rule._refreshGateValueBestEffort.bind(rule);
			rule._refreshGateValueBestEffort = () => { refreshCalled = true; origRefresh(); };

			// Count timer sets before timer fires to detect rescheduling.
			const timerSetsBefore = calls.timerSet.length;

			// Fire stop timer — gate is null, session must NOT end and timer must be rescheduled.
			rule.onTimer({
				id: 'sess:stopDelay:a.b.c',
				at: now + 5_000,
				kind: 'session.stopDelay',
				data: { targetId: 'a.b.c' },
			});

			expect(calls.endUpsert).to.have.length(0);
			expect(calls.timerSet.length).to.be.greaterThan(timerSetsBefore);
			expect(calls.timerSet.slice(timerSetsBefore).some(([id]) => id === 'sess:stopDelay:a.b.c')).to.equal(true);
...
describe('SessionRule ruleTemplateCatalog compliance', () => {
	const { ruleTemplateCatalog } = require('../constants');

	function collectAllEmittedMetricKeys(calls) {
		const keys = new Set();
		for (const u of [...(calls.startUpsert ?? []), ...(calls.endUpsert ?? [])]) {
			for (const k of Object.keys(u.metrics ?? {})) keys.add(k);
		}
		for (const m of [...(calls.startMetrics ?? []), ...(calls.endMetrics ?? [])]) {
	...
	it('emits exactly the catalog-declared metric keys', () => {
		const calls = {
			startUpsert: [],
			startMetrics: [],
			endUpsert: [],
		...
		const ctx = {
			api: {
				log: { debug: () => undefined, error: msg => calls.logError.push(String(msg || '')) },
				i18n: { t: key => key },
				constants: { lifecycle: { state: { deleted: 'deleted' } } },
				store: {
					getMessageByRef: ref => byRef[ref] || null,
					removeMessage: ref => { calls.removeStartMessage.push(ref); return true; },
				},
				iobroker: {
					objects: { getForeignObject: async () => ({ common: {} }) },
					states: { getForeignState: async () => null },
				},
			},
			meta: { plugin: { baseOwnId: 'msghub.0.IngestStates.0', regId: 'IngestStates:0' } },
		};
		const startWriter = {
			presetId: 'startPreset',
			onUpsert: (ref, info) => { calls.startUpsert.push({ ref, ...info }); return true; },
			onMetrics: (ref, info) => { calls.startMetrics.push({ ref, ...info }); return true; },
			onClose: ref => { calls.endClose.push({ ref }); return true; },
		};
		const endWriter = {
			presetId: 'endPreset',
			onUpsert: (ref, info) => { calls.endUpsert.push({ ref, ...info }); return true; },
			onMetrics: (ref, info) => { calls.endMetrics.push({ ref, ...info }); return true; },
			onClose: ref => { calls.endClose.push({ ref }); return true; },
		};
		const messageWritersByPresetKey = {
			SessionStartId: startWriter,
		...
		const timers = {
			_timers: new Map(),
			get: id => timers._timers.get(id) ?? null,
			set: (id, at, kind, data) => { timers._timers.set(id, { at, kind, data }); calls.timerSet.push(id); },
			delete: id => { timers._timers.delete(id); calls.timerDelete.push(id); },
		};
		...
		// Supply counter and price values so session-counter and session-cost are emitted.
		rule.onStateChange('counter', { val: 100 });
		rule.onStateChange('price', { val: 2 });

		rule.onStateChange('a.b.c', { val: 60 }); // start → startUpsert

		rule.onStateChange('counter', { val: 103 });
		rule.onStateChange('a.b.c', { val: 10 }); // stop → endUpsert

		assertExactCatalogMatch(collectAllEmittedMetricKeys(calls), 'session', null);
	});
});
