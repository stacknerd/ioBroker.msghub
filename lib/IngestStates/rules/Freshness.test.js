...
describe('IngestStates FreshnessRule', () => {
	...
	it('opens on stale and sets lastSeenAt via onUpsert metrics', () => {
		...
		expect(calls.onUpsert[0].defaultTitle).to.contain('without updates');
		expect(calls.onUpsert[0].defaultText).to.contain('{{m.lastSeenAt|durationSince}}');
		expect(calls.onUpsert[0].defaultText).to.contain('{{m.lastSeenAt|datetime}}');
		expect(calls.onUpsert[0].defaultText).to.contain('within 1m');
		expect(calls.onUpsert[0].actions.map(a => a.type)).to.deep.equal(['ack', 'snooze']);
		expect(calls.onUpsert[0].metrics.lastSeenAt).to.deep.equal({ val: 1000, unit: 'ms' });
	...
	it('closes on recovery once and keeps patching lastSeenAt while active', () => {
		...
		expect(calls.onMetrics[0].set.lastSeenAt).to.deep.equal({ val: 200_000, unit: 'ms' });
	...
	it('supports evaluateBy=lc (unchanged wording)', () => {
		...
		expect(calls.onUpsert).to.have.length(1);
		expect(calls.onUpsert[0].defaultTitle).to.contain('unchanged');
		expect(calls.onUpsert[0].defaultText).to.contain('has not changed');
		expect(calls.onUpsert[0].defaultText).to.contain('does not change within 1m');
	});
