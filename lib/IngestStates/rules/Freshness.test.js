...
describe('IngestStates FreshnessRule', () => {
	...
	it('formats local durations with the same compact buckets as MsgRender', () => {
		const { ctx, messageWritersByPresetKey } = createHarness();
		const rule = new FreshnessRule({
			ctx,
			targetId: 'a.b.c',
			ruleConfig: { everyValue: 1, everyUnit: 60, evaluateBy: 'ts' },
		...
		expect(rule._formatDuration(56_000)).to.equal('56s');
		expect(rule._formatDuration(34 * 60_000)).to.equal('34m');
		expect(rule._formatDuration((3 * 60 + 45) * 60_000)).to.equal('3h 45m');
		expect(rule._formatDuration((24 * 60 + 4 * 60) * 60_000)).to.equal('1d 4h');
	});
});

describe('FreshnessRule ruleTemplateCatalog compliance', () => {
	...
	it('emits exactly the catalog-declared metric keys', () => {
		const calls = { onUpsert: [], onMetrics: [], onClose: [] };
		const ctx = {
			api: {
				log: { debug: () => undefined },
				i18n: { t: key => key },
				iobroker: {
					objects: { getForeignObject: async () => ({ common: { name: 'Sensor' } }) },
					states: { getForeignState: async () => ({ ts: 0, lc: 0 }) },
				},
			},
		...
		try {
			...
			// Supply ts, lc and val so all three optional fields are emitted.
			rule.onStateChange('a.b.c', { ts: now - 200_000, lc: now - 200_000, val: 42 });
			rule.onTick(now); // stale → open with all metrics in onUpsert

			// Advance time and supply fresh ts → recovery (emits state-recovered-at).
			now += 1000;
			rule.onStateChange('a.b.c', { ts: now, lc: now });

			assertExactCatalogMatch(collectAllEmittedMetricKeys(calls), 'freshness', null);
		} finally {
			Date.now = originalNow;
		}
	});
