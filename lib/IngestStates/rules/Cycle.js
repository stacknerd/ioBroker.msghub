...
class CycleRule {
	...
	_open(now) {
		...
		const actions = [
			...
			{ id: 'close', type: closeType },
		];

		const metrics = {};
		const name = typeof this._name === 'string' ? this._name.trim() : '';
		if (name) {
			metrics['state-name'] = { val: name, unit: '' };
		}
	...
	_patchMetrics(now) {
		...
		if (this._timeMs > 0) {
			...
			set['cycle-elapsedMs'] = { val: elapsedMs, unit: 'ms' };
			set['cycle-remainingTimeMs'] = { val: Math.max(0, Math.trunc(this._timeMs - elapsedMs)), unit: 'ms' };
		}
