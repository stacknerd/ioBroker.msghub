...
class TriggeredRule {
	...
	onStateChange(id, state) {
		if (id === this._triggerId) {
			...
			if (wasActive && !nextActive) {
				...
				this._cancelWindow();
				if (this._conditionActive && this._badStateActive) {
					this._patchRecoveredAt(now);
				}
	...
	_buildMetricsSet() {
		...
		const triggerName = this._triggerNameMetricValue();
		const trgVal = this._triggerValRaw;
		const set = {
			'state-name': { val: stateName, unit: '' },
			'state-value': { val: stateVal, unit: this._unit || '' },
			'trigger-name': { val: triggerName, unit: '' },
	...
	/**
	 * Resolve the best-available trigger label for metrics.
	 *
	 * Guarantees a stable metric key (`trigger-name`) even when object metadata is unavailable.
	 *
	 * @returns {string|null} Trigger label or null when no fallback can be derived.
	 */
	_triggerNameMetricValue() {
		const name = typeof this._triggerName === 'string' ? this._triggerName.trim() : '';
		if (name) {
			return name;
		}
		const fallback = this._fallbackName(this._triggerId);
		const fallbackName = typeof fallback === 'string' ? fallback.trim() : '';
		return fallbackName || null;
	}
	...
	_loadObjectMeta() {
		...
		Promise.resolve()
			...
			.then(obj => {
				...
				if (typeof n === 'string' && n.trim()) {
					const next = n.trim();
					if (next !== this._triggerName) {
						this._triggerName = next;
						patchName = true;
					}
				}
				...
				if (patchName && this._isActive && this._badStateActive) {
					...
					if (writer && typeof writer.onMetrics === 'function') {
						writer.onMetrics(this._getRef(), {
							set: { 'trigger-name': { val: this._triggerName, unit: '' } },
							now: Date.now(),
							force: true,
						});
					}
