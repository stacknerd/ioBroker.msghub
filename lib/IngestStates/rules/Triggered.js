 ...
 *
 * Recovery:
 * - If the expectation becomes true after the message exists, it is closed via `msg.resetOnNormal`/`resetDelay`.
...
class TriggeredRule {
	 ...
	 * @param {object} info.ruleConfig Rule config (`trg.*`).
	...
	constructor({ targetId, ruleConfig = {}, message, timers = null, traceEvents = false }) {
		...
		if (!this._windowMs) {
			throw new Error('TriggeredRule: invalid trg.window config');
		}
	...
	_trace(msg) {
		...
		const prefix = this.message?.ctx?.meta?.plugin?.baseFullId || 'IngestStates';
		this._log.debug(`${prefix}: TriggeredRule: ${msg}`);
	}
