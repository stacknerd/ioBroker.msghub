...
describe('IngestStates NonSettlingRule', () => {
		...
		try {
			const rule = new NonSettlingRule({
				...
				ruleConfig: {
					...
					trendWindowValue: 2,
					trendWindowUnit: 1,
					trendQuietGapValue: 0,
					trendQuietGapUnit: 60,
					minTotalDelta: 5,
				},
				messageWritersByPresetKey,
				timers,
	...
	it('trend: closes after trendQuietGap via recovery timer without follow-up event', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createMessageStub();
		...
		try {
			...
			const openTimer = timers.get('ns:a.b.c:open:trend');
			now = openTimer.at;
			rule.onTimer({
				id: 'ns:a.b.c:open:trend',
				at: openTimer.at,
				kind: openTimer.kind,
				data: openTimer.data,
			});
			...
			const recoveryTimer = timers.get('ns:a.b.c:recover:trend');
			expect(recoveryTimer).to.be.ok;
			expect(recoveryTimer.kind).to.equal('nonSettling.trend.recover');

			now = recoveryTimer.at;
			rule.onTimer({
				id: 'ns:a.b.c:recover:trend',
				at: recoveryTimer.at,
				kind: recoveryTimer.kind,
				data: recoveryTimer.data,
			});
	...
	it('trend: keeps trendQuietGap deadline unchanged for stable events within minDelta', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createMessageStub();
		...
		try {
			...
			rule.onStateChange('a.b.c', { val: 0, ts: now });
			now += 500;
			rule.onStateChange('a.b.c', { val: 4, ts: now });
			now += 500;
			rule.onStateChange('a.b.c', { val: 8, ts: now });
			...
			const recoveryTimer = timers.get('ns:a.b.c:recover:trend');
			expect(recoveryTimer).to.be.ok;
			const expectedRecoveryAt = recoveryTimer.at;
			const expectedStartedAt = recoveryTimer.data.startedAt;

			now += 500;
			rule.onStateChange('a.b.c', { val: 9, ts: now }); // no directional progress (> max + minDelta)
			let nextRecoveryTimer = timers.get('ns:a.b.c:recover:trend');
			expect(nextRecoveryTimer.at).to.equal(expectedRecoveryAt);
			expect(nextRecoveryTimer.data.startedAt).to.equal(expectedStartedAt);

			now += 500;
			rule.onStateChange('a.b.c', { val: 10, ts: now }); // still no directional progress
			nextRecoveryTimer = timers.get('ns:a.b.c:recover:trend');
			expect(nextRecoveryTimer.at).to.equal(expectedRecoveryAt);
			expect(nextRecoveryTimer.data.startedAt).to.equal(expectedStartedAt);
			...
			rule.onTimer({
				id: 'ns:a.b.c:recover:trend',
				at: nextRecoveryTimer.at,
				kind: nextRecoveryTimer.kind,
				data: nextRecoveryTimer.data,
			});
	...
	it('trend: resets trendQuietGap timer only on directional progress', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createMessageStub();
		...
		try {
			...
			const initialRecoveryTimer = timers.get('ns:a.b.c:recover:trend');
			expect(initialRecoveryTimer).to.be.ok;
			const staleRecoveryTimer = {
				at: initialRecoveryTimer.at,
				kind: initialRecoveryTimer.kind,
				data: { ...initialRecoveryTimer.data },
			};

			now += 500;
			rule.onStateChange('a.b.c', { val: 6, ts: now }); // directional progress -> reset quiet window
			const shiftedRecoveryTimer = timers.get('ns:a.b.c:recover:trend');
			expect(shiftedRecoveryTimer.at).to.be.greaterThan(staleRecoveryTimer.at);
			expect(shiftedRecoveryTimer.data.startedAt).to.equal(now);

			now = staleRecoveryTimer.at;
			rule.onTimer({
				id: 'ns:a.b.c:recover:trend',
				at: staleRecoveryTimer.at,
				kind: staleRecoveryTimer.kind,
				data: staleRecoveryTimer.data,
			});
			expect(calls.onClose).to.have.length(0);

			now = shiftedRecoveryTimer.at;
			rule.onTimer({
				id: 'ns:a.b.c:recover:trend',
				at: shiftedRecoveryTimer.at,
				kind: shiftedRecoveryTimer.kind,
				data: shiftedRecoveryTimer.data,
			});
			expect(calls.onClose).to.have.length(1);
		} finally {
			Date.now = originalNow;
	...
	it('trend: disables quiet-gap auto-reset when trendQuietGapValue is zero', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createMessageStub();
		...
		try {
			...
			expect(calls.onUpsert).to.have.length(1);
			expect(timers.get('ns:a.b.c:recover:trend')).to.equal(null);
			...
			now += 1;
			rule.onStateChange('a.b.c', { val: 0, ts: now }); // direction break keeps legacy close behavior
			expect(calls.onClose).to.have.length(1);
		} finally {
			Date.now = originalNow;
		}
	...
	it('trend(any): auto-recovers after quiet-gap once direction resolves to down', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createMessageStub();
		...
		try {
			...
			rule.onStateChange('a.b.c', { val: 10, ts: now });
			now += 500;
			rule.onStateChange('a.b.c', { val: 9, ts: now }); // direction resolves: down
			now += 500;
			rule.onStateChange('a.b.c', { val: 6, ts: now });
			...
			expect(calls.onUpsert).to.have.length(1);
			expect(calls.onUpsert[0].metrics.trendDir.val).to.equal('down');
...
describe('NonSettlingRule ruleTemplateCatalog compliance', () => {
	...
	function createStub() {
		const calls = { onUpsert: [], onMetrics: [], onClose: [], timerSet: [], timerDelete: [] };
		const byRef = {};
		const ctx = {
			api: {
				log: { debug: () => undefined },
				i18n: { t: key => key },
				store: { getMessageByRef: ref => byRef[ref] || null },
				constants: { lifecycle: { state: { open: 'open', closed: 'closed', expired: 'expired', deleted: 'deleted' } } },
				iobroker: { objects: { getForeignObject: async () => ({ common: { name: 'Sensor', unit: 'W' } }) } },
			},
			meta: { plugin: { baseOwnId: 'msghub.0.IngestStates.0' } },
		};
		const writer = {
			presetId: 'preset',
			onUpsert: (ref, info) => { byRef[ref] = { ref, lifecycle: { state: 'open' } }; calls.onUpsert.push({ ref, ...info }); return true; },
			onMetrics: (ref, info) => { calls.onMetrics.push({ ref, ...info }); return true; },
			onClose: ref => { delete byRef[ref]; calls.onClose.push({ ref }); return true; },
		};
		const timers = {
			_timers: new Map(),
			get: id => timers._timers.get(id) ?? null,
			set: (id, at, kind, data) => { timers._timers.set(id, { at, kind, data }); calls.timerSet.push([id, { at, kind, data }]); },
	...
	it("emits exactly the catalog-declared metric keys for subset 'activity'", () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createStub();
		...
		try {
			...
			// Trigger continuous activity to start the open timer.
			rule.onStateChange('a.b.c', { val: 0, ts: now });
			now += 500;
			rule.onStateChange('a.b.c', { val: 2, ts: now });
			now += 500;
			...
			// Fire the open timer → emits onUpsert with all activity metrics.
			const t = timers.get('ns:a.b.c:open:activity');
			now = t.at;
			rule.onTimer({ id: 'ns:a.b.c:open:activity', at: t.at, kind: t.kind, data: t.data });

			// Trigger recovery: stable value held past quietGap → state-recovered-at emitted.
			now += 1000;
			rule.onStateChange('a.b.c', { val: 5, ts: now });
			now += 2000;
			rule.onStateChange('a.b.c', { val: 5, ts: now });

			assertExactCatalogMatch(collectAllEmittedMetricKeys(calls), 'nonSettling', 'activity');
		} finally {
			Date.now = originalNow;
		}
	});

	it("emits exactly the catalog-declared metric keys for subset 'trend'", () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createStub();
		...
		try {
			...
			// Monotonically increasing values to build an 'up' trend.
			rule.onStateChange('a.b.c', { val: 0, ts: now });
			now += 500;
			rule.onStateChange('a.b.c', { val: 3, ts: now });
			now += 500;
			...
			// Fire the trend open timer → emits onUpsert with all trend metrics.
			const t = timers.get('ns:a.b.c:open:trend');
			now = t.at;
			rule.onTimer({ id: 'ns:a.b.c:open:trend', at: t.at, kind: t.kind, data: t.data });

			// Break the 'up' trend → _transitionToGood emits state-recovered-at, then onClose.
			now += 1000;
			rule.onStateChange('a.b.c', { val: 0, ts: now });

			assertExactCatalogMatch(collectAllEmittedMetricKeys(calls), 'nonSettling', 'trend');
		} finally {
			Date.now = originalNow;
		}
	});
