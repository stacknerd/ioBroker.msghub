'use strict';

const { expect } = require('chai');
const { format } = require('node:util');
const { ThresholdRule } = require('./Threshold');

describe('IngestStates ThresholdRule', () => {
	function createHarness({ unit = 'W' } = {}) {
		const calls = {
			onUpsert: [],
			onMetrics: [],
			onClose: [],
			timerSet: [],
			timerDelete: [],
		};

		const ctx = {
			api: {
				log: { debug: () => undefined },
				i18n: {
					t: (key, ...args) => format(String(key), ...args),
				},
				iobroker: {
					objects: {
						getForeignObject: async () => ({ common: { name: 'My Sensor', unit } }),
					},
					states: {
						getForeignState: async () => null,
					},
				},
			},
			meta: {
				plugin: { baseOwnId: 'msghub.0.IngestStates.0' },
			},
		};

		const writer = {
			onUpsert: (ref, info) => {
				calls.onUpsert.push({ ref, ...info });
				return true;
			},
			onMetrics: (ref, info) => {
				calls.onMetrics.push({ ref, ...info });
				return true;
			},
			onClose: ref => {
				calls.onClose.push({ ref });
				return true;
			},
		};

		const messageWritersByPresetKey = { DefaultId: writer, $fallback: writer };

		const timers = {
			_timers: new Map(),
			get: id => (timers._timers.has(id) ? timers._timers.get(id) : null),
			set: (id, at, kind, data) => {
				const timer = { at, kind, data };
				timers._timers.set(id, timer);
				calls.timerSet.push([id, timer]);
			},
			delete: id => {
				timers._timers.delete(id);
				calls.timerDelete.push(id);
			},
		};

		return { ctx, messageWritersByPresetKey, timers, calls };
	}

	it('does not patch state-name from object meta before any message is active', async () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createHarness({ unit: 'W' });
		new ThresholdRule({
			ctx,
			targetId: 'a.b.c',
			ruleConfig: { mode: 'lt', value: 10, hysteresis: 0, minDurationValue: 0, minDurationUnit: 1 },
			messageWritersByPresetKey,
			timers,
		});

		await new Promise(resolve => setImmediate(resolve));
		expect(calls.onMetrics).to.have.length(0);
	});

	it('opens on bootstrap when the current value is already violating and minDuration is off', async () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createHarness({ unit: 'W' });
		ctx.api.iobroker.states.getForeignState = async () => ({ val: 9 });

		const originalNow = Date.now;
		const now = Date.UTC(2025, 0, 1, 12, 0, 0);
		Date.now = () => now;
		try {
			// Constructor triggers an async bootstrap read via getForeignState().
			new ThresholdRule({
				ctx,
				targetId: 'a.b.c',
				ruleConfig: { mode: 'lt', value: 10, hysteresis: 0, minDurationValue: 0, minDurationUnit: 1 },
				messageWritersByPresetKey,
				timers,
			});

			// Allow promise chain in _initValueFromForeignState() to settle.
			await new Promise(resolve => setImmediate(resolve));

			expect(calls.onUpsert).to.have.length(1);
			expect(calls.onUpsert[0].startAt).to.equal(now);
			expect(calls.onUpsert[0].metrics['state-name']).to.deep.equal({ val: 'My Sensor', unit: '' });
			expect(calls.onUpsert[0].metrics['state-min']).to.deep.equal({ val: 10, unit: 'W' });
			expect(calls.onUpsert[0].metrics['state-value']).to.deep.equal({ val: 9, unit: 'W' });
			expect(calls.onUpsert[0].actions.map(a => a.type)).to.deep.equal(['ack', 'snooze']);

			expect(calls.onMetrics.length).to.be.at.least(1);
			const metricCall = calls.onMetrics.find(c => c?.set?.['state-value']);
			expect(metricCall.ref).to.equal(calls.onUpsert[0].ref);
			expect(metricCall.set['state-name'].unit).to.equal('');
			expect(['c', 'My Sensor']).to.include(metricCall.set['state-name'].val);
			expect(metricCall.set['state-value']).to.deep.equal({ val: 9, unit: 'W' });
		} finally {
			Date.now = originalNow;
		}
	});

	it('opens on lt violation and delays message creation by minDuration', async () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createHarness({ unit: '' });

		const originalNow = Date.now;
		const now = Date.UTC(2025, 0, 1, 12, 0, 0);
		Date.now = () => now;
		try {
			const rule = new ThresholdRule({
				ctx,
				targetId: 'a.b.c',
				ruleConfig: { mode: 'lt', value: 10, hysteresis: 2, minDurationValue: 5, minDurationUnit: 1 },
				messageWritersByPresetKey,
				timers,
			});

			rule.onStateChange('a.b.c', { val: '9' });

			expect(calls.onUpsert).to.have.length(0);
			expect(calls.timerSet).to.have.length(1);

			rule.onTimer({
				id: 'thr:a.b.c',
				at: now + 5000,
				kind: 'threshold.minDuration',
				data: { targetId: 'a.b.c' },
			});

			expect(calls.onUpsert).to.have.length(1);
			expect(calls.onUpsert[0].startAt).to.equal(now);
			expect(calls.onUpsert[0].actions.map(a => a.type)).to.deep.equal(['ack', 'snooze']);
			expect(calls.onUpsert[0].metrics['state-name'].unit).to.equal('');
			expect(['c', 'My Sensor']).to.include(calls.onUpsert[0].metrics['state-name'].val);
			expect(calls.onUpsert[0].metrics['state-min']).to.deep.equal({ val: 12, unit: '' });
			expect(calls.onUpsert[0].metrics['state-value']).to.deep.equal({ val: 9, unit: '' });
			expect(calls.onMetrics.length).to.be.at.least(1);
			const valueMetricCall = calls.onMetrics.find(c => c?.set?.['state-value']);
			expect(valueMetricCall.set['state-name'].unit).to.equal('');
			expect(['c', 'My Sensor']).to.include(valueMetricCall.set['state-name'].val);
			expect(valueMetricCall.set['state-value']).to.deep.equal({ val: 9, unit: '' });

			// object meta lookup is async -> state-name may arrive later
			await new Promise(resolve => setImmediate(resolve));
			const nameMetricCall = calls.onMetrics.find(c => c?.set?.['state-name']?.val === 'My Sensor');
			expect(nameMetricCall.set['state-name']).to.deep.equal({ val: 'My Sensor', unit: '' });
		} finally {
			Date.now = originalNow;
		}
	});

	it('provides recovery bounds metrics for outside mode (min/max + hysteresis)', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createHarness({ unit: '' });

		const rule = new ThresholdRule({
			ctx,
			targetId: 'a.b.c',
			ruleConfig: { mode: 'outside', min: 10, max: 20, hysteresis: 1, minDurationValue: 0, minDurationUnit: 1 },
			messageWritersByPresetKey,
			timers,
		});

		rule.onStateChange('a.b.c', { val: 5 });
		expect(calls.onUpsert).to.have.length(1);
		expect(calls.onUpsert[0].metrics['state-min'].val).to.equal(11);
		expect(calls.onUpsert[0].metrics['state-max'].val).to.equal(19);
	});

	it('patches all threshold live metrics while still in bad-state', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createHarness({ unit: '' });

		const originalNow = Date.now;
		let t = Date.UTC(2025, 0, 1, 12, 0, 0);
		Date.now = () => t;
		try {
			const rule = new ThresholdRule({
				ctx,
				targetId: 'a.b.c',
				ruleConfig: { mode: 'lt', value: 10, hysteresis: 2, minDurationValue: 0, minDurationUnit: 1 },
				messageWritersByPresetKey,
				timers,
			});

			rule.onStateChange('a.b.c', { val: 9 }); // active
			t += 1000;
			rule.onStateChange('a.b.c', { val: 8 }); // still bad -> live patch
			expect(calls.onMetrics.length).to.be.at.least(1);
			const patch = calls.onMetrics[calls.onMetrics.length - 1].set;
			expect(patch).to.have.property('state-name');
			expect(patch).to.have.property('state-value');
			expect(patch).to.not.have.property('state-recovered-at');
		} finally {
			Date.now = originalNow;
		}
	});

	it('patches no threshold live metrics in good-state', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createHarness({ unit: '' });

		const originalNow = Date.now;
		let t = Date.UTC(2025, 0, 1, 12, 0, 0);
		Date.now = () => t;
		try {
			const rule = new ThresholdRule({
				ctx,
				targetId: 'a.b.c',
				ruleConfig: { mode: 'lt', value: 10, hysteresis: 2, minDurationValue: 0, minDurationUnit: 1 },
				messageWritersByPresetKey,
				timers,
			});

			rule.onStateChange('a.b.c', { val: 9 }); // bad/open
			t += 1000;
			rule.onStateChange('a.b.c', { val: 12 }); // good/recover+close
			const afterRecoveryIdx = calls.onMetrics.length;

			t += 1000;
			rule.onStateChange('a.b.c', { val: 15 }); // still good
			rule.onTick(t);

			const goodStatePatches = calls.onMetrics.slice(afterRecoveryIdx);
			expect(goodStatePatches.some(c => c?.set?.['state-name'])).to.equal(false);
			expect(goodStatePatches.some(c => c?.set?.['state-value'])).to.equal(false);
			expect(goodStatePatches.some(c => c?.set?.['state-min'])).to.equal(false);
			expect(goodStatePatches.some(c => c?.set?.['state-max'])).to.equal(false);
		} finally {
			Date.now = originalNow;
		}
	});

	it('closes on recovery using hysteresis', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createHarness({ unit: '' });

		const originalNow = Date.now;
		const now = Date.UTC(2025, 0, 1, 12, 0, 0);
		let t = now;
		Date.now = () => t;
		try {
			const rule = new ThresholdRule({
				ctx,
				targetId: 'a.b.c',
				ruleConfig: { mode: 'lt', value: 10, hysteresis: 2, minDurationValue: 0, minDurationUnit: 1 },
				messageWritersByPresetKey,
				timers,
			});

			rule.onStateChange('a.b.c', { val: 9 }); // active
			expect(calls.onUpsert).to.have.length(1);

			t += 1000;
			rule.onStateChange('a.b.c', { val: 11 }); // not yet ok (needs >= 12)
			expect(calls.onClose).to.have.length(0);
			const metricCountBeforeRecovery = calls.onMetrics.length;

			t += 1000;
			rule.onStateChange('a.b.c', { val: 12 }); // ok
			expect(calls.onMetrics).to.have.length(metricCountBeforeRecovery + 1);
			const recoveryPatch = calls.onMetrics[calls.onMetrics.length - 1];
			expect(recoveryPatch.set).to.deep.equal({
				'state-recovered-at': { val: t, unit: 'ms' },
			});
			const afterRecoveryIdx = calls.onMetrics.length;
			expect(calls.onClose).to.have.length(1);
			expect(calls.onClose[0].ref).to.equal(calls.onUpsert[0].ref);

			t += 1000;
			rule.onStateChange('a.b.c', { val: 20 }); // still good
			rule.onTick(t);
			const goodStatePatches = calls.onMetrics.slice(afterRecoveryIdx);
			expect(goodStatePatches).to.have.length(0);
			expect(goodStatePatches.some(c => c?.set?.['state-name'])).to.equal(false);
			expect(goodStatePatches.some(c => c?.set?.['state-value'])).to.equal(false);
		} finally {
			Date.now = originalNow;
		}
	});

	it('keeps state-value live while the message remains open in the hysteresis band', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createHarness({ unit: '' });

		const originalNow = Date.now;
		let t = Date.UTC(2025, 0, 1, 12, 0, 0);
		Date.now = () => t;
		try {
			const rule = new ThresholdRule({
				ctx,
				targetId: 'a.b.c',
				ruleConfig: { mode: 'lt', value: 10, hysteresis: 2, minDurationValue: 0, minDurationUnit: 1 },
				messageWritersByPresetKey,
				timers,
			});

			rule.onStateChange('a.b.c', { val: 9 }); // bad/open
			const metricCountAfterOpen = calls.onMetrics.length;

			t += 1000;
			rule.onStateChange('a.b.c', { val: 11 }); // hysteresis band: still open, not yet recovered

			expect(calls.onClose).to.have.length(0);
			expect(calls.onMetrics).to.have.length(metricCountAfterOpen + 1);
			const patch = calls.onMetrics[calls.onMetrics.length - 1].set;
			expect(patch).to.have.property('state-name');
			expect(patch['state-value']).to.deep.equal({ val: 11, unit: '' });
			expect(patch).to.not.have.property('state-recovered-at');

			t += 1000;
			rule.onTick(t);
			const tickPatch = calls.onMetrics[calls.onMetrics.length - 1].set;
			expect(tickPatch).to.have.property('state-name');
			expect(tickPatch['state-value']).to.deep.equal({ val: 11, unit: '' });
			expect(tickPatch).to.not.have.property('state-recovered-at');
		} finally {
			Date.now = originalNow;
		}
	});

	it('cancels a pending minDuration timer when the condition clears before it fires', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createHarness({ unit: '' });

		const originalNow = Date.now;
		const now = Date.UTC(2025, 0, 1, 12, 0, 0);
		Date.now = () => now;
		try {
			const rule = new ThresholdRule({
				ctx,
				targetId: 'a.b.c',
				ruleConfig: { mode: 'lt', value: 10, hysteresis: 2, minDurationValue: 5, minDurationUnit: 1 },
				messageWritersByPresetKey,
				timers,
			});

			rule.onStateChange('a.b.c', { val: 9 }); // active -> schedule timer
			expect(calls.timerSet).to.have.length(1);
			expect(calls.onUpsert).to.have.length(0);

			rule.onStateChange('a.b.c', { val: 12 }); // ok -> cancel timer
			expect(calls.timerDelete).to.deep.equal(['thr:a.b.c']);

			rule.onTimer({
				id: 'thr:a.b.c',
				at: now + 5000,
				kind: 'threshold.minDuration',
				data: { targetId: 'a.b.c' },
			});
			expect(calls.onUpsert).to.have.length(0);
		} finally {
			Date.now = originalNow;
		}
	});

	it('supports boolean truthy mode and closes when the value becomes FALSE', () => {
		const { ctx, messageWritersByPresetKey, timers, calls } = createHarness({ unit: '' });

		const rule = new ThresholdRule({
			ctx,
			targetId: 'a.b.c',
			ruleConfig: { mode: 'truthy', minDurationValue: 0, minDurationUnit: 1 },
			messageWritersByPresetKey,
			timers,
		});

		rule.onStateChange('a.b.c', { val: 'true' });
		expect(calls.onUpsert).to.have.length(1);

		rule.onStateChange('a.b.c', { val: 'false' });
		expect(calls.onClose).to.have.length(1);
	});
});

describe('ThresholdRule ruleTemplateCatalog compliance', () => {
	const { ruleTemplateCatalog } = require('../constants');

	function collectAllEmittedMetricKeys(calls) {
		const keys = new Set();
		for (const u of calls.onUpsert ?? []) {
			for (const k of Object.keys(u.metrics ?? {})) keys.add(k);
		}
		for (const m of calls.onMetrics ?? []) {
			for (const k of Object.keys(m.set ?? {})) keys.add(k);
		}
		return keys;
	}

	function getCatalogKeys(ruleId, presetSubset) {
		const rule = ruleTemplateCatalog[ruleId];
		return new Set(
			Object.keys(rule.metrics).filter(key => {
				const e = rule.metrics[key];
				return e.subset === null || e.subset.includes(presetSubset);
			}),
		);
	}

	function assertExactCatalogMatch(emittedKeys, ruleId, presetSubset) {
		const expected = getCatalogKeys(ruleId, presetSubset);
		const unexpected = [...emittedKeys].filter(k => !expected.has(k));
		const missing = [...expected].filter(k => !emittedKeys.has(k));
		expect(unexpected, `unexpected metric keys for ${ruleId}/${presetSubset}`).to.deep.equal([]);
		expect(missing, `missing metric keys for ${ruleId}/${presetSubset}`).to.deep.equal([]);
	}

	function createHarness() {
		const calls = { onUpsert: [], onMetrics: [], onClose: [], timerSet: [], timerDelete: [] };
		const ctx = {
			api: {
				log: { debug: () => undefined },
				i18n: { t: key => key },
				iobroker: {
					objects: { getForeignObject: async () => ({ common: { name: 'Sensor', unit: 'W' } }) },
					states: { getForeignState: async () => null },
				},
			},
			meta: { plugin: { baseOwnId: 'msghub.0.IngestStates.0' } },
		};
		const writer = {
			onUpsert: (ref, info) => { calls.onUpsert.push({ ref, ...info }); return true; },
			onMetrics: (ref, info) => { calls.onMetrics.push({ ref, ...info }); return true; },
			onClose: ref => { calls.onClose.push({ ref }); return true; },
		};
		const timers = {
			_timers: new Map(),
			get: id => timers._timers.get(id) ?? null,
			set: (id, at, kind, data) => { timers._timers.set(id, { at, kind, data }); calls.timerSet.push(id); },
			delete: id => { timers._timers.delete(id); calls.timerDelete.push(id); },
		};
		const messageWritersByPresetKey = { DefaultId: writer, $fallback: writer };
		return { ctx, messageWritersByPresetKey, timers, calls };
	}

	const scenarios = [
		{
			subset: 'lt',
			ruleConfig: { mode: 'lt', value: 10, hysteresis: 0, minDurationValue: 0, minDurationUnit: 1 },
			open: { val: 5 },
			recover: { val: 15 },
		},
		{
			subset: 'gt',
			ruleConfig: { mode: 'gt', value: 10, hysteresis: 0, minDurationValue: 0, minDurationUnit: 1 },
			open: { val: 15 },
			recover: { val: 5 },
		},
		{
			subset: 'inside',
			ruleConfig: { mode: 'inside', min: 40, max: 60, hysteresis: 0, minDurationValue: 0, minDurationUnit: 1 },
			open: { val: 50 },
			recover: { val: 10 },
		},
		{
			subset: 'outside',
			ruleConfig: { mode: 'outside', min: 40, max: 60, hysteresis: 0, minDurationValue: 0, minDurationUnit: 1 },
			open: { val: 10 },
			recover: { val: 50 },
		},
		{
			subset: 'truthy',
			ruleConfig: { mode: 'truthy', minDurationValue: 0, minDurationUnit: 1 },
			open: { val: true },
			recover: { val: false },
		},
		{
			subset: 'falsy',
			ruleConfig: { mode: 'falsy', minDurationValue: 0, minDurationUnit: 1 },
			open: { val: false },
			recover: { val: true },
		},
	];

	for (const { subset, ruleConfig, open: openState, recover: recoverState } of scenarios) {
		it(`emits exactly the catalog-declared metric keys for subset '${subset}'`, () => {
			const { ctx, messageWritersByPresetKey, timers, calls } = createHarness();
			const rule = new ThresholdRule({ ctx, targetId: 'a.b.c', ruleConfig, messageWritersByPresetKey, timers });
			rule.onStateChange('a.b.c', openState);
			rule.onTick(Date.now() + 1000);
			rule.onStateChange('a.b.c', recoverState);
			assertExactCatalogMatch(collectAllEmittedMetricKeys(calls), 'threshold', subset);
		});
	}
});
