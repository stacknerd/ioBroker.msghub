...
class NonSettlingRule {
	...
	constructor({ ctx, targetId, ruleConfig = {}, messageWritersByPresetKey, timers = null, traceEvents = false }) {
		...
		this._trendQuietGapMs =
			this._profile === 'trend' ? this._resolveDurationMs('trendQuietGapValue', 'trendQuietGapUnit') : 0;
		...
		this._trace(
			`start targetId='${this.targetId}' profile='${this._profile}' minDelta=${this._minDelta} maxContinuousMs=${this._maxContinuousMs} trendWindowMs=${this._trendWindowMs} trendQuietGapMs=${this._trendQuietGapMs}`,
		);
	}
	 ...
	 */
	onTick(now) {
		if (!this._isMessageActive() || !this._badStateActive) {
			return;
		}
	...
	dispose() {
		...
		this._cancelActivityRecoveryTimer();
		this._cancelTrendRecoveryTimer();
	}
	...
	onTimer(timer) {
		...
		if (kind === 'nonSettling.activity.recover') {
			if (this._profile !== 'activity') {
				return;
			}
			const startedAt =
				typeof data.startedAt === 'number' && Number.isFinite(data.startedAt) ? Math.trunc(data.startedAt) : 0;
			if (!startedAt || this._activity.candidateStartedAt !== startedAt) {
				return;
			}
			const now = Date.now();
			if (!this._isActivityCandidateStableAt(now)) {
				return;
			}
			this._handleActivityRecovery(now);
			return;
		}
		if (kind === 'nonSettling.trend.recover') {
			if (this._profile !== 'trend') {
				return;
			}
			if (!this._trendQuietGapMs) {
				this._cancelTrendRecoveryTimer();
				this._trend.quietSinceAt = 0;
				return;
			}

			const startedAt =
				typeof data.startedAt === 'number' && Number.isFinite(data.startedAt) ? Math.trunc(data.startedAt) : 0;
			if (!startedAt || this._trend.quietSinceAt !== startedAt) {
				return;
			}

			const now = Date.now();
			if (now - startedAt < this._trendQuietGapMs) {
				this._ensureTrendRecoveryTimer();
				return;
			}

			// Trend auto-recovery: no progress for trendQuietGap.
			this._transitionToGood(now);
			this._closeRequested = false;
			this._close(now);
			return;
		}
	...
	_validateConfigOrThrow() {
		...
		if (profile === 'trend') {
			...
			if (
				typeof cfg.trendQuietGapValue !== 'number' ||
				!Number.isFinite(cfg.trendQuietGapValue) ||
				cfg.trendQuietGapValue < 0
			) {
				throw new Error('NonSettlingRule(trend): nonset-trendQuietGapValue must be a number >= 0');
			}
			if (
				typeof cfg.trendQuietGapUnit !== 'number' ||
				!Number.isFinite(cfg.trendQuietGapUnit) ||
				cfg.trendQuietGapUnit <= 0
			) {
				throw new Error('NonSettlingRule(trend): nonset-trendQuietGapUnit must be a positive number (seconds)');
			}
			return;
		}
	}
	...
	_handleActivity(value, now) {
		...
		this._ensureActivityRecoveryTimer();

		const stable = this._isActivityCandidateStableAt(now);
	...
	_handleTrend(value, now) {
		...
		if (!this._trend.startedAt) {
			...
			if (brokeUp || brokeDown) {
				...
				const prevMin = this._trend.min;
				const prevMax = this._trend.max;
				this._trend.lastValue = value;
				this._trend.dir = effectiveDir || this._trend.dir;

				// "Progress" is directional extension beyond the noise threshold.
				if (effectiveDir === 'up' && Number.isFinite(prevMax)) {
					trendProgress = value > prevMax + minDelta;
				} else if (effectiveDir === 'down' && Number.isFinite(prevMin)) {
					trendProgress = value < prevMin - minDelta;
				}
		...
		if (openAt <= now && (dir === 'up' || dir === 'down') && delta >= this._minTotalDelta) {
			...
			if (!this._isMessageActive() || !this._badStateActive) {
				...
				// Keep an already-open message stable: update metrics and recovery window only.
				this._syncTrendRecoveryWindow(now, trendProgress);
				this._patchMetrics(now);
			}
			return;
		}
	...
	/**
	 * Build the recovery-timer id for the activity profile.
	 *
	 * @returns {string} Timer id for activity recovery checks.
	 */
	_activityRecoveryTimerId() {
		return `ns:${this.targetId}:recover:activity`;
	}

	/**
	 * Build the recovery-timer id for the trend profile.
	 *
	 * @returns {string} Timer id for trend recovery checks.
	 */
	_trendRecoveryTimerId() {
		return `ns:${this.targetId}:recover:trend`;
	}
	...
	/**
	 * Ensure a persisted activity-recovery timer for the current stable candidate.
	 *
	 * @returns {void}
	 */
	_ensureActivityRecoveryTimer() {
		if (this._profile !== 'activity' || !this.timers) {
			return;
		}
		if (!this._quietGapMs || !this._activity.candidateStartedAt) {
			this._cancelActivityRecoveryTimer();
			return;
		}

		const id = this._activityRecoveryTimerId();
		const at = this._activity.candidateStartedAt + this._quietGapMs;
		const data = {
			profile: 'activity',
			targetId: this.targetId,
			startedAt: this._activity.candidateStartedAt,
			min: this._activity.candidateMin,
			max: this._activity.candidateMax,
		};
		const existing = this.timers.get(id);
		if (existing && existing.at === at) {
			this.timers.set(id, at, 'nonSettling.activity.recover', data);
			return;
		}
		this.timers.set(id, at, 'nonSettling.activity.recover', data);
	}
	...
	/**
	 * Cancel the activity-recovery timer (best-effort).
	 *
	 * @returns {void}
	 */
	_cancelActivityRecoveryTimer() {
		if (!this.timers) {
			return;
		}
		this.timers.delete(this._activityRecoveryTimerId());
	}

	/**
	 * Ensure a persisted trend-recovery timer while the message is active.
	 *
	 * @returns {void}
	 */
	_ensureTrendRecoveryTimer() {
		if (this._profile !== 'trend' || !this.timers) {
			return;
		}
		if (!this._trendQuietGapMs) {
			this._cancelTrendRecoveryTimer();
		...
		if (!this._trend.quietSinceAt || !Number.isFinite(this._trend.quietSinceAt)) {
			return;
		}
		const id = this._trendRecoveryTimerId();
		const at = this._trend.quietSinceAt + this._trendQuietGapMs;
		const data = {
			profile: 'trend',
			targetId: this.targetId,
			startedAt: this._trend.quietSinceAt,
			dir: this._trend.dir || '',
		};
		const existing = this.timers.get(id);
		if (existing && existing.at === at) {
			this.timers.set(id, at, 'nonSettling.trend.recover', data);
			return;
		}
		this.timers.set(id, at, 'nonSettling.trend.recover', data);
	}
	...
	/**
	 * Keep trend quiet-gap window state/timer aligned with current progress.
	 *
	 * @param {number} now Current timestamp (ms).
	 * @param {boolean} didProgress True when the trend advanced directionally.
	 * @returns {void}
	 */
	_syncTrendRecoveryWindow(now, didProgress) {
		if (this._profile !== 'trend') {
			return;
		}
		if (!this._trendQuietGapMs) {
		...
		if (didProgress || !this._trend.quietSinceAt) {
			this._trend.quietSinceAt = Math.trunc(now);
		}
		this._ensureTrendRecoveryTimer();
	}

	/**
	 * Cancel the trend-recovery timer (best-effort).
	 *
	 * @returns {void}
	 */
	_cancelTrendRecoveryTimer() {
		if (!this.timers) {
			return;
		}
		this.timers.delete(this._trendRecoveryTimerId());
	}
	...
	_restorePendingTimerState() {
		...
		const openExisting = this.timers.get(this._openTimerId());
		const openData = openExisting?.data;
		if (openData && typeof openData === 'object' && !Array.isArray(openData)) {
			if (openData.profile === 'activity') {
				this._activity.nonStableSinceAt =
					typeof openData.startedAt === 'number' && Number.isFinite(openData.startedAt)
						? openData.startedAt
						: 0;
				this._activity.startValue =
					typeof openData.startValue === 'number' && Number.isFinite(openData.startValue)
						? openData.startValue
						: NaN;
				this._activity.min =
					typeof openData.min === 'number' && Number.isFinite(openData.min) ? openData.min : NaN;
				this._activity.max =
					typeof openData.max === 'number' && Number.isFinite(openData.max) ? openData.max : NaN;
			}

			if (openData.profile === 'trend') {
				if (
					typeof openData.startedAt === 'number' &&
					Number.isFinite(openData.startedAt) &&
					typeof openData.startValue === 'number' &&
					Number.isFinite(openData.startValue) &&
					typeof openData.min === 'number' &&
					Number.isFinite(openData.min) &&
					typeof openData.max === 'number' &&
					Number.isFinite(openData.max)
				) {
					this._trend.startedAt = openData.startedAt;
					this._trend.startValue = openData.startValue;
					this._trend.min = openData.min;
					this._trend.max = openData.max;
					this._trend.dir = openData.dir === 'up' || openData.dir === 'down' ? openData.dir : '';
					this._trend.lastValue = typeof this._value === 'number' ? this._value : NaN;
				}
			}
		}
		...
		if (this._profile === 'activity') {
			const recoveryExisting = this.timers.get(this._activityRecoveryTimerId());
			const recoveryData = recoveryExisting?.data;
			if (
				recoveryData &&
				typeof recoveryData === 'object' &&
				!Array.isArray(recoveryData) &&
				typeof recoveryData.startedAt === 'number' &&
				Number.isFinite(recoveryData.startedAt) &&
				recoveryData.startedAt > 0
			) {
				this._activity.candidateStartedAt = Math.trunc(recoveryData.startedAt);
				this._activity.candidateMin =
					typeof recoveryData.min === 'number' && Number.isFinite(recoveryData.min)
						? recoveryData.min
						: this._activity.candidateMin;
				this._activity.candidateMax =
					typeof recoveryData.max === 'number' && Number.isFinite(recoveryData.max)
						? recoveryData.max
						: this._activity.candidateMax;
			}
		}
		if (this._profile === 'trend') {
			const recoveryExisting = this.timers.get(this._trendRecoveryTimerId());
			const recoveryData = recoveryExisting?.data;
			if (
				recoveryData &&
				typeof recoveryData === 'object' &&
				...
				this._trend.quietSinceAt = Math.trunc(recoveryData.startedAt);
			}
		}
	}

	/**
	 * Check whether the current activity candidate qualifies as stable at `now`.
	 *
	 * @param {number} now Current timestamp (ms).
	 * @returns {boolean} True when quietGap elapsed and movement stayed within minDelta.
	 */
	_isActivityCandidateStableAt(now) {
		if (
		...
		if (!Number.isFinite(this._activity.candidateMin) || !Number.isFinite(this._activity.candidateMax)) {
			return false;
		}
		return this._activity.candidateMax - this._activity.candidateMin <= this._minDelta;
	}

	/**
	 * Handle activity recovery once a quiet/stable phase is confirmed.
	 *
	 * @param {number} now Current timestamp (ms).
	 * @returns {void}
	 */
	_handleActivityRecovery(now) {
		this._cancelOpenTimer();
		this._cancelActivityRecoveryTimer();
	...
	_open(now) {
		...
		if (this._profile === 'trend') {
			this._syncTrendRecoveryWindow(now, true);
		}
	}
	...
	/**
	 * Mark bad->good transition and emit recovery timestamp once.
	 *
	 * @param {number} now Current timestamp (ms).
	 * @returns {void}
	 */
	_transitionToGood(now) {
		if (this._isMessageActive() && !this._recoveredAtPatched) {
			const writer = this._getMessageWriter('NonSettlingId');
			if (writer && typeof writer.onMetrics === 'function') {
				writer.onMetrics(this._getRef(), {
					set: {
						'state-recovered-at': { val: Math.trunc(now), unit: 'ms' },
					},
					now: Math.trunc(now),
					force: true,
				});
	...
	_buildMetricsSet() {
		...
		if (m) {
			...
			set.trendStartValue = { val: m.startValue, unit: this._unit || '' };
			set.trendMin = { val: m.min, unit: this._unit || '' };
			set.trendMax = { val: m.max, unit: this._unit || '' };
			set.trendMinToMax = { val: m.max - m.min, unit: this._unit || '' };
			...
			set.trendStartValue = { val: this._activity.startValue, unit: this._unit || '' };
			set.trendMin = { val: this._activity.min, unit: this._unit || '' };
			set.trendMax = { val: this._activity.max, unit: this._unit || '' };
			set.trendMinToMax = { val: this._activity.max - this._activity.min, unit: this._unit || '' };
	...
	_loadObjectMeta() {
		...
		Promise.resolve()
			...
			.then(obj => {
				...
				if (patchName && this._isMessageActive() && this._badStateActive) {
					const writer = this._getMessageWriter('NonSettlingId');
					if (writer && typeof writer.onMetrics === 'function') {
						writer.onMetrics(this._getRef(), {
