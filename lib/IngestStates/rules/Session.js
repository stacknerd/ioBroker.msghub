...
class SessionRule {
	...
	constructor({ ctx, targetId, ruleConfig = {}, messageWritersByPresetKey, timers = null, traceEvents = false }) {
		...
		const baseOwnId = typeof this.ctx?.meta?.plugin?.baseOwnId === 'string' ? this.ctx.meta.plugin.baseOwnId : '';
		const baseFullId =
			typeof this.ctx?.meta?.plugin?.baseFullId === 'string' && this.ctx.meta.plugin.baseFullId.trim()
				? this.ctx.meta.plugin.baseFullId
				: baseOwnId;
		this._baseOwnId = `${baseOwnId}.session.${this.targetId}`;
		this._baseFullId = `${baseFullId}.session.${this.targetId}`;
		this._ids = Object.freeze({
			sessionCount: { own: `${this._baseOwnId}.sessionCount`, full: `${this._baseFullId}.sessionCount` },
			sessionActive: { own: `${this._baseOwnId}.sessionActive`, full: `${this._baseFullId}.sessionActive` },
		});
	 ...
	 */
	requiredStateIds() {
		const ids = new Set([this.targetId, this._ids.sessionCount.full]);
	...
	onStateChange(id, state) {
		if (id === this._ids.sessionCount.full) {
			this._onSessionCountStateChange(state);
			return;
		}
	...
	onTimer(timer) {
		...
		if (kind === 'session.stopDelay') {
			...
			if (gate === null) {
				// Gate state unknown at expiry — reschedule and refresh gate before next check.
				// Without the refresh, _gateVal stays null forever if no state changes arrive,
				// causing the timer to loop indefinitely without ending the session.
				this._ensureStopTimer(Date.now());
	...
	_startSession(now) {
		...
		// Defensive: if a start message already exists in the store, restore its session-counter-start
		// value instead of latching the current counter. This preserves the original start value when
		// the rule instance is replaced mid-session (e.g., due to an Engine rescan).
		let counterStartVal = null;
		if (summaryEnabled) {
			const startRef = this._getRef('_start');
			const existingMsg = this.ctx.api.store.getMessageByRef(startRef, 'quasiOpen');
			const stored =
				existingMsg?.metrics instanceof Map ? existingMsg.metrics.get('session-counter-start')?.val : undefined;
			if (typeof stored === 'number' && Number.isFinite(stored)) {
				counterStartVal = stored;
			} else if (typeof this._counterVal === 'number' && Number.isFinite(this._counterVal)) {
				counterStartVal = this._counterVal;
			}
		}
		...
		// A new start supersedes any still-open end task from the previous session.
		this._completePreviousEndMessageIfOpen();
	...
	_endSession(now) {
		...
		this._setSessionCount(this._sessionCount + 1);
		...
		if (endWriter) {
			...
			// Apply preset close policy immediately on end creation (manual-close action or policy-driven completion behavior).
	...
	_buildSessionMetrics(_now) {
		...
		const metrics = {
			...
			'session-count': { val: this._sessionCount, unit: '' },
		};
		if (!this._isSummaryEnabled()) {
			return metrics;
		}

		metrics['session-counter-start'] = {
	...
	/**
	 * Mark the previous end message as resolved when a new session starts.
	 *
	 * This is intentionally stronger than MessageWriter.onClose(): manual-close end presets must stay open
	 * after the session ended, but a later start proves the previous end task has been superseded.
	 *
	 * @returns {void}
	 */
	_completePreviousEndMessageIfOpen() {
		...
		this.ctx.api.store.completeAfterCauseEliminated(ref, { actor: this._actor() });
	}
	...
	/**
	 * Ensure the persistent session states exist and load their current values.
	 *
	 * @returns {void}
	 */
	_bootstrapSessionStates() {
		...
		Promise.resolve()
			.then(async () => {
				await objects.setObjectNotExists(this._ids.sessionCount.own, {
					type: 'state',
					common: {
						name: 'Session count',
						type: 'number',
				...
				await objects.setObjectNotExists(this._ids.sessionActive.own, {
					type: 'state',
					common: {
						name: 'Session active',
						type: 'boolean',
				...
				const current = await states.getForeignState(this._ids.sessionCount.full).catch(() => null);
				const next = this._normalizeSessionCount(current?.val);
				this._setSessionCount(next === null ? 0 : next);
				this._setSessionActive(this._isActive());
			})
			.catch(() => undefined);
	}

	/**
	 * Handle manual session counter corrections.
	 *
	 * @param {object} state ioBroker state.
	 * @returns {void}
	 */
	_onSessionCountStateChange(state) {
		if (state?.ack === true) {
			return;
		}

		const next = this._normalizeSessionCount(state?.val);
		if (next === null) {
			return;
		}

		this._setSessionCount(next);
		this._patchStartMetrics(Date.now(), true);
	}

	/**
	 * Normalize a session count value.
	 *
	 * @param {any} val Raw value.
	 * @returns {number|null} Non-negative integer or null when invalid.
	 */
	_normalizeSessionCount(val) {
		if (typeof val === 'string' && !val.trim()) {
			return null;
		}
		const parsed = this._parseNumber(val);
	...
	/**
	 * Update the session counter cache and mirror it to ioBroker.
	 *
	 * @param {number} value New counter value.
	 * @returns {number} Stored counter value.
	 */
	_setSessionCount(value) {
		const next = this._normalizeSessionCount(value);
		if (next === null) {
			return this._sessionCount;
		}
		this._sessionCount = next;
		this._setAck(this._ids.sessionCount.full, next);
		return next;
	}

	/**
	 * Mirror the current active-session state to ioBroker.
	 *
	 * @param {boolean} active Active flag.
	 * @returns {void}
	 */
	_setSessionActive(active) {
		this._setAck(this._ids.sessionActive.full, active === true);
	}
