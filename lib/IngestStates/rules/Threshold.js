...
class ThresholdRule {
	...
	onStateChange(id, state) {
		...
		// Hysteresis band: the message stays open even though the hard bad-state no longer matches.
		// Keep `state-value` live so the open message does not show a stale number.
		this._patchCurrentValue(now);
	}
	...
	_patchCurrentValue(now) {
		if (this._value === null || !this._isActive) {
			return;
		}
