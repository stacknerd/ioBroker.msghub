...
class ThresholdRule {
	...
	onStateChange(id, state) {
		...
		if (active) {
			...
			if (this._minDurationMs > 0 && !this._isActive) {
				...
				this._isActive = true;
				const startAt =
					typeof this._activeSinceAt === 'number' && Number.isFinite(this._activeSinceAt) ? this._activeSinceAt : now;
				this._open(now, startAt);
				this._patchCurrentValue(now);
			}
			return;
	...
	_open(now, startAt) {
		...
		this.message.openActive({ defaultTitle, defaultText, now, startAt, actions });
	}
