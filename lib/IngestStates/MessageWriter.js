...
const PRESET_LOGGED_BY_META = new WeakMap();
...
class TargetMessageWriter {
	...
	constructor(
		ctx,
		{ targetId, ruleType, messageConfig, startMessageConfig, presetProvider = null, traceEvents = false },
	) {
		this.ctx = ctx;
		...
		this._tracePresetPreview();
	}

	/**
	 * Best-effort debug helper: log the referenced preset JSON once per presetId.
	 *
	 * This is intentionally non-functional (read-only) and exists only to verify wiring:
	 * - jsonCustom selects a preset id
	 * - Engine loads it into the cache
	 * - MessageWriter can read it via `presetProvider.getPreset(presetId)`
	 *
	 * @returns {void}
	 */
	_tracePresetPreview() {
		if (!this._traceEvents || typeof this._log?.debug !== 'function' || !this._presetProvider) {
			return;
		}

		const logSet = this._presetLoggedSet();
		const handleCfg = (cfg, source) => {
			...
			for (const [key, value] of Object.entries(cfg)) {
				...
				const seen = logSet ? logSet.has(presetId) : false;
				if (logSet && !seen) {
					logSet.add(presetId);
				}
				...
				const preset = this._presetProvider.getPreset(presetId);
				if (!preset) {
					this._log.debug(
						`MessageWriter: preset preview: presetId='${presetId}' source='${source}.${key}' -> <missing>`,
					);
					continue;
				}

				this._log.debug(
					`MessageWriter: preset preview: presetId='${presetId}' source='${source}.${key}' -> ${JSON.stringify(
						preset,
						null,
						2,
					)}`,
		...
		handleCfg(this.messageConfig, 'messageConfig');
		handleCfg(this.startMessageConfig, 'startMessageConfig');
	}

	/**
	 * @returns {Set<string>|null} Shared "already logged" set (per plugin runtime).
	 */
	_presetLoggedSet() {
		...
		let set = PRESET_LOGGED_BY_META.get(meta);
		if (!set) {
			set = new Set();
			PRESET_LOGGED_BY_META.set(meta, set);
		}
		return set;
	}
		...
		if (metricsPatch?.set && existingMetrics.size) {
			for (const [key, next] of Object.entries(metricsPatch.set)) {
				...
				if (prevVal === next?.val && prevUnit === next?.unit) {
					delete metricsPatch.set[key];
				}
			}
			if (Object.keys(metricsPatch.set).length === 0) {
				metricsPatch = null;
			}
		}
