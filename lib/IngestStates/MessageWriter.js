...
class TargetMessageWriter {
	...
	_ensureRoomsIndexLoaded() {
		...
		try {
			const res = getForeignObjects('enum.rooms.*');
	...
	_clearScheduledReset(ref) {
		...
		const msg = this.ctx.api.store.getMessageByRef(ref);
		const resetAt = msg?.metrics instanceof Map ? msg.metrics.get(key)?.val : undefined;
		if (typeof resetAt !== 'number' || !Number.isFinite(resetAt)) {
			return;
		}
		try {
			this._traceStore('updateMessage(clearScheduledReset)', ref, { metrics: { delete: [key] } });
	...
	openActive({
		...
		let existing = this.ctx.api.store.getMessageByRef(ref);
		const prior = existing;
		const foundState = prior?.lifecycle?.state;
		if (
			foundState === this.ctx.api.constants.lifecycle.state.deleted ||
			foundState === this.ctx.api.constants.lifecycle.state.expired
		) {
			existing = null;
		}
		...
		if (existing && isActive) {
			...
			if (actions !== undefined) {
				patch.actions = Array.isArray(actions) ? actions : [];
			}
		...
		const existingState = prior?.lifecycle?.state;
		const closedState = this.ctx.api.constants.lifecycle.state.closed;
		const deletedState = this.ctx.api.constants.lifecycle.state.deleted;
		const statusKind = this.ctx.api.constants.kind.status;
		if (
			cooldownMs > 0 &&
			(existingState === closedState || (existingState === deletedState && kind === statusKind))
		) {
			const closedAt =
				typeof prior?.lifecycle?.stateChangedAt === 'number' ? prior.lifecycle.stateChangedAt : 0;
		...
		return this.ctx.api.store.updateMessage(ref, reopenPatch);
	}
	...
	updateActive({ defaultTitle, defaultText, now = Date.now(), details = undefined, actions = undefined } = {}) {
		...
		const shouldPatchActions = actions !== undefined;
		const nextActions = shouldPatchActions ? (Array.isArray(actions) ? actions : []) : undefined;
		...
		this._traceStore('updateMessage(updateActive)', ref, patch);
	...
	closeOnNormal() {
		...
		if (this.messageConfig?.resetOnNormal === false) {
			...
			const patch = {
				actions: nextActions,
				...(Number.isFinite(existing?.timing?.notifyAt)
					? {}
					: { timing: { notifyAt: this._farFutureNotifyAt(Date.now()) } }),
			};
		...
		this._traceStore('updateMessage(closeOnNormal.schedule)', ref, schedulePatch);
		this.ctx.api.store.updateMessage(ref, schedulePatch);

		const ms = Math.max(0, resetAt - Date.now());
		this._trace(
			`scheduled close ref='${ref}' inMs=${ms} resetAt=${resetAt} actor='${actor}' metricKey='${metricKey}'`,
		);
		this._pendingResetHandle = this.ctx.meta.resources.setTimeout(() => {
			this._pendingResetHandle = null;
			try {
				this._clearScheduledReset(ref);
				this._traceStore('completeAfterCauseEliminated(closeOnNormal.due)', ref);
				this.ctx.api.store.completeAfterCauseEliminated(ref, { actor });
			} catch (e) {
				this.ctx.api.log.warn(`closeOnNormal failed for '${ref}': ${e?.message || e}`);
			}
		}, ms);
	...
	tryCloseScheduled({ now = Date.now() } = {}) {
		...
		this._cancelPendingReset();
		this._clearScheduledReset(ref);
		this._traceStore('completeAfterCauseEliminated(tryCloseScheduled)', ref);
		return this.ctx.api.store.completeAfterCauseEliminated(ref, { actor: this._actor() });
	}
