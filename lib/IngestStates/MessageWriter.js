...
class TargetMessageWriter {
	 ...
	 *
	 * Domain timestamps (create-only):
	 * - `startAt`: planned/expected start (or actual start when first known)
	 * - `endAt`: planned/expected end (optional)
	 * - `dueAt`: derived from preset `timing.dueInMs` (deadline relative to `startAt`)
	 * - `expiresAt`: derived from preset `timing.expiresInMs` (relative to create time)
	 *
	 ...
	 * @param {number} [info.endAt] Optional domain timestamp forwarded to `timing.endAt` on create.
	...
	onUpsert(
		...
		const hasPresetIcon = preset?.message && Object.prototype.hasOwnProperty.call(preset.message, 'icon');
		const rawPresetIcon = hasPresetIcon ? preset.message.icon : null;
		const nextIcon = hasPresetIcon && typeof rawPresetIcon === 'string' ? this._maybeT(rawPresetIcon).trim() : null;
		...
		const endAtsafe = typeof endAt === 'number' && Number.isFinite(endAt) ? Math.trunc(endAt) : undefined;
		...
		const expiresInMs = preset.message?.timing?.expiresInMs;
		const timing = {
			notifyAt: nextNotifyAt,
			startAt: startAtsafe,
			...(endAtsafe !== undefined ? { endAt: endAtsafe } : {}),
			...(typeof remindEvery === 'number' && Number.isFinite(remindEvery) && remindEvery > 0
				? { remindEvery }
				: {}),
			...(typeof cooldown === 'number' && Number.isFinite(cooldown) && cooldown > 0 ? { cooldown } : {}),
			...(typeof timeBudget === 'number' && Number.isFinite(timeBudget) && timeBudget > 0 ? { timeBudget } : {}),
			...(typeof dueInMs === 'number' && Number.isFinite(dueInMs) && dueInMs > 0
				? { dueAt: startAtsafe + dueInMs }
				: {}),
			...(typeof expiresInMs === 'number' && Number.isFinite(expiresInMs) && expiresInMs > 0
				? { expiresAt: nextNotifyAt + expiresInMs }
				: {}),
		};
		...
		const desiredIcon = hasPresetIcon ? String(preset.message.icon ?? '') : null;
		const existingIcon = typeof existing.icon === 'string' ? existing.icon : null;
		if (existingIcon !== desiredIcon) {
	 ...
	 */
	onClose(ref) {
		// Close handling is only meaningful for active/quasi-open messages.
		const existing = this.ctx.api.store.getMessageByRef(ref, 'quasiOpen');
		if (!existing) {
			return false;
		}
		...
		if (preset?.policy?.resetOnNormal === false) {
			...
			const textRecovered = this._maybeT(this._unescapeNewlines(preset?.message?.textRecovered));
			if (typeof textRecovered === 'string' && textRecovered.trim() && existing.text !== textRecovered) {
				patch.text = textRecovered;
			}
