...
class TargetMessageWriter {
	...
	_trace(msg) {
		...
		this._log.debug(`${this.ctx?.meta?.plugin?.baseFullId || 'IngestStates'}: MessageWriter: ${msg}`);
	}
	...
	_roomsCache() {
		...
		if (!meta.__ingestStatesRoomsCache) {
			meta.__ingestStatesRoomsCache = { byMember: new Map(), inFlight: null, loadedAt: 0 };
		}

		return meta.__ingestStatesRoomsCache;
	}
	...
	_ensureRoomsIndexLoaded() {
		...
		cache.inFlight = Promise.resolve()
			.then(() => getForeignObjects('enum.rooms.*'))
			.then(enums => {
				const next = new Map();
	...
	openActive({
		...
		if (!existing) {
			...
			this._traceStore('addOrUpdateMessage(openActive.create)', ref, created);
