...
class IoArchiveResolver {
	 ...
	 * @returns {Promise<{createStorageBackend:(onMutated?: () => void)=>any, archiveRuntime:{configuredStrategyLock:string,effectiveStrategy:'native'|'iobroker',effectiveStrategyReason:string,nativeRootDir:string,nativeProbeError:string}}>} Resolved backend factory and runtime strategy metadata.
	 */
	...
	async resolve() {
		...
		const createStorageBackend =
			effectiveStrategy === 'native'
				? onMutated => this._createNativeBackend(onMutated)
				: onMutated => this._createIobrokerBackend(onMutated);
